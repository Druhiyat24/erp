              ...
              <div class='form-group'>
                ...
                <select class='form-control select2' style='width: 100%;' name='txtid_dayterms'>
                  <?php 
                    ...
                    IsiCombo($sql,$id_terms,'Pilih Day Terms');
                  ?>
                </select>
              </div>
            </div>			
      ...
      <tbody>
        ...
        $sql="select if(t_it_po_cx=t_it_po,'Cancelled','') status_po,a.app,a.id,pono,podate,supplier,
          ...
          (select group_concat(distinct jo_no) jo_no,ac.kpno,ac.styleno,poit.id_jo,poit.id_po,ms.supplier buyer,count(*) t_it_po,
            ...
            left join 
            (select s.id_po,a.id_po_item,sum(a.qty) tqtybpb from bpb a inner join po_item s on a.id_po_item=s.id group by s.id_po)
