...
$rs=mysqli_fetch_assoc(mysqli_query($con_new,"select up.fullname,po.revise,po.jenis status,po.pono,ac.kpno,po.notes,
  ...
  from po_header_dev po inner join po_item_dev poit on po.id=poit.id_po 
  inner join jo_det_dev jod on poit.id_jo=jod.id_jo inner join so_dev so on jod.id_so=so.id inner join 
  act_development ac on ac.id=so.id_cost 
...
{   
    ...
    { $wsdet = flookup("group_concat(distinct concat(' ',jod.jo_no))","po_header_dev po inner join po_item_dev poit 
      on po.id=poit.id_po inner join jo_dev jod on poit.id_jo=jod.id","po.id='$_GET[id]' and poit.cancel='N' order by jod.jo_no");
    }
    else
    { $wsdet = flookup("group_concat(distinct concat(' ',ac.kpno))","po_header_dev po inner join po_item_dev poit 
      on po.id=poit.id_po inner join jo_det_dev jod on poit.id_jo=jod.id_jo inner join so_dev on jod.id_so=so.id inner join 
      act_development ac on ac.id=so.id_cost","po.id='$_GET[id]' and poit.cancel='N' order by ac.kpno");
    }
}
else
{ 
...
if($jenis_company=="VENDOR LG")
{	$partnodet = "";
  $sql=mysqli_query($con_new, "select distinct mp.product_group from po_header_dev po inner join po_item_dev poit 
    on po.id=poit.id_po inner join jo_det_dev jod on poit.id_jo=jod.id_jo inner join so_dev so on jod.id_so=so.id inner join 
    act_development ac on ac.id=so.id_cost inner join masterproduct mp on ac.id_product=mp.id   
...
{ $sqldatatable = 'select po_header.discount,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
  ...
  masteritem.itemdesc,po_header.eta from po_header_dev INNER JOIN po_item_dev 
...
{ if ($jenis_company=="VENDOR LG") 
  ...
  $sqldatatable = 'select po_header.discount,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
    sum(po_item.qty) qty, 
    sum(po_item.qty * po_item.price)as totalin, po_item.curr, po_item.unit,
    '.$flddesc.' itemdesc,po_header.eta from po_header_dev INNER JOIN po_item_dev 
    on po_header.id=po_item.id_po inner join 
...
$sqldataAlamatSP = 'select mastersupplier.supplier, 
                ...
                from po_header_dev po INNER JOIN mastersupplier ON
...
<body >
    ...
    <table align="right" style="width:50%; margin-top:15px; border-collapse: collapse;font-size:12px;">
            ...
            <tr>
                ...
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; border-top:solid 2px black; padding:5px;"><p style="text-align:right;">'.$currency.' '.number_format($disc,2).'</p></td>
            </tr>
            <tr>
