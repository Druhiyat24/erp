...
{ $query = mysql_query("SELECT a.*,s.curr,f.kode_mkt FROM po_header_dev a inner join po_item_dev s 
    on a.id=s.id_po inner join reqnon_header_dev d on s.id_jo=d.id 
...
<script type="text/javascript">
  ...
  { var kode_dept = $('#cboDept').val();
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_po_dev.php?modeajax=get_list_req',
  ...
  { var id_jo = $('#cboJO').val();
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_po_dev.php?modeajax=view_list_req',
        data: {id_jo: id_jo},
        async: false
    }).responseText;
    if(html)
...
<?php if ($mod=="gendev9" or $mod=="gendev9e") { ?>
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_po_dev.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>              
...
<div class="box">
  <div class="box-header">
    ...
    <a href='../pur/?mod=gendev9' class='btn btn-primary btn-s'>
    ...
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        $query = mysql_query("select a.app,a.id,pono,podate,supplier,
          ...
          from po_header_dev a inner join 
          ...
          (select poit.id_jo,poit.id_po,group_concat(distinct reqno) buyer,
            count(*) t_row,sum(if(cancel='Y',1,0)) t_rows_cancel from po_item_dev poit inner join reqnon_header_dev rnh on poit.id_jo=rnh.id  group by poit.id_po) 
        ...
        { if($data['t_row']>0 and $data['t_row']=$data['t_rows_cancel'])
          ...
          echo "<tr $bgcol>";
            ...
            { echo "
              <td>
                <a href='?mod=gendev9e&id=$data[id]'
...
<?php } else if (($mod=="gendev9" or $mod=="gendev9e") and $id_po!="") { ?>
<div class="box">
  <div class="box-header">
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        from po_item_dev l inner join reqnon_header_dev m on l.id_jo=m.id 
        ...
        { echo "<tr>";
            ...
            { echo "
              <td>
                <a href='?mod=gendev9ei&id=$data[id]'
                  $tt_ubah</a>
              </td>
              <td>
                <a href='d_po_dev.php?mod=$mod&id=$data[id]&idd=$id_po'
                  $tt_hapus";?> 
