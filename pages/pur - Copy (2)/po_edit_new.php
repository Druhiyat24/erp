...
{ 
  ...
  $query = mysql_query("SELECT a.*,s.curr,b.tipe_com FROM po_header a 
inner join po_item s on a.id=s.id_po 
inner join (select id id_draft,tipe_com from po_header_draft where id = (select id_draft from po_header where id = '$id_po')) b on a.id_draft = b.id_draft   
...
//	  {
  ...
  { <?php echo "window.location.href='../pur/?mod=33xa&id=$id_po';"; ?>
...
<!--       <form method='post' name='form' action='s_po.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'> -->
        <form method='post' name='form' action='s_po_edit_new.php?id_po=<?php echo $id_po; ?>' >
...
$jumlahdatafix = $jumlahdata + 4;   
?>

          <select class='form-control select'  multiple='multiple'  size='<?php echo $jumlahdatafix ?>'; name='listjo' style ='overflow-x: auto;'>
  <?php 
  $sql=mysql_query("select DISTINCT(jo_no),group_concat(jo_no , ' - ', ms.supplier, ' - ', ac.styleno) nm_jo from po_item a 
    inner join jo on a.id_jo = jo.id
...
<div class="box">
  ...
  <div class="box-body">
    ...
    <table id="example2" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        { $query = mysql_query("select l.id,jo_no,concat(a.matclass,' ',a.goods_code,' ',a.itemdesc,' ',
          ...
          { $query = mysql_query("select pid.id,ac.kpno,jo_no, mi.itemdesc, round(sum(sd.qty)*k.cons,2) qty_bom,k.unit unit_bom, pid.unit, pid.qty,pid.price, pid.qty * pid.price total , pid.cancel from po_header phd 
inner join po_item pid on phd.id = pid.id_po
...
");
        ...
        $result_total_price = mysql_query("SELECT SUM(price) AS value_sum_price FROM po_item where id_po='$id_po' and cancel='N' group by id_po");         
