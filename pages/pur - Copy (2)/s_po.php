...
$id_dayterms=$_POST['txtid_dayterms'];
...
$d_installment = '';
$n_installment = '';
if(ISSET($_POST['fg_installment'])){
	$fg_installment = $_POST['fg_installment'];
	
}
if(ISSET($_POST['d_installment'])){
	$d_installment = $_POST['d_installment'];
}
if(ISSET($_POST['n_installment'])){
	$n_installment = $_POST['n_installment'];
}
...
{	
	if($pono=="")
	{
		...
		$sql="insert into po_header (username,pono,podate,etd,eta,expected_date,id_supplier,id_terms,
			notes,n_kurs,ppn,tax,discount,pph,jenis,jml_pterms)
			values ('$user','$pono','$podate','$etddate','$etadate','$expdate','$id_supplier','$id_terms',
			'$notes','$n_kurs','$tax','$tax','$disc','$pph','$jenis','$txtdays')";
		insert_log($sql,$user);
	}
...
// {	$cekbpb=flookup("bpbno","bpb","pono='$pono' & cancel !='Y'"); RMN
// 	if($cekbpb=="")
	{	$id_po=flookup("id","po_header","pono='$pono'");
					$sql="update po_header set ppn='$tax',tax='$tax',discount='$disc',pph='$pph',notes='$notes',
			id_terms='$id_terms',jml_pterms='$txtdays',etd='$etddate',eta='$etadate',
			expected_date					=   '$expdate',
			id_dayterms 					=   '$id_dayterms',
			fg_pkp 							=   '$pkp',
			fg_installment        			=  	'$fg_installment',
			d_start_installment     		=  	'$d_installment',
			n_installment        			=  	'$n_installment'			
			where id='$id_po'";
		insert_log($sql,$user);
	...
	// }
	// else
	// {	$_SESSION['msg'] = "XData Tidak Bisa Dirubah, Karena Sudah Ada BPB : ".$cekbpb;	} RMN
}
if ($mod=="9")
{	
