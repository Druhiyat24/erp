...
$rs=mysqli_fetch_assoc(mysqli_query($con_new,"select up.fullname,po.revise,po.jenis status,po.pono,po.jml_pterms,ac.kpno,po.notes,
	count(distinct poit.id_jo) totjo,count(distinct mp.product_group) totprodgroup,
  if(mpt.nama_pterms='',mpt.kode_pterms,mpt.nama_pterms) nama_pterms,   concat(nama_pterms, ' (' ,po.jml_pterms, ' days - After',' ', mdt.kode_pterms,')') ket_terms,
  poit.curr,phd.tipe_com   
  ...
  left join masterdayterms mdt on po.id_dayterms=mdt.id
  left join userpassword up on po.username=up.username
  left join po_header_draft phd on phd.id=po.id_draft     
  where po.id='$_GET[id]' and poit.cancel='N' /*and phd.app = 'A'*/ group by po.pono"));
$PONO = $rs['pono'];
$cekidgen=flookup("count(*)","po_item","id_gen=0 and id_po='$_GET[id]'");
$revise = $rs['revise'];
$createby = $rs['fullname'];
$tipe_com = $rs['tipe_com'];
if($rs['curr']=="USD")
{
  $decim=4;
}
...
if($rs['totjo']>1 or substr($rs['pono'],0,2)=="C/")
{   
    // $wsno = "(Combine)";
    $wsno = "";
    if($jenis_company=="VENDOR LG")
    { $wsdet = flookup("group_concat(distinct concat(' ',jod.jo_no))","po_header po inner join po_item poit 
      on po.id=poit.id_po inner join jo jod on poit.id_jo=jod.id","po.id='$_GET[id]' and poit.cancel='N' order by jod.jo_no");
    }
    else
    { $wsdet = flookup("group_concat(distinct concat(' ',ac.kpno))","po_header po inner join po_item poit 
...
$ptermsday = $rs['jml_pterms'];
$pketterms = $rs['ket_terms'];
$ttd = 
"<table>
      <td>JAJAL</td>
    </table>";
...
if ($status=="P")
{ $sqldatatable = 'select po_header.discount,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
    sum(po_item.qty) qty, 
...
{ if ($jenis_company=="VENDOR LG") 
  ...
  $sqldatatable = 'select po_header.discount,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
    ...
    left join masterdesc md on masteritem.id_gen = md.id
    left join mastercolor mc on md.id_color = mc.id
    left join masterweight mw on mc.id_weight = mw.id
    left join masterlength ml on mw.id_length = ml.id
    left join masterwidth mwd on ml.id_width = mwd.id
    left join master_size_new msn on mwd.kode_width = msn.size
    WHERE po_header.pono = "'.$PONO.'" and po_item.cancel="N" group by po_item.price,po_item.unit,'.$sql_join.' 
    order by  md.nama_desc asc, mc.nama_color asc, msn.urutan asc 
...
if (mysqli_num_rows($result)) {
    ...
    <tr>
        ...
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:10% "><p style="text-align:right;">'.number_format($row["price"],$decim).'</p></td>
        <td nowrap style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:20% "><p style="text-align:right;">'.$row["curr"].' '.number_format($row["totalin"], $decim).'</p></td>
    </tr>';
    $disc = $row["discount"];
    $jumlah += $row["totalin"];
    $taxnya = $row["tax"];
...
<body >
    ...
    <p style="font-size:14px">Purchase Order Confirmation '.$PONO.' '.$revise.' '.$wsno.'</p>

    <table style="width:70%;font-size:12px;">
        <tr>
            <td style="width:40%"><p style="font-weight:bold;">Our Order Reference:</p></td>
            <td style="width:30% "><p style="font-weight:bold;">Order Date:</p></td>            
            <td style="width:35% "><p style="font-weight:bold;">Validate By:</p></td>
    ...
    <table align="right" style="width:50%; margin-top:15px; border-collapse: collapse;font-size:12px;">
            <tr>
                ...
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; border-top:solid 2px black; padding:5px;"><p style="text-align:right;">'.$currency.' '.number_format($jumlah,$decim).'</p></td>
            </tr>
            <tr>
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; border-top:solid 2px black; padding:5px;"><p style="font-weight:bold">Discount</p></td>
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; border-top:solid 2px black; padding:5px;"><p style="text-align:right;">'.$currency.' '.number_format($disc,$decim).'</p></td>
            </tr>
            <tr>
                <td style="border-collapse: collapse; border-bottom:solid 2px black; padding:5px;"><p style="font-weight:bold">PPN ('.$taxnya.'%)</p></td>
                <td style="border-collapse: collapse; border-bottom:solid 2px black; padding:5px;"><p style="text-align:right;">'.$currency.' '.number_format($bunga,$decim).'</p></td>
            </tr>
            <tr>
                <td style="padding:5px;"><p style="font-weight:bold">Total</p></td>
                <td style="padding:5px;"><p style="text-align:right; font-size:10px; font-weight:bold">'.$currency.' '.number_format($nambah,$decim).'</p></td>
            </tr>
    </table>
