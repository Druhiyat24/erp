...
{ $query = mysql_query("SELECT a.*,s.curr,if(pono like '%GLB%','GLB','STD') cek_po FROM po_header a inner join po_item s on a.id=s.id_po 
...
<script type='text/javascript'>
  ...
  function add_item()
  { window.location.href='../pur/?mod=3ea';
  };
  function validasi()
...
<script type="text/javascript">
  ...
  function getJO_add_item()
  { var id_jo = $('#cboJO').val();
    var id_supp = document.form.txtid_supplier.value;
    var jenis_item = document.form.txtJItem.value;
    var cek_po = document.form.txtcek_po.value;
    if (cek_po == 'STD')
    {
      cek_url ='ajax_po.php?modeajax=view_list_jo';
    }
    else
    {
      cek_url ='ajax_po.php?modeajax=view_list_jo_global';
    }
    var html = $.ajax
    ({  type: "POST",
        url: cek_url,
    ...
    <div class='row'>
      <form method='post' name='form' action='s_po.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        ...
        <div class='col-md-3'>
          ...
          <div class='form-group'>
            <label>Kurs</label>
            <input row='5' class='form-control' name='n_kurs' placeholder='Masukkan Kurs'><?php echo $n_kurs;?></input>
          </div>
          <?php if ($mod=="3ea") { ?>
          <div class='form-group'>
            <label>Job Order # *</label>
            <input type='hidden' readonly class='form-control' name='txtcek_po' value='<?php echo $cek_po;?>' >
            <select class='form-control select2' multiple='multiple' style='width: 100%;' 
              name='txtJOItem' id='cboJO' onchange='getJO_add_item()'>
            <?php 
            $id_supp = $id_supplier;
            $jenis_item = $jenis_item;
            if($pr_need_app=="Y") { $sql_app=" and a.app='A' "; } else { $sql_app=""; }
            if ($cek_po == "STD"){
              $sql = "select tmppr.Y_tampil,tmppr.id isi,tmppr.vtampil tampil from  
              (select mi.id_item,CONCAT(mi.itemdesc,'-',mi.add_info)item_nya,a.id,concat(jo_no,' - ',ms.supplier,' - ',ac.styleno) vtampil,
              SUM(sod.qty*s.cons) qtybom,tmppo.qty_po
							,if(SUM(sod.qty*s.cons) > ifnull(tmppo.qty_po,0),'Y','N'  )Y_tampil
							from 
			  ...
			  inner join masteritem mi ON mi.id_gen = s.id_item
              inner join so_det sod on s.id_so_det=sod.id  
              left join (select id_jo,id_gen,sum(qty) qty_po from po_item where cancel='N' group by id_jo,id_gen) tmppo 
              on tmppo.id_jo=s.id_jo and tmppo.id_gen=s.id_item
              where (s.id_supplier='$id_supp' or s.id_supplier2='$id_supp') 
              and s.status='$jenis_item' and s.cancel='N' $sql_app group by jo_no,mi.id_item) tmppr 
              where tmppr.Y_tampil = 'Y' GROUP BY tmppr.id";             
            }
            else if ($cek_po == "GLB")
            {
              $sql = "select jo.id isi, CONCAT(jo.jo_no, ' - ', ms.supplier, ' - ', ac.styleno ) tampil 
    ...
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        $sql="select if(t_it_po_cx=t_it_po,'Cancelled','') status_po,a.app,a.id,pono,podate,supplier,
          nama_pterms,tmppoit.buyer,a.notes,a.n_kurs,a.revise,tmppoit.kpno,tmppoit.jo_no,tmppoit.styleno from po_header a inner join 
          ...
          (select group_concat(distinct jo_no) jo_no,ac.kpno,ac.styleno,poit.id_jo,poit.id_po,ms.supplier buyer,count(*) t_it_po,
            sum(if(poit.cancel='Y',1,0)) t_it_po_cx from po_item poit 
