...
$akses = flookup("cutting_output","userpassword","username='$user'");
if ($akses=="0") 
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_cut.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">List Cutting Output</h3>
    <a href='../prod/?mod=2' class='btn btn-primary btn-s'>
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $query = mysql_query("select ac.kpno,ac.styleno,ms.supplier,co.dateinput,co.dateoutput,
          co.qty qtyout,sod.id,so.so_no,so.buyerno,sod.dest,sod.color,sod.size,sod.qty,so.unit
          from so inner join so_det sod on so.id=sod.id_so
          inner join act_costing ac on so.id_cost=ac.id 
          inner join mastersupplier ms on ac.id_buyer=ms.id_supplier 
          inner join cut_out co on sod.id=co.id_so_det order by co.dateoutput desc "); 
        $no = 1; 
