...
?>
	...
	<script type='text/javascript'>
		...
		var id_buyer = document.form.txtid_buyer.value;
			...
			var attach_file = document.getElementById('output').src;

			var attach_file2 = document.getElementById('txtattach_file');
		

			if (id_buyer == '') { swal({ title: 'Nama Buyer Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (styleno == '') { swal({ title: 'Style # Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (id_smode == '') { swal({ title: 'Ship Mode Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (qty == '') { swal({ title: 'Qty Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (smv_min == '') { document.form.txtsmv_min.focus(); swal({ title: 'SMV (Min) Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (smv_sec == '') { document.form.txtsmv_sec.focus(); swal({ title: 'SMV (Sec) Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (book_min == '') { document.form.txtbook_min.focus(); swal({ title: 'Book (Min) Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (book_sec == '') { document.form.txtbook_sec.focus(); swal({ title: 'Book (Sec) Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

			 if (deldate == '') { document.form.txtdeldate.focus(); swal({ title: 'Delivery Date Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}

		//	 if (file_exists(attach_file)) { document.form.txtattach_file.focus(); swal({ title: 'Attach File Tidak Boleh Kosong', <?=$img_alert ?> }); return  false;}
			if(!ValidateFileUpload(attach_file2)){
				return false;
			}
		...
		<div class='row'>
				...
				<div class='col-md-3'>								
					...
					<script>
					  ...
					  if(output.src != ""){
						...
						swal({ title: 'Gambar Tidak Boleh Kosong!', <?php echo $img_alert; ?> }); 
            ...
            <?php } ?>
				...
				<div class='col-md-3'>				
      			...
      			<input type="file" name='txtattach_file' id='txtattach_file' accept="image/*" onchange="loadFile(event)">
...
$sql="select a.id,cost_no,cost_date,supplier,styleno,
		    	...
		    	a.username,a.dateinput,kpno,a.revise from act_costing a inner join mastersupplier s 
		    	...
		    	$sql_where ORDER BY deldate DESC";				
...
//echo $sql;				
			  ...
			  {	echo "
					...
					<tr>
						...
						<td>$rs[fullname] ($rs[dateinput])</td>
