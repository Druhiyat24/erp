...
class Model{
    ...
    public function get_report($jo_id)
    {
        $sql = "
            ...
            FROM 
                jo_dev jo
                INNER JOIN jo_det_dev jd ON jo.id = jd.id_jo
                INNER JOIN so_dev so ON so.id=jd.id_so
                LEFT JOIN masterseason mseas ON so.id_season=mseas.id_season
                INNER JOIN so_det_dev sod ON so.id = sod.id_so
...
?>
    ...
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td class="text-left" colspan="7"><?=$jo->product_group?></td>
        </tr>
    </table>
    ...
    <?php if(count((array) $so)):?>
        ...
        <?php if(count((array) $so_total->dests)):?>
            ...
            <table class="table table-bordered">
                <thead>
                <tr>
                    <th>Total Assortment Based Destination</th>
                    <?php foreach($so_total->sizes as $size):?>
                        <th><?=$size?></th>
                    <?php endforeach;;?>
                    <th>TOTAL</th>
                ...
                <?php foreach($so_total->dests as $dest):?>
                    <tr>
                        <th><?=$dest?></th>
                        <?php foreach($so_total->sizes as $size):?>
                            <td><?=isset($so_total->dest->$dest->$size) ? number_format($so_total->dest->$dest->$size) : 0?></td>
                        <?php endforeach;?>
                        <td><?=$so_total->dest->SUM_DEST->$dest?></td>
                    </tr>
                <?php endforeach;?>
                <tr>
                    <th>TOTAL</th>
                    <?php foreach($so_total->sizes as $size):?>
                        <td><?=isset($so_total->dest->SUM_SIZE->$size) ? number_format($so_total->dest->SUM_SIZE->$size) : 0?></td>
                    <?php endforeach;?>
                    <td><?=$so_total->dest->SUM_ALL?></td>
                </tr>
                </tbody>
            </table>
        <?php endif;?>
