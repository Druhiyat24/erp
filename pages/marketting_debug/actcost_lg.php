...
{	$query = mysql_query("SELECT * FROM act_costing where id='$id_item'");
	...
	$buyer=$data['id_buyer'];
	$kdbuyer=flookup("supplier_code","mastersupplier","id_supplier='$data[id_buyer]'");
...
{	echo "
	<script type='text/javascript'>
		...
		{	var id_buyer = document.form.txtid_buyer.value;
			var kode_buyer = document.form.txtkodecust.value;
			...
			else if (kode_buyer == '') { swal({ title: 'Kode Buyer Tidak Boleh Kosong', $img_alert }); valid = false;}
...
<script type="text/javascript">
  ...
  { var html = $.ajax
    ...
    { $("#cbopr_it").html(html); }
  	jQuery.ajax
    ({  
      url: "ajax_pre_cost.php?mdajax=get_part_name",
      method: 'POST',
      data: {cri_item: cri_item},
      dataType: 'json',
      success: function(response)
      {	var $newOption = $("<option selected='selected'></option>").val(response[0]).text(response[1])
        $("#cbopr_it").append($newOption).trigger('change');
      },
      error: function (request, status, error) 
      { alert(request.responseText); },
    });
  };
  function getModel(cri_item)
  { jQuery.ajax
    ({  
      url: "ajax_pre_cost.php?mdajax=get_model_LG",
      method: 'POST',
      data: {cri_item: cri_item},
      dataType: 'json',
      success: function(response)
  ...
  function cek_kode_cust()
  { var id_buyer = document.form.txtid_buyer.value;
  	jQuery.ajax
    ({  
      url: "ajax_act_cost.php?mdajax=get_kode_cust",
      method: 'POST',
      data: {id_buyer: id_buyer},
      dataType: 'json',
      success: function(response)
      {	$('#txtkodecust').val(response[0]); },
      error: function (request, status, error) 
      { alert(request.responseText); },
    });
  };
  ...
  { var id_item_cd = document.getElementById('txtid_item_cd').value;
    jQuery.ajax
    ({  
      url: "ajax_act_cost.php?mdajax=get_base_default",
      method: 'POST',
      data: {id_item_cd: id_item_cd},
      dataType: 'json',
      success: function(response)
      { if (response[0] == 'IDR')
        { $('#txtprice_idr_cd').val(response[1]); 
          $('#txtprice_cd').val(0);
        }
        else
        { if ($('#txtprice_idr_cd') == '')
          { $('#txtprice_idr_cd').val(0); 
            $('#txtprice_cd').val(response[1]);
          }
        }
        $('#txtmaterial_source_cd').select2('val',[response[2]]); 
      },
      error: function (request, status, error) 
      { alert(request.responseText); },
    });
...
<div class='box'>
	<div class='box-body'>
		<div class='row'>
				...
				<div class='col-md-3'>				
					<div class='form-group'>
						...
						<select class='form-control select2' style='width: 100%;' 
							name='txtid_buyer' onchange='cek_kode_cust()'>
			      ...
			      <input type='hidden' name='txtkodecust' id='txtkodecust' value='$kdbuyer'>
					</div>				
					<div class='form-group'>
            ...
            <?php if ($id_cs=="") { ?>
            <select class='form-control select2' style='width: 100%;' 
            	id='cbopr_it' name='txtid_product' onchange="getModel(this.value)">
            </select>
            <?php } else { ?>
            <select class='form-control select2' style='width: 100%;' 
            ...
            <?php } ?>
					...
					<div class='form-group'>
						...
						<input type='text' class='form-control' 
							name='txtstyle' id='txtstyle' value='<?php echo $styleno; ?>' readonly>
					</div>
					<div class='form-group'>
						<label>Costing #</label>
  ...
  <div class="box-body">
  	<div class='col-md-3'>								
			<div class='form-group'>
				...
				<select class='form-control select2' style='width: 100%;' 
					id='txtid_item_cd' onchange='getBaseData()'>
					<?php
						if($nm_company=="PT. Youngil Leather Indonesia")
						{	$listmat="('A','F','M','C')";	}
						else
						{	$listmat="('A','F','M')";	}
						$sql = "select id_item isi,concat(IFNULL(goods_code,'-'),' ',IFNULL(itemdesc,'-'),' ',
							IFNULL(color,'-'),' ',IFNULL(size,'-')) 
              tampil from masteritem where mattype in $listmat ";
    ...
    <table id="examplefix" class="display responsive" style="width:100%; font-size:12px;">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $query = mysql_query("SELECT mattype,
		    	concat(goods_code,' ',itemdesc,' ',ifnull(color,''),' ',ifnull(size,'')) itemdesc,
		    	price,cons,s.unit,allowance,material_source,s.id,s.jenis_rate 
		    	from act_costing a inner join act_costing_mat s on 
		    	a.id=s.id_act_cost inner join masteritem d on s.id_item=d.id_item
        ...
        $no = 1; 
            ...
            echo "<td>$data[mattype]</td>";
						...
						echo "
						<td>
							<button class='btn btn-info' onclick='edit_cd($data[id])'>
					  		<i class='fa fa-pencil'></i>
					  	</button>
					  	<a $cl_hapus href='d_add_item_cs.php?mod=$mod&mode=$mode&
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">Manufacturing - <?php echo $caption[6]?></h3>
  	<!--
  ...
  <div class="box-body">
  	<div class='col-md-3'>								
			<div class='form-group'>
				<label><?php echo $caption[6]?> *</label>
				<select class='form-control select2' style='width: 100%;' 
					id='txtid_item_mf'>
					<?php 
						$sql = "SELECT id_item isi,
              concat(goods_code,' ',itemdesc) tampil
              FROM masteritem where mattype='C'  
              ORDER BY goods_code DESC";
						IsiCombo($sql,$id_item_mf,'Pilih '.$caption[6]);
					?>
				</select>
			</div>				
    ...
    <table id="example1" class="display responsive" style="width:100%;font-size:12px;">
      ...
      <tbody>
        <?php
        # QUERY TABLE
		    $query = mysql_query("SELECT d.goods_code cfcode,d.itemdesc cfdesc,price,cons,s.unit,
        ...
        $no = 1; 
            ...
            echo "<td>$data[cfcode]</td>";
						...
						echo "
						<td>
							<button class='btn btn-info' onclick='edit_mf($data[id])'>
					  		<i class='fa fa-pencil'></i>
					  	</button>
					  	<a $cl_hapus href='d_add_mfg_cs.php?mod=$mod&mode=$mode&
					  		id=$id_cs&idd=$data[id]&mod=$mod'
		            $tt_hapus";?> 
		            onclick="return confirm('Apakah anda yakin akan menghapus ?')">
		            <?PHP echo $tt_hapus2."</a>
      ...
      <tbody>
        ...
        $no = 1; 
            ...
            echo "<td>$data[otherscode]</td>";
						...
						echo "
						<td>
							<button class='btn btn-info' onclick='edit_ot($data[id])'>
					  		<i class='fa fa-pencil'></i>
					  	</button>
							<a $cl_hapus href='d_add_oth_cs.php?mod=$mod&mode=$mode&
					  		id=$id_cs&idd=$data[id]&mod=$mod'
		            $tt_hapus";?> 
		            onclick="return confirm('Apakah anda yakin akan menghapus ?')">
		            <?PHP echo $tt_hapus2."</a>
...
<div class="box">
  ...
  <div class="box-body">
      ...
      <tbody>
        <?php
        # QUERY TABLE
		    if ($groupp=="DIREKTUR") { $sql_where=""; } else { $sql_where="where d.kode_mkt='$kd_mkt'"; }
		    $sql="select a.id,cost_no,cost_date,supplier,styleno,
			  ...
			  {	$cek=flookup("id","so","id_cost='$rs[id]'");
          if ($cek!="") {$bgcol=" style='color: red; font-weight:bold;'";} else {$bgcol="";}
          echo "
					<tr $bgcol>
						<td>$rs[cost_no]</td>
            ...
            if ($rs['username']==$user OR $groupp=="DIREKTUR")
            {	echo "<td><a href='?mod=5&id=$rs[id]'
              data-toggle='tooltip' title='View'><i class='fa fa-pencil'></i></a></td>"; 
            }
            else
            { echo "<td></td>"; }
            echo "
