...
$id_item=$_GET['id'];
$rscs=mysqli_fetch_array(mysqli_query($con_new, "select * from act_development where id='$id_item'"));
//print_r($rscs);
...
$tot_cd = flookup("sum((if(jenis_rate='B',price/rate_beli,price)*cons)+((if(jenis_rate='B',price/rate_beli,price)*cons)*allowance/100))",
    ...
    inner join (SELECT * FROM masterrate WHERE v_codecurr IN('COSTING3','COSTING6','COSTING8','COSTING12')) d on 'USD'=d.curr and s.deldate=d.tanggal","id_act_cost='$id_item'");
...
class Model{
     ...
     * @param $act_development_id Actual Costing ID
     * @return array
     */
    public function get_report_fabric($act_development_id){
        return $this->query("SELECT *, 
          ...
          WHERE a.id = $act_development_id
            AND nama_group LIKE '%FABRIC%' and price > 0 ")->result();
    }

    public function get_report_accesories($act_development_id){
        return $this->query("SELECT *, 
                d.nama_group mattype,
                concat(nama_group,' ',nama_sub_group,' ',nama_type,' ',nama_contents) itemdesc,
                price,
          ...
          INNER JOIN mastersupplier i ON a.id_buyer=i.Id_Supplier
          WHERE a.id = $act_development_id
            AND nama_group LIKE '%ACCESORIES%' and price > 0  ")->result();
    }

    public function get_report_manufacturing($act_development_id){
        return $this->query("SELECT * FROM act_development a INNER JOIN act_development_mfg s ON a.id=s.id_act_cost 
                             INNER JOIN mastercf i ON s.id_item = i.id
                             WHERE a.id = $act_development_id
                             ")->result();
    }

    public function get_report_other($act_development_id){
        return $this->query("SELECT * FROM act_development a INNER JOIN act_development_oth s ON a.id=s.id_act_cost 
...
$sql="SELECT s.supplier,d.product_item,a.qty,up.fullname,up2.fullname app1_by_name,app1_date,
    ...
    from act_development a inner join mastersupplier s on a.id_buyer=s.id_supplier 
