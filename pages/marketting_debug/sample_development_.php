...
{	$query = mysql_query("SELECT * FROM act_development where id='$id_item'");
//echo "QUERY: SELECT * FROM act_costing where id='$id_item'";
	...
	$tot_cd = flookup("sum((if(jenis_rate='B',price/rate_beli,price)*cons)+((if(jenis_rate='B',price/rate_beli,price)*cons)*allowance/100))",
		"act_development_mat a inner join act_development s on a.id_act_cost=s.id
	...
	$tot_mf = flookup("sum((if(jenis_rate='B',price/rate_beli,price)*cons)+((if(jenis_rate='B',price/rate_beli,price)*cons)*allowance/100))",
		"act_development_mfg a inner join act_development s on a.id_act_cost=s.id
		inner join masterrate d on 'USD'=d.curr and s.cost_date=d.tanggal","id_act_cost='$id_item'");
	...
	$tot_ot = flookup("sum(if(jenis_rate='B',price/rate_beli,price))",
		"act_development_oth a inner join act_development s on a.id_act_cost=s.id
		inner join masterrate d on 'USD'=d.curr and s.cost_date=d.tanggal","id_act_cost='$id_item'");
...
<script src="js/SampleDevelopment.js"></script>
<script type="text/javascript">
  ...
  { var smvnya = document.form.txtsmv_min.value;
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=calc_smv_min",
      method: 'POST',
  ...
  { 
	...
	if(Data == 'edit'){
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=calc_usd_idr",
      method: 'POST',
  ...
  { 
	...
	if(data == 'edit'){
  	...
  	var tglnya = document.form.txtcost_date.value;
	tanggalGlobal = document.form.txtcost_date.value;
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=calc_usd_idr",
      method: 'POST',
...
console.log($('#txtprice_idr_mf2').val());		
  ...
  var pxnya = document.getElementById('txtprice_idr_cd').value;
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=calc_idr_usd",
      method: 'POST',
      data: {pxnya: pxnya,tglnya: tglnya},
      dataType: 'json',
      success: function(response)
  ...
  { var smvnya = document.form.txtsmv_sec.value;
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=calc_smv_sec",
      method: 'POST',
      data: {smvnya: smvnya,qtynya: qtynya},
      dataType: 'json',
      success: function(response)
...
<div class='box'>
	<div class='box-body'>
		<div class='row'>
			<form method='post' name='form' enctype='multipart/form-data' action="s_sample_development.php?mod=<?php echo $mod; ?>&id=<?php echo $id_cs; ?>&pro=<?php echo $pro; ?>" onsubmit='return validasi()'>
				<div class='col-md-3'>								
					<?php if ($id_cs=="") { ?>
            ...
            <?php } ?>
					...
					<div class='form-group'>
						...
						<input type='text' class='form-control' readonly name='txtcost_no' placeholder='Masukkan Sample Development #' value='<?php echo $cost_no;?>' >
					</div>				
					<div class='form-group'>
					...
					<div class='form-group'>
						...
						<input type='text' class='form-control' readonly name='txtkpno' placeholder='Masukkan Number Sample #' value='<?php echo $kpno;?>' >
					</div>					
				</div>
				<div class='col-md-3'>
					<div class='form-group'>
					...
					<div class='form-group'>
						<label>SMV (Min) *</label>
						<input type='text' class='form-control' readonly name='txtsmv_min' id='txtsmv_min' placeholder='Masukkan SMV (Min)' onchange='CalcSMVMin()' value='<?php echo $smv_min;?>' >
					</div>				
					<div class='form-group'>
						<label>SMV (Sec) *</label>
						<input type='text' class='form-control' readonly name='txtsmv_sec' id='txtsmv_sec' placeholder='Masukkan SMV (Sec)' onchange='CalcSMVSec()' value='<?php echo $smv_sec;?>' >
					</div>				
					<div class='form-group'>
				...
				<div class='col-md-3'>				
					...
					<div class='form-group'>
						<label>Status *</label>
						<select class='form-control select2' style='width: 100%;' 
							name='txtstatus' id='xstatus'>
							<?php 
								$sql = "select nama_pilihan isi,nama_pilihan tampil from 
									masterpilihan where kode_pilihan='ST_CST_DEV'";
								IsiCombo($sql,$status,'Pilih Status');
								echo "<script>$('#xstatus').val('$status').triger('change')</script>";
							?>
						</select>
					</div>
					<div class='form-group'>
...
<select name ="categorys"  id="categoryFilter"  onchange="filterCategory(this)" class="form-control" style="width:auto">
	...
	<option value="Sample Detail">Sample Detail</option>
	<option value="Manufacturing - Complexity">Manufacturing - Complexity</option>
	<option value="Other Cost">Other Cost</option>
</select>
		
	
	<table id="example1_new" class="display responsive" style="auto;font-size:13px;">
      <thead>
      <tr>
				...
				<th>Value (IDR)</th>
				<th >Percent</th>
				<th style="text-align:left">Material Source</th>
...
SELECT 	d.nama_group cod,
		concat(nama_group,' ',nama_sub_group,' ',nama_type,' ',nama_contents)  it,
		 ...
		 a.id idCostings,
		 '1' category
		    	from act_development a 
				inner join act_development_mat s on 
...
SELECT d.otherscode cod,
		...
		a.id idCostings,
		'3' category 
		    	from act_development a inner join act_development_oth s on 
		...
		$sumOtherCostUsd = 0;
			  ...
			  { 
		  									...
		  									$urls = '';
						if($data['category']  == '1'){
							$urls = 'd_add_item_cs_dev.php';
						}
						if($data['category'] == '2' ) {
							$urls = 'd_add_mfg_cs_dev.php';
						}
						if($data['category'] == '3') {
							$urls = 'd_add_oth_cs_dev.php';
						}	
		  ...
		  echo "<tr>";
			...
			echo "<td style='display:none'>$data[id] </td>";
						...
						echo "<td style='text-align:center'>".fn($persens,2)."</td>";
						echo "<td style='text-align:center'>$data[material_source]</td>";
						echo "
						<td>
...
<table id="example1_news" class="display responsive" style="auto;font-size:13px;">

		  <tr>
			  <td style="padding:0" > <select name ="category"  id="category"onchange="changecategory(this)" class="form-control" style="width:100px;display:none" >
						<option value="1">-- Choose Kategory --</option>

					</select>
				<div class="selectCD" style="display:none">
							<select class='form-control select2' style='width: 100px;padding:0;position:absolute' 
					id='txtid_item_cd'>
					<?php 
						$sql = "select e.id isi,concat(nama_group,' ',nama_sub_group,' ',nama_type,' ',nama_contents) 
              tampil from mastergroup a inner join mastersubgroup s on 
			...
			<td style="width:80px">	
			 ...
			 <div class="selectMC" style="display:none">
			  ...
			  <td>&nbsp;</td>
			  <td style="float:right">				 <div class="selectCD" style="display:none">
				<select class='form-control select2'  
			  ...
			  <td>				&nbsp; </td>
			  <td>	<a href="#" onclick='Routes("add")' /><button class='btn btn-primary'>Add Item</button> </a> 
  	...
  	<table width="70%" style="font-weight:bold;">
  		...
  		<tr>
  			...
  			<td id="footerCommissionFeeidr"><?php echo "0" ?></td>
		  		...
		  		<tr>
  			<td>Total Sample (USD)</td>
  			<td>&nbsp:&nbsp</td>
  			<td id="footerTotalCosting"><?php echo "0"; ?></td>
  			<td>&nbsp &nbsp</td>
  			<td>Total Sample (IDR)</td>
  			<td>&nbsp:&nbsp</td>
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">List Sample Development</h3>
    <a href='../marketting/?mod=2X' class='btn btn-primary btn-s'>
...
$sql="select a.id,cost_no,cost_date,supplier,styleno,
		    	qty,deldate,fullname,status,mp.product_item,mp.product_group,
		    	a.username from act_development a inner join mastersupplier s 
		    	on a.id_buyer=s.id_supplier inner join userpassword d 
		    	on a.username=d.username inner join masterproduct mp 
		    	on a.id_product=mp.id
		    	$sql_where ORDER BY cost_date DESC";				
//echo $sql;				
			  ...
			  {	echo "
					<tr>
						...
						<td>$rs[fullname]</td>
						<td>
							<a class='btn-s' href='pdfSampleDevelopment.php?id=$rs[id]'
							data-toggle='tooltip' title='Save PDF' target='_blank'><i class='fa fa-file-pdf-o'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	            if ($rs['username']==$user)
	            {	echo " <a class='btn-s' href='?mod=2X&id=$rs[id]'
	              data-toggle='tooltip' title='Edit'><i class='fa fa-pencil'></i></a>&nbsp;&nbsp;&nbsp;"; 
	            }
	            echo "
	            <a class=' btn-s' href='?mod=2X&id=$rs[id]&pro=Copy'
...
*/	
  ...
  {		
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=edit_cost_detail",
      method: 'POST',
...
//ComplexityModal.showModal();  
 // var id_cs = <?php echo $id_cs; ?>;
  var id_cs = <?php echo "'".$id_cs."'"; ?>;
  	...
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=edit_cost_mf",
      method: 'POST',
      data: {id_cs: id_cs,id_cd: id_cd},
      dataType: 'json',
      success: function(response)
 ...
 var id_cs = <?php echo "'".$id_cs."'"; ?>;
  	jQuery.ajax
    ({  
      url: "ajax_act_sample_development.php?mdajax=edit_cost_ot",
      method: 'POST',
      data: {id_cs: id_cs,id_cd: id_cd},
      dataType: 'json',
      success: function(response)
...
//  	var cek_so = "<?php echo flookup("so_no","so","id_cost='$id_cs'"); ?>";
 ...
 /*   });
  	...
  	{swal({ title: 'Sample Development Tidak Bisa Dirubah Karena Sudah Dibuat SO', <?php echo $img_alert; ?> }); 
			exit;
		}
...
//		else if (unit_mf == '')
  ...
  {	
   //var id_cs = <?php echo $id_cs; ?>;
	...
	console.log(id_ot);
	console.log(j_rate_ot);
	console.log(id_item_ot);
	console.log(price_ot);
	console.log(price_idr_ot);
	}
		if(Data == 'edit'){
