...
if ($modenya=="view_list_jo" or $modenya=="view_list_req")
{	echo "<head>";
...
{ $jenis_item = $_REQUEST['jenis_item'];
  if($pr_need_app=="Y") { $sql_app=" and a.app='A' "; } else { $sql_app=""; }
  $sql = "select d.id_supplier isi,concat(supplier,' (',if(pkp='','NON PKP',pkp),')') tampil from 
    jo_dev a inner join bom_dev_jo_item s on a.id=s.id_jo
    ...
    union all 
    select d.id_supplier isi,concat(supplier,' (',if(pkp='','NON PKP',pkp),')') tampil from 
    jo_dev a inner join bom_dev_jo_item s on a.id=s.id_jo
  ...
  ?>
    ...
    <tbody>
      ...
      { $sql="select ac.id id_cs,k.id_jo,0 idsubgroup,k.id_item,l.color,l.size,
        ...
        k.unit,m.supplier,ac.kpno 
        from bom_dev_jo_item k inner join so_det_dev l on k.id_so_det=l.id
        inner join so_dev so on so.id=l.id_so inner join act_development ac on ac.id=so.id_cost 
      ...
      { $query = mysql_query($sql); 
        ...
        { echo "<tr>";
            ...
            $qtySdhPO=flookup("sum(qty)","po_item","id_jo='$idjo2' and id_gen='$id2' and cancel='N' group by id_gen");
            #$qtyPO=($allowq+$data['qty_bom']) - $sisa_stock;
            $qtyPO=($allowq+$data['qty_bom']);
            $qtyPO=$qtyPO - $qtySdhPO;
            if ($jenis_company=="VENDOR LG")
            { $price_cs=flookup("price","act_development_mat","id_item='$data[id_item]' 
            ...
            echo "
            <td>
              <input type ='text' style='width:70px;' name ='itemprice[$id]' class='priceclass' value='$price_cs'>
  ...
  <table id="examplefix" class="display responsive" style="width:100%">
    ...
    <tbody>
      ...
      $sql="select rh.notes,k.id_reqno,0 idsubgroup,k.id_item,j.color,j.size,
        ...
        where k.id_reqno in ($id_req) group by k.id_reqno,k.id_item";
      #echo $sql;
      if ($id_req!='""')
      { $query = mysql_query($sql); 
        $no = 1; 
