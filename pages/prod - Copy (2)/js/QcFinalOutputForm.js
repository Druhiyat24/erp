$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Qc_F_Out.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_qc_final_out_header;
	}
 	setTimeout(function(){
		table.ajax.url( "webservices/getListQcFinalOutputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300); 
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListQcFinalOutputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columnDefs": [
					{
				...
				"columns": [
					...
					},					
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.balance_qc_final_out);
						},
					},
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_qc_final_out);
						},
					},	
					{
						"data": null,
...
function Save() {
	...
	getTable.each(function (i) {
		...
		if($tds.length > 0){
			...
			_js.is_qty					= $("#is___qty_qc_final_out__"+$serpihan_id[2]).val();
...
/* 		_js = {};
		...
		_js = {
			...
			is_id_sew_qc_final_out_header_det	: $tds.data('id_sew_qc_final_out_header_det'),
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_QcFinalOutputForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					setTimeout(function(){
						window.location = "../prod/?mod=QC_F_O_Page";
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_qc_final_out_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/QcFinalOutputForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	if(kondisi =='1'){
	return [
		Data.id_sew_qc_final_out_header		= decodeURIComponent(json.records[0].id_sew_qc_final_out_header),	
		...
		Data.no_sew_qc_final_out       = decodeURIComponent(json.records[0].no_sew_qc_final_out)                     
	];                                           
	}
	else{
		return 1;
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_sew_qc_final_out").val(json.no_sew_qc_final_out),
