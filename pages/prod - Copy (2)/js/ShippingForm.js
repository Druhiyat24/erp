$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Shp.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_finishing_shp_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListShippingDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListShippingDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_shp);
						},
					},	
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_ShippingForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=SHP_Page";
					//window.location = "../prod/?mod=SHP_Form&id_finishing_shp_header="+d.id_finishing_shp_header;
				}
				if (d.respon == '201'){
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_finishing_shp_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/ShippingForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_finishing_shp_header			= decodeURIComponent(json.records[0].id_finishing_shp_header),	
		...
		Data.no_finishing_shp          = decodeURIComponent(json.records[0].no_finishing_shp)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_finishing_shp").val(json.no_finishing_shp),
