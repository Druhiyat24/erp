$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Sew_Qc_Out.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_qc_out;
	}
 	setTimeout(function(){
		table.ajax.url( "webservices/getListSewingQcOutputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300); 
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListSewingQcOutputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columnDefs": [
					{
				...
				"columns": [
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_sew_qc_out);
						},
					},	
					{
						"data": null,
...
function Save() {
	...
	getTable.each(function (i) {
		...
		for(var i=0;i<$tds.length;i++){
			...
			_js.is_qty_qc_output		= $tds[i].value;
		...
		if($tds.length > 0){
			...
			_js.is_qty_qc_output		= $("#is___qty_sew_qc_out__"+$serpihan_id[2]).val();
			...
			_js.is_id_sew_qc_out_det	= $("#"+$tds[0].id).attr('data-id_sew_qc_out_det');
...
/* 		_js = {};
		...
		_js = {
			...
			is_id_sew_qc_out_det	: $tds.data('id_sew_qc_out_det'),
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/saveSewingQcOutput.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					setTimeout(function(){
						window.location = "../prod/?mod=11WP";
						//window.location = "../prod/?mod=11LA&id_sew_qc_out="+d.id_sew_qc_out;
				...
				if (d.respon == '203') {
					hide_loading();
					alert('Qty Input Must be Greater Then 0 On Row '+d.baris);
					return false;
				}	
				if (d.respon == '204') {
					hide_loading();
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Sewing Qc Out Must be Smaller Than Qty Steam! On Row "+d.baris);
					return false;
				}	
				if (d.respon == '207') {
					hide_loading();
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_qc_out
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/Sew_Qc_Out_GetListData.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	if(kondisi =='1'){
	return [
		Data.id_sew_qc_out		= decodeURIComponent(json.records[0].id_sew_qc_out_header),	
		...
		Data.no_sew_qc_out       = decodeURIComponent(json.records[0].no_sew_qc_out)                     
	];                                           
	}
	else{
		return 1;
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_sew_qc_out").val(json.no_sew_qc_out),
