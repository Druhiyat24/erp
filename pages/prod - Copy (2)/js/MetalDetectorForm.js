$(window).on("load", function () {
	...
	format = 1;		
	Data = {
		id_finishing_metal_detector_header: '0',
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Met_Det.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_finishing_metal_detector_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListMetalDetectorDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListMetalDetectorDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
					...
					},						
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.balance_metal_detector);
						},
					},
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_metal_detector);
						},
					},	
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_MetalDetectorForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=Met_Dec_Page";
					//window.location = "../prod/?mod=Met_Dec_Form&id_finishing_metal_detector_header="+d.id_finishing_metal_detector_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Metal Detector Must be Smaller Than Qty Qc Audit Buyer!");
					return false;
				}				
			}
		});
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_finishing_metal_detector_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/MetalDetectorForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_finishing_metal_detector_header			= decodeURIComponent(json.records[0].id_finishing_metal_detector_header),	
		...
		Data.no_finishing_metal_detector          = decodeURIComponent(json.records[0].no_finishing_metal_detector)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_finishing_metal_detector").val(json.no_finishing_metal_detector),
