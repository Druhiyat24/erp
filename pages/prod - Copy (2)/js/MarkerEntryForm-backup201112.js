...
function del() {
	...
	$.ajax({
		...
		cache: false,
		url: "webservices/saveMarkerEntryDetailAdd.php",
		data: { code: '1', format: format, data: id },
		success: function (response) {
			data = response;
			// console.log(data);
			d = JSON.parse(data);
...
function save_all() {
	...
	$.ajax({
		...
		cache: false,
		url: "webservices/saveMarkerEntryDetail.php",
		data: { code: '1', format: '2', data: Data, id_cost: $id_cost, clr: $color, d_api: d_api, id: $id_url, tab: panel },
		success: function (response) {
			data = response;
			d = JSON.parse(data);
			console.log(d.message);
...
function sync_perhitungan_marker() {
