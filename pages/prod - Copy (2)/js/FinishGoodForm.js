$(window).on("load", function () {
	...
	format = 1;		
	Data = {
		id_finishing_fg_header: '0',
		id_finishing_fg_header_det: '',
		no_finishing_fg :'',
		id_cost: '',
		id_so: '',
		id_line: '',
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Fg.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_finishing_fg_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListFinishGoodDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListFinishGoodDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.tot_qty_audit_buyer);
						},
					},		
					{
						"data": null,
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_fg);
						},
					},	
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_FinishGoodForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=FG_Page";
					//window.location = "../prod/?mod=FG_Form&id_finishing_fg_header="+d.id_finishing_fg_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Finish Good Must be Smaller Than Qty Shippinh!");
					return false;
				}				
			}
		});
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_finishing_fg_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/FinishGoodForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_finishing_fg_header			= decodeURIComponent(json.records[0].id_finishing_fg_header),	
		...
		Data.no_finishing_fg          = decodeURIComponent(json.records[0].no_finishing_fg)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_finishing_fg").val(json.no_finishing_fg),
