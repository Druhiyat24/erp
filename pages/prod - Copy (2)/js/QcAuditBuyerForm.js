$(window).on("load", function () {
	...
	format = 1;		
	Data = {
		id_finishing_qc_audit_header: '0',
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Qc_A_B.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_finishing_qc_audit_header;
	}
 	setTimeout(function(){
		table.ajax.url( "webservices/getListQcAuditBuyerDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300); 
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListQcAuditBuyerDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columnDefs": [
					{
				...
				"columns": [
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.tot_qty_metal_detector);
						},
					},						
					{
						"data": null,
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.balance_audit_buyer);
						},
					},					
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_audit_buyer);
						},
					},	
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___rpr);
						},
					},	
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___select___defect);
						},
					},						
				],
...
function Save() {
	...
	getTable.each(function (i) {
		...
		if($tds.length > 0){
			$pecah_id = $("#"+$tds[0].id).attr('id'); //is___qty_sew_qc_out__
			console.log($pecah_id);
		 	$serpihan_id = $pecah_id.split("__"); 
			_js.is_qty				= $("#is___qty_audit_buyer__"+$serpihan_id[2]).val();
			_js.is_rpr					= $("#is___rpr__"+$serpihan_id[2]).val();
			_js.is_id_defect			= $("#defect__"+$serpihan_id[2]).val();
...
/* 		_js = {};
		...
		_js = {
			...
			is_id_finishing_qc_audit_header_det	: $tds.data('id_finishing_qc_audit_header_det'),
			is_balance				: $tds.data('balance'),
			is_qty_qc_output		: $tds.val(),
		}		
		detail.push(_js); */
	});
	//console.log(Data);
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_QcAuditBuyerForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					setTimeout(function(){
						window.location = "../prod/?mod=QC_A_Page";
						//window.location = "../prod/?mod=QC_A_Form&id_finishing_qc_audit_header="+d.id_finishing_qc_audit_header;
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Qc Audit Buyer Must be Smaller Than Qty Finish Good!");
					return false;
				}	
				if (d.respon == '207') {
					hide_loading();
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_finishing_qc_audit_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/QcAuditBuyerForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	if(kondisi =='1'){
	return [
		Data.id_finishing_qc_audit_header		= decodeURIComponent(json.records[0].id_finishing_qc_audit_header),	
		...
		Data.no_finishing_qc_audit       = decodeURIComponent(json.records[0].no_finishing_qc_audit)                     
	];                                           
	}
	else{
		return 1;
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_finishing_qc_audit").val(json.no_finishing_qc_audit),
