...
<script type="text/javascript">
  ...
  { var id_jo = document.form.txtJOItem.value;
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_sew.php?modeajax=view_list_jo_in',
        data: {id_jo: id_jo},
        async: false
    }).responseText;
    if(html)
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_sew_in.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>
        ...
        <div class='col-md-3'>
          ...
          <div class='form-group'>
            ...
            <select class='form-control select2' style='width: 100%;' 
              ...
              from act_costing #INNER JOIN cut_out ON act_costing.id=cut_out.id_act_costing 
              order by kpno";
...
<div class="box">
  <div class="box-header">
    ...
    <a href='../prod/?mod=8' class='btn btn-primary btn-s'>
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $query = mysql_query("select ac.styleno,ms.supplier,ac.kpno,co.dateinput,co.jam,co.dateoutput,co.qty qtyout,sod.id,so.so_no,so.buyerno,sod.dest,sod.color,sod.size,sod.qty,so.unit
          from so inner join so_det sod on so.id=sod.id_so
          inner join sew_in co on sod.id=co.id_so_det
