...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_tot_qty_steam_out"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.tot_qty_qc_final_in next_qty
			FROM(
			{$sql_balance }
	)TABLE_BALANCE
				LEFT JOIN(
