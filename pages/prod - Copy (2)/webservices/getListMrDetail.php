...
$main_query 				= $Proses->get_main_query($id_jo,$color,$key_det,$id_number,$bppbno_req); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$bppbno_req,$__data,$_key_det){
		...
		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >";
			...
			$_str = "<input type='text' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' >";
		}
		return $_str;
	}
...
/* 					if($j == $trigger){
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				...
				$_attr = "";
				$_attr = $this->generate_attr($_attribut,$_id_roll_det,$bppbno_req,$_data['data'][$i],$_key_det);
				//if ($outp != "") {$outp .= ",";}
 					if($k == $trigger){
...
/*  		print_r($records);
	...
	public function get_main_query($id_jo,$color,$key_det,$id_number,$bppbno_req){
/* 		echo $key_det;
		die(); */
		if($key_det =='1'){
