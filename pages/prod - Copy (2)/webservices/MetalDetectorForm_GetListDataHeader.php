...
$code = $_POST['code'];
$id = $_POST['id'];
 //print_r($id_sew_in);die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id);
...
class Proses {
	...
	public function get_list_data($id){
		$sql = "SELECT id_finishing_metal_detector_header,date_output,no_finishing_metal_detector,notes,id_cost,id_line,username,dateinput,time 
					FROM prod_finishing_metal_detector_header WHERE id_finishing_metal_detector_header ='{$id}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_finishing_metal_detector_header":"'.rawurlencode($row[$i]['id_finishing_metal_detector_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_finishing_metal_detector":"'. rawurlencode($row[$i]["no_finishing_metal_detector"]).'",';	
