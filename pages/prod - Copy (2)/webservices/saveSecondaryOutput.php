...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_id($_no_sec_out){
		$sql = "SELECT id_sec_out_header FROM prod_sec_out WHERE no_sec_out = '{$_no_sec_out}' LIMIT 1";
...
/* 	echo $del;
	...
	public function insert($data,$username,$no_sec_out)
	{
		...
		$insert = "INSERT INTO prod_sec_out (notes,id_cost,username,dateinput,date_output,no_sec_out,inhouse_subkon,dept_subkon) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$username}',
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->tanggalinput));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sec_out SET dept_subkon='{$data->deptsubcon}',inhouse_subkon='{$data->inhousesubcon}', notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sec_out_header = '{$data->id_sec_out_header}'";
