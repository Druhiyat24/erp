...
// $table = "(SELECT c.id, c.kpno, mi.goods_code,mi.id_item, mi.itemdesc
// FROM masteritem AS mi 
// LEFT JOIN act_costing_mat AS cm ON cm.id_item=mi.id_item
// LEFT JOIN act_costing AS c ON c.id=cm.id_act_cost
// INNER JOIN cut_in AS ci ON ci.id_act_costing=c.id 
// WHERE c.id='$id_cost' AND mi.matclass='FABRIC') AS X"; 
...
$table = "(
	...
	INNER JOIN (
		SELECT * FROM cut_in_detail WHERE id_cut_in='{$id_cut_in}'
	)Y ON 
	X.id_cost			   		= Y.id_cost	
	AND X.id_so                = Y.id_so      
...
//echo $row['n_post'];
	...
	$button .="<input type='button' id='cutout_".$row['id_cut_in_detail']."' onclick='Insert(".'"'.$row['id_cut_in_detail'].'"'.")' class='btn btn-primary addrow' value='Tambah'>
				<input type='hidden' 
						...
						id='tmp_html_".$row['id_cut_in_detail']."' value=''>
