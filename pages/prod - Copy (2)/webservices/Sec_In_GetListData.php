...
class Proses {
	...
	public function get_list_data($id_sew_qc_out){
		$sql = "SELECT    A.id_sec_in_header
			,A.date_output
			,A.inhouse_subkon
			,A.dept_subkon
			,A.no_sec_in
			...
			,GROUP_CONCAT(DISTINCT(C.id_proses))id_proses
				...
				INNER JOIN (SELECT id_sec_in_header,id_panel,id_proses FROM prod_sec_in_detail WHERE id_sec_in_header ='{$id_sew_qc_out}' GROUP BY id_panel )B
				ON A.id_sec_in_header = B.id_sec_in_header
				INNER JOIN (SELECT id_sec_in_header,id_panel,id_proses FROM prod_sec_in_detail WHERE id_sec_in_header ='{$id_sew_qc_out}' GROUP BY id_proses )C
				ON A.id_sec_in_header = B.id_sec_in_header				
				 WHERE A.id_sec_in_header ='{$id_sew_qc_out}'
				 GROUP BY A.id_sec_in_header";
		...
		*/
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_sec_in_header":"'.rawurlencode($row[$i]['id_sec_in_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_sec_in":"'. rawurlencode($row[$i]["no_sec_in"]).'",';	
