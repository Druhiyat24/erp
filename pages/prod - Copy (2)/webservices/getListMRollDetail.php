...
if($row['color'] == $id_color && $row['item'] == $id_item){
	...
	$table = "(
		...
		SELECT 
			...
			mr.cons_ws,
			mr.cons_m,
			(fabric_used / mr.qty_cutting) AS cons_act,
			(cons_act - mr.cons_ws) AS cons_balance,
			((cons_balance / mr.cons_ws) * 100) AS percentage,
			mr.binding,
			mr.actual_balance,
			(mr.binding + mr.actual_balance + used_total) AS actual_total,
			(actual_total - mr.qty_sticker) AS short_roll,
			mr.spread_sheet,
			srn.ratio,
			(srn.ratio * mr.spread_sheet) AS qty_pcs,
			srn.p_markers,
			srn.efficiency,
			((used_total * srn.efficiency) / 100) AS fabric_total_act,
			(100 - srn.efficiency) AS majun,
			(used_total - fabric_total_act) AS majun_kg,
			mr.ampar,
			(mr.ampar) AS total,
