...
class Proses {
	...
	public function get_list_data($id){
		$sql = "SELECT    A.id_sec_out_header
			,A.date_output
			,A.inhouse_subkon
			,A.dept_subkon
			,A.no_sec_out
			...
			,GROUP_CONCAT(DISTINCT(C.id_proses))id_proses
				FROM prod_sec_out A
				INNER JOIN (SELECT id_sec_out_header,id_panel,id_proses FROM prod_sec_out_detail WHERE id_sec_out_header ='{$id}' GROUP BY id_panel )B
				ON A.id_sec_out_header = B.id_sec_out_header
				INNER JOIN (SELECT id_sec_out_header,id_panel,id_proses FROM prod_sec_out_detail WHERE id_sec_out_header ='{$id}' GROUP BY id_proses )C
				ON A.id_sec_out_header = B.id_sec_out_header				
				 WHERE A.id_sec_out_header ='{$id}'
				 GROUP BY A.id_sec_out_header";
		...
		*/
		...
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_sec_out_header":"'.rawurlencode($row[$i]['id_sec_out_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_sec_out":"'. rawurlencode($row[$i]["no_sec_out"]).'",';	
