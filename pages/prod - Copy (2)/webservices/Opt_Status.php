...
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_populasi_status();
...
class Proses {
	...
	public function get_populasi_status(){
		$sql = "SELECT status isi,status id FROM act_costing WHERE status IS NOT NULL OR status !='' GROUP BY status";
