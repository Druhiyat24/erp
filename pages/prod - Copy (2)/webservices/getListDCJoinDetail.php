...
// 			sqc.qty_sec_qc,
// 			sqc.approve_sec,
// 			IFNULL(cqc.qty_cut_qc, IF(cqc.secondary = 'Y', sqc.qty_sec_qc, cqc.qty_cut_qc)) AS qty_qc,
// 			'' AS qty_reject_dc_join,
// 			'' AS qty_dc_join,
// 			IF(cqc.approve = 'Y', 'Approved', 'Not Approved') AS status_approve
...
// 				sqc.qty_sec_qc,
// 				sqc.approve_sec
// 			FROM prod_sec_qc AS sqc
// 		) AS sqc ON sqc.id_cut_qc = cqc.id_cut_qc
// 		INNER JOIN (
// 			SELECT 
...
// 		) AS cod ON cod.id_cut_out_detail = cqc.id_cut_out_detail
// 		WHERE cqc.id_cost = '{$id_ws}'
		...
		FROM (
			...
			INNER JOIN (
				SELECT 
					...
					cnd.number_sack
				FROM prod_cut_number_detail AS cnd
			) AS cnd ON cnd.id_cut_number_detail = djd.id_cut_number_detail
			INNER JOIN (
				SELECT 
					cod.id_cut_out_detail,
					cod.fabric_desc,
