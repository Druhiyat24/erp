...
function getListData(){
	...
	if(!id_coa){
		alert("Id COA Harus Diisi");
		return false;		
	}
	$(".list").css("display","");

	GenerateTable(from,to,id_coa);
...
function get_coa(){
        ...
        type:"POST",
        cache:false, 
        url:"webservices/get_ms_coa.php", 
        data : { code : '1' },     // multiple data sent using ajax
        success: function (response) {
			data = response;
			var opt = "";
				opt += "<option >Select Coa</option>";
				d = JSON.parse(data);
				if(d.message == '1'){
				for(var i=0;i<d.records.length;i++){
					opt += "<option value='"+decodeURIComponent(d.records[i].id)+"'>"+decodeURIComponent(d.records[i].isi)+"</option>";
				}					
					$("#id_coa").append(opt);
				}
				if(d.message == '2'){
...
function generate_saldo(){
	var id_coa = $("#id_coa").val();
	    $.ajax({		
        type:"POST",
        cache:false, 
        url:"webservices/generateDailySaldo.php", 
        data : { code : '1',id_coa :id_coa },     // multiple data sent using ajax
        success: function (response) {
			data = response;
			console.log(data);
				d = JSON.parse(data);
...
function GenerateTable(from_,to_,id_coa_){
	table = $('#laporan_jurnal').DataTable( {
	  ...
	  "lengthMenu": [[10, 25, 50, 999999999], [10, 25, 50, "All"]],
        ...
        "ajax":  "webservices/getDataMasterSaldoAwal.php?from="+from_+"&to="+to_+"&id_coa="+id_coa_,
        "columns": [
        {
			"data":           "d_daily",
        } ,
