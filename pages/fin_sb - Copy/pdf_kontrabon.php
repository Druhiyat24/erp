...
SELECT 
		...
		INNER JOIN (SELECT fin_journal_d.*,po_header.id, po_header.pono FROM fin_journal_d inner join po_header on po_header.id = fin_journal_d.id_po WHERE id_journal LIKE '%-PK-%' AND is_retur_bh = 'Y' AND debit > 0 AND id_coa NOT IN('15204','15207') AND reff_doc IS NOT NULL  ) FD  ON FD.id_bppb = bpb.id 
			WHERE 1=1
                 AND poi.cancel = 'N' 	AND FD.id_journal ='$id'		AND poh.id IS NOT NULL
