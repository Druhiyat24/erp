...
//        }
    ...
    public function save($d)
    {
        ...
        $sql_ = "
            ...
            ;
        ";
		insert_log_nw($sql_,$_SESSION['username']);
        $this->query($sql_);		
...
die();    */
            foreach($dataBpb as $_bpb){
                ...
                $sql = "
                    ...
                    VALUES 
                      ('{$_bpb['id_journal']}', '{$_bpb['row_id']}', '{$_bpb['id_coa']}', '{$_bpb['nm_coa']}', '{$_bpb['curr']}', '{$_bpb['debit']}', '{$_bpb['credit']}'
                      ,'".htmlspecialchars($_bpb['description'])."','{$_bpb['dateadd']}', '{$_bpb['useradd']}','{$_bpb['reff_doc']}','{$_bpb['reff_doc2']}'
					  ,'{$_bpb['id_bpb']}'
					  ,'{$_bpb['id_po']}'
					  ,'{$_bpb['id_po_det']}'
					  ,'{$_bpb['id_item']}'
                ...
                case Config::$journal_usage['KONTRABON']:
                    $arr_count = count($data['multiple']->arraynya);				
 					if($arr_count > 0){
						...
						for($i=0;$i<$arr_count;$i++){
							$is_retur = check_retur($data['multiple']->arraynya[$i]->bpb);
/* 							echo $is_retur;
							die(); */
								if($is_retur == '1'){
/* 							echo $is_retur;
							die(); 	 */				
									update_bpb_ir_ret($data['multiple']->arraynya[$i]->bpb, $data['id_journal'],$data['multiple']->arraynya[$i]->journal_reff,0);									
								}else{
                ...
                <div class='col-md-3'>
                    ...
                    <div class='form-group'>
                     ...
                     <div class='form-group'>
						 ...
						 <input type='text' name='matauang' id='curr_header' class='form-control' readonly placeholder='auto'>
