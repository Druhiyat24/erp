...
/*  		print_r($pph_nya);
        ...
        $sql = "
			...
			WHERE id_journal = '{$id}' AND row_id = '{$pph_nya[$i]->id}';
        ";
...
die(); */
            foreach($dataBpb as $_bpb){
                ...
                $sql = "
                    INSERT INTO fin_journal_d
                      (id_journal, row_id, id_coa, nm_coa, curr, debit, credit
                      ,description, dateadd, useradd,reff_doc,reff_doc2,id_bpb)
                    VALUES 
                      ('{$_bpb['id_journal']}', '{$_bpb['row_id']}', '{$_bpb['id_coa']}', '{$_bpb['nm_coa']}', '{$_bpb['curr']}', '{$_bpb['debit']}', '{$_bpb['credit']}'
                      ,'{$_bpb['description']}','{$_bpb['dateadd']}', '{$_bpb['useradd']}','{$_bpb['reff_doc']}','{$_bpb['reff_doc2']}','{$_bpb['id_bpb']}'
                      )
...
<script>
    ...
    function lookup_bpb(){
        ...
        ({  type: "POST",
            ...
            data: "bpbno=" +$("#bpbNo").val()+"&bpbno_internal="+$("#bpbNoInternal").val()+"&posted=1&supplier="+$("#suppliercari").val()+"&bpbdate="+$("#bpbdatecari").val(),
            dataType: "json",
            async: false
        }).responseText;
        ...
        if(request.status == true){
            ...
            if(bpb_list.length) {
                for (var i in bpb_list) {
					...
					rows += '<td>'+ bpb_list[i].curr +" "+ bpb_list[i].bpb_amount + '</td>';
