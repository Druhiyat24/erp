...
while ($data=mysql_fetch_array($query)) {// perulangna untuk menampilkan data  ?>
   <tr>
      ...
      <td style="width:15%;text-align:right;"><?php echo $data['matauang']." ".number_format((float)$data['debit'], 2, '.', ',') ?></td>
      <td style="width:15%;text-align:right;"><?php echo $data['matauang']." ".number_format((float)$data['credit'], 2, '.', ',') ?></td>
      <td style="width:15%;text-align:center;"><?php echo fd_view($data['jatuh_tempo']) ?></td>
    </tr>
<?php
$mata_uang = $data['matauang'];
$total = $total + $data['debit'];
$total_curr  = $total_curr + $data['credit'];
...
<table width="100%" border="0" style="font-size:10px">
	<tr>
		...
		<td style="text-align:right">
			<?php echo $mata_uang." ".number_format((float)$total, 2, '.', ',') ?>
		</td>		
	</tr>
	<tr>
		<td width="70%">
