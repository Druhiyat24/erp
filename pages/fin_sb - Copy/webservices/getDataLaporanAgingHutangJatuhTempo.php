...
SELECT 		YY.id_coa
		   ...
		   ,YY.jml_pterms
		   ,YY.id_dayterms
		   ,SUM(ifnull(YY.credit,0))tot_cre
		   ,MAX(YY.id_supplier) id_supplier
		   ,YY.top
		   ,YY.kode_pterms
		   ,YY.rate
		   ,YY.row_id
FROM(
	...
	SELECT       a.id_coa
				...
				,if(a.curr='USD',(ifnull(MR.rate,0)*((ifnull(a.credit,0))) ),(ifnull(a.credit,0) ) )credit
				...
				GROUP BY Y.row_id,Y.id_journal
				)YY GROUP BY YY.id_supplier,YY.id_coa,YY.id_journal
	...
	SELECT 		 YY.id_coa
		...
		SELECT   a.id_coa
				...
				IS NULL 
				#GROUP BY TRIGGERS.id_supplier,a.id_coa,a.id_journal
				) Y GROUP BY Y.row_id,Y.id_journal
				
				)YY 
				GROUP BY YY.id_supplier,YY.id_coa,YY.id_journal
...
SELECT
	...
	SELECT     a.id_coa
				...
				,TRIGGERS.id_supplier id_supplier
				,a.row_id
				,if(a.curr='USD',(ifnull(MR.rate,0)*((ifnull(a.debit,0))) ),(ifnull(a.debit,0) ) )debit
				FROM fin_journal_d a
					INNER JOIN(SELECT id_journal,date_journal,fg_post FROM fin_journal_h WHERE fg_post ='2')b
						ON a.id_journal = b.id_journal
					INNER JOIN(SELECT id_journal,MAX(id_supplier)id_supplier FROM fin_journal_d WHERE id_journal LIKE '%PK%'
				...
				)Y #GROUP BY Y.id_supplier,Y.id_coa,Y.id_journal
	)KB_PENGURANGAN_LAIN ON KB.id_journal=KB_PENGURANGAN_LAIN.id_journal
	LEFT JOIN(
	/* pengurangan retur */
		SELECT   YY.id_coa
				,YY.nm_coa
				,SUM(ifnull(YY.debit,0))tot_deb
				,MAX(YY.id_supplier)id_supplier
				,YY.id_journal
				,YY.row_id		
		FROM(
		...
		SELECT   Y.id_coa
				,Y.nm_coa
				,(ifnull(Y.debit,0))debit
				,(Y.id_supplier)id_supplier
				,Y.id_journal
				,Y.row_id
		FROM (		
		...
		SELECT   a.id_coa
				...
				#GROUP BY Y.id_supplier,Y.id_coa,Y.id_journal
				)YY 
				GROUP BY YY.id_supplier,YY.id_coa,YY.id_journal
...
SELECT 	
	...
	SELECT
		...
		,if(curr='USD',(ifnull(MR.rate,0)*((ifnull(b.debit,0))) ),(ifnull(b.debit,0) ) )debit
		,MR.rate
		...
		LEFT JOIN
			(SELECT reff_doc
				...
				,nm_coa
				,curr
				#,SUM(ifnull(debit,0))tot_deb
				#,MAX(id_supplier)id_supplier 
				,(ifnull(debit,0))debit
				,id_supplier		
				FROM fin_journal_d 
				WHERE nm_coa LIKE '%UTANG%' AND debit >0 AND id_bpb IS NOT NULL 
		#GROUP BY reff_doc,id_coa
			)b ON a.id_journal = b.id_journal
			LEFT JOIN (SELECT v_codecurr,rate,tanggal FROM masterrate)MR ON MR.tanggal = a.date_journal
		...
		GROUP BY b.row_id,a.id_journal
		)Y GROUP BY Y.id_supplier,Y.id_coa,Y.kb_reff
...
SELECT 	
		...
		,b.nm_coa
		,b.row_id
		,if(curr='USD',(ifnull(MR.rate,0)*((ifnull(b.credit,0))) ),(ifnull(b.credit,0) ) )credit
		,MR.rate
		FROM fin_journal_h a
		LEFT JOIN
			(SELECT reff_doc
				...
				,nm_coa
				,curr
				#,SUM(ifnull(credit,0))tot_cre
				#,MAX(id_supplier)id_supplier 
				,(ifnull(credit,0))credit
				,id_supplier		
				FROM fin_journal_d  WHERE nm_coa LIKE '%UTANG%' AND credit >0 AND id_bppb IS NOT NULL GROUP BY reff_doc,id_coa
			)b ON a.id_journal = b.id_journal
		LEFT JOIN (SELECT v_codecurr,rate,tanggal FROM masterrate)MR ON MR.tanggal = a.date_journal	
		WHERE a.type_journal ='3' AND a.fg_post ='2'  AND b.id_supplier IS NOT NULL AND b.id_supplier >0
		#GROUP BY b.id_supplier,b.id_coa,b.reff_doc
		GROUP BY b.row_id,a.id_journal
		)Y GROUP BY Y.id_supplier,Y.id_coa,Y.reff_doc
/* ADA RETUR DI PAYMENT */
	)PAYMENT_RETUR ON KB.id_journal=PAYMENT_RETUR.reff_doc
	
	
	LEFT JOIN(
	/* KURANG PAYMENT MANUAL */
SELECT 	
...
SELECT 	
		...
		LEFT JOIN
			(SELECT reff_doc
				...
				,(ifnull(debit,0))debit
				,id_supplier		
				FROM fin_journal_d  WHERE nm_coa LIKE '%UTANG%' AND debit  > 0 AND id_bppb IS NULL AND id_bpb IS NULL GROUP BY reff_doc,id_coa
			)b ON a.id_journal = b.id_journal
			LEFT JOIN (SELECT v_codecurr,rate,tanggal FROM masterrate)MR ON MR.tanggal = a.date_journal
