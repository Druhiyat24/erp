...
class getListData {
	...
	public function get_nilai_kb($id){
...
SELECT id_journal
...
)X WHERE X.id_journal = '{$id}' GROUP BY X.id_journal";
	...
	public function get($id){
		...
		WHERE A.id_journal = '$id' AND A.id_bpb IS NOT NULL 
		AND A.credit > 0 GROUP BY A.id_bpb
...
SELECT   A.id_journal
		...
		LEFT JOIN fin_journal_h FH_ref ON A.reff_doc2 = FH_ref.id_journal
		LEFT JOIN mtax MT ON A.n_pph = MT.idtax
		WHERE A.id_journal = '$id' AND A.id_bppb IS NOT NULL
		AND A.credit > 0 GROUP BY A.id_bppb
		...
		$pajak = 0;
		$nama  = "";
		$total_nilai_final = $this->get_nilai_kb($id);
