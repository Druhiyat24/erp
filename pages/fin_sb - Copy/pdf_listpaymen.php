...
$value_utang = 0;
$sql="
	SELECt GLOBAL.*,SUM(GLOBAL.value_other)value_other,SUM(GLOBAL.nilai_kontrabon)nilai_kontrabon,SUM(GLOBAL.value_pph),SUM(GLOBAL.value_ppn),SUM(GLOBAL.amount_original)amount_original FROM(
		SELECT A.*
				,if(B.is_utang = '1',0,if(B.id_coa = '15207' OR B.id_coa = '15204',0,B.deb ))debits
				,if(B.id_coa = '15207' OR B.id_coa = '15204',B.deb,0 )pajaks
				,B.Cre credit
				,B.id_coa
				,pph.amount nilai_kontrabon
				,pph.pph value_pph
				,pph.amount_original
				,pph.value_ppn
				,pph.value_pengurangan value_other
				,pph.value_penambahan
				,pph.value_pengurangan
				,B.row_id
				,B.is_utang
...
$journal_detail
				...
				ON A.v_nojournal = C.id_journal
					...
					)SUPPLIER ON pph.id_supplier = SUPPLIER.Id_Supplier				
					LEFT JOIN(
			...
			WHERE A.v_listcode = '$id' AND A.v_source = 'KB' AND B.deb !='0' GROUP BY A.v_nojournal
...
$sql_utang = "	SELECT A.v_nojournal,A.v_listcode,SUM(ifnull(B.nilai,0))nilai
	FROM fin_status_journal_ap A INNER JOIN (		
			SELECT   a.reff_doc bpb_ref
					,a.id_bpb
					,a.is_retur_bh
					,a.n_pph
					...
					LEFT JOIN mapping_utang c ON c.id_coa = a.id_coa
					LEFT JOIN (SELECT idtax,percentage FROM mtax)MT ON MT.idtax = a.n_pph WHERE
					a.debit > 0 AND a.is_retur_bh = 'N' AND
					a.id_coa NOT IN('15204','15207') AND c.id_coa IS NOT NULL GROUP BY a.id_journal,a.row_id
					)B ON A.v_nojournal = B.id_journal WHERE A.v_listcode = '$id' GROUP BY v_listcode";

$rs=mysql_fetch_array(mysql_query($sql));
$rs_utang=mysql_fetch_array(mysql_query($sql_utang));
$v_listcode=$rs['v_listcode'];
//$my_utang = $rs_utang['nilai'];
...
//if($data['id_coa'] == '15207' || $data['id_coa'] == '15204' ){
		$amount__pajak = $amount__pajak + $data['value_ppn'];
		$total = $total + ($data['amount_original'] );
		$total_other = $total_other + $data['value_pengurangan'] + $data['value_penambahan'];
	//}
   ...
   <tr>
	 ...
	 <!-- <td style="width:15%;text-align:center;"><?php // echo $data['kode_pterms'] ?></td> -->
      <td style="width:15%;text-align:right;"><?php 
		...
		//	}else{
				echo $data['matauang']." ".number_format((float)$data['nilai_kontrabon'], 2, '.', ','); 
		//	}?>
		</td>
      <td style="width:15%;text-align:right;"><?php 			
	//  if($data['is_utang'] == '1' ){
...
<?php
$total_by_journal = $total_by_journal + $data['nilai_kontrabon'];
$mata_uang = $data['matauang'];
if($data['v_source']=='KB' ){
	//if($id_journal == '' || ($id_journal != $data['v_nojournal'])){
		$pph_kb = $pph_kb + $data['value_pph'];
	//}
}
if($data['v_source']=='PO'){
	//if($id_journal == '' || ($id_journal != $data['v_nojournal'])){
		$pph_po = $pph_po + $data['value_pph'];
	//}	
...
// $total_curr = $total + $my_utang;
 $total_curr = $total;
 $total_curr_awal = $total;
 $total_pph = $pph_kb + $pph_po;
 
 if($total_other > 0){
	 $total_curr_awal =$total; //+  (-1*$total_other);
 }
 //$utang = $total_curr;
 // $ori_utang = $utang - $amount__pajak;
 //if($total_pph != '0'){
	 $total_curr_pph = $total_curr + $amount__pajak;
 //}else{
...
<table width="100%" border="0" style="font-size:12px">
	<tr>
		...
		<td style="text-align:right">
			<?php echo $mata_uang." ".number_format((float)$total_curr_awal, 2, '.', ',') ?>
		</td>		
	</tr>
	...
	<tr>
		...
		<td style="width:1%">:</td>
		<td style="text-align:right">
			<?php echo $mata_uang." ".number_format((float)$total_other, 2, '.', ',') ?>
		</td>		
	</tr>
...
<td style="width:1%">:</td>
		<td style="text-align:right">
			<?php echo $mata_uang." ".number_format((float)$total_by_journal, 2, '.', ','); ?>
		</td>		
	</tr>	
</table>
