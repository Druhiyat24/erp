	...
	<section class="content">
		...
		<div class="box">
			...
			<div class="box-body">
				...
				<table id="examplelaporan"  rules="all" class="stripe" style="width:100%;font-size:12px;" >
					...
					<tbody>
						...
						<?php  if(isset($_GET['submit'])){  ?>
							...
							SELECT Y.umur_utang_kontrabon
								 ...
								 ,Y.Id_Supplier
								 ,Y.Supplier
								 ,ifnull(if(Y.supplier_code='','-',Y.supplier_code),'-')supplier_code
...
FROM(
		...
		SELECT       a.reff_doc bpb_ref
					...
					INNER JOIN (SELECT id_journal,type_journal,fg_tax,n_pph,date_journal FROM fin_journal_h
						WHERE type_journal = '14' AND  id_journal NOT IN ('NAG-PK-1912-00104') AND  id_journal NOT IN ('NAG-PK-1912-00105') AND  id_journal NOT IN ('NAG-PK-2001-00067')
...
UNION ALL
			SELECT   a.reff_doc bpb_ref
					...
					LEFT JOIN mastercoa h ON h.id_coa=a.id_coa
					WHERE 1=1 AND
					  e.Id_Supplier IS NOT NULL
					  AND
					  e.Supplier IS NOT NULL
					  AND
					a.debit > 0 AND
					a.id_coa NOT IN('15204','15207') AND c.id_coa IS NOT NULL GROUP BY a.id_journal,a.row_id
					)MASTER GROUP BY MASTER.id_journal ORDER BY MASTER.id_journal, MASTER.Id_Supplier ASC)X ORDER BY X.Id_Supplier)Y GROUP BY Y.Id_Supplier, Y.ddayspterms 
					
					ORDER BY Y.Id_Supplier DESC
";				
		 // echo "<pre>$sql</pre>";
			  ...
			  // rumus perhitungan ap = 365/(total utang)/ (rata rata utang)
                { 
                	...
                	// $total_utang = $data['jt1_31']+$data['jt31_61']+$data['jt61_91']+$data['jt90'];
                	$ap_days = 0;
              ...
              echo "<td>$data[id_coa]</td>"; // COA post to
			  ...
			  echo "<td style='text-align: right;'>".(number_format($total_utang))."</td>"; //TOTAL
			  echo "<td style='text-align: right;'>".(number_format($ap_days  ,2,',','.'))."</td>"; //TOTAL
