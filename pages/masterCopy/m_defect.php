  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
		    $query = mysql_query("select * from master_defect order by id_defect desc ");
			  $no = 1; 
				while($data = mysql_fetch_array($query))
			  { echo "<tr>";
				  echo "<td>$no</td>"; 
          echo "<td>$data[jenis_defect]</td>";
          echo "<td>$data[mattype]</td>";
          echo "<td>$data[kode_defect]</td>";
          echo "
            <td>$data[nama_defect]</td>
            <td>$data[kode_posisi]</td>
            <td>$data[nama_posisi]</td>
            <td>$data[remark]</td>";
          echo "<td><a $cl_ubah href='../master/?mod=$mod&mode=$mode&id=$data[id_defect]' $tt_ubah </a>";
				  echo " <a $cl_hapus href='d_defect.php?mod=$mod&mode=$mode&id=$data[id_defect]' $tt_hapus";?> 
