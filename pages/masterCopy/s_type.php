...
$cek=flookup("kode_type","mastertype2","id_sub_group='$id_sub_group' and kode_type='$type'");
if ($cek!="")
{ $_SESSION['msg'] = "XData Sudah Ada";
  echo "<script>window.location.href='index.php?mod=$mod';</script>";
}
else if ($id_item!="")
{ $cek=flookup("id_type","mastercontents","id_type='$id_item'");
	if($cek=="")
	{	$sql="update mastertype2 set id_sub_group='$id_sub_group',
			kode_type='$type',nama_type='$type_desc' where id='$id_item'";
	  insert_log($sql,$user);
