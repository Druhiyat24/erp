    ...
    <table id="examplefix" class="" style="width:100%">
      ...
      <tbody>
		...
		$sql = "select a.id,cost_no,cost_date,supplier,styleno,
          ...
          where status IN('CONFIRM','SAMPLE','BOOKING') and app1='W' group by a.cost_no";
    ...
    <table id="examplefix2" class="" style="width:100%">
      ...
      <tbody>
        ...
        $query = mysql_query("select a.id,a.jo_no,a.jo_date,fullname,msup.supplier,ac.styleno,sod.deldate_det AS first_garment_delv,bld.balance_days
          ...
          inner join userpassword up on a.username=up.username where a.app='W' group by a.jo_no ORDER BY first_garment_delv asc"); 
        $no = 1; 
  ...
  <div class="box-body">
    <table id="examplefix3" class="" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $sql="select tmppoit.kpno,tmppoit.styleno,a.revise,a.jenis,a.app,a.id,a.draftno,a.draftdate,supplier,
          nama_pterms,tmppoit.buyer,tmppoit.deldate_det AS first_garment_delv,tmppoit.balance_days from po_header_draft a inner join 
          mastersupplier s on a.id_supplier=s.id_supplier inner join 
          masterpterms d on a.id_terms=d.id
          inner join 
          (select ac.kpno,ac.styleno,poit.id_jo,poit.id_po_draft,ms.supplier buyer,sod.deldate_det,bld.balance_days from po_item_draft poit 
          ...
          where poit.cancel='N' 
          group by poit.id_po_draft) tmppoit 
          on tmppoit.id_po_draft=a.id where a.app='W' and jenis in ('M','P')
          union all 
          select '','',a.revise,a.jenis,a.app,a.id,draftno,draftdate,supplier,
          nama_pterms,tmppoit.buyer,tmppoit.deldate_det,tmppoit.balance_days from po_header_draft a inner join 
          mastersupplier s on a.id_supplier=s.id_supplier inner join 
          masterpterms d on a.id_terms=d.id
          LEFT join 
          (select poit.id_jo,poit.id_po_draft,'' buyer,sod.deldate_det,bld.balance_days from po_item_draft poit 
          ...
          where poit.cancel='N' 
          group by poit.id_po_draft) tmppoit 
          on tmppoit.id_po_draft=a.id where a.app='W' and jenis='N' ORDER BY first_garment_delv asc
          ";
        $query = mysql_query($sql); 
        $no = 1; 
        while($data = mysql_fetch_array($query))
