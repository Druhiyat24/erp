...
class getListData {
	...
	LEFT JOIN (
		SELECT MAX(n_id_dest)destination,id_so,id FROM so_det GROUP BY id_so,n_id_dest,id
	)SOD ON SOD.id_so = SO.id
	LEFT JOIN(  
		SELECT confirm,bppbno_int,bppbno,id_item,id_jo,id_so_det,id,qty FROM bppb
...
LEFT JOIN(
	SELECT qty,bppbno_int FROM bppb 
)BPPB_QTY ON BPPB_QTY.bppbno_int = BPPB.bppbno_int	
	
	
	
	WHERE BPPB.confirm='Y' AND ACT.id_buyer = '$id_buyer' AND BPPB.bppbno_int IS NOT NULL
	...
	GROUP BY BPPB.bppbno_int
		...
		if($MyList == '0'){
			//$result = '{ "status":"ok", "message":"2", "records":"0"}';
				...
				if ($outp != "") {$outp .= ",";}
				$outp .= '{"checkbox":" ",'; 
				$outp .= '"ws":"'.(ISSET($row['kpno']) ? $row['kpno'] :'' ).'",'; 
				$outp .= '"destination":" ",'; 
				$outp .= '"qty":" ",'; 
				$outp .= '"customer":" ",'; 
				$outp .= '"bppbno_int":" ",';
			    ...
			    if (!is_object($MyList)) {
					...
					if($EXP[1]){
						...
						while($row = mysqli_fetch_array($MyList)){
							$checkbox = '';//idcosting
						if($id_inv != '0'){
							$checkbox .= "<input type='checkbox' data-destination='$row[destination]' data-idcosting='$row[idcosting]' data-idcustomer='$row[idcustomer]' data-bppbno_int='$row[bppbno_int]' data-bppbno_int='$row[bppbno_int]' checked='checked' disabled onclick='GenerateDetail(this)' id='__$row[id_bppb]' />";
						}else{
							$checkbox .= "<input type='checkbox' data-destination='$row[destination]' data-idcosting='$row[id_bppb]' data-idcustomer='$row[idcustomer]' data-bppbno_int='$row[bppbno_int]' data-bppbno_int='$row[bppbno_int]' onclick='GenerateDetail(this)' id='__$row[id_bppb]' />";
						}			
							if ($outp != "") {$outp .= ",";}
							...
							$outp .= '"id_bppb":"'.$row['id_bppb'].'",'; 
							$outp .= '"idcosting":"'.$row['id_bppb'].'",'; 
