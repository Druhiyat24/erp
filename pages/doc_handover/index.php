...
<head>
  ...
  <script type="text/javascript">
    ...
    $("table tbody tr").on("click", "#showdet", function(){                 
        ...
        var ket_mtn = $(this).closest('tr').find('td:eq(4)').attr('value');
        // alert(nomtn);
        $.ajax({
            type:'POST',
   ...
   <script type="text/javascript">
      $("table tbody tr").on("click", ".cancel_maintain", function(){                 
        var no_maintain = $(this).closest('tr').find('td:eq(0)').attr('value');
        // alert(no_transfer);

        $.ajax({
          type:'POST',
          url:'cancel_maintain.php',
          data: {'no_maintain':no_maintain},
            // close: function(e){
            //     e.preventDefault();
            // },
            success: function(data){                
    ...
    <script type="text/javascript">
      $("#form-simpan").on("click", "#transf_bpb", function(){             
        ...
        $.ajax({
          ...
          data: {'doc_num':doc_num, 'unik_code':unik_code},
          cache: 'false',
          close: function(e){
            e.preventDefault();
          },
    ...
    <script type="text/javascript">
      $("#form-simpan").on("click", "#maintain_bpb", function(){             
        var doc_num = document.getElementById('no_doc').value;
        var tanggal_doc = document.getElementById('trfdate').value;
        var keterangan = document.getElementById('txt_keterangan').value;
        var unik_code = document.getElementById('unik_code').value; 

        $.ajax({
          type:'POST',
          url:'insert_maintain_h.php',
          data: {'doc_num':doc_num, 'tanggal_doc':tanggal_doc, 'keterangan':keterangan, 'unik_code':unik_code},
          cache: 'false',
          close: function(e){
            e.preventDefault();
          },
          success: function(response){
            ...
            $("input[type=checkbox]:checked").each(function () {
              ...
              var confirm_by = $(this).closest('tr').find('td:eq(10)').attr('value');
              var confirm_date = $(this).closest('tr').find('td:eq(11)').attr('value');
              var dateinput = $(this).closest('tr').find('td:eq(12)').attr('value');
              ...
              $.ajax({
                type:'POST',
                url:'insert_maintain_det.php',
                data: {'no_bpb':no_bpb, 'tgl_bpb':tgl_bpb, 'pono':pono, 'Supplier':Supplier, 'jml_pterms':jml_pterms, 'kode_pterms':kode_pterms, 'keterangan':keterangan, 'curr':curr, 'total':total, 'confirm_by':confirm_by, 'confirm_date':confirm_date, 'dateinput':dateinput, 'qty':qty, 'tgl_transfer':tgl_transfer, 'unik_code':unik_code},
                cache: 'false',
                close: function(e){
                  e.preventDefault();
                },
                success: function(response){
                  console.log(response);
                  window.location = '../doc_handover/?mod=maintain_bpb';
                },
                error: function (xhr, ajaxOptions, thrownError) {
                  console.log(xhr);
                  alert(xhr);
