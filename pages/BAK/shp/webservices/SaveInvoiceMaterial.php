...
class proses{
	...
	public function generate_invoice($date_nya){
		...
		$years = $pecah_date[0];
		$sql = "SELECT COUNT(years)jlh,ifnull(seq,0)seq FROM shp_inv_scrap_pool WHERE years = '{$date_nya}' AND code ='MTRL'";
		$tmp_array = $this->eksekusi_query($sql,"generate_invoice");		
		if($tmp_array[0]['jlh'] == '0' && $tmp_array[0]['seq'] == '0'){
			$seq = '1';
			$insert = "INSERT INTO shp_inv_scrap_pool (code,code_company,years,seq)VALUES('MTRL','NAG'
			,'{$years}','1')";
		...
		return sprintf('%03d', $seq)."/INV/MTRL-NAG/".$years_.sprintf('%02d', $pecah_date[1]);
		
	}	
	public function insert_header($my_header,$no_invoice,$username,$date_invoice){
		$insert = "INSERT INTO shp_invoice_scrap_header(type_invoice,invno,date_invoice,user_insert,fg_ppn,id_pterms,id_buyer,id_coa) VALUES('101','{$no_invoice}','{$date_invoice}','{$username}','{$my_header['fg_ppn']}','{$my_header['id_terms']}','{$my_header['id_supplier']}','{$my_header['id_coa']}')";
