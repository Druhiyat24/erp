...
$row = $_POST['start'];/* 
print_r($_POST); */
	$table = "(SELECT A.invno,A.id,A.id_buyer,A.user_insert,A.d_insert,A.user_post,A.d_post,A.user_update,A.d_update,MS.Supplier buyer
 FROM shp_invoice_scrap_header A
INNER JOIN mastersupplier MS ON A.id_buyer = MS.Id_Supplier
WHERE A.type_invoice = '100'
)X
";
	$colomn = " 
		 X.id
...
//PdfInvoiceScrap
	$input = '';
	$input .="<a href='?mod=InvocieScrapForm&id=".$row['id']."' title='Edit' target='_blank'><i class='fa fa-pencil'></i></a>";
