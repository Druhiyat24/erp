...
<script type="text/javascript">
  ...
  { 
	...
	if(x.length > 0){
		for(var i=0;i<x.length;i++){
			...
			var split = '';
			split = x[i].text.split("|");
			arr_text[i].text = split[2];
			arr_text[i].id = x[i].id;
			tmp_id_value = arr_text[i].id 
		}
		var keyNew = parseFloat(x.length) - 1;
		var keyExist = parseFloat(x.length) - 2;		
		

		if(keyExist < 0){
			return false;
		}else{
		console.log(arr_text[keyExist].text);
		console.log(arr_text[keyNew].text);	
			if(arr_text[keyExist].text == arr_text[keyNew].text){
			}else{
				
				
    var $select = $("#cbokp");
    var idToRemove = tmp_id_value;
 ...
 {
	if(that.id){
		window.location.href = '/pages/shp/?mod=3&type_invoice='+that.value;
	}else{
    ...
    <div class='box-body'>
      <div class='row'>
        <div class='col-md-3'>              
          ...
          <div class='form-group'>
            <label>WS # *</label>
            <select class='form-control select2' multiple="multiple" style='width: 100%;' name='txtwsno[]' id='cbokp' onchange="getJO(this)">
            </select>
          </div>
          <div class='form-group'>
            <label>Faktur Pajak</label>
