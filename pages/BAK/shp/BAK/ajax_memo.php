...
{	$id_inv = json_encode($_REQUEST['id_inv']); }
	...
	if ($modenya == 'simpan_temp')
	{
		...
		if ($id_kat_add!="")
		{	
			$sql = "insert into memo_det_tmp (id_ctg,id_sub_ctg,biaya,inv_vendor,user) 
			values ('$id_kat_add','$id_sub_kat_add','$biaya_add',upper('$inv_vendor_add'),'$user')";
			insert_log($sql,$user);	
		...
		echo "<table id='examplefix1' class='display responsive' style='width: 100%;font-size:14px;'>";
				echo "
					...
					<tbody>";
						$sql="select a.id,a.id_ctg, b.nm_ctg, a.id_sub_ctg, c.nm_sub_ctg, a.biaya,a.inv_vendor, a.user from memo_det_tmp a
