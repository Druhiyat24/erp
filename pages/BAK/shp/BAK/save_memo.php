...
if ($mod == 'simpan_header') {
	...
	$sql = "insert into memo_h (nm_memo, tgl_memo, kepada, id_supplier, jns_trans, jns_pengiriman, id_buyer, 
			ditagihkan, curr, jatuh_tempo, dok_pendukung, date_input, status, jns_inv, user,notes, inv_buyer,id_item) 
	...
	if ($cekheader != "") {
		...
		$sql_insert_det = "insert into memo_det (id_h,id_ctg,nm_ctg,id_sub_ctg,nm_sub_ctg,biaya,inv_vendor,cancel)
			select $cekheader,a.id_ctg, b.nm_ctg, a.id_sub_ctg, c.nm_sub_ctg, a.biaya, a.inv_vendor, 'N' from memo_det_tmp a
...
if ($mod == 'simpan_header_non_inv') {
	...
	$sql = "insert into memo_h (nm_memo, tgl_memo, kepada, id_supplier, jns_trans, jns_pengiriman, id_buyer, 
			ditagihkan, curr, jatuh_tempo, dok_pendukung, date_input, status, jns_inv, user, notes, inv_buyer,id_item) 
			values ('$kodepay_f','$txtmemodate','$cbokpd','$cbosupp','$jns_trans','$jns_pengiriman','$cbobuyer',
			'$ditagihkan','$curr','$txtjth_tempo','$dok_pendukung','$dateinput','DRAFT','NON INVOICE','$user','$txtnotes','$inv_buyer','$id_item')";
	insert_log($sql, $user);
...
if ($mod == 'tambah_det') {
	...
	$sql = "insert into memo_det (id_h,id_ctg,nm_ctg,id_sub_ctg,nm_sub_ctg,inv_vendor,biaya,cancel) 
	select '$id_h',b.id_ctg, b.nm_ctg, c.id_sub_ctg, c.nm_sub_ctg,'$inv_vendor_add', '$biaya_add', 'N' from
	master_memo_ctg b 
