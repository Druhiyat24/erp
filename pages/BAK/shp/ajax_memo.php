...
{	$id_inv = json_encode($_REQUEST['id_inv']); }
	...
	if ($modenya == 'simpan_temp')
	{
		...
		if ($id_kat_add!="")
		{	
			if ($inv_vendor_add != '-' || $inv_vendor_add != '') {

			$sql_fil  = mysql_query("select sum(id) filter from (select count(id_ctg) id from memo_det where cancel = 'N' and inv_vendor = '$inv_vendor_add' and biaya = '$biaya_add' Union select count(id_ctg) id from memo_det_tmp where inv_vendor = '$inv_vendor_add' and biaya = '$biaya_add') a");
