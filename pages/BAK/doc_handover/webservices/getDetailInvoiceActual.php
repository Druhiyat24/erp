...
class getListData {
	public function get($id){
		...
		,ACT.kpno
		,ACT.comm_cost
		,ifnull(MS_TO.Supplier,MS_H.Supplier)supplier
		,ifnull(MS_TO.Id_Supplier,MS_H.Id_Supplier)id_supplier
		...
		LEFT JOIN mastersupplier MS_H  ON MS_H.Id_Supplier = IH.id_buyer
		LEFT JOIN mastersupplier MS_TO  ON MS_TO.Id_Supplier = A.v_to
...
";
		...
		while($row = mysql_fetch_array($stmt)){
			...
			$outp .= '{"id":"'.rawurlencode($row['n_id']).'",';
				...
				$outp .= '"id_buyer":"'. rawurlencode($row["id_supplier"]). '",'; 
				$outp .= '"nama_buyer":"'. rawurlencode($row["supplier"]). '",'; 
