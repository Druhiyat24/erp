$(document).ready(function () {
	...
	format = '1';
	  ...
	  Option("webservices/getListBKBSMaterial.php")//OPTION BKB
...
/* 		if($detail[i].discount == '0'){
	...
	$.ajax({
		type: "POST",
		cache: false,
		url: "webservices/SaveInvoiceMaterial.php",
...
function getListDataHeader(my_id,kondisi){
	...
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/getListDataInvoiceMaterialHeader.php",
		{
			type: "POST",
			data: {id:my_id},
			success: function (data) {
...
/* 	i_trigger =i_trigger +1; 
	...
	//if(i_trigger > 1){
		TableDetail = $('#inv_scrap_detail').DataTable({
			...
			"destroy": true,
			"method" :"POST",
			"ajax": "webservices/getInvoiceMaterialDetail.php?id_bkb=" + id_bkb+"&id_url="+id_url_,
			"columns": [
				// { "data": "kpno" },
				{ "data": "bkb" },
				{ "data": "tipe_scrap" },
