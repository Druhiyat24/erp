...
{	$crinya = $_REQUEST['id_jo'];
	...
	{	$ponya=$crinya; 
		$id_jo=$crinya;
		//INI ADALAH SINTAKS UNTUK MENG EXTRAK ID_JO YG BERUPA ARRAY
		//KORELASI DARI QUERY ADALAH PADA BAGIAN :
		//"where k.id_jo = $id_jo and k.cancel='N' and k.status='P' group by k.id_item"
		//DIGANTI/DIUBAH MENJADI
		//"where k.id_jo IN $where and k.cancel='N' and k.status='P' group by k.id_item"
		$count = count($id_jo);
		$params = $count - 1;
		$value_nya ="";
		for($i=0;$i<$count;$i++){
		...
		echo "<table id='examplefix2' style='width: 100%;'>";
			echo "
				...
				<tbody>";
				$sql="select POH.pono,k.id_item,l.color,l.size,mi.goods_code,mi.itemdesc,
			  l.qty qty_gmt,k.cons,round(sum(l.qty*k.cons),2) qty_bom,
			  k.unit from bom_jo_item k 
			  inner join so_det l on k.id_so_det=l.id 
			  inner join po_item POI on POI.id_jo = k.id_jo
