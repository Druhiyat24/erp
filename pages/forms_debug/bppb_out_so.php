...
{ $id_item = "";
  ...
  $invno = $data['invno'];
  if($mode=="FG" and $mod=="32e")
  { $jono = flookup("jo_no","jo a inner join jo_det s on a.id=s.id_jo inner join so_det sod on s.id_so=sod.id_so","sod.id='$data[id_so_det]'"); }
  else
...
echo "<div class='box'>";
  echo "<div class='box-body'>";
    echo "<div class='row'>";
      echo "<form method='post' name='form' action='save_data_bppb_out_so.php?mod=$mod&mode=$mode&noid=$bppbno&id=$id_line' onsubmit='return validasi()'>";
          ...
          <div class='col-md-6'>
            <div class='form-group'>
              ...
              <input type='text' maxlength='6' class='form-control' name='txtbcaju' 
                placeholder='Masukan Nomor Aju' value='$bcaju' readonly>
            </div>
          </div>";
          echo "
          <div class='col-md-6'>
            <div class='form-group'>
              ...
              <input type='text' class='form-control' name='txttglaju' readonly id='datepicker3' 
        ...
        echo "<div class='col-md-3'>";
          ...
          echo "<div class='form-group'>";
            ...
            echo "<input type='text' class='form-control' name='txtinvno'  
              placeholder='$cmas $c41' value='$invno'>";
          echo "</div>";
          echo "
          <div class='col-md-6'>
            <div class='form-group'>";
          ...
          <div class='col-md-6'>
            <div class='form-group'>
              <label>$c53 *</label>
              <input type='text' class='form-control' name='txtbppbdate' onchange='getSat(this.value)'  
                placeholder='$cmas $c53' value='$bppbdate'>
            </div>
          </div>";
        echo "</div>";
        $list_id_so_det=flookup("group_concat(id_so_det)","bppb","bppbno='$bppbno'");
        ?>
        <div class='box-body'>
          <?php if($mod=="32e") 
          { echo "<table id='examplefix2' style='width: 100%;'>";
            echo "
              ...
              <tbody>";
              $sql="select mi.buyerno,mi.color,mi.size,breq.nomor_rak,breq.id line_item,breq.id_supplier,breq.qty qtyout,
                mi.goods_code,mi.itemname itemdesc,tbl_in.id_item,tbl_in.id_jo,tbl_in.qty_in, 
                ifnull(tbl_out.qty_out,0) qty_out, tbl_in.unit, ac.kpno,ac.styleno,
                mbuyer.supplier buyer,so.jo_no  
                from masterstyle mi inner join 
                (select id_item,id_jo,id_so_det,sum(qty) qty_in,unit from bpb where id_so_det in ($list_id_so_det) 
                  group by id_item) as tbl_in on mi.id_item=tbl_in.id_item 
                left join 
                (select id_item,id_so_det,sum(qty) qty_out from bppb where id_so_det in ($list_id_so_det) 
                  group by id_item) as tbl_out on tbl_in.id_item=tbl_out.id_item 
                inner join bppb breq on mi.id_item=breq.id_item inner join 
                (select jo_no,sod.id,id_cost,min(sod.deldate_det) mindeldate from so inner join so_det sod on so.id=sod.id_so 
                  inner join jo_det jod on sod.id_so=jod.id_so inner join jo on jod.id_jo=jo.id  
                  group by sod.id) so on breq.id_so_det=so.id 
                inner join act_costing ac on so.id_cost=ac.id inner join mastersupplier mbuyer on ac.id_buyer=mbuyer.id_supplier 
