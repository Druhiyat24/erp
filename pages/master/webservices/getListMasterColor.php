...
$searchQuery = " AND (
	...
	OR X.id_contents	               LIKE'%".$searchValue."%'		
	OR X.tampil               	   LIKE'%".$searchValue."%'	
	OR X.kode_color       		   LIKE'%".$searchValue."%'
	OR X.nama_color                LIKE'%".$searchValue."%'
	OR X.phantom	               LIKE'%".$searchValue."%'
	)
	";
}
...
$button = '';
		...
		$button .="<a $cl_ubah href='../master/?mod=10&mode=$mode&id=$row[id]' $tt_ubah 

                    data-toggle='tooltip' ><i class='fa fa-pencil'></i>

                  </a>";
		$button .="<a $cl_hapus href='d_col.php?mod=10&mode=$mode&id=$row[id]' $tt_hapus 

                   data-toggle='tooltip' ><i class='fa fa-trash'></i>

                   </a>";		
				if($row["aktif"] == 'Y'){
				$button .="<a  href='dea_group.php?mod=10&mode=$mode&id=$row[id]&is_active=N&part=color' data-original-title='Inactive'
						data-toggle='tooltip' ><i class='fa fa-ban'></i>
						</a>";				
			}else{
				$button .="<a  href='dea_group.php?mod=10&mode=$mode&id=$row[id]&is_active=Y&part=color' data-original-title='Inactive'
           ...
           $data[] = array(
			...
			"kode_color"=>htmlspecialchars($row['kode_color']),
			"nama_color"=>htmlspecialchars($row['nama_color']),
			"phantom"=>htmlspecialchars($row['phantom']),
