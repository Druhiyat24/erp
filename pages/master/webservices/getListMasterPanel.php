...
$table = "(
			SELECT * FROM masterpanel ORDER BY id ASC limit 500
...
$searchQuery = " AND (
	...
	OR X.nama_panel                LIKE'%".$searchValue."%'	
	OR X.description        	   LIKE'%".$searchValue."%'
	OR X.v_codepanel        	   LIKE'%".$searchValue."%'
	)
	";
}
...
$button = '';
		...
		$button .="<a $cl_ubah href='../master/?mod=mpanel&mode=$mode&id=$row[id]' $tt_ubah 

                    data-toggle='tooltip' ><i class='fa fa-pencil'></i>

                  </a>";
		$button .="<a $cl_hapus href='d_panel.php?mod=mpanel&mode=$mode&id=$row[id]' $tt_hapus 
           ...
           $data[] = array(
			...
			"v_codepanel"=>htmlspecialchars($row['v_codepanel']),
