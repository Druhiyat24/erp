...
$table = "(
		SELECT concat(a.nama_group,' ',
          ...
          e.nama_contents,' ',f.nama_width) tampil
          ,g.* FROM 
...
$button = '';
		...
		$button .="<a $cl_ubah href='../master/?mod=8&mode=$mode&id=$row[id]' $tt_ubah 

                    data-toggle='tooltip' ><i class='fa fa-pencil'></i>

                  </a>";
		$button .="<a $cl_hapus href='d_len.php?mod=8&mode=$mode&id=$row[id]' $tt_hapus 
			...
			if($row["aktif"] == 'Y'){
				$button .="<a  href='dea_group.php?mod=8&mode=$mode&id=$row[id]&is_active=N&part=length' data-original-title='Inactive'
						data-toggle='tooltip' ><i class='fa fa-ban'></i>
						</a>";				
			}else{
				$button .="<a  href='dea_group.php?mod=8&mode=$mode&id=$row[id]&is_active=Y&part=length' data-original-title='Inactive'
           ...
           $data[] = array(
			...
			"kode_length"=>htmlspecialchars($row['kode_length']),
			"nama_length"=>htmlspecialchars($row['nama_length']),
