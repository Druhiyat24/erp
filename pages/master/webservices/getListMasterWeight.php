...
$button = '';
		...
		$button .="<a $cl_hapus href='d_weight.php?mod=9&mode=$mode&id=$row[id]' $tt_hapus 
			...
			if($row["aktif"] == 'Y'){
				$button .="<a  href='dea_group.php?mod=9&mode=$mode&id=$row[id]&is_active=N&part=weight' data-original-title='Inactive'
						data-toggle='tooltip' ><i class='fa fa-ban'></i>
						</a>";				
			}else{
				$button .="<a  href='dea_group.php?mod=9&mode=$mode&id=$row[id]&is_active=Y&part=weight' data-original-title='Inactive'
