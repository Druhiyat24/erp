...
$akses = flookup("hr_backpay","userpassword","username='$user'");
if ($akses=="0") 
...
{ $cri=split(":",$id_item);
  $nik = $cri[0];
  $paydate = $cri[1];
  $query = mysql_query("SELECT * FROM hr_backpay where 
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_backpay.php?mod=<?php echo $mod;?>&id=<?php echo $id_item;?>' onsubmit='return validasi()'>
        <div class='col-md-3'>                
...
<div class="box">
  <div class="box-header">
      ...
      <a href='../hr/?mod=24' class='btn btn-primary btn-s'>
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
		    $query = mysql_query("SELECT a.*,s.nama FROM hr_backpay a inner join hr_masteremployee s 
          on a.nik=s.nik ");
        $no = 1; 
        ...
        { $cri=$data['nik'].":".$data['paydate'];
          ...
          echo "
          <td>
            <a $cl_ubah href='../hr/?mod=24&id=$cri' 
              class='btn btn-info btn-s' $tt_ubah
            </a>
            <a $cl_hapus href='del_data.php?mod=$mod&id=$cri&pro=BPay' $tt_hapus";?> 
