...
{ $akses = flookup("hr_bagian","userpassword","username='$user'"); }
else if ($mod=="m17p")
{ $akses = flookup("hr_man_abs","userpassword","username='$user'"); }
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">Data Mutasi Karyawan</h3>
    <a href='../hr/form_mutasi_karyawan.php' class='btn btn-primary btn-s'>
      <i class='fa fa-plus'></i> New
    </a>
  </div>
  <div class="box-body">
    <table id="example1" class="display responsive" style="width:100%;font-size:11px;">
    <?php 
    $sql= "SELECT a.*, b.nama, b.bagian, b.department AS dpt FROM emp_history a INNER JOIN hr_masteremployee b where a.nik=b.nik ORDER BY department ASC";
    ...
    <thead>
      <tr>
        ...
        <th>Department Lama</th>
        <th>Department Baru</th>
        <th>Bagian</th>
        <th>Telah Diajukan</th>
        <th>Diketahui HR-GA</th>
        <th>Diketahui Manager Dept</th>
        <th>Disetujui General Manager</th>
        <th>Disetujui GM Produksi</th>
    ...
    { echo '
      <tr>
        <td>'.$row["nama"].'</td>
        <td>'.$row["work_information"].'</td>
        <td>'.$row["tanggal_PL"].'</td>
        <td>'.$row["dpt"].'</td>
        <td>'.$row["department"].'</td>
        <td>'.$row["bagian"].'</td>
        <td>'.$row["chief_diajukan"].'</td>
        <td>'.$row["hr_diketahui"].'</td>
        <td>'.$row["manager_diketahui"].'</td>
        <td>'.$row["general_disetujui"].'</td>
        <td>'.$row["gm_pro_disetujui"].'</td>
        <td>
          <a class="btn btn-primary btn-s" href="form_apply_karyawan.php?id='.$row["nik"].'" 
            data-toggle="tooltip" title="Update"><i class="fa fa-pencil"></i>
          </a> 
          <a class="btn btn-danger btn-s" href="del_data.php?pro=Apply&id='.$row["nik"].'"'; 
            ?>
            onclick="return confirm('Apakah anda yakin akan menghapus ?')"
            <?php 
            echo 'data-toggle="tooltip" title="Delete"><i class="fa fa-trash"></i>
          </a>  
          <!--<a class="btn btn-warning btn-s" href="pdfAply.php?id='.$row["nik"].'" 
