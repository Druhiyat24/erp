...
{	
	if($pono=="")
	{
		...
		$sql="insert into po_header (username,pono,podate,etd,eta,expected_date,id_supplier,id_terms,
			notes,ppn,tax,discount,pph,jenis,jml_pterms,type_ot)
			values ('$user','$pono','$podate','$etddate','$etadate','$expdate','$id_supplier','$id_terms',
			'$notes','$tax','$tax','$disc','$pph','M','$txtdays','$type_ot')";
		insert_log($sql,$user);
	}
	$id_po=flookup("id","po_header","pono='$pono'");
	# PO DETAIL
	$id_gen = $_POST['txtItem'];
	$id_jo = $joselect;
	$qty_po=$_POST['txtqty'];
	$curr_po=$_POST['txtcurr'];
	$unit_po=$_POST['txtunit'];
	$price_po=$_POST['txtprice'];
...
echo "<script>window.location.href='../pur/?mod=poaddL';</script>";	
?>