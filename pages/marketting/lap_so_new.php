...
echo "<div class='box'>";
  echo "<div class='box-body'>";
    ...
    echo "Tgl SO : ".$from." - ".$to." Buyer ".$buyer_cap . " Tipe ".$tipe; 
  echo "</div>";
echo "</div>";
echo "<div class='box'>";
  echo "<div class='box-body'>";
      ...
      echo "</thead>";
        $sql=mysql_query("select so_date, so_no, ac.kpno,ms.supplier,ac.styleno,sd.color, sd.size, sd.qty, sd.unit, so.buyerno,ac.smv_min, ac.smv_sec, so.fob, product_group, product_item, ac.brand, ac.main_dest,so.curr,sd.price,season 
          from so 
                                ...
                                left join masterseason msn on so.id_season = msn.id_season
        ...
        { $qtyout = flookup("sum(a.qty)","bppb a inner join so_det sod on a.id_so_det=sod.id","sod.id_so='$rs[id_so]'");
          echo "<tr>";
            ...
            echo "
              ...
              <td>$rs[brand]</td>
              <td>$rs[main_dest]</td>
              <td class='text'>$rs[styleno]</td>
              <td>$rs[color]</td>
              <td class='text'>$rs[size]</td>
              <td>$rs[qty]</td>
              <td>$rs[unit]</td>
              <td>$rs[curr]</td>
              <td>$rs[price]</td>              
