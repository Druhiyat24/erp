...
SELECT d.otherscode cod,
		...
		'3' category 
		    	...
		    	where a.id='$id_cs'	) X ORDER BY  X.id
	...
	"); 
		...
		$sumidr = 0;
		  ...
		  echo "<tr>";
            ...
            echo "<td>$data[cod]</td>";
			echo "<td style='display:none'>$data[category] </td>";
						echo "<td>$data[it]</td>";
				  ...
				  $no++; // menambah nilai nomor urut
					...
					$total_cost_plus = $sumusd + $total_vat + $total_deal + $total_ga_cost;
					$total_cost_plus_idr = $sumidr + $total_vat_idr + $total_deal_idr + $g;
			  ?> 
      </tbody>
       <tfoot>
		  <tr>
			  <td style="padding:0" > <select name ="category"  id="category"onchange="changecategory(this)" class="form-control" style="width:200px">
						<option value="1">-- Choose Kategory --</option>
						<option value="1">Costing Detail</option>
						<option value="2">Manufacturing - Complexity</option>
						<option value="3">Others Cost</option>
					</select>
			  </td>
			<td >	
			...
			<td style="padding:0">	
			 ...
			 <div class="selectMC" style="display:none">
			  ...
			  <select class='form-control select2' style='width: 100%;' 
				...
				<td>
				<a href="#" onclick='Routes("add")' />Add Item </a>  
				</td>
			  </tr>
    ...
    Sub Total (USD) :  <?php echo fn($sumusd,4)  // echo fn($tot_mf,4); ?>
    <br>
		Sub Total (IDR) : <?php echo fn($sumidr,2); ?>
...
<div class="box">
  	...
  	<table width="45%" style="font-weight:bold;">
  		...
  		<tr>
  			...
  			<td>&nbsp:&nbsp</td>
  			<td><?php echo fn($sumusd,4); ?></td>
  			<td>&nbsp &nbsp</td>
  			<td>Total Costing (IDR)</td>
  			<td>&nbsp:&nbsp</td>
  			<td><?php echo fn($sumidr,2); ?></td>
  		</tr>
  		<tr>
  			<td>VAT (USD)</td>
  			<td>&nbsp:&nbsp</td>
		    ...
		    $sql="select a.id,cost_no,cost_date,supplier,styleno,
			  ...
			  {	echo "
					<tr>
						...
						<td>$rs[fullname]</td>
						<td>
							<a class='btn btn-primary btn-s' href='pdfCost.php?id=$rs[id]'
	              data-toggle='tooltip' title='Save PDF' target='_blank'><i class='fa fa-file-pdf-o'></i></a>";
	            if ($rs['username']==$user)
	            {	echo " <a class='btn btn-info btn-s' href='?mod=5&id=$rs[id]'
	              data-toggle='tooltip' title='View'><i class='fa fa-pencil'></i></a>"; 
	            }
	            echo "
	            <a class='btn btn-warning btn-s' href='?mod=5&id=$rs[id]&pro=Copy'
