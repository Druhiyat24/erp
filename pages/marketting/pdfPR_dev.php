...
$tblbomjoit="bom_dev_jo_item_".$dateskrg;
...
class Model{
    ...
    public function get_report($jo_id,$tblbomjoit)
    {
        ...
        $sql_h = "
            ...
            from jo_dev a inner join jo_det_dev s on a.id=s.id_jo inner join so_dev d on s.id_so=d.id 
            inner join act_development f on d.id_cost=f.id inner join mastersupplier g on f.id_buyer=g.Id_Supplier
            inner join masterproduct h on f.id_product=h.id
            inner join so_det_dev i on s.id_so=i.id_so
        ...
        $sql = "
            ...
            From $tblbomjoit k INNER JOIN so_det_dev l on k.id_so_det=l.id 
...
$id = $_GET['id'];
$cekerr=flookup("posno","(select posno,count(distinct cons) jcons from bom_dev_jo_item where 
...
/*$sql="create table $tblbomjoit 
  ...
  from bom_dev_jo_item a INNER JOIN so_det_dev l on a.id_so_det=l.id 
...
*/$sql="select posno,rule_bom,id_item,nama_type 
  from bom_dev_jo_item a inner join masterdesc md on a.id_item=md.id 
...
$sql_pro="select posno,if(rule_bom='','ALL COLOR ALL SIZE',rule_bom) rule_bom,md.id_item,md.matclass nama_type  
  from bom_dev_jo_item a inner join masteritem md on a.id_item=md.id_item  
