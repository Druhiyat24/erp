...
{   
    ...
    $wsno = "";
    $wsdet = "";
    $sql=mysqli_query($con_new, "select distinct ac.kpno from po_header_dev po inner join po_item_dev poit 
      on po.id=poit.id_po inner join jo_det_dev jod on poit.id_jo=jod.id_jo inner join so_dev so on jod.id_so=so.id inner join 
      act_development ac on ac.id=so.id_cost  
      where po.id='$_GET[id]' and poit.cancel='N' order by ac.kpno");
...
if ($status=="P")
{ $sqldatatable = 'select po_header_dev.app_by,po_header_dev.tax,po_header_dev.podate, po_item_dev.price, 
    sum(po_item_dev.qty) qty, 
  sum(po_item_dev.qty * po_item_dev.price)as totalin, po_item_dev.curr, po_item_dev.unit,
  masteritem.itemdesc,po_header_dev.eta from po_header_dev INNER JOIN po_item_dev 
  on po_header_dev.id=po_item_dev.id_po inner join 
  masteritem ON po_item_dev.id_gen = masteritem.id_item
  WHERE po_header_dev.pono = "'.$PONO.'" and po_item_dev.cancel="N" group by masteritem.id_item';
}
else
{ if ($jenis_company=="VENDOR LG") 
  ...
  $sqldatatable = 'select po_header_dev.app_by,po_header_dev.tax,po_header_dev.podate, po_item_dev.price, 
    sum(po_item_dev.qty) qty, 
    sum(po_item_dev.qty * po_item_dev.price)as totalin, po_item_dev.curr, po_item_dev.unit,
    '.$flddesc.' itemdesc,po_header_dev.eta from po_header_dev INNER JOIN po_item_dev 
    on po_header_dev.id=po_item_dev.id_po inner join 
    masteritem ON po_item_dev.id_gen = '.$sql_join.' 
    WHERE po_header_dev.pono = "'.$PONO.'" and po_item_dev.cancel="N" group by '.$sql_join.'';
  #echo $sqldatatable;
}
