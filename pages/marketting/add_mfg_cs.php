<?php 
  ...
  {	$arrid_cs=explode("&",$id_cs);
	...
	{	$rs=mysql_fetch_array(mysql_query("select * from act_costing_mfg where
			id_act_cost='$id_cs' and id='$id_cs_mfg' "));
		$id_item = $rs['id_item'];
		$price = $rs['price'];
		$cons = $rs['cons'];
		$unit = $rs['unit'];
...
<div class='box'>
	<div class='box-body'>
		<div class='row'>
			<form method='post' name='form' action='s_add_mfg_cs.php?mod=<?php echo $mod; ?>&id=<?php echo $id_cs; ?>&idd=<?php echo $id_cs_mfg; ?>' onsubmit='return validasi()'>
				<div class='col-md-6'>								
					<div class='form-group'>
		              ...
		              FROM mastercf 
		              ORDER BY id DESC";
								IsiCombo($sql,$id_item,'Pilih Complexity');
							?>
						</select>
					</div>				
					<div class='form-group'>
