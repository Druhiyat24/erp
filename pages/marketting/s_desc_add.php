...
{ $cek=flookup("id_item","masteritem","id_gen='$id_item'");
  if($cek=="")
  { $sql="update masterdesc set add_info='$type'  
    where id='$id_item'";
    insert_log($sql,$user);
...
{ $sql="insert into masterdesc (id_color,kode_desc,nama_desc,add_info) 
    values ('$id_sub_group','-','-','$type')";
  insert_log($sql,$user);
  $sql="insert into masteritem (mattype,id_gen,matclass,size,color,goods_code,itemdesc,add_info)
    ...
    if(j.nama_desc='-','',concat(' ',j.nama_desc))) itemdesc,j.add_info  
