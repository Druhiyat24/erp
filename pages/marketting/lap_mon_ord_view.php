...
<script>
  ...
  { 
    var html = $.ajax
    ({  type: "POST",
        url: '../forms/ajax_bpb_jo.php?modeajax=view_list_det_mon_ord',
...
echo "<div class='box'>";
	echo "<div class='box-body'>";
		...
		echo "<table id='examplemon' $tbl_border style='font-size:11px; width: 100%;'>";
			echo "<thead>";
				echo "
				<tr>
					<th>No</th>
      ...
      $sqldatatable = "select jo.app_date pr_date,jo.id id_jo,jo.jo_date,so.so_date,ac.id,ac.cost_date,ac.app1_date,ac.kpno,ac.styleno,mb.supplier buyer,
		  	...
		  	(select id_so,sum(qty) tqty_so,min(deldate_det) min_del from so_det where cancel='N' group by id_so) sod on jod.id_so=sod.id_so
				...
				where min_del between '".fd($from)."' and '".fd($to)."' $sqlbuy $sqlsty   
				group by ac.kpno ";
      ...
      {	$cost_date = new DateTime($data['cost_date']);
          ...
          echo "
          <td $col_fab>".fd_view($eta_fab)."</td>
  				<td $col_fab>$duefab</td>
  				<td $col_fab>".fd_view($etd_fab)."</td>";
  				if($excel=="N")
          { echo "
            <td $col_fab><a href='#' style='color: black;' data-toggle='modal' 
          ...
          { echo "<td $col_sew>".$tpo_sew.' of '.$tpr_sew."</td>"; }
  			  echo "
          <td $col_sew>".fd_view($eta_sew)."</td>
  				<td $col_fab>$duesew</td>
  				<td $col_sew>".fd_view($etd_sew)."</td>";
          if($excel=="N")
          { echo "
            <td $col_fab><a href='#' style='color: black;' data-toggle='modal' 
          ...
          { echo "<td $col_pac>".$tpo_pac.' of '.$tpr_pac."</td>"; }
  				echo "
          <td $col_pac>".fd_view($eta_pac)."</td>
  				<td $col_fab>$duepac</td>
  				<td $col_pac>".fd_view($etd_pac)."</td>";
  			echo "
  			</tr>";
    		$no++;
      }
