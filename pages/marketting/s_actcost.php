...
if (isset($_POST['txtmaindest'])) {$txtmaindest = nb($_POST['txtmaindest']);} else {$txtmaindest = "";}
...
{	$date=fd($txtcost_date);
	...
	if($tipe_ws=="GLOBAL")	
	{	$cri2="GLB/".$kd_buyer."/".date('my',strtotime($date)); 
		$txtkpno=urutkan_ws("GLB-".$kd_buyer."-".date('Y',strtotime($date)),$cri2);
		$app1 = "A";
		$app1_by = "SYSTEM";
...
{	$cek = flookup("count(*)","so inner join act_costing ac on so.id_cost=ac.id","cost_no='$txtcost_no'");
	...
	{	if ($txtattach_file!="") {$sql_attach=",attach_file='$txtattach_file'";} else {$sql_attach="";}
		$sql = "update act_costing set qty='$qty',curr='$txtcurr',status='$status'
			...
			,deldate='$txtdeldate', brand = '$txtbrand', main_dest = '$txtmaindest' where cost_no='$txtcost_no'"; 
		insert_log($sql,$user);
