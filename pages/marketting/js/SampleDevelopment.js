...
function GetDetailData(ids){
	//date = document.form.txtcost_date.value;
		...
		success: function (response) {
			...
			if(d.status == 'ok'){	
				//data.subtotalusd 			  = d.records[0].
				//data.subtotalidr 			  = d.records[0].
				...
				data.totalcostingidr		  = decodeURIComponent(d.records[0].tot_cd_idr)
				data.totalcostingusd		  = decodeURIComponent(d.records[0].tot_cd)
...
$("#footerTotalCosting").text(formatRupiah(data.totalcostingusd));
$("#footerTotalCostingIdr").text(formatRupiah(data.totalcostingidr));	
