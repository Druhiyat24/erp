...
{	$sql = "insert into so (id_cost,buyerno,so_no,so_date,qty,unit,curr,fob,nm_file,username,tax,id_season,id_terms,jml_pterms,jns_so)
		values ('$txtid_cost','$txtbuyerno','$txtso_no','$txtso_date','$txtqty','$txtunit','$txtcurr','$txtfob',
		'$nama_file','$user','$txttax','$txtseason','$txtterms','$txtdays','$txtjns_so')";
	insert_log($sql,$user);
...
{	$cek=flookup("count(*)","jo_det","id_so='$id_so'");
	...
	{	$sql = "update so set buyerno='$txtbuyerno',
			...
			tax='$txttax',id_season='$txtseason',id_terms='$txtterms',jml_pterms='$txtdays',jns_so='$txtjns_so' 
			where id='$id_so'";
		insert_log($sql,$user);
