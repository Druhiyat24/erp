...
<head>
  ...
  <script type="text/javascript">
    function carinopabean(val) { 
      var jenis_dok = val; 
        // alert(jenis_dok);
        $.ajax({
          type: 'POST', 
          url: 'cari_nopabean.php', 
          data: {'jenis_dok':jenis_dok},
          success: function(response) { 
            $('#pil_no_daftar').html(response); 
          }
        });
      }

      function searchdatadetail() {
  var jenis_dok = $('select[name=jenis_dok] option').filter(':selected').val();
  var pil_no_daftar = $('select[name=pil_no_daftar] option').filter(':selected').val();

  $.ajax({
    type: 'POST',
    url: 'cari_list_data.php',
    data: {'jenis_dok': jenis_dok, 'pil_no_daftar': pil_no_daftar},
    beforeSend: function() {
      $('#tbody_monitoring').html('<tr><td colspan="100%" style="text-align: center">Loading...</td></tr>');
    },
    success: function(response) {

      if ($.fn.DataTable.isDataTable('#examplefix3')) {
        $('#examplefix3').DataTable().destroy();
      }
      $('#tbody_monitoring').empty().html(response);

      $('#examplefix3').DataTable({
        scrollY: "300px",
        scrollCollapse: true,
        paging: true,
        pageLength: 10,
    ...
    error: function(xhr, status, error) {
      ...
      $('#tbody_monitoring').html('<tr><td colspan="100%">Error loading data</td></tr>');
