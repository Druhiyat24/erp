...
<script type='text/javascript'>
  ...
  { 
    var cri_item = $('#cbows').val();
    var cri_tipe = $('#cbotipe').val();
    var cri_id_item = $('#cboitem').val();
    var cri_tipe_rak = $('#cbotipe_rak').val();
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_bpb_jo.php?modeajax=view_list_temp_loc',
        data: {cri_item: cri_item, cri_tipe: cri_tipe, cri_id_item: cri_id_item, cri_tipe_rak: cri_tipe_rak},
        async: false
    }).responseText;
    if(html)
    {  
  ...
  { 
    ...
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_bpb_jo.php?modeajax=view_list_item_temp_loc',
        data: {cri_item: cri_item, cri_tipe: cri_tipe },
        async: false
    }).responseText;
    if(html)
    {  
  ...
  // function getListData1(cri_tipe)
  // { var html = $.ajax

  //   ({  type: "POST",
  //       url: 'ajax_bpb_jo.php?modeajax=view_list_temp_loc',
  //       data: {cri_tipe: cri_tipe},
  //       async: false
  //   }).responseText;
  //   if(html)
  //   {  
    ...
    <div class="box-body">
      <table id="examplefix3" class="display responsive" style="width:100%;font-size:12px;">
        ...
        <tbody>
          <?php
          # QUERY TABLE
          $sql="SELECT brh.cancel,bpb.bpbdate,brh.dateinput,brh.location,brh.id,brh.bpbno,ac.kpno,mi.goods_code,mi.itemdesc,br.roll_no,
            ...
            where brh.location='Ok' and bpb.bpbdate >= '$tglf' and bpb.bpbdate <= '$tglt' 
          ...
          { 
            ...
            if($data['cancel']=="Y") { $colnya = "style='color:red;font-weight:bold;'"; } else { $colnya = ""; }
			echo "
            <tr $colnya>";
              if($bpbno_int!="")
        ...
        echo "<form method='post' name='form' action='save_data_bpb_roll_loc.php?mod=$mod&mode=$mode&id=$id_item' onsubmit='return validasi()'>";
          ...
          <div class='col-md-3'>
            ...
            <div class='form-group'>
              ...
              <select class='form-control select2' style='width: 100%;' name='txttipe' id='cbotipe' 
               onchange='collectdata()'>";
              $sql="select mattype isi, case when mattype = 'A' then 'ACCESORIES' else 'FABRIC' end as tampil  from masteritem 
              where mattype in ('A','F')group by mattype";
            ...
            <div class='form-group'>
              <label>Item *</label>
              <select class='form-control select2' style='width: 100%;' name='cboitem' id='cboitem' 
            ...
            <div class='form-group'>
              ...
              <select class='form-control select2' style='width: 100%;' name='txttipe_rak' id='cbotipe_rak' 
               onchange='getListData()'>";
              $sql="select '' isi,'-' tampil
              union 
              select * from
              (select id isi,concat(kode_rak,' ',nama_rak) tampil from master_rak order by kode_rak asc) a ";
