	...
	if(ISSET($type_nya)){
		...
		$sql="	SELECT MASTER.* FROM (
            ...
            WHERE 
				...
				)MASTER WHERE 1=1 AND MASTER.area NOT IN('F','LINE') AND MASTER.is_jasa = '$is_jasa'";
	...
	if($source=="LOOKUP"){
		...
		SELECT reff_doc FROM(
					SELECT A.fg_post,A.id_journal,A.reff_doc,B.v_nojournal FROM fin_journal_h A 
						INNER JOIN(
							SELECT v_listcode,is_lunas,v_nojournal,n_id FROM fin_status_journal_ap WHERE is_lunas = 'Y'
						)B ON A.reff_doc = B.v_listcode
					WHERE A.type_journal = '3' AND A.fg_post = '2' AND B.is_lunas = 'Y' GROUP BY B.n_id) X
		) AND A.v_status !='C'";
	}	
    $query = str_replace('{WHERE}', $WHERE, $query);			
	//echo $query;
...
$sql = "SELECT MASTER.id_journal
		...
		FROM(	
	SELECT 
				((IFNULL(MT.percentage,0)/100)*(FD.amount_original)    )value_pph
                 ,bpb.bpbno
				,FD.id_journal
				,FD.qty
				,FD.price
		...
		LEFT JOIN (SELECT B.id_bpb,B.amount_original,B.qty,B.price,MT.percentage pph_h,A.n_pph pph_hd,B.description,B.credit,B.curr,A.id_journal journal_h,A.fg_tax,B.row_id,B.id_journal,B.reff_doc,B.n_pph FROM fin_journal_h A
LEFT JOIN(
...
WHERE A.fg_tax = '1') FD ON FD.id_bpb = bpb.id
		...
		LEFT JOIN mtax MT ON FD.n_pph = MT.idtax
			WHERE 1=1 AND FD.id_journal IS NOT NULL 
                 AND poi.cancel = 'N' AND poh.id IS NOT NULL AND id_journal = '$id'
		group by bpb.id
...
UNION ALL
		...
		SELECT       a.reff_doc bpb_ref
					...
					WHERE 1=1 AND a.id_coa IS NOT NULL AND c.id_coa IS NULL
					AND a.is_retur_bh = 'N' AND
					a.debit > 0 AND  a.id_coa NOT IN('15204','15207') AND a.reff_doc IS NULL GROUP BY a.id_journal,a.row_id
UNION ALL
