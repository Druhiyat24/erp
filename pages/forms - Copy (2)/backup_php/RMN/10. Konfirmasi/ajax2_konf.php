...
{	$crinya = $_REQUEST['cri_item2'];
	...
	{	$arrnya=explode(":",$crinya);
		...
		$sql1="select sum(a.qty) as total, $nm_fld trans_no,a.id_jo,a.id_item,$fld_mat,s.goods_code,$fld_desc,s.color,a.qty,
			a.unit,a.confirm $add_cri from $nm_tbl a inner join $nm_mst s 
			on a.id_item=s.id_item where $nm_fld='$bppbno' ";			
		echo $sql;
...
///// RMN			
			...
			echo "<th colspan = '5' style='text-align:left'>$data1[total]</th>";
			echo "</tr>";
			echo "</tfoot>";
		}
					...
					echo "<tr>";
						...
						{ if($nm_tbl=="bpb" and $logo_company=="S")
							{	
								...
								echo "<td>$cekroll</td>";
								echo "<td>$upddok</td>";	
								if(($cekroll=="Ok" and $upddok=="Ok" and ($data['mattype']=="A" or $data['mattype']=="F") ) 
									or ($upddok=="Ok" and $data['mattype']=="N") 
									or $data['mattype']=="M" or $data['mattype']=="C"
									or substr($data['trans_no'],0,2)=="FG")
								{	$statchk="checked='checked'"; }
								else
