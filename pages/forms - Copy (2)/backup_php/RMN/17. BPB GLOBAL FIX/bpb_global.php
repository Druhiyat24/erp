...
<script type="text/javascript">
  ...
  { var id_supp = document.form.cbosupp.value;
    ...
    ({  type: "POST",
        ...
        data: {id_supp: id_supp, id_tipe: id_tipe},
        async: false
    }).responseText;
    if(html)
    {  
  ...
  { var id_ws = $('#cbows').val();
    ...
    ({  type: "POST",
        ...
        data: {id_ws: id_ws, id_tipe: id_tipe, id_supp: id_supp},
        async: false
    }).responseText;
    if(html)
    {  
...
clearInterval(intervalBpb);}
  ...
  name='cbosupp' id='cbosupp' onchange='getWS()'>
              <?php 
  ...
  <select class='form-control select2' style='width: 100%;' 
  name='cbotipe' id='cbotipe' onchange='getWS()'>
              <?php 
        ...
        <div class='form-group'>
            ...
            <input type='text' class='form-control' name='txtinvno'
  placeholder='Masukkan Nomor Invoice / SJ' value='<?php echo $txtinvno;?>'>
  ...
  <div class="box-body">
    <table id="examplefix3" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        if (bpb.qty = br.roll_qty, 'Done', 'Need') stat_lokasi, confirm, confirm_by, confirm_date, bpb.cancel
        from bpb 
        inner join bom_jo_global_item a on a.id_jo = bpb.id_jo
        inner join mastersupplier ms on bpb.id_supplier = ms.id_supplier
        inner join jo_det jd on a.id_jo = jd.id_jo
        inner join so on jd.id_so = so.id
        inner join act_costing ac on so.id_cost = ac.id
        left join bpb_roll_h brh on bpb.bpbno = brh.bpbno and bpb.id_item = brh.id_item and bpb.id_jo = brh.id_jo
        left join (select id_h,sum(roll_qty) roll_qty from bpb_roll group by id_h) br on brh.id = br.id_h
        where bpb.bpbdate >= '$tglf' and bpb.bpbdate <= '$tglt' and bpb.bpbno like 'A%' or bpb.bpbno like 'F%' 
        group by bpbno
        ...
        { 
          ...
          echo "<tr $fontcol>";
            ...
            {
              echo"
            <a href='../forms/?mod=bpb_global_item&mode=Bahan_Baku&bpbno=$data[bpbno]'
      ...
      <form method='post' name='form' action='save_bpb_global.php?mod=update&bpbno=<?php echo $bpbno; ?>'>
        ...
        <div class='form-group'>
          ...
          <div class='form-group'>
            <label>Tipe Material #</label>
            <select class='form-control select2' style='width: 100%;' 
            name='cbotipe' id='cbotipe' disabled >
              <?php 
              $sql="select distinct(mattype) isi, case mattype when 'F' then 'FABRIC'
              when 'A' then 'ACCESSORIES'
              end tampil
        ...
        <div class='form-group'>
            <label>Nomor Invoice / SJ *</label>
            <input type='text' class='form-control' name='txtinvno'
  placeholder='Masukkan Nomor Invoice / SJ' value='<?php echo $invno;?>'>
          </div> 
          <div class='form-group'>
            <label>Keterangan</label>
      ...
      <tbody>
        ...
        inner join masterproduct mp on ac.id_product = mp.id
				left join bpb_roll_h brh on bpb.id_item = brh.id_item and bpb.id_jo = brh.id_jo and bpb.bpbno = brh.bpbno
				left join (select id_h,sum(roll_qty) roll_qty from bpb_roll group by id_h) br on brh.id = br.id_h
        ...
        { 
          ...
          echo "<tr>";
            ...
            <td>
          <a 
            href='../forms/?mod=bpb_global_item_lokasi&mode=Bahan_Baku&bpbno=$bpbno&id_item=$id_item';
...
<script type='text/javascript'>
  ...
  { var total = document.getElementById('total').value;
    ...
    if (total > sisa_qty_a) { swal({ title: 'Qty tidak Boleh Melebihi Sisa', <?php echo $img_alert; ?> }); valid = false;}
    else valid = true;
    return valid;
    exit;
  }
...
function getListData()
  { var cri_item = document.form.txtroll.value;
    ...
    ({  type: "POST",
        ...
        data: {cri_item: cri_item,sat_nya: sat_nya,txtrak: txtrak},
        async: false
    }).responseText;
    if(html)
    {  
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='save_bpb_global.php?mod=simpan_lokasi' onsubmit='return validasi()'>
        <div class='col-md-3'>
        <div class='form-group'>
            <label>Nomor BPB</label>
            <input type='text' class='form-control' disabled value = '<?php echo $bpbno_int ?>'>
      ...
      <div class='col-md-3'> 
       ...
       <div class='col-md-3'> 
        ...
        <div class='form-group'>
                  <label>No. Rak *</label>
                  <select class='form-control select2' style='width: 100%;' name='txtrak' 
