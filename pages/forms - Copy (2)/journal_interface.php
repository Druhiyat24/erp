...
class Config{
    ...
    public static $journal_type = array(
		...
		'17' => 'Jurnal Subcon',
		'18' => 'Jurnal Reverse',
		'19' => 'Jurnal Retur Pengeluaran',
		'20' => 'Jurnal Retur Pemasukkan',		
    );
...
function generate_coa_id($company_code, $journal_code, $journal_date){
    ...
    $new_seq = 1;
    if(is_null($pool)){
		$sql = "INSERT INTO fin_journal_pool (segment, `year`, sequence) VALUES ('$journal_code', '$year', $new_seq);";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new,$sql);
    }else{
        $new_seq = $pool->sequence + 1;
		$sql = "UPDATE fin_journal_pool SET sequence = $new_seq WHERE segment = '$journal_code' and `year`='$year';";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new, $sql);
    }
...
function generate_packing_list($journal_code, $journal_date,$con_new__){
    ...
    $year = $date; //date('Y', strtotime($journal_date));
    $sql = "
            SELECT sequence FROM fin_journal_pool WHERE segment = '$journal_code' and year='$year';
        ";
		//echo $sql;
    ...
    if(is_null($pool)){
		$sql = "INSERT INTO fin_journal_pool (segment, year, sequence) VALUES ('$journal_code', '$year', '$new_seq');";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new__,$sql);
    }else{
        $new_seq = intval($pool->sequence) + 1;
		$sql = "UPDATE fin_journal_pool SET sequence = '$new_seq' WHERE segment = '$journal_code' and `year`='$year'";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new__,$sql);
    }
...
function generate_rekap_id($type){
    ...
    $new_seq = 1;
    if(is_null($pool)){
		$sql = "INSERT INTO fin_journal_pool (segment, `year`, sequence) VALUES ('$type', '$year', $new_seq)";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new, $sql);
    }else{
        $new_seq = $pool->sequence + 1;
		$sql = "UPDATE fin_journal_pool SET sequence = $new_seq WHERE segment = '$type' and `year`='$year'";
		insert_log_nw($sql,$_SESSION['username']);
        mysqli_query($con_new,$sql);
    }
...
if($type_nya == "WIP"){
	if($j_type=='14'){
		...
		//echo "SELECT trim(reff_doc) FROM fin_journal_d WHERE trim(reff_doc) LIKE '%".trim($id)."%' AND trim(reff_doc) LIKE '%".trim($id2)."%'";
		//die();
			}else{                                                                          
				$used = $Bm->query("SELECT trim(reff_doc) FROM fin_journal_d WHERE trim(reff_doc) LIKE '%".trim($id)."%'")->row();
			}				
...
function get_id_coa_gr_from_bpb($bpb)
{
    global $con_new;
    $Bm = new Basemodel($con_new);
		//echo " SELECT mp.gr_d, mcd.nm_coa nm_coa_d, mp.gr_k, mck.nm_coa nm_coa_k
       //id_group = '{$bpb['id_group']}' and vendor_cat = '{$bpb['vendor_cat']}";
		//die();
    $mapping_coa = $Bm->query("
...
function check_null_pono_bpb($bpbno_int){
			 global $con_new;
	    $Bm = new Basemodel($con_new);	
$sql="SELECT bpbno_int,ifnull(MAX(pono),'0')pono FROM bpb WHERE bpbno_int ='{$bpbno_int}' GROUP BY bpbno_int";
		$_bpb = $Bm->query($sql)->result();		
		if(!ISSET($_bpb[0]->pono)){
			$pono = NULL;
		}else{
			$pono = $_bpb[0]->pono;
		}
...
$sql="	SELECT MASTER.* FROM (
	...
	$sql="SELECT MASTER.* FROM (
            ...
            WHERE 
                bpb.bpbno_int = '$id'  group by bpb.id )MASTER";		
...
function get_journal_nya($id){
	...
	$Bm = new Basemodel($con_new);
            ...
            SELECT * FROM
            fin_journal_h jh
            WHERE TRIM(reff_doc) = '".TRIM($id)."' AND type_journal IN('2','17','1','19') LIMIT 1;
        ")->row();
...
SELECT 
                ...
                ,(id.qty * sd.price) invoice_amount
                ,so.curr
				,so.tax
                ,ms.supplier
                ,ms.vendor_cat
...
function update_inv_sales($invno)
{
...
function generate_period($date){
	$pecah = explode("-",$date);
	$years = $pecah[2];
	$month = $pecah[1];
	$cb_period = $month."/".$years;
	return $cb_period;
}
...
function insert_inv_sales($invno)
{
   ...
   $id_journal = generate_coa_id("NAG", $journal_code, $journal_date);
    if($invoice_request['status']){
        ...
        $dateadd = date('Y-m-d H:i:s', time());
        @$user = $_SESSION['username'];
/* echo "JURNAL:".$id_journal;
die(); */
 $company = get_master_company();
		$date_journal = $invoice_request['data'][0]->invdate;
       $dateadd = date('Y-m-d H:i:s', time());
		$period  = generate_period($date_journal);
       @$user   = $_SESSION['username'];
       $d = array(
           'company' => $company->company,
           ...
           'num_journal' => '',//$_POST['num_journal'],
           'id_journal' => $id_journal,
           'date_journal' => $date_journal,  //date('Y-m-d', time()),
...
function GetQuery_Detail_Payment($source,$param){
	...
	$query = "SELECT  A.v_nojournal
			...
			,A.n_id
			,JD.curr
			,round(JD.amount,2) bpb_amount
			,MS.supplier nm_supplier			
				FROM fin_status_journal_ap A LEFT JOIN
					(
					$j_detail
					)JD ON A.v_nojournal = JD.id_journal 
				LEFT JOIN mastersupplier MS ON MS.Id_Supplier = JD.id_supplier 
				WHERE 1 {WHERE}  AND A.v_source = 'KB' AND A.is_deleted = 'N'
			...
			SELECT AP.v_nojournal
					...
					WHERE AP.v_source = 'PO' AND AP.is_deleted = 'N' GROUP BY POI.id 
...
function GetQuery_AP_Cicilan($source,$param){
...
SELECT  AA.v_nojournal
	   ...
	   FROM(SELECT  A.v_nojournal
			...
			,A.is_deleted
			,A.n_id
			,JD.amount bpb_amount
			,JD.curr
			,MS.supplier nm_supplier	
			,JD.id_supplier id_supplier
			,MS.terms_of_pay
				FROM fin_status_journal_ap A LEFT JOIN
					(
						/* Journal Detail */
				...
				GROUP BY A.n_id,A.v_listcode
				)AA  WHERE 1  GROUP BY AA.n_id
			UNION ALL
			SELECT   A.v_nojournal
					,A.v_listcode no_payment
					,A.v_source
			...
			SELECT AP.v_nojournal
					...
					LEFT JOIN mastersupplier MS ON MS.Id_Supplier = BPB.id_supplier 
					WHERE AP.v_source = 'PO' AND AP.is_deleted = 'N'  GROUP BY POI.id 
					)A WHERE 1 {WHERE}  GROUP BY A.n_id";
	...
	if($source=="LOOKUP"){
		$WHERE .= " AND A.v_listcode NOT IN(										
		SELECT reff_doc FROM(
					SELECT A.fg_post,A.id_journal,A.reff_doc,B.v_nojournal FROM fin_journal_h A 
						INNER JOIN(
							SELECT v_listcode,is_lunas,v_nojournal,n_id FROM fin_status_journal_ap
						)B ON A.reff_doc = B.v_listcode
					WHERE A.type_journal = '3' GROUP BY B.n_id) X
		) AND A.v_status !='C'";
	}	
    $query = str_replace('{WHERE}', $WHERE, $query);			
	return $query;
...
function check_pajak($id_journal){
			global $con_new; 
			$Bm = new Basemodel($con_new);	
			$q="SELECT A.id_journal,A.row_id,A.id_po,A.is_retur_bh,MAX(A.n_ppn)n_ppn,
				if(MAX(A.n_ppn) > 0,1,0 )key_nya
				FROM fin_journal_d A 
				LEFT JOIN po_header B ON A.id_po = B.id
				WHERE A.id_journal = '$id_journal'
				AND A.credit > 0 AND A.is_retur_bh = 'N'
				GROUP BY id_journal";
...
function check_pajak_ret($id_journal){
			global $con_new; 
			$Bm = new Basemodel($con_new);	
			$q="SELECT A.id_journal,A.row_id,A.id_po,A.is_retur_bh,MAX(A.n_ppn)n_ppn,
				if(MAX(A.n_ppn) > 0,1,0 )key_nya
				...
				LEFT JOIN po_header B ON A.id_po = B.id
				WHERE A.id_journal = '$id_journal'
				AND A.credit > 0 AND A.is_retur_bh = 'Y'
				GROUP BY id_journal";
    $pajak_detail = $Bm->query($q)->result();
if(!$pajak_detail){
	 $pajaknya = array(
...
function check_nilai($id_journal){
			global $con_new; 
			$Bm = new Basemodel($con_new);	
	$q = "SELECT A.id_journal,A.row_id,A.id_po,A.is_retur_bh,IFNULL(A.n_ppn,0)n_ppn,A.id_bpb
			,A.qty
			,A.price
			,((A.qty*A.price) * (A.n_ppn/100))nilai_ppn
			FROM fin_journal_d A 
			LEFT JOIN po_header B ON A.id_po = B.id
			WHERE A.id_journal = '$id_journal'
			AND A.credit > 0 AND A.is_retur_bh = 'N'
			AND id_bpb IS NOT NULL
";		
...
function check_nilai_ret($id_journal){
			global $con_new; 
			$Bm = new Basemodel($con_new);	
	$q = "SELECT A.id_journal,A.row_id,A.id_po,A.is_retur_bh,IFNULL(A.n_ppn,0)n_ppn,A.id_bppb
			,A.qty
			,A.price
			,((A.qty*A.price) * (A.n_ppn/100))nilai_ppn
			FROM fin_journal_d A 
...
function insert_bpb_ir_pajak($id_journal)
		{	
			...
			$check_pajak_ret = check_pajak_ret($id_journal);			
			if($check_pajak['keys'] == '1'){
				...
				foreach($total_nilai as $_total_nilai){
					$nilai_pajak = $nilai_pajak + $_total_nilai->nilai_ppn;
						array_push($bpb__,$_total_nilai->id_bpb);
						array_push($ppn__,$_total_nilai->nilai_ppn);	
				}
				//echo $utang_before_pajak;
				//die();
				$amount_ppn = $nilai_pajak;
...
/* 				print_r($row_nya);
			...
			// GR
				...
				for($q=0;$q<count($bpb__);$q++){
					$sql1 = "UPDATE fin_journal_d SET debit =(debit + ((".$ppn__[$q]."))) WHERE id_journal = '".$id_journal."' AND is_retur_bh ='N' AND debit > 0 AND (id_bpb) ='".($bpb__[$q])."'  AND id_coa NOT IN('15204','15207')";
					insert_log_nw($sql1,$_SESSION['username']);
					$Bm->query($sql1);
					$sql = "UPDATE fin_journal_d SET credit =(credit + ((".$ppn__[$q]."))) WHERE id_journal = '".$id_journal."' AND is_retur_bh ='N' AND credit > 0 AND (id_bpb) ='".($bpb__[$q])."'  AND id_coa NOT IN('15204','15207')";
					insert_log_nw($sql,$_SESSION['username']);
					$Bm->query($sql);							
...
//die();
			...
			if($check_pajak_ret['keys'] == '1'){
				$total_nilai = check_nilai_ret($id_journal);
				$bpb__ = array();
				$ppn__ = array();
				...
				foreach($total_nilai as $_total_nilai){
						...
						array_push($bpb__,$_total_nilai->id_bppb);
...
/* 				print_r($row_nya);
			...
			// GR
                ...
                $_credit = array(
                    ...
                    'id_journal' => $id_journal,
                    'id_coa' => '15204',
                    'nm_coa' => 'Pajak Dibayar Dimuka PPn Masukan(Retur)',					
                    'curr' => $j_detail['data']->curr,
                    'debit' => 0,
                    'credit' => $amount_ppn,
                    'description' => 'Pajak Dibayar Dimuka PPn Masukan(Retur)',
                    'dateadd' => $j_detail['data']->dateadd,
                    'useradd' => $_SESSION['username'],
					'reff_doc' => '',
					'reff_doc2' => '',
				...
				//die();
				for($q=0;$q<count($bpb__);$q++){
					$sql = "UPDATE fin_journal_d SET debit =(debit + ((".$ppn__[$q].") WHERE id_journal = '".$id_journal."' AND is_retur_bh ='Y' AND debit > 0 AND (id_bppb) ='".($bpb__[$q])."'  AND id_coa NOT IN('15204','15207')";
					insert_log_nw($sql,$_SESSION['username']);
					$Bm->query($sql);
					$sql = "UPDATE fin_journal_d SET credit =(credit + ((".$ppn__[$q].")) WHERE id_journal = '".$id_journal."' AND is_retur_bh ='Y' AND credit > 0 AND (id_bppb) ='".($bpb__[$q])."'  AND id_coa NOT IN('15204','15207')";
					insert_log_nw($sql,$_SESSION['username']);
					$Bm->query($sql);							
...
function exist_bpb_kontra_bon($journal_nya,$bpb_nya,$id_bpb_nya){

		 global $con_new;
	    $Bm = new Basemodel($con_new);	
    $bpb_exist = $Bm->query(
"SELECT id_journal,row_id FROM fin_journal_d WHERE id_journal = '{$journal_nya}' AND id_bpb = '{$id_bpb_nya}' LIMIT 1"		
		)->row();
		if(count($bpb_exist) > 0 ){
			$key = '0';
...
SELECT X.id_journal
       ...
       ,X.id_coa
       ,X.nm_coa
	   ,SUM( (ifnull(X.percentage_nya,0)/100)* ifnull(X.amount_original,0) )val_pph
       ...
       FROM (
		SELECT 	 A.id_journal
				...
				,A.nm_coa
				,A.row_id
				,(MT.percentage) val_pph
				,IFNULL((SELECT X.percentage FROM( 
					SELECT mt.percentage,a.id_journal,a.n_pph FROM fin_journal_h a
					INNER JOIN mtax mt ON a.n_pph = mt.idtax
					WHERE a.id_journal = '$id'
					)X),MT.percentage)percentage_nya
				FROM fin_journal_d A
				INNER JOIN mtax MT ON MT.idtax = A.n_pph
			WHERE A.id_journal = '$id'
			AND credit AND id_bpb IS NOT NULL
			GROUP BY row_id)X GROUP BY id_journal";		
...
function journal_detail(){
	$q= "	
SELECT X.id_journal
		,MAX(X.id_supplier)id_supplier
		,MAX(X.curr)curr
		,SUM(X.value_pengurangan)value_pengurangan
		,SUM(X.value_penambahan)value_penambahan
		,SUM(X.pajak)value_ppn
		,SUM(X.pph)pph
		,SUM(X.amount_original)amount_original
		,MAX(id_po)id_po
		,'' nilai
		,SUM(X.amnt)amount FROM(
/*DETAIL PPH*/
SELECT   A.id_journal
		,(A.amount_original + ( (ifnull(A.n_ppn,0)/100)*A.amount_original  ) - ((ifnull(B.percentage,0)/100)*A.amount_original ) )amnt
		,A.reff_doc
		,A.reff_doc2	
		,B.percentage
		,A.row_id
		...
		,0 value_pengurangan
		,0 value_penambahan
		,( (ifnull(A.n_ppn,0)/100)*A.amount_original  )pajak
		,((ifnull(B.percentage,0)/100)*A.amount_original )pph
		,A.id_po
		,A.amount_original
		FROM fin_journal_d A 
		INNER JOIN (SELECT * FROM fin_journal_h WHERE type_journal =14 AND n_pph = '0' )A_A ON A_A.id_journal = A.id_journal
			LEFT JOIN
		mtax B ON B.idtax = A.n_pph
		WHERE A.credit >0 AND A.id_bpb IS NOT NULL
		GROUP BY A.row_id,A.id_journal
	UNION ALL	
		#HEADER PPH
		SELECT   A.id_journal
		,(A.amount_original + ( (ifnull(A.n_ppn,0)/100)*A.amount_original  ) - ((ifnull(B.percentage,0)/100)*A.amount_original) )amnt
		...
		INNER JOIN (SELECT * FROM fin_journal_h WHERE type_journal =14 AND n_pph != '0' )A_A ON A_A.id_journal = A.id_journal
			LEFT JOIN
		mtax B ON B.idtax = A_A.n_pph
		WHERE A.credit >0 AND A.id_bpb IS NOT NULL
		GROUP BY A.row_id,A.id_journal
	UNION ALL	
/*PENGURANGAN UTANG */	
SELECT id_journal
		,(-1*amount_original)amnt
		,reff_doc
		,reff_doc2
		,'' percentage
		,row_id
		...
		,0 pph
		,id_po
		,(-1*amount_original)amount_original
		FROM fin_journal_d WHERE 1=1 AND id_journal LIKE '%-PK-%'
		AND id_coa IN (SELECT id_coa FROM mapping_utang WHERE 1=1) 
		AND debit >0 AND id_bppb IS NOT NULL
UNION ALL
/*PENAMBAHAN UTANG MANUAL*/	
SELECT id_journal
		,credit amnt
		,reff_doc
		,reff_doc2
		...
		FROM fin_journal_d WHERE 1=1 AND id_journal LIKE '%-PK-%' 
		AND id_coa IN (SELECT id_coa FROM mapping_utang WHERE 1=1) 
			AND id_coa IN (SELECT id_coa FROM mapping_utang WHERE 1=1) AND
		credit >0 AND id_item IS NULL		
		UNION ALL
/*PENGURANGAN UTANG MANUAL*/
SELECT id_journal
		,(-1*debit) amnt
		,reff_doc
		,reff_doc2
		...
		FROM fin_journal_d WHERE 1=1 AND id_journal LIKE '%-PK-%' 
			AND id_coa IN (SELECT id_coa FROM mapping_utang WHERE 1=1) AND
		debit >0 AND id_item IS NULL	
)X GROUP BY X.id_journal
...
function insert_bpb_gr_ret($bpbno)
{
    global $con_new;
    ...
    $Bm = new Basemodel($con_new);
	$type_nya = substr($bpbno,0,3);
	$journal_code = Config::$journal_code['19'];

	$usages ='RETPENGELUARAN';
    $journal_date = date('Y-m-d', time());
   // $id_journal = $journal['id_journal'];
...
die(); */
    if($bpb_request['status']){
		$id_journal = generate_coa_id("NAG", $journal_code, $bpb_request['data'][0]->bppbdate);
/* 		echo $id_journal;
		die(); */
        $company = get_master_company();
        $dateadd = date('Y-m-d H:i:s', time());
        ...
        $d = array(
            'company' => $company->company,
            'period' => date('m/Y', strtotime($bpb_request['data'][0]->bppbdate)),
            'num_journal' => '',//$_POST['num_journal'],
            'id_journal' => $id_journal, 
            //'date_journal' => date('Y-m-d', time()),
			'date_journal' => $bpb_request['data'][0]->bppbdate,
            'type_journal' =>  Config::$journal_usage[$usages],
            'reff_doc' => $bpbno,
            'src_reference' => 'BPB',
            'dateadd' => $dateadd,
       ...
       $dataBpb = array();
		...
		$___bpb['fg_pkp'] = 0;
		$___bpb['ppn'] = $ppn_percentage;
		list($coa_debit, $coa_credit) =	get_id_coa_gr_from_bpb_ret($___bpb);
            // GR
            $_debit = array(
                'row_id' => '1',
                'id_journal' => $id_journal,
...
function update_bpb_gr_ret($bpbno)
{
...
function get_id_coa_gr_from_bpb_ret($bpb)
{
    global $con_new;
    $Bm = new Basemodel($con_new);

    $mapping_coa = $Bm->query("
            SELECT mp.ret_gr_d, mp.ret_gr_k,mcd_ret.nm_coa nm_coa_d ,mck_ret.nm_coa nm_coa_k 
            FROM mapping_coa mp

            LEFT JOIN mastercoa mcd_ret  ON mcd_ret.id_coa = mp.ret_gr_d
            LEFT JOIN mastercoa mck_ret  ON mck_ret.id_coa = mp.ret_gr_k			
		...
		echo "";
    $coa_debit = array(
        'id' => $mapping_coa->ret_gr_d,
        'nm' => $mapping_coa->nm_coa_d
    );
    $coa_credit = array(
        'id' => $mapping_coa->ret_gr_k,
        'nm' => $mapping_coa->nm_coa_k
    );
    return array(
        $coa_debit,
...
function check_bppb_exist($bpbno){
...
$type_journal = '19';
		...
		//
		$sql = "SELECT reff_doc,type_journal,id_journal FROM fin_journal_h WHERE trim(reff_doc) = '".trim($bpbno)."'  LIMIT 1";
    $bpb_exist = $Bm->query($sql)->row();
		if(count($bpb_exist) > 0 ){
				$key_1 = '1';
		}else{
			$key_1 = '0';
...
/* echo "SELECT reff_doc,type_journal,id_journal FROM fin_journal_h WHERE trim(reff_doc) = '".trim($bpbno)."'  LIMIT 1";
die(); */
    return $key_1;
}


function check_bppb_update($id){
		 global $con_new;
	    $Bm = new Basemodel($con_new);	

$sql_check_bppbno = "SELECT bppbno FROM bppb WHERE bppbno = '{$id}'";
$res = $Bm->query($sql_check_bppbno)->result();
if(count($res) == '0'){
	$sql_bppbno = "SELECT bppbno FROM bppb WHERE bppbno_int = '{$id}' LIMIT 1";
	$res = $Bm->query($sql_bppbno)->result();
	$id = $res[0]->bppbno;
}
		$sql="
				SELECT 
				...
				,poh.id SAVE_id_po
				,poh.pono SAVE_pono
				,bppb.id_item SAVE_id_item
				,ifnull(bppb.qty,0)SAVE_qty
				,poi.price SAVE_price
				,((ifnull(bppb.qty,0))*poi.price)SAVE_amount_ori
				,poi.id SAVE_id_po_det
				,bppb.id_supplier SAVE_id_supplier
				,'3' SAVE_type_bpb		
				,poh.ppn SAVE_ppn
                ,bppb.bppbno
				,bppb.bppbno_int
				...
				,((ifnull(poh.ppn,0)/100)*((bppb.qty*poi.price)) + bppb.qty*poi.price )nilai
            FROM 
                bppb bppb
				INNER JOIN view_bppbno_ret_pengeluaran VBPPB ON VBPPB.id = bppb.id
				INNER JOIN bpb bpb ON bpb.bpbno = bppb.bpbno_ro AND bpb.id_item = bppb.id_item AND bpb.id_jo = bppb.id_jo
                LEFT JOIN mastersupplier ms ON bpb.id_supplier = ms.Id_Supplier
			...
			#	INNER JOIN po_item poi ON poi.id_gen = mi.id_gen AND bpb.id_jo = poi.id_jo
				...
				LEFT JOIN po_item poi ON poh.id = poi.id_po AND poi.id_gen = mi.id_gen AND poi.id_jo = bppb.id_jo
								
				WHERE bppb.bppbno = '$id' AND poh.app  ='A' AND poi.cancel !='C'
				GROUP BY bppb.id
				";
...
function update_bpb_ir_ret($bpbno, $id_journal,$reff_doc2,$pph)
    {
			global $con_new; 
			$Bm = new Basemodel($con_new);
		$type_nya = substr($bpbno,0,3);	
...
function get_id_coa_ir_from_bpb_ret($bpb){
	    global $con_new;
    $Bm = new Basemodel($con_new);

        $mapping_coa = $Bm->query("
            SELECT mp.ret_ir_d, mcd.nm_coa nm_coa_d, mp.ret_ir_k, mck.nm_coa nm_coa_k
            FROM mapping_coa mp
            LEFT JOIN mastercoa mcd ON mcd.id_coa = mp.ret_ir_d
            LEFT JOIN mastercoa mck ON mck.id_coa = mp.ret_ir_k
            WHERE 
            id_group = '{$bpb['id_group']}' and vendor_cat = '{$bpb['vendor_cat']}' 
        ")->row();

        $coa_debit = array(
            'id' => $mapping_coa->ret_ir_d,
            'nm' => $mapping_coa->nm_coa_d
        );
        $coa_credit = array(
            'id' => $mapping_coa->ret_ir_k,
            'nm' => $mapping_coa->nm_coa_k
        );
        return array(
            $coa_debit,
	...
	function check_retur($bppbno){
		 global $con_new;
	    $Bm = new Basemodel($con_new);	
    $is_return = $Bm->query(
			"SELECT bppbno FROM bppb WHERE bppbno ='$bppbno'  ")->row();
