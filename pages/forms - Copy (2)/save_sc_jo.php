...
function generate_bpb_inhouse_in($bpb_date){
	...
	$quenya = "SELECT type_flag,category,code,month,years,sequence FROM bpb_inhouse_flag WHERE 1=1 AND category='OUT' AND month='{$month}' AND years='{$years}' ";
/* 	echo $quenya;
	die(); */
	...
	$bpb_no_int = trim($code."/".$category."/".sprintf("%'.02d",TRIM($month)).SUBSTR($years,2,2)."/".$sequence);
	return $bpb_no_int;
}
