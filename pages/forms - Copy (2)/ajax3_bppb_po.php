	...
	{	$ponya=$crinya;
		...
		echo "<table id='examplefix2' style='width: 100%;font-size:12px;'>";
			...
			echo "
				...
				$jenispo=flookup("jenis","po_header","id='$ponya'");
				$id_po=$ponya;
/*  echo $id_po;
die();  */
				...
				{	/*$sql="select ac.kpno,mp.product_group,'' id_supplier,0 qtyreq,mi.goods_code,mi.itemdesc,tbl_in.id_item,tbl_in.id_jo,tbl_in.qty_in,
					if(tbl_out.qty_out is null,0,tbl_out.qty_out) qty_out,
					tbl_in.unit,tbl_in.id_item_bppb 
					from masteritem mi 

INNER JOIN(
SELECT A.id,A.pono,A.app,POI.id id_poi,POI.id_gen,POI.id_jo FROM po_header A
INNER JOIN po_item POI ON A.id = POI.id_po
WHERE A.app = 'A' AND TRIM(A.pono) = TRIM('$crinya')
)PO ON PO.id_gen = mi.id_item
					inner join 
					(select id_po_item,id_item,id_jo,(qty) qty_in,unit,id_item_bppb from bpb 
						where bpbno_int LIKE '%WIP/%' AND id_supplier ='432'
						group by id_item,id_jo,id_item_bppb
				) as tbl_in ON
					tbl_in.id_jo = PO.id_jo AND mi.id_item=tbl_in.id_item
					left join 
					(select id_item,id_jo,sum(qty) qty_out,id_item_bb from bppb 
						 group by id_item,id_jo,id_item_bb) as tbl_out
					on tbl_in.id_item=tbl_out.id_item and tbl_in.id_jo=tbl_out.id_jo
					inner join jo_det jod on PO.id_jo=jod.id_jo 
...
*/
				...
				{	$sql__="SELECT   mp.product_group
		,bppb.bppbno_int bppb_exist
		,bppb.qty_exist
		,AB.id id_bpb
		,POH.id_supplier
		,0 qtyreq
		...
		,AB.id_item
		,AB.id_jo
		,IFNULL(E.sum_qty,0) qtys_in
		,C.id_po
		,C.unit
		,C.qty qtys_po
		,AB.qty qtys_out
		,(AB.qty - ifnull(bppb.qty_exist,0))stock
		,(AB.qty - bppb.qty_exist)sisa
		,AB.bpbno_int
		,E.id_item_bppb
		FROM (
	SELECT A.* FROM bpb A WHERE A.id_supplier = '432'  AND A.bpbno_int LIKE '%WIP%')AB
	LEFT JOIN(
		SELECT * FROM po_item WHERE cancel = 'N')C 
	ON AB.id_jo = C.id_jo AND AB.id_item = C.id_gen 
	LEFT JOIN(
		SELECT SUM(qty)sum_qty,id_po_item,id_item,id_item_bppb FROM bpb WHERE bpbno_int LIKE '%WIP%' AND 
	id_supplier !='432' AND id_po_item !=''  GROUP BY id_po_item,id_item )E
	ON E.id_po_item = C.id_gen
	
	LEFT JOIN (SELECT id,pono,id_supplier FROM po_header WHERE app = 'A')POH ON POH.id = C.id_po
	LEFT JOIN(SELECT *,SUM(qty)qty_exist FROM bppb WHERE bppbno_int LIKE '%WIP%' GROUP BY id_bpb)bppb ON 
	bppb.id_bpb = AB.id
	inner join jo_det jod on C.id_jo=jod.id_jo 
	inner join jo on jod.id_jo=jo.id 
	inner join so on jod.id_so=so.id
	inner join act_costing ac on so.id_cost=ac.id 
	inner join masterproduct mp on ac.id_product=mp.id
	inner join masteritem mi ON mi.id_item = C.id_gen
	WHERE C.id_po = '$id_po'";
				...
				$query_=mysql_query($sql__);
/* 				echo "<pre>";
				print_r(mysqli_fetch_object($query_));
				echo "</pre>"; */
				
				$jenismat = 'WIP';
				while($data=mysql_fetch_array($query_))

				{	$id=$data['id_item'].":".$data['id_jo'].":".$data['id_bpb'];
					...
					echo "
						<tr>
							...
							<td>$data[itemdesc]</td>
							<td>$eksdok</td>
							<td><input type ='text' size='4' name ='qtysisa[$id]' value='$stock' id='qtysisa$i' class='qtysisaclass' readonly></td>
							<td>
							...
							<td style='display:none'><input type ='text' value='IDR' style='width:70px;' name ='curr[$id]' id='curr$i' class='curr'>

							</td>
							<td style='display:none'><input type ='text' value='0' style='width:70px;' name ='price[$id]' id='price$i' class='price'></td>
						</tr>";
					$i++;
				};
		...
		echo "<table id='examplefix2' style='width: 100%;'>";
			echo "
				...
				{	
  				...
  				$crinya = str_replace("]","",$crinya);
  				$sql="select jo_no,mi.goods_code,concat(mi.itemdesc,' ',mi.add_info) itemdesc,mi.id_item,k.id_jo,
						IFNULL(tbl_in.qty_in,0) qty_in,
						IFNULL(tbl_out.qty_out,0) qty_out,
	...
	{	if($modenya=="view_list_stock_req")
		...
		echo "<table id='examplefix2' style='width: 100%;font-size:12px;'>";
			echo "
				...
				<tbody>";
				if($modenya=="view_list_stock_req")
				{	$sql="select jod.jo_no,breq.id_supplier,breq.qty qtyreq,mi.goods_code,
