...
/* 			print_r(json_decode($_POST['txtid_supplier']));
			die(); */
$nm_tbl="bpb";
...
{ # COPAS SAVE ADD
	...
	{	$JmlArray 		= $_POST['qtysc'];
		...
		$bppbno_nya 			= json_decode($_POST['txtid_supplier']);
		...
		{	if (is_numeric($value))
			...
			//SAVE BPB SCRAP JIKA BPPBNO UNDEFINED 
			//AMBIL BPPBNO DARI $_POST['txtid_supplier']
			//EDIT BY TAUFAN 2020 07 02
			/*-START-*/
			if(!ISSET($cri[2])){
				$txtreqno=$bppbno_nya[2];
			}else{
				$txtreqno=$cri[2];
			}
			/*-END-*/
		  	...
		  	$cbomat = flookup("mattype","masteritem","id_item='$txtid_item'");
				...
				{	$cek="0";
					...
					{	$sql = "insert into $nm_tbl (id_item,id_item_fg,qty,unit,curr,price,remark,berat_bersih,berat_kotor,nomor_mobil,id_supplier,invno,bcno,bcdate,
							...
							nomor_rak,status_retur,tujuan,subtujuan,id_jo,id_item_bppb,jenis_trans) values ('$txtid_item','$txtid_item_fg',
							...
							'$txtkpno','$txtnomor_rak','$retur','$txttujuan','$txtsubtujuan','$txtid_jo','$txtid_item_bppb','$txtjns_in')";
						insert_log($sql,$user);
