...
<script type='text/javascript'>
  ...
  { 
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_mut_rak.php?modeajax=view_list_rak',
        data: "raknya=" +raknya,
        async: false
    }).responseText;
    if(html)
...
<?php } else { ?>
  <form method='post' name='form' enctype='multipart/form-data' 
    action='s_mut_rak.php?mod=<?=$mod?>' onsubmit='return validasi()'>
    <div class='box'>
      <div class='box-body'>
        <div class='row'>
          <div class='col-md-3'>
            <div class='form-group'>
              <label>Rak #</label>
              <?php
              $sql="select a.id_rak_loc isi,concat(s.kode_rak,' ',s.nama_rak) tampil 
            ...
            <div class='form-group'>
              <label>Mutasi Rak #</label>
              <?php
              $sql="select id isi,concat(kode_rak,' ',nama_rak) tampil from master_rak 
                where aktif='Y' order by kode_rak";
              echo "<select class='form-control select2' style='width: 100%;' name='cbomutrak'>";
              IsiCombo($sql,'',$cpil.' Mutasi Rak #');
              echo "</select>";
              ?>
            </div>
          </div>
