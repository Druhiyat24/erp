...
// CADANGAN SQL DEST
	...
	// $sqlk = "SELECT goods_code AS kode_brg,itemname AS nama_brg,no_ws,styleno,color,size,dest,'PCS' AS unit,
	// 			SUM(qtysawal) AS saldoawal, SUM(QTYIN) AS qtyin, SUM(QTYRI) AS qtyri, SUM(QTYOUT) AS qtyout, SUM(QTYRO) AS qtyro FROM
	// 			(SELECT  A.NO_WS,b.styleno,c.product_group as goods_code,c.product_item as itemname,A.COLOR,A.SIZE,A.DEST,A.QTY AS QTYSAWAL,0 AS QTYIN, 0 AS QTYRI,0 AS QTYOUT, 0 AS QTYRO 
	// 			FROM saldoawal_gd AS a 
	// 			INNER JOIN act_costing AS B ON A.NO_WS =B.kpno 
	// 			INNER JOIN masterproduct as c on c.id=b.id_product
	// 			WHERE a.PERIODE='$tglf' AND A.KAT='FG' 
	// 			GROUP BY A.NO_WS,b.styleno,c.product_group,c.product_item,A.COLOR,A.SIZE,A.DEST
	// 			UNION
	// 			SELECT TZ.kpno AS no_ws,TY.Styleno,TY.goods_code,TY.itemname,ty.Color,ty.Size,TZ.dest,0 AS QTYSAWAL, SUM(TX.QTYIN) AS QTYIN, SUM(TX.QTYRI) AS QTYRI, SUM(TX.QTYOUT) AS QTYOUT, SUM(TX.QTYRO) AS QTYRO 
	// 			FROM
	// 			(SELECT a.id_item,a.id_so_det, SUM(a.qty) qtyin,0 AS qtyri,0 AS QTYOUT, 0 AS QTYRO FROM bpb a
	// 			WHERE a.qty>0 AND LEFT(a.bpbno,2)='FG' AND A.bpbno_int LIKE '%IN%' AND a.bpbdate>='$tglf' AND a.bpbdate<='$tglt' 
	// 			GROUP BY a.id_item ,a.id_so_det
	// 			UNION 
	// 			SELECT a.id_item,a.id_so_det,0 AS qtyin, SUM(a.qty) qtyri,0 AS QTYOUT, 0 AS QTYRO FROM bpb a 
	// 			WHERE a.qty>0 AND LEFT(a.bpbno,2)='FG' AND A.bpbno_int LIKE '%RI%' AND a.bpbdate>='$tglf' AND a.bpbdate<='$tglt' 
	// 			GROUP BY a.id_item,a.id_so_det 
	// 			UNION 
	// 			SELECT a.id_item,a.id_so_det,0 AS QTYIN, 0 AS QTYRI,SUM(a.qty) QtyOUT,0 AS QTYRO FROM bppb a 
	// 			WHERE a.qty>0 AND MID(a.bppbno,4,2)='FG' AND a.bppbno_int LIKE 'FG/OUT%' AND a.bppbdate>='$tglf' AND a.bppbdate<='$tglt' 
	// 			GROUP BY a.id_item ,a.id_so_det 
	// 			UNION 
	// 			SELECT a.id_item,a.id_so_det,0 AS QTYIN, 0 AS QTYRI,SUM(a.qty) QtyOUT,0 AS QTYRO FROM bppb a 
	// 			WHERE a.qty>0 AND MID(a.bppbno,4,2)='FG' AND a.bppbno_int LIKE 'FG/RO%' AND a.bppbdate>='$tglf' AND a.bppbdate<='$tglt' 
	// 			GROUP BY a.id_item,a.id_so_det) AS TX
	// 			INNER JOIN masterstyle TY ON TX.id_item=TY.id_item 
	// 			LEFT OUTER JOIN
	// 			(SELECT sod.id,ac.kpno,ac.styleno,sod.dest 
	// 			FROM act_costing ac INNER JOIN so ON ac.id=so.id_cost 
	// 			INNER JOIN so_det sod ON so.id=sod.id_so GROUP BY sod.id) AS TZ ON TZ.ID=TX.ID_SO_DET
	// 			GROUP BY  TZ.kpno,TY.Styleno,TY.goods_code,TY.itemname,ty.Color,ty.Size,TZ.dest) AS MGBX
	// 			GROUP BY no_ws,goods_code,itemname,color,size,unit,dest
	// 						";

	$sqlk = "SELECT goods_code AS kode_brg,itemname AS nama_brg,no_ws,styleno,color,size,'PCS' AS unit,
				SUM(qtysawal) AS saldoawal, SUM(QTYIN) AS qtyin, SUM(QTYRI) AS qtyri, SUM(QTYOUT) AS qtyout, SUM(QTYRO) AS qtyro FROM
				(SELECT  A.NO_WS,b.styleno,c.product_group as goods_code,c.product_item as itemname,A.COLOR,A.SIZE,A.QTY AS QTYSAWAL,0 AS QTYIN, 0 AS QTYRI,0 AS QTYOUT, 0 AS QTYRO 
				FROM saldoawal_gd AS a INNER JOIN act_costing AS B ON A.NO_WS =B.kpno 
				INNER JOIN masterproduct as c on c.id=b.id_product
				WHERE a.PERIODE='$tglf' AND A.KAT='FG' 
				GROUP BY A.NO_WS,b.styleno,c.product_group,c.product_item,A.COLOR,A.SIZE
				UNION
				SELECT TZ.kpno AS no_ws,TY.Styleno,TY.goods_code,TY.itemname,ty.Color,ty.Size,0 AS QTYSAWAL, SUM(TX.QTYIN) AS QTYIN, SUM(TX.QTYRI) AS QTYRI, SUM(TX.QTYOUT) AS QTYOUT, SUM(TX.QTYRO) AS QTYRO 
				FROM
				(SELECT a.id_item,a.id_so_det, SUM(a.qty) qtyin,0 AS qtyri,0 AS QTYOUT, 0 AS QTYRO FROM bpb a
				WHERE a.qty>0 AND LEFT(a.bpbno,2)='FG' AND A.bpbno_int LIKE '%IN%' AND a.bpbdate>='$tglf' AND a.bpbdate<='$tglt' 
				GROUP BY a.id_item ,a.id_so_det
				...
				INNER JOIN masterstyle TY ON TX.id_item=TY.id_item 
				LEFT OUTER JOIN
				(SELECT sod.id,ac.kpno,ac.styleno 
				FROM act_costing ac INNER JOIN so ON ac.id=so.id_cost 
				INNER JOIN so_det sod ON so.id=sod.id_so GROUP BY sod.id) AS TZ ON TZ.ID=TX.ID_SO_DET
				GROUP BY  TZ.kpno,TY.Styleno,TY.goods_code,TY.itemname,ty.Color,ty.Size) AS MGBX
				GROUP BY no_ws,goods_code,itemname,color,size,unit
							";
//cadangan yang d atas
