...
if  ($st_company!="KITE" AND $rpt=='wip_cut') {$header_cap="LAPORAN PERTANGGUNGJAWABAN MUTASI WIP CUTTING";}
...
<?PHP 
	...
	{	
			echo "<a class='btn btn-info btn-sm' href='?mod=view_mut_wip_cut&parfrom=$tglf&parto=$tglt&parfromv=$perf&partov=$pert&rptid=$rpt&dest=xls'>Save To Excel</a>";	
			echo "<br>";
			echo "-";
...
##---TEMP PENCARIAN SALDO AWAL DAN TRANSAKSI-----------------------------------------------------------------------------------------------------------------------------------
	...
	<tr>
		...
		<?php
		 if ($rpt=="hasil")
			...
			echo "<th>GROSS CUTTING OUTPUT (PCS)</th>";
			echo "<th>PERGANTIAN REJECT GARMENT (PCS)</th>";
			echo "<th>OUT KE DC (PCS)</th>";
			echo "<th>OUT (PENGGANTI REJECT GARMENT) (PCS)</th>";
	...
	$sqlk= "SELECT * from lap_wip_cut where periode >= '$tglf'
							";
	...
	{ #ketika data tabel mahasiswa di array kan maka lakukan perulangan hingga mahasiswa terakhir
		...
		$gross_cut_out=$data['gross_cut_out'];
		
		$in_ganti_reject_garment=$data['in_ganti_reject_garment'];
		$out_dc=$data['out_dc'];

		$out_ganti_reject_garment=$data['out_ganti_reject_garment'];
		...
		{	
			...
			<tr>
				...
				{	echo "
						...
						<td align='right'>$gross_cut_out</td>
						<td align='right'>$in_ganti_reject_garment</td>
						<td align='right'>$out_dc</td>
						<td align='right'>$out_ganti_reject_garment</td>
