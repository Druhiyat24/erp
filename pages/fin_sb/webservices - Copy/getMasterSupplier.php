...
class getListData {
	...
	public function getDefaultCode($idJournal){
		//print_r("ABC");
		include __DIR__ .'/../../../include/conn.php';
		$q = "SELECT A.v_idjournal,A.v_idkonsumen,B.supplier FROM fin_prosescashbank A LEFT JOIN(select supplier_code,id_supplier,supplier from mastersupplier  )B ON A.v_idkonsumen = B.supplier_code WHERE A.v_idjournal = '$idJournal' AND A.v_idkonsumen !='' ";
		//print_r($q);
		$stmt = mysql_query($q);		 
		$numberjournal = array();
		$id = array();
