...
SELECT a.id_journal id_journal_pembelian
		...
		,		a.reff_doc keterangan_pembelian
		,		a.id_coa
		,		d.supplier_code
		,		d.Supplier
		,		IF(a.curr='USD', ROUND(a.credit,2), '0')pembelian_usd
		,		IF(a.curr='USD', ROUND(a.credit*a.rate,2), a.credit)pembelian_idr_total
		,		IF(a.curr='IDR',a.credit,0)pembelian_idr
		...
		WHERE 1=1 AND (i.id_journal IS NULL OR i.id_journal !='' ) AND ((ifnull(DATE_ADD(a.date_journal, INTERVAL g.days_pterms DAY),DATE_ADD(a.date_journal, INTERVAL g.days_pterms DAY)) >= '{$d_from}') AND (ifnull(DATE_ADD(a.date_journal, INTERVAL g.days_pterms DAY),DATE_ADD(a.date_journal, INTERVAL g.days_pterms DAY)) <= '{$d_to}') ) 

		GROUP BY a.date_journal asc
)X";
## Search
$searchQuery = " ";
if($searchValue != ''){
$searchQuery = " AND (
	...
	OR X.pembelian_idr_total             			LIKE'%".$searchValue."%'
	OR X.keterangan_pembelian             		LIKE'%".$searchValue."%'
	OR X.ap_days             	LIKE'%".$searchValue."%'
...
$colomn = "		 
			...
			,X.period
			,X.pembelian_idr
			,X.pembelian_idr_total
			,X.keterangan_pembelian
			,X.ap_days
			";
...
"pembelian_idr"=>htmlspecialchars(number_format((float)$row['pembelian_idr'], 2, '.', ',')), //ok
"pembelian_idr_total"=>htmlspecialchars(number_format((float)$row['pembelian_idr_total'], 2, '.', ',')), //ok
 "keterangan_pembelian"=>htmlspecialchars($row['keterangan_pembelian']), //ok days_pterms
