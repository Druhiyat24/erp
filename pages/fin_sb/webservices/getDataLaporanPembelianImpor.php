...
$table = "(
			  SELECT mi.goods_code
                ...
                FROM bpb WHERE jenis_dok='BC 2.3' AND id_jo!='' and confirm='Y')b ON b.bpbno_int=fjh.reff_doc
                ...
                INNER JOIN (SELECT id,id_po,qty,cancel FROM po_item WHERE cancel='N') poi ON poi.id=b.id_po_item

                INNER JOIN (SELECT id,ppn, app, podate from po_header WHERE app='A') poh ON poh.id=poi.id_po
                ...
                WHERE 1=1 AND (fjh.date_journal >='$d_from' AND fjh.date_journal <='$d_to' ) AND fjh.fg_post='2' AND fjh.type_journal='2' AND ms.area='I'
				
				GROUP BY b.bpbno_int 
...
"no_bl"=>htmlspecialchars('-'),
"tgl_bl"=>htmlspecialchars('-'),
...
"bea_masuk"=>htmlspecialchars(number_format((float)$row['bea_masuk'], 2, '.', ',')),
"pph22"=>htmlspecialchars(number_format((float)$row['pph22'], 2, '.', ',')),
