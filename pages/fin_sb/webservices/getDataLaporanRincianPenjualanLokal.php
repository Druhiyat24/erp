...
FROM (SELECT ih.id, ih.invno, ih.invdate, ih.id_buyer, ih.id_pterms, ih.n_typeinvoice, ih.n_post, ih.n_idcoa, ic.bpbno FROM invoice_header ih LEFT JOIN invoice_commercial ic ON ic.n_idinvoiceheader=ih.id WHERE ih.n_typeinvoice='1' AND ih.n_post='2') a 
INNER JOIN (SELECT id, id_inv, id_so_det, qty, unit, price, bppbno FROM invoice_detail) b ON a.id=b.id_inv
INNER JOIN (SELECT Id_Supplier, Supplier, tipe_sup, supplier_code, short_name, id_coa FROM mastersupplier WHERE tipe_sup = 'C') c ON c.Id_Supplier=a.id_buyer
...
INNER JOIN (SELECT bppbno_int, bppbdate, id_item, bcno, bcdate, invno, jenis_dok, confirm, no_fp, tgl_fp FROM bppb WHERE jenis_dok='BC 4.1' AND confirm='Y')h ON h.bppbno_int=a.bpbno
