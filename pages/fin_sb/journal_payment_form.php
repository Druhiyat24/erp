...
class Model{
   ...
   public function check_bpb($id,$no_rekap,$type_nya, $checking = TRUE){
	if(ISSET($type_nya)){
        ...
        $bpb_detail = $this->query("SELECT MASTER.*,AP.n_id id_payment FROM (
				SELECT 
				 bpb.id SAVE_id_bpb
				,poh.id SAVE_id_po
				,poh.pono SAVE_pono
            ...
            WHERE 
				...
				INNER JOIN(SELECT * FROM fin_journal_d WHERE id_bpb ='$id' AND credit > 0 AND id_bpb IS NOT NULL)FD 
				ON FD.reff_doc = MASTER.bpbno_int
				INNER JOIN(SELECT n_id,v_nojournal FROM fin_status_journal_ap )AP ON AP.v_nojournal = FD.id_journal
				WHERE MASTER.is_jasa = '$is_jasa'")->result();
        ...
        if ($_POST['mode'] == 'save') {
            ...
            switch ($lookup_by) {
                ...
                case 'rekap_ap':
                    ...
                    $M->insert_rekap_payment($data['id_journal'], $coa_bank, $amount,$data['multiple']);
                default:
            ...
            if ($journal->num_journal != $data['num_journal'] and $M->get_journal_by_num($data['num_journal'])) {
                ...
                case 'rekap_ap':
                    $data['src_reference'] = 'REKAP_AP';
                    $M->insert_rekap_payment($id, $coa_bank, $amount,$data['multiple']);
                default:
                    $M->insert_bank_payment($data['id_journal'], $coa_bank, $amount);
            }
