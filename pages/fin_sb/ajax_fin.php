 ...
 public function lookup_posted_bpb($param = array()){
	 $where_po = $this->get_where_po();
      /*  $sql = "	
            ...
            LEFT JOIN (
					SELECT * FROM fin_journal_h WHERE type_journal IN ('2','17') AND reff_doc2 != ''
				) kb ON fh.id_journal = kb.reff_doc2
            WHERE fh.type_journal IN '2','17' 
				AND fh.fg_post = '2'
            ...
            GROUP BY 
				...
				WHERE 1 AND FINAL.bpbno_int NOT IN(SELECT JOINNYA.reff_doc FROM(
SELECT fh.id_journal
		,fh.type_journal 
		,fd.reff_doc
		FROM fin_journal_h fh
LEFT JOIN (SELECT id_journal,reff_doc FROM fin_journal_d WHERE 1 GROUP BY id_journal)fd
ON fh.id_journal = fd.id_journal
WHERE fh.type_journal = '14') JOINNYA)
        ...
        if(isset($param['bpbdate'])){
            $sql_where .= " AND bpb.bpbdate like '%{$param['bpbdate']}%' ";
        }				
		
		$sql_where .= "AND bpb.bpbno_int NOT IN (SELECT XX.reff_doc FROM (
...
INNER JOIN (SELECT id_journal,fg_post,type_journal FROM fin_journal_h)JH ON JD.id_journal = JH.id_journal
WHERE JH.type_journal =14 AND JH.fg_post != 2 GROUP BY JD.reff_doc)XX)";
...
$sql = "SELECT FH.id_journal
		...
		,BPB.id_supplier
		,BPPB.bppbno
		,ifnull(BPB.bpbno_int,BPPB.bppbno)bpbno_int
	...
	LEFT JOIN bppb BPPB ON BPPB.bppbno = FH.reff_doc
...
";
        ...
        $sql_where = '';
		$sql_where .="AND BPB.bpbno NOT IN($where_po)";
        if(isset($param['bpbno'])){
            $sql_where .= " AND (BPB.bpbno like '%{$param['bpbno']}%' OR BPPB.bppbno like '%{$param['bpbno']}%' ) ";
        }
        if(isset($param['bpbno_int'])){
            $sql_where .= " AND (BPB.bpbno_int like '%{$param['bpbno_int']}%' OR BPPB.bppbno like '%{$param['bpbno_int']}%') ";
        }
        if(isset($param['pono'])){
