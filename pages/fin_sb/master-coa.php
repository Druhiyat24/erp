...
.modal-dialog {
  ...
  vertical-align: middle;
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST CHART OF ACCOUNT</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="?mod=master-coa" method="post">        
        <div class="form-row">
           <div class="col-md-3">
            <label for="nama_type"><b>Category 2</b></label>            
              <select style="background-color: gray;" class="form-control select2bs4" name="nama_ctg2" id="nama_ctg2" required>
                ...
                $sql = mysql_query("select id_ctg2,ind_name, CONCAT(id_ctg2,' - ',ind_name) as name from master_coa_ctg2 group by id",$con);
            ...
            <label for="nama_group"><b>Category 5</b></label>            
              <select style="background-color: gray;" class="form-control select2bs4" name="nama_ctg5" id="nama_ctg5" required>
                ...
                $sql = mysql_query("select id_ctg5,ind_name, CONCAT(id_ctg5,' - ',ind_name) as name from master_coa_ctg5 group by id",$con);
                while ($row = mysql_fetch_array($sql)) {
                    $data = $row['name'];
                    $id_ctg5 = $row['id_ctg5'];
...
<?php
        $querys = mysqli_query($conn_li,"select useraccess.menu as menu,useraccess.username as username, useraccess.fullname as fullname, menurole.id as id from useraccess inner join menurole on menurole.menu = useraccess.menu where username = '$user' and useraccess.menu = 'Create List payment'");
...
(select id,ind_name as idnindir, eng_name as engindir from tbl_master_cashflow) indir on indir.id = coa.id_indirect) $where",$con);
