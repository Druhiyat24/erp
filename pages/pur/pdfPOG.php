...
$rs=mysqli_fetch_assoc(mysqli_query($con_new,"select po.jenis status,po.pono,'' kpno,
  po.notes,count(distinct poit.id_jo) totjo,up.fullname ,  concat(nama_pterms, ' (' ,po.jml_pterms, ' days - After',' ', mp.kode_pterms,')') ket_terms
  from po_header po inner join po_item poit on po.id=poit.id_po 
  inner join reqnon_header rnh on poit.id_jo=rnh.id 
    left join masterpterms mp on po.id_terms = mp.id
  left join userpassword up on po.username=up.username  
  where po.id='$_GET[id]' and poit.cancel='N' group by po.pono"));
$PONO = $rs['pono'];
$tipe_com =flookup("tipe_com","po_header_draft a inner join po_header b on b.id_draft = a.id","b.id='$_GET[id]'");
...
{ $sqldatatable = 'select masteritem.color,masteritem.size,po_header.pph,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
  ...
  WHERE po_header.pono = "'.$PONO.'" and po_item.cancel="N" 
  group by masteritem.id_item,po_item.price';
}
else
{ $sql_join="masteritem.id_item";
  $sqldatatable = 'select masteritem.color,masteritem.size,po_header.pph,po_header.app_by,po_header.tax,po_header.podate, po_item.price, 
    ...
    WHERE po_header.pono = "'.$PONO.'" and po_item.cancel="N" 
	group by '.$sql_join.',po_item.price';
  #echo $sqldatatable;
}
...
if (mysqli_num_rows($result)) {
    ...
    <tr>
        ...
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:13%; text-align:right; "><p style="text-align:right;">'.number_format($row["price"],2).'</p></td>
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:4%; text-align:right;">'.$row["curr"].'</td>
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:10%; text-align:right; "><p style="text-align:right;">'.number_format($row["totalin"],2).'</p></td>
...
<body >
    ...
    <table style="width:85%;font-size:12px;">
        <tr>
            <td style="width:30%"><p style="font-weight:bold;">Our Order Reference:</p></td>
            <td style="width:25% "><p style="font-weight:bold;">Order Date:</p></td>            
            <td style="width:25% "><p style="font-weight:bold;">Validate By:</p></td>
            <td style="width:15% "><p style="font-weight:bold;">Tipe PO:</p></td>
        </tr>
        <tr>
            <td style=" ">'.$PONO.'</td>
    ...
    <table style="width:100%; margin-top:25px; border-collapse: collapse;font-size:11px;">
          <tr>
              ...
              <td style="border-collapse: collapse; border-bottom:solid 2px black; width:15%; text-align:right;"><p style="font-weight:bold; text-align:center;">Unit Price</p></td>
              <td style="border-collapse: collapse; border-bottom:solid 2px black; width:15%; text-align:right;" colspan="2"><p style="font-weight:bold; text-align:center;">Net Price</p></td>
          </tr>
            '.$tableatas.'
            <tr>
    ...
    <table align="right" style="width:50%; margin-top:15px; border-collapse: collapse;font-size:12px;">
            ...
            $ppnnya = $datappn['tot_ppn'];

            $subnya = $jumlah + $total_add;
            $tot_jpph = ($subnya*$pphnya/100);
            $tot_ppn  = $bunga - $ppnnya;
            $totalnya = $subnya + $tot_jpph + $tot_ppn;
            ...
            $html .='<tr>
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px;">
                  <p style="font-weight:bold">PPh '.$pphnya.' %</p>
                </td>
                <td style="text-align:right;border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px;">
                  <p style="text-align:right;">'.$currency.' '.fn($tot_jpph,2).'</p>
                </td>
            </tr>
            <tr>
                ...
                <td style="text-align:right;border-collapse: collapse; border-bottom:solid 2px black; padding:5px;">
                  <p style="text-align:right;">'.$currency.' '.number_format(($tot_ppn),2).'</p>
                </td>
            </tr>
            <tr>
                <td style="padding:5px;"><p style="font-weight:bold">Total</p></td>
                <td style="padding:5px;text-align:right;"><p style="text-align:right; font-size:18px; font-weight:bold">'.$currency.' '.number_format($totalnya,2).'</p></td>
            </tr>
    </table>
