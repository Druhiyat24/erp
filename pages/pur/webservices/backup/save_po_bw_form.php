...
{	if ($fld!="" AND $tbl!="" AND $criteria!="")
	...
	public function insert_header($header,$username){
		$insert = "insert into po_header_draft (username,draftno,draftdate,etd,eta,expected_date,id_supplier,id_terms,
			notes,n_kurs,ppn,tax,discount,pph,jenis,jml_pterms,fg_pkp,id_dayterms)
			...
			'{$header['txtnotes']}','{$header['n_kurs']}','{$header['ppn_nya']}','{$header['ppn_nya']}','{$header['txtdisc']}','{$header['txtdisc']}','{$header['jenis_item']}','{$header['txtdays']}','{$header['pkp']}','{$header['txtid_dayterms']}')";
/* 			echo $insert;
			die();  */
