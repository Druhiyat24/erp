...
{ 
  ...
  $query = mysql_query("SELECT a.*,s.curr,if(draftno like '%GLB%','global','regular') cek_po FROM po_header_draft a inner join po_item_draft s on a.id=s.id_po_draft 
...
<?php 
    $query_ok = mysql_query("
SELECT  
count(a.id) tot,
if(k.id is null,'Check BOM','Ok') status
FROM po_item_draft_upload a
left join masteritem mi on a.itemname = mi.nm_bom
left join jo on a.jo = jo.jo_no
left join (
select id_po_draft, curr from po_item_draft where id_po_draft= '$id_po' and cancel = 'N' limit 1 
...
where a.id_po_draft = '$id_po' and if(k.id is null,'Check BOM','Ok') = 'Ok'
group by if(k.id is null,'Check BOM','Ok')     
    
    ");
    $data_cek_ok = mysql_fetch_array($query_ok);
    $ok = $data_cek_ok['tot'];

    $query_gagal = mysql_query("
SELECT  
count(a.id) tot,
if(k.id is null,'Check BOM','Ok') status
FROM po_item_draft_upload a
...
group by if(k.id is null,'Check BOM','Ok')     
    ...
    ");
    $data_gagal= mysql_fetch_array($query_gagal);
    $cek = $data_gagal['tot'];
...
<div class='box'>
  ...
  <div class='box-body'>
      ...
      <div class='col-md-6'>
        <form method="post" enctype="multipart/form-data" action="upload_aksi.php?id_po=<?php echo $id_po?>">
          <label>File Upload : </label>
          <input name="fileimport" type="file" required="required" class='form-control'>
      </div>
      <div class='col-md-6'>
        <div style="padding:24px">
          <button name="upload" type="submit" value="Import" class='btn btn-warning'><i class='fa fa-upload'></i> Import</button>
          </form>
          <button onclick="window.location.href='../pur/contoh/import.xls'" class='btn btn-info'><i class="fa fa-download"></i> Contoh</button>    
        </div>
      </div>
      <div class='col-md-2'>
      <input type='text' style="color: green;" name='data_ok' id='data_ok'  value='<?php echo $ok;?> Ok' disabled >
      </div>
      <div class='col-md-2'>
      <input type='text' style="color: red;" name='data_cek' id='data_cek'  value='<?php echo $cek;?> Check Bom' disabled>
      </div>      

      <table id="examplemcc" class="display responsive" style="width: 100%;font-size:13px;">
        <thead>
          <tr>
            <th style : width='2%' ;>No</th>
            <th style : width='10%' ;>ID Gen</th>
            <th>JO#</th>
            <th>Item Name</th>
            <th>Qty</th>
            <th>Unit</th>
...
where a.id_po_draft = '$id_po'

      ");
          $no = 1;
          while ($data = mysql_fetch_array($query)) {

            $status_fix = $data['status'];
            if ($status_fix == 'Ok') {
              $fontcol = 'green';
            } else {
              $fontcol = 'red';
...
<div style="float: left;">      
        <form method="post" action="upload_undo.php?id_po=<?php echo $id_po?>">
          <button name="undo" type="submit" value="undo" class='btn btn-danger'><i class='fa fa-refresh'></i> Undo</button>
          </form> 
        </div>
<div style="float: right;">      
  <form method="post" action="upload_simpan.php?id_po=<?php echo $id_po?>">
          <button name="simpan" type="submit" value="simpan" class='btn btn-success'><i class='fa fa-check'></i> Simpan</button>
          </form> </div>
...
$query = mysql_query("select pid.id,ac.kpno,jo_no, mi.itemdesc, k.qty qty_bom,k.unit unit_bom, pid.unit, pid.qty,pid.price, pid.qty * pid.price total , pid.cancel from po_header_draft phd 
...
inner join bom_jo_global_item k on k.id_jo = pid.id_jo and k.id_item = mi.id_item
where id_po_draft = '$id_po'
