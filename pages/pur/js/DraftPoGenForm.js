$(document ).ready(function() {
	...
	console.log($split_nya);
	  Opt("webservices/Opt_dept.php")//OPTION jenis item
	  .then(data 				    => generate_option(data,'Choose Dept'))//generate bkb 
	  .then(inj_option 				=> injectOptionToHtml(inj_option,'cboDept')) //generate bkb 
	  ...
	  .then(AAA						=> Opt("webservices/Opt_supplier.php")) //generate supplier 
	  .then(data					=> generate_option(data,'Choose Supplier')) //generate supplier 
	  .then(inj_option 				=> injectOptionToHtml(inj_option,'cbosupp')) // generate supplier 
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		...
		$("#n_installment").val(decodeURIComponent(json.records[0].n_installment)),
		$("#datepicker_installment").datepicker().datepicker("setDate", decodeURIComponent(json.records[0].d_installment)),
		
		setTimeout(function(){
			console.log(json.records[0].fg_pkp);
			if(decodeURIComponent(json.records[0].fg_pkp) == '1' ){
				$("#fg_installment").prop( "checked", true );
				console.log("checklist_installment");
			}			
			...
			getJOList();
			setTimeout(function(){
					var t_kode_dept = [];		//cboDept			
					for(var i=0;i<json.kode_dept.length;i++){
						t_kode_dept.push(json.kode_dept[i].kode_dept);
					}
					$("#cboDept").val(t_kode_dept).trigger("change");
...
{ var id_jo = $('#cboJO').val();
if($G_kondisi == '1'){
	var url_nya = "ajax_po_draf.php?modeajax=view_list_req";
	var data_nya = { id_draft_po : $G_id_url,id_jo: id_jo }
}else{
	var url_nya = "ajax_po.php?modeajax=view_list_req";
	var data_nya = { id_jo: id_jo };		
}
var html = $.ajax
({  type: "POST",
	url: url_nya,
...
function getListDataHeader(my_id,kondisi) {
	if(kondisi == '1'){
		return new Promise(function (resolve, reject) {
			$.ajax({
				type: "POST",
				url: "webservices/GetListDataPoDraftGen_Form.php",
			data: { code: '1', id: my_id },     // multiple data sent using ajax
...
//	  {
	...
	function Save(){
		...
		$data.fg_installment        =$('#fg_installment').val(); //n_installment
		$data.jumlah_installment    =$('#n_installment').val();
		$data.start_installment     =$('#datepicker_installment').val();
    ...
    for (var i = 0; i < chks.length; i++)
    {
    	...
    	if($data.txtdrafpodate ==''){
    		swal({ title: 'Tanggal Tidak Boleh Kosong', imageUrl: '../../images/error.jpg' }); return false;;
...
/* 		if(qtyover >0){
	...
	$.ajax("webservices/save_po_gen_form.php",
	{
		type: "POST",
