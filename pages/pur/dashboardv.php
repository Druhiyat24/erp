...
<script>
  $(document).ready(function() {
    var table = $('#examplefix_po_da').DataTable
    ({  scrollY: "300px",
        scrollCollapse: true,
        paging: true,
...
echo "<div class='box'>";
	echo "<div class='box-body'>";
		...
		echo "<table id='examplefix_po_da' $tbl_border style='font-size:11px; width: 100%;'>";
			echo "<thead>";

      // if($tpr_fab==$tpo_fab) && ($tpr_sew==$tpo_sew) && ($tpr_pac==$tpo_pac)
      // {
      //   echo "<tr></tr>";
      // }
...
inner join (select id_so,deldate_det, current_date() 
	  ...
	  if($tprs_pac == $tpr_pac){
          ...
          { echo "<td $col_sew>".$tpo_fab.' of '.$tpr_fab."</td>"; }
          echo "";
  				if($excel=="N")
          { echo "
            <td $col_sew><a href='#' style='color: black;' data-toggle='modal' 
	  ...
	  if($tpo_pac == $tpr_pac){
		  ...
		  /* SEGMENT 2 */
		if(($point_tprs_fab == '1' && $point_2_tprs_fab == '1' && $point_tprs_sew == '1' && $point_2_tprs_sew =='1' && $point_tprs_pac == '1' && $point_2_tprs_pac == '1' ) ){
			$check = '1';
		}else{
			$check = '0';
		}
...
/*       if  ( 
          // ( ($tprs_fab == $tpo_fab) && ($tpr_fab == $tpo_fab) 
          //   && ($tprs_sew == $tpo_sew) && ($tpr_sew == $tpo_sew) 
          //   && ($tprs_pac == $tpo_pac) && ($tpr_pac == $tpo_pac) )

( ($tpo_fab) == ($tpr_fab && $trps_fab) 
   && ($tpo_sew && $tpr_sew) == ($tpr_sew && $trps_sew) 
   && ($tpo_pac) && ($tpr_pac && $tprs_pac) )
