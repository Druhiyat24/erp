...
$sql = "select po.jenis status,po.draftno,'' kpno,
 ...
 // echo "<pre>$sql";
 // die();
$rs=mysqli_fetch_assoc(mysqli_query($con_new,$sql));
$PONO = $rs['draftno'];
$tipe_com =flookup("tipe_com","po_header_draft","id='$_GET[id]'");
if ($rs['totjo']>1)
{   $wsno = "(Combine)";
    $wsdet = "";
    $sql=mysqli_query($con_new, "select distinct ac.kpno from po_header_draft po inner join po_item_draft poit 
...
$space=" ";
if ($status=="P")
{ $sqldatatable = 'select masteritem.color,masteritem.size,po_header_draft.pph,po_header_draft.app_by,po_header_draft.tax,po_header_draft.draftdate, po_item_draft.price, 
    sum(po_item_draft.qty) qty, 
  sum(po_item_draft.qty * po_item_draft.price)as totalin, po_item_draft.curr, po_item_draft.unit,
  concat(masteritem.itemdesc,"'.$space.'",masteritem.color,"'.$space.'",masteritem.size) itemdesc,po_header_draft.eta from po_header_draft INNER JOIN po_item_draft 
...
{ $sql_join="masteritem.id_item";
  $sqldatatable = 'select masteritem.color,masteritem.size,po_header_draft.pph,po_header_draft.app_by,po_header_draft.tax,po_header_draft.draftdate, po_item_draft.price, 
    ...
    po_header_draft.eta from po_header_draft INNER JOIN po_item_draft 
    on po_header_draft.id=po_item_draft.id_po_draft inner join 
    masteritem ON po_item_draft.id_gen = '.$sql_join.' 
    WHERE po_header_draft.draftno = "'.$PONO.'" and po_item_draft.cancel="N" group by '.$sql_join.'';
  #echo $sqldatatable;
}
...
if (mysqli_num_rows($result)) {
    ...
    <tr>
        ...
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:10%; text-align:right; "><p style="text-align:right;">'.number_format($row["price"],2).'</p></td>
...
<body >
    ...
    <p style="font-size:14px">'.$PONO.' '.$wsno.'</p>
    ...
    <table style="width:100%; margin-top:25px; border-collapse: collapse;font-size:11px;">
          <tr>
              ...
              <td style="border-collapse: collapse; border-bottom:solid 2px black; width:12%; text-align:right;"><p style="font-weight:bold; text-align:center;">Unit Price</p></td>
