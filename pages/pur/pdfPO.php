...
<body >
    ...
    <table align="right" style="width:50%; margin-top:15px; border-collapse: collapse;font-size:12px;">
            <tr>
                ...
                <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; border-top:solid 2px black; padding:5px;text-align:right"><p style="text-align:right;">'.$currency.' '.number_format($jumlah,$decim).'</p></td>
            </tr>';

             $sqladdbiaya = mysql_query("select nama_kategori,if(kategori = 'Plus',sum(total),(sum(total) * -1)) total from po_add_biaya a inner join po_master_pilihan b on b.id = a.id_kategori inner join po_header c on c.id_draft = a.id_po_draft where c.id = '$_GET[id]' GROUP BY a.id_kategori");
            // echo $sqlsaldo;
            $total_add = 0;
            ...
            $sqlppnbiaya = mysql_query("select round(sum(total * if(kategori = 'Min',a.ppn,(a.ppn * -1))/100),2) tot_ppn from po_add_biaya a inner join po_master_pilihan b on b.id = a.id_kategori inner join po_header c on c.id_draft = a.id_po_draft where c.id = '$_GET[id]'");
            // echo $sqlsaldo;
            $tot_ppn = 0;
