...
echo "<div class='box'>";
	echo "<div class='box-body'>";
		...
		if ($excel=="N") 
		{ echo "<br><a href='?mod=$mod&mode=$mode&frexc=$from&txtsupplier=$txtsupplier&txtbuyerr=$txtbuyer&toexc=$to&dest=xls'>Save To Excel</a></br>"; }
	echo "</div>";	
echo "</div>";
echo "<div class='box'>";
	echo "<div class='box-body'>";
		...
		echo "<table id='example1' $tbl_border style='font-size: 12px; width: 100%;' class='display responsive'>";
		  ...
		  $sqldatatable = "select mi.id_item,poh.app,poh.app_by,poh.app_date,poh.pono,ac.kpno,poh.app_by,poh.tax,poh.podate, poi.price, 
		    ...
		    mi.goods_code,mi.itemdesc,mi.color,mi.size, mb.id_supplier id_buyer,
		    mb.supplier buyer,ms.supplier,ms.country,poh.eta,poh.etd, phd.tipe_com, mcon.id id_contents
		    ...
		    left join masterdesc mdesc on mi.id_gen = mdesc.id
				left join mastercolor mcol on mdesc.id_color = mcol.id
				left join masterweight mwe on mcol.id_weight = mwe.id
				left join masterlength mle on mwe.id_length = mle.id
				left join masterwidth mwd on mle.id_width = mwd.id
				left join mastercontents mcon on mwd.id_contents = mcon.id
		    left join (select id,tipe_com from po_header_draft) phd on poh.id_draft = phd.id
		    ...
		    where 
		    poh.podate between '".fd($from)."' and '".fd($to)."' and poh.jenis='M' $supplier $buyer
		    union all 
		    select mi.id_item,poh.app,poh.app_by,poh.app_date,poh.pono,jod.reqno,poh.app_by,poh.tax,poh.podate, poi.price, 
		    poi.qty qtypo,tmp_bpb.qty_bpb,tmp_bpb.qtyreject_bpb,tmp_bpb.fullqty_bpb,so.qty as qty_order,if(poi.cancel='Y','Cancelled','') status_cx, 
		    poi.price pricepo,poi.curr, poi.unit,
		    mi.goods_code,mi.itemdesc,mi.color,mi.size,'-' id_buyer,
		    '-' buyer,ms.supplier,ms.country,poh.eta,poh.etd, phd.tipe_com, mcon.id id_contents
		    from po_header poh INNER JOIN po_item poi on poh.id=poi.id_po inner join 
		    ...
		    on tmp_bpb.id_po_item=poi.id
		    where 
		    poh.podate between '".fd($from)."' and '".fd($to)."' and poh.jenis='N' $supplier $buyer_gen
  			union all 
  			select mi.id_item,poh.app,poh.app_by,poh.app_date,poh.pono,ac.kpno,poh.app_by,poh.tax,poh.podate, poi.price, 
		    poi.qty qtypo,tmp_bpb.qty_bpb,tmp_bpb.qtyreject_bpb,tmp_bpb.fullqty_bpb,so.qty as qty_order,if(poi.cancel='Y','Cancelled','') status_cx, 
		    poi.price pricepo,poi.curr, poi.unit,
		    mi.goods_code,mi.itemdesc,mi.color,mi.size,mb.id_supplier id_buyer,
		    mb.supplier buyer,ms.supplier,ms.country,poh.eta,poh.etd, phd.tipe_com, mcon.id id_contents
		    ...
		    on tmp_bpb.id_po_item=poi.id
		    where 
		    poh.podate between '".fd($from)."' and '".fd($to)."' and poh.jenis='P' $supplier $buyer "; 
  	// echo $sqldatatable;
