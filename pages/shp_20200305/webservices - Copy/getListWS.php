...
class getListData {
	...
	LEFT JOIN (
		SELECT MAX(dest)destination,id_so,id FROM so_det GROUP BY id_so,dest
	...
	AND SUBSTRING(BPPB.bppbno,4,1) != 'C'
		...
		$my_id = 1; 
 		while($row = mysqli_fetch_array($MyList)){
			$checkbox = '';//idcosting
		if($id_inv != '0'){
			$checkbox .= "<input type='checkbox' data-idcosting='$row[idcosting]' data-idcustomer='$row[idcustomer]' data-bppbno_int='$row[bppbno_int]' data-bppbno_int='$row[bppbno_int]' checked='checked' disabled onclick='GenerateDetail(this)' id='$row[bppbno]' />";
		}else{
			$checkbox .= "<input type='checkbox' data-idcosting='$row[idcosting]' data-idcustomer='$row[idcustomer]' data-bppbno_int='$row[bppbno_int]' data-bppbno_int='$row[bppbno_int]' onclick='GenerateDetail(this)' id='$row[bppbno]' />";
		}			
