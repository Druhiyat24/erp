	...
	public function get($bppb){
		...
		//print_r($multiple);
			...
			for($x=0;$x<$count;$x++){
				...
				$id_inv = $val_data_type = $multiple->arraynya[$x]->id_inv;
			}
		if($id_inv == '0'){
			$where_inv = '';
		...
		LEFT join 
        (select a.id_so_det,a.qty qty,a.bppbno,a.bppbno_int from bppb a inner join so_det s on a.id_so_det=s.id inner join so d on s.id_so=d.id where d.id_cost IN ($val_idcosting)) co on sod.id=co.id_so_det
        left join 
        (select a.id_so_det,a.qty qty,a.lot,a.carton,a.price price_invoice,a.bppbno bppbno_inv,a.id id_inv_det,a.id_inv from invoice_detail a inner join so_det s on a.id_so_det=s.id inner join so d on s.id_so=d.id where d.id_cost IN ($val_idcosting)) mo on sod.id=mo.id_so_det
