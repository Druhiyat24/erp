...
FROM(
	...
	SELECT   BKB.bppbno_int
			...
			LEFT JOIN(
			SELECT B.bpbno,A.id id_inv_det,A.id_inv,A.id_so_det,A.qty,A.price,A.lot,A.carton,A.composition,A.bppbno FROM invoice_detail A
					LEFT JOIN
			...
			LEFT JOIN(
			SELECT A.id id_inv_det,A.id_inv,A.id_so_det,A.qty,A.price,A.lot,A.carton,A.composition,A.bppbno FROM invoice_detail A
 ...
 if($id == '00'){
	...
	$row['id_inv_det'] = '0';
		...
		$price .= "<input onkeyup='handledetail(this)' readonly style='width:50%' type='text' value='".$row['price_so']."' class='PRICE__".$my_id."'>";
		$qty_invoice .= "<input onkeyup='handledetail(this)' style='width:50%' readonly type='text' value='".$row['qty']."' class='QTYINV__".$my_id."'>";
