...
function CallDetailServices($stringify__){
  ...
  $.ajax({		
        type:"POST",
        cache:false, 
        url:"webservices/getListInvoiceDetail.php?bppbno_int="+$stringify__,
...
function handledetail(that){
	...
	console.log($myjson_detail.data[ind].price_val);		
		...
		$myjson_detail.data[ind].lot_val = that.value;
	}	
...
/*     TableDetail.clear();
	...
	TableDetail.destroy();	
		...
		TableDetail = $('#TableDetail').DataTable( {
				//"ajax": "webservices/getDataLaporanTrialBalance.php?from="+from+"&to="+to,
					"ajax":  "webservices/getListInvoiceDetail.php?bppbno_int="+$stringify,
				"columns": [
						{ "data": "bppbno_int"}, 
						{ "data": "ws"},
						{ "data": "styleno"},
						...
						"data":           null,
						"mRender": function (data) {
							return decodeURIComponent(data.carton);
									}
					} ,  
						{
						"orderable":      '',
