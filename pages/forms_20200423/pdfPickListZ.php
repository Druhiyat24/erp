    ...
    public function get_report($bppbno)
    {
        ...
        $sql = "
            ...
            FROM 
                ...
                INNER JOIN 
                (select jo_no,id_so,id_jo from jo_det jod inner join jo on jod.id_jo=jo.id 
                    group by id_jo)  jod on b.id_jo=jod.id_jo 
                inner join 
                (select so.id,id_cost,min(sod.deldate_det) mindeldate from so inner join so_det sod on so.id=sod.id_so group by so.id) so on jod.id_so=so.id 
                inner join act_costing ac on so.id_cost=ac.id 
                left join 
                (select id_item,id_jo,group_concat(distinct nomor_rak) no_rak,sum(qty) qtyloc,unit unitloc from bpb 
                  group by id_item,id_jo) tbllok
...
<h3 class="text-center">PICK LIST</h3>
<?php
$jolist=flookup("group_concat(distinct jo_no)","bppb_req a inner join jo  
    on a.id_jo=jo.id","bppbno='$id' group by bppbno");
?>
    ...
    <table class="table table-bordered table-condensed">
        ...
        <?php foreach($d as $_d):?>
            <tr>
                ...
                <td><?=$_d->color?></td>
                <td><?=$_d->location?></td>
                <td class="text-right"><?=$_d->loc_qty?></td>
                <td><?=$_d->loc_unit?></td>
