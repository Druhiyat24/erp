		...
		{	if (is_numeric($value))
		    ...
		    $txtid_item=$cri[0];
		  	...
		  	$cbomat = flookup("mattype","masteritem","id_item='$txtid_item'");
				...
				if ($txtqty>0)
				{	$sql = "update bpb set qty='$txtqty',remark='$txtremark',jam_masuk='$txtjam_masuk',
						berat_bersih='$txtberat_bersih',berat_kotor='$txtberat_kotor',nomor_mobil='$txtnomor_mobil',
						invno='$txtinvno',nomor_kk_bc='$txtkkbc',nomor_rak='$txtnomor_rak' where bpbno='$txtbpbno' and id='$line_item'";
					insert_log($sql,$user);
					calc_stock($cbomat,$txtid_item);
				}
			}
		}
		$sql = "update bpb set bpbdate='$txtbpbdate',bcno='$txtbcno',bcdate='$txtbcdate',nomor_aju='$txtbcaju',
			tanggal_aju='$txttglaju' where bpbno='$txtbpbno' ";
		insert_log($sql,$user);
	}
	# END COPAS SAVE ADD
}
