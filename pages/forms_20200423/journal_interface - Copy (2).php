...
function insert_bpb_gr($bpbno)
{
    ...
    $bpb_request = check_bpb($bpbno,$type_nya);
//print_r($bpb_request);
...
function get_journal_nya($id){
	...
	$Bm = new Basemodel($con_new);
            ...
            SELECT * FROM
            fin_journal_h jh
            WHERE TRIM(reff_doc) LIKE '%".TRIM($id)."%' AND type_journal IN('2','17') LIMIT 1;
        ")->row();
...
function update_bpb_gr($bpbno)
{
    ...
    $bpb_request = check_bpb_update($bpbno,$type_nya);
    if($bpb_request['status']){
        $company = get_master_company();
        $dateadd = date('Y-m-d H:i:s', time());
        @$user = $_SESSION['username'];
...
function insert_inv_sales($invno,$faktur_pajak,$discount_nya,$fg_discount_nya)
{
...
function GetQuery_Allokasi_AR($source){
	...
	/*$query = "SELECT AR.id_rekap
		...
		,(SUM(FD.nilai_journal) - SUM(ifnull(FD_PPH.debit,0)))nilai_ori_after_potongan
		,SO.curr
		,ID.id_so_det 
		,ID.jqty
		,MS.Supplier
		...
		LEFT JOIN(
			SELECT debit,id_coa,nm_coa,id_journal  FROM fin_journal_d WHERE id_journal LIKE '%IJ%' AND 
...
WHERE FH.type_journal ='1' AND fg_post = '2' AND FH.reff_doc IS NOT NULL AND  AR.id_rekap NOT IN(SELECT reff_doc2 FROM fin_journal_h WHERE type_journal IN ('4','13')) GROUP BY AR.id_rekap";
		//echo $query;		
	return $query;
}
			...
			,MS.supplier nm_supplier			
				FROM fin_status_journal_ap A LEFT JOIN
					(SELECT id_journal
						...
						,reff_doc
						,reff_doc2
						FROM fin_journal_d WHERE id_coa !='15204' GROUP BY id_journal	
...
SELECT 	
			...
			,SUM(C.debit)debit
			,SUM(C.utang)utang	
			,(SUM(C.debit) - ((ifnull(C.percentage,0)/100) * (SUM(C.debit))) + SUM(C.pajak) - SUM(C.utang))nilai
			,C.bpb_ref		
			,C.journal_ref		
			,C.fg_tax		
			,C.n_pph		
...
LEFT JOIN (select id_jo,id_po,id_gen,cancel FROM po_item)POI_WIP ON BPB.id_jo = POI_WIP.id_jo AND BPB.id_item = POI_WIP.id_gen
...
function update_bpb_ir($bpbno, $id_journal,$reff_doc2)
    {
			global $con_new; 
			$Bm = new Basemodel($con_new);
		$type_nya = substr($bpbno,0,3);	
