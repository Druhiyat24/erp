...
{	
	...
	{	
		$fld="trans_no,username,sesi,id_item,tanggal_aju,nomor_aju,supplier,
	  	...
	  	,styleno,kpno,rakno";	
		$fld2="'Out','$user','$sesi',id_item,bppbdate,concat(if(bppbno_int!='',bppbno_int,bppbno),' ',ifnull(jo_no,'')),
	...
	$SQL = "insert into upload_tpb ($fld) select $fld2 from 
		...
		left join jo_det jod on jo.id=jod.id_jo 
	...
	{	
		$fld="trans_no,username,sesi,id_item,tanggal_aju,nomor_aju,supplier,jumlah_satuan,id_supplier
	  	...
	  	,styleno,kpno,rakno";
		$fld2="'In','$user','$sesi',a.id_item,bpbdate,concat(if(a.bpbno_int!='',bpbno_int,a.bpbno),' ',ifnull(jo_no,'')),s.supplier,0,s.supplier,
	  	'' kode_barang,a.qty,0,concat(jenis_dok,' (',bcno,')'),
	  	concat(pono,' / ',a.styleno)
	  	,ac.styleno,ac.kpno,tmprak.rakno";
	}
	else
	{	
