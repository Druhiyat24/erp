      ...
      echo "<form method='post' name='form' action='save_data_bpb_sc_sj.php?mod=$mod&mode=$mode&noid=$bppbno&id=$id_line' onsubmit='return validasi()'>";
        echo "<div class='col-md-3'>";
          echo "<div class='form-group'>";
            ...
            $sql="select a.bppbno isi,group_concat(distinct concat(a.bppbno,' ',jo_no,' ',ac.styleno,' ',msup.supplier)) tampil from 
              ...
              and right(bppbno,1)!='R' and a.id_jo!='' group by bppbno";
  ...
  <div class="box-body">
    <table id="examplefix3" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        $sql="SELECT jo_no,ac.kpno wsno,a.*,s.goods_code,$fld_desc itemdesc,supplier 
          FROM bpb a inner join $tbl_mst s on a.id_item=s.id_item
          inner join mastersupplier ms on a.id_supplier=ms.id_supplier 
          left join jo on a.id_jo=jo.id inner join jo_det jod on jo.id=jod.id_jo 
          inner join so on jod.id_so=so.id 
          inner join act_costing ac on so.id_cost=ac.id 
          where $fldnyacri and bppbno!='' 
