  	...
  	// $ponya=$crinya;
		...
		echo "<table id='examplefix2' style='width: 100%;'>";
			echo "
				...
				<tbody>";
				$sql="select jo.jo_no,k.id_item,l.color,l.size,mi.goods_code,concat(mi.matclass,' ',mi.itemdesc) itemdesc,
				  l.qty qty_gmt,k.cons,round(sum(l.qty*k.cons),2) qty_bom,
				  k.unit,k.id_jo,b.qty_reject from bom_jo_item k inner join so_det l on k.id_so_det=l.id 
				  inner join masteritem mi on k.id_item=mi.id_item
				  inner join jo on k.id_jo=jo.id
				  inner join bpb b on b.id_jo=k.id_jo 
				  where k.id_jo in ($id_jo) and k.cancel='N' and k.status='P' group by k.id_jo,k.id_item";
				#echo $sql;
				$i=1;
				...
				{	$id=$data['id_item'].":".$data['id_jo'];
					...
					echo "
						<tr>
							...
							<td>
								<input type ='text' style='width:100px;' name ='qtyreject[$id]' id='qtyreject$i' 
									class='form-control qtyreject'>
							</td>
							<td>
