...
$rs=mysql_fetch_array(mysql_query("select po.pono,ac.kpno,po.notes,count(distinct poit.id_jo) totjo 
  ...
  where po.id='$_GET[id]' group by po.pono"));
$PONO = $rs['pono'];
if ($rs['totjo']>1)
{   $wsno = "(Combine)";
    $wsdet = "";
    $sql=mysql_query("select distinct ac.kpno from po_header po inner join po_item poit 
...
$sqldatatable = 'select po_header.tax,po_header.podate, po_item.price, 
    sum(po_item.qty) qty, 
  ...
  WHERE po_header.pono = "'.$PONO.'" group by masteritem.id_gen';
...
if (mysqli_num_rows($result)) {
    ...
    <tr>
        ...
        <td style="border-collapse: collapse; border-bottom:solid 2px #f5f5f5; padding:5px; width:10% "><p style="text-align:right;">'.$row["price"].'</p></td>
...
<body >
    ...
    <p style="font-size:20px">Purchase Order Confirmation '.$PONO.' '.$wsno.'</p>
