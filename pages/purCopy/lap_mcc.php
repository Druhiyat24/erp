		...
		echo "<table id='examplemcc' $tbl_border style='font-size:11px; width: 100%;'>";
		  ...
		  $sqldatatable = "select ac.deldate deldatecost,ac.id,ac.cost_date,ac.kpno,ac.styleno,mb.supplier buyer,
		  	...
		  	(select id_so,sum(qty) tqty_so,min(deldate_det) min_del from so_det where cancel='N' group by id_so) sod on jod.id_so=sod.id_so
				...
				group by ac.kpno ";
  		#and ac.kpno='ADV/0719/00001' untuk trial
  		#echo $sqldatatable;
        ...
        $rs=mysqli_fetch_array(mysqli_query($con_new, "select * from masterrate where  curr='USD' 
      		...
      		and tanggal='".fd($deldatecost)."'"));
				...
				$sql_cost="select a.id_act_cost,
					round(sum((if(jenis_rate='B' or jenis_rate='',price/rate_beli,price)*cons)+((if(jenis_rate='B' or jenis_rate='',price/rate_beli,price)*cons)*allowance/100)),4) act_usd,
					round(sum((if(jenis_rate='J',price*rate_jual,price)*cons)+((if(jenis_rate='J',price*rate_jual,price)*cons)*allowance/100)),2) act_idr 
				...
				$sql_mfg="select 
					a.id_act_cost,
					round(sum((if(jenis_rate='B' or jenis_rate='',price/rate_beli,price)*cons)+((if(jenis_rate='B' or jenis_rate='',price/rate_beli,price)*cons)*allowance/100)),4) act_mfg_usd,
					round(sum((if(jenis_rate='J',price*rate_jual,price)*cons)+((if(jenis_rate='J',price*rate_jual,price)*cons)*allowance/100)),2) act_mfg_idr 
				...
				$sql_oth="select 
					a.id_act_cost,
					round(sum((if(jenis_rate='B' or jenis_rate='',price/rate_beli,price))),4) act_oth_usd,
					round(sum((if(jenis_rate='J',price*rate_jual,price))),2) act_oth_idr 
