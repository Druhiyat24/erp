...
<script type="text/javascript">
  ...
  { var id_po = document.form.txtpono.value;
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_po_dev.php?modeajax=get_list_item_prorata',
        data: {id_po: id_po},
        async: false
    }).responseText;
    if(html)
  ...
  { var id_po = document.form.txtpono.value;
    ...
    jQuery.ajax
    ({  
      url: "ajax_po_dev.php?modeajax=get_qty_po",
      method: 'POST',
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_prorata_dev.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>                
          <div class='form-group'>
            <label>PO # *</label>
            <select class='form-control select2' style='width: 100%;' name='txtpono' onchange='getDetList()'>
              <?php 
                $sql = "select a.id isi,concat(a.pono,'|',ms.supplier) tampil from po_header_dev a inner join 
                  (select id_po,count(distinct id_jo) t_jo from po_item_dev group by id_po) tmp_det 
