		...
		if($MyList == '0'){
			$result = '{ "status":"ok", "message":"2", "records":"0"}';
				...
				while($row = mysqli_fetch_array($MyList)){
					...
					$outp .= '"amnt_view":"'.rawurlencode(number_format($row["bpb_amount"],2,'.',',')). '",';
					$outp .= '"amount_":"'.rawurlencode($row["bpb_amount"]).'"}';
				}
				$result = '{ "status":"ok", "message":"1", "records":['.$outp.'],"total_nilai" : "'.number_format($amnt,2,'.',',').'"}';	
			}		
		}
		return $result;
	}
