...
//ECHO $q;
		...
		while($row = mysql_fetch_array($stmt)){
			...
			$debit = $row['debit'];
			array_push($debitArray,$row['debit']);
			array_push($creditArray,$row['credit']);
			//$periods = $row['period'];
if($codeJournal[$x] == "RV" ){
		...
		$array[] = array();
		$array['debit']  = $debitArray;
		$array['credit'] = $creditArray;		
		$array['posisi'] = array();
		...
		if(ISSET($count) && $count > 0 ){
			...
			for($x=0;$x<$count;$x++){
				$totalRowBySupplier = $this->aktual_total_awal($code_journal,$ListSupplier['data']['id_supplier'][$x],$from,$to,$pencarian);
					array_push($basedon['debit'],$totalRowBySupplier['debit']);
					array_push($basedon['credit'],$totalRowBySupplier['credit']);					
