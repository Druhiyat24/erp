...
if($data['code'] == '1' ){
	$getListData = new getListData();
		...
		for($i=0;$i<$count;$i++){
			...
			if(check_bppb_exist($array_bpb[$i]) == '1' ){
				update_bpb_gr_ret(trim(preg_replace('/\s+/', ' ', $array_bpb[$i])));
			}else if(check_bppb_exist($array_bpb[$i]) == '0'){
				insert_bpb_gr_ret(trim(preg_replace('/\s+/', ' ', $array_bpb[$i])));
			}else{
