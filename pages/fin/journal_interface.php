...
WHERE A.fg_tax = '1') FD ON FD.reff_doc = bpb.bpbno_int AND TRIM(mi.itemdesc) = TRIM(FD.description) AND mi.id_item = bpb.id_item
		LEFT JOIN mtax MT ON FD.n_pph = MT.idtax
			WHERE 1=1 AND FD.id_journal IS NOT NULL 
                 AND poi.cancel = 'N' AND poh.id IS NOT NULL AND (SUBSTR(bpb.bpbno_int,1,3) NOT IN ('FG/','WIP') )
		group by bpb.id
...
WHERE A.fg_tax = '1') FD ON FD.reff_doc = bpb.bpbno_int AND TRIM(mi.itemdesc) = TRIM(FD.description) AND mi.id_item = bpb.id_item
		LEFT JOIN mtax MT ON FD.n_pph = MT.idtax
			WHERE 1=1 AND FD.id_journal IS NOT NULL 
                  AND poi.cancel = 'N' AND poh.id IS NOT NULL AND (SUBSTR(bpb.bpbno_int,1,3) NOT IN ('WIP') ) AND SUBSTR(bpb.bpbno_int,1,3) IN ('FG/')
		group by bpb.id
...
function journal_detail(){
	...
	SELECT D.is_normal
       ...
       ,D.debit
       ,D.utang	
       ,(D.nilai - SUM(ifnull(pph.value_pph,0)))nilai
	   ,SUM(ifnull(pph.value_pph,0))value_pph
	   ,D.nilai nilai_before_pph
       ,D.bpb_ref		
	   ,D.coa_utang
	   ,D.nm_utang
       ,D.journal_ref		
...
WHERE A.fg_tax = '1') FD ON FD.reff_doc = bpb.bpbno_int AND TRIM(mi.itemdesc) = TRIM(FD.description) AND mi.id_item = bpb.id_item
		LEFT JOIN mtax MT ON FD.n_pph = MT.idtax
			WHERE 1=1 AND FD.id_journal IS NOT NULL 
                 AND poi.cancel = 'N' AND poh.id IS NOT NULL AND bpb.bpbno_int ='FG/'
		group by bpb.id
		)MASTER GROUP BY MASTER.id_journal)pph ON pph.id_journal = D.id_journal
