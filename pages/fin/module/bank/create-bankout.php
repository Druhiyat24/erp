<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">FORM OUTGOING BANK</h4>
<div class="box">
...
<form id="form-data" method="post">
        ...
        <input type="hidden" style="font-size: 15px;" name="txt_rate" id="txt_rate" class="form-control" 
            value="<?php 
...
<!--            <div class="col-md-3 mb-3">            
    ...
    <div class="box body">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                ...
                if ($ref_num == 'Payment') {
                    echo '<tr>
                            ...
                            <th style="width:100px;">Equivalent IDR</th>
            ...
            if ($ref_num == 'Payment') {
                $sql = mysqli_query($conn2,"select no_kbon,curr,nama_supp,no_payment,tgl_payment,tgl_tempo,curr,(sum(amount) - (select COALESCE(sum(for_balance),0) from b_bankout_det where no_reff = no_payment)) as total_kbon,sum(pph_value) as pph_value, 'REG' as kode from sb_list_payment where status_int = '4' and nama_supp = '$nama_supp' and tgl_payment BETWEEN '$start_date' and '$end_date'  group by no_payment
union
select no_kbon,valuta,supplier,no_pay,tgl_pay,due_date,valuta,total,pph,'SAL' as kode from sb_saldo_awal where status_int = '4' and no_pay not like '%LP/NAG%' and supplier = '$nama_supp' and tgl_pay BETWEEN '$start_date' and '$end_date' group by no_pay
union
...
select '',list_payment_dp.curr as valuta, list_payment_dp.nama_supp as supplier, list_payment_dp.no_payment as no_pay, list_payment_dp.tgl_payment as tgl_pay, list_payment_dp.tgl_tempo, list_payment_dp.curr as valuta, list_payment_dp.total_kbon as total,  list_payment_dp.pph_value, 'DP' as kode from list_payment_dp  where list_payment_dp.nama_supp = '$nama_supp' and list_payment_dp.status_int = '4' and total_kbon != 0 and tgl_payment BETWEEN '$start_date' and '$end_date' group by no_pay");
                ...
                $sql = mysqli_query($conn2,"select a.nama_supp,a.no_pv,a.pv_date,max(b.due_date) as due_date,a.curr,a.subtotal, a.ppn, a.pph ,a.total,a.status, a.frm_akun, if(a.frm_akun = '-','-',c.bank_name) as bank_name, c.b_code from tbl_pv_h a inner join sb_pv b on b.no_pv = a.no_pv left join b_masterbank c on c.bank_account = a.frm_akun where a.nama_supp = '$nama_supp' and a.pv_date BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' and a.outstanding != '0' group by a.no_pv");
            }else{
                '';
            }
            ...
            while($row = mysqli_fetch_array($sql)){
                    ...
                    echo '<tr>
                         ...
                         while($row = mysqli_fetch_array($sql)){
                            ...
                            $sqly = mysqli_query($conn1,"select ROUND(rate,2) as rate , tanggal  FROM masterrate where tanggal = '$tgl_payment' and v_codecurr = 'PAJAK'");
                            $rowy = mysqli_fetch_array($sqly);
                            $rates = isset($rowy['rate']) ? $rowy['rate'] : 1;


                            $sqlz = mysqli_query($conn1,"select COALESCE(sum(for_balance),0) as balance from sb_b_bankout_det where no_reff = '$no_pay'");
                            $rowz = mysqli_fetch_array($sqlz);
                            $balance = isset($rowz['balance']) ? $rowz['balance'] : 0;
                            ...
                            if ($rates == '0') {
                                ...
                                $sqlz = mysqli_query($conn1,"select ROUND(rate,2) as rate , tanggal  FROM masterrate where id = '$maxid' and v_codecurr = 'PAJAK'");
                                $rowz = mysqli_fetch_array($sqlz);
                                $rate = $rowz['rate'];
                            }else{
                                $rate = $rowy['rate'];
                            ...
                            if ($kode != 'REG') {
                            $querysaa = mysqli_query($conn2,"select if(count(no_kbon) = 0,1,count(no_kbon)) as total_no from sb_list_payment where no_kbon = '$no_kbon'");
                            $rowsaa = mysqli_fetch_array($querysaa);
                            $total_no = isset($rowsaa['total_no']) ? $rowsaa['total_no'] : 1;
                            $tot_sub = $total - $balance;
                            $tot_pph = $pph / $total_no;
                            $tot_ppn = 0; 
                            }else{
                            ...
                            $querys = mysqli_query($conn2,"select sum(b.amount) as sub,sum(a.tax) as ppn from sb_kontrabon_h a inner join sb_list_payment b on b.no_kbon = a.no_kbon where b.no_payment = '$no_pay' group by b.no_payment");
                            $rows = mysqli_fetch_array($querys);
                            $sub = isset($rows['sub']) ? $rows['sub'] : 0;
                            $ppn = isset($rows['ppn']) ? $rows['ppn'] : 0; 
                            $tot_ppn = $ppn/$total_no;
                            $tot_pph = $pph/$total_no;
                            $tot_sub = ($sub - $tot_ppn + $tot_pph) - $balance;
                        ...
                        <tr>
                           <td style="width:10px;"><input type="checkbox" id="select" name="select[]" value="" <?php if(in_array("1",$_POST[select])) echo "checked=checked";?></td>                        
                            ...
                            <td style="width:100px;" value-sub="'.$tot_sub.'">'.number_format($tot_sub,2).'</td>
                            <td style="width:100px;" value-ppn="'.$tot_ppn.'">'.number_format($tot_ppn,2).'</td>
                            <td style="width:100px;" value-pph="'.$tot_pph.'">'.number_format($tot_pph,2).'</td>
                            <td style="width:100px;" data-total="'.$total.'">'.number_format($total,2).'</td>
                            <td style="" data-total-idr="'.$total_idr.'">'.number_format($total_idr,2).'</td>
                            <td style="display:none;" value-curr="'.$row['curr'].'">'.$row['curr'].'</td>
                            ...
                            <td style="display: none" data-rates="'.$ratess.'">'.number_format($ratess,2).'</td> 
...
<div class="box footer">   
    ...
    <tbody id="tbody3">
        <tr style="display:none">
            ...
            <td style="width: 200px;">
                <select class="form-control" name="nomor_coc" id="nomor_coc" style="width: 250px"> <option value="-" > - </option> <?php $sqlcoc = mysqli_query($conn1," select no_cc as code_combine,cc_name as cost_name from b_master_cc where status = 'Active'"); foreach ($sqlcoc as $coc) : echo'<option value="'.$coc["code_combine"].'"> '.$coc["cost_name"].' </option>'; endforeach; ?>
                </select>
            </td>
            <td>
...
<script type="text/javascript">
    ...
    var kali = 0;      
    $("input[type=checkbox]:checked").each(function () {   
    var ratess = parseFloat(document.getElementById('rate').value,10) || 0;
    var curren = document.getElementById('valuta').value;
    var ttl_debit = parseFloat(document.getElementById('jml_debit').value,10) || 0;   
    ...
    var balance_usd = parseFloat($(this).closest('tr').find('td:eq(7)').attr('data-total'),10) || 0;
    var kb2 = parseFloat($(this).closest('tr').find('td:eq(7)').attr('data-total'),10) || 0;
    var select_amount = $(this).closest('tr').find('td:eq(11) input'); 
    if (curren == 'IDR') {             
    if(amount > balance){
        sum_kb += kb;
    ...
    sum_total = sum_kb - sum_amount;        
        ...
        sum_kb += kb2;
        debit = ttl_debit;
        select_amount.val(balance_usd);
        sum_amount += balance;
        sum_amount2 += balance_usd * ratess;
        tot_debit = sum_amount2 + debit;
        sum_total = sum_kb2 - sum_amount2;
    }else{
    sum_kb2 += kb2;
    sum_amount += amount;
    sum_amount2 += amount * ratess;
    debit = ttl_debit;
    tot_debit = sum_amount2 + debit;
    sum_total = sum_kb2 - sum_amount2;        
    }  
    ...
    $("#total_cek_idr").val(formatMoney(sum_amount2));
    $("#total_cek_idr_h").val(sum_amount2);
    $("#total_cek_h").val(sum_amount);
    $("#nominaldeb_h").val(tot_debit.toFixed(2));  
...
<script type="text/javascript">
   $('#accountid').change(function(){
    ...
    valu = 'BK'+'/'+'MDR'+kode+'/'+'NAG'+'/'+bulan+tahun;
}else{
...
<script type="text/javascript">
   ...
   $("#nomcredit").val(total_credit.toFixed(2));
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            data: {'no_bankout':no_bankout, 'tgl_bankout':tgl_bankout, 'reff_doc':reff_doc, 'no_coa':no_coa, 'no_costcntr':no_costcntr, 'buyer':buyer, 'no_ws':no_ws, 'curr':curr, 'debit':debit, 'credit':credit, 'deskripsi':deskripsi, 'rate':rate},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
        ...
        var bankout_date = document.getElementById('tgl_active').value;
        var no_coa = $(this).closest('tr').find('td:eq(1)').find('select[name=nomor_coa] option').filter(':selected').val();
        var no_coc = $(this).closest('tr').find('td:eq(2)').find('select[name=nomor_coc] option').filter(':selected').val();     
        var reff_doc = $(this).closest('tr').find('td:eq(3) input').val();
        ...
        var amount_input = $(this).closest('tr').find('td:eq(11) input').val() || 0;
        var nama_supp = $('select[name=nama_supp] option').filter(':selected').val();
        var akun = document.getElementById('accountid').value;
        var amount = document.getElementById('nominal_h').value;  
        var t_credit = document.getElementById('nominalcre_h').value;
        var t_debit = document.getElementById('nominaldeb_h').value;
        var curr_h = document.getElementById('valuta').value;
        var rate_h = document.getElementById('rate').value;
