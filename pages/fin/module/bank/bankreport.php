<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">REPORT BANK ACCOUNT BALANCE</h4>
<div class="box">
       ...
       <form id="form-data" action="bankreport.php" method="post">        
        <div class="form-row">
            ...
            <label for="accountid" class="col-form-label" ><b>Account </b></label>  
                ...
                <?php 
                        $sqlacc = mysqli_query($conn1,"select CONCAT(bank_account,' (',upper(bank_name),')') name_bank, upper(bank_name) as bank,curr,bank_account as account,RIGHT(bank_account,4) as kode from b_masterbank where status = 'Active' and beneficiary_name != 'Syenni Santosa'");
...
$sqlsaldo = mysqli_query($conn1,"select amount from sb_saldoawal_bank where account = '$accountid'");
...
FROM
   (select transaksi_date as date, no_doc as doc_num,deskripsi,debit,credit,curr from b_reportbank where akun = '$accountid' and transaksi_date < '$start_date' and transaksi_date >= '2023-01-01' and status != 'Cancel') AS q1 JOIN
...
$sqlsaldoidr = mysqli_query($conn1,"select eqv_idr from sb_saldoawal_bank where account = '$accountid'");
$rowsaldoidr = mysqli_fetch_array($sqlsaldoidr);
$saldo_ = isset($rowsaldoidr['eqv_idr']) ? $rowsaldoidr['eqv_idr'] : 0;
$saldo__ = $saldoawal * $rates;
...
<?php
   ...
   if(empty($accountid) and empty($start_date) and empty($end_date)){
    echo '';
    $sqlswl = mysqli_query($conn1,"select amount from sb_saldoawal_bank where account = '$accountid'");
...
$rates2 = $rowyss2['rate'];
    ...
    if($credit == '0'){
     ...
     $sqlswl3 = mysqli_query($conn1,"select amount from sb_saldoawal_bank where account = '$accountid'");
