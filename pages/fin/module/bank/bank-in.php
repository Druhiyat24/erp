<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->    
    <div class="col p-4">
        <h4 class="text-center">INCOMING BANK</h4>
<div class="box">
    <div class="box header">
<form id="form-data" action="bank-in.php" method="post">
        <div class="form-row">
            <div class="col-md-6">
...
<table id="mytable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            $sql = mysqli_query($conn1,"select  a.*,b.beneficiary_name from (select * from (select doc_num, customer, date, ref_data, akun, bank, curr, amount, outstanding, status,deskripsi,approve_date from sb_bankin_arcollection $where group by doc_num order by id asc) a
                UNION
                select * from (select doc_num, customer, date, ref_data, akun, bank, curr, amount, outstanding, status,deskripsi,approve_date from tbl_bankin_arcollection $where group by doc_num order by id asc) a) a left join b_masterbank b on a.akun = b.bank_account where beneficiary_name = 'PT Nirwana Alabare Garment'");
               // echo $start_date;
               //  echo $end_date;                                                       
                while($row = mysqli_fetch_array($sql)){
           ...
           echo'<tr>                       
                        ...
                        if ($row['status'] == 'Cancel') {
                                ...
                                echo '<a href="pdf_bankin.php?doc_num='.$row['doc_num'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
                            }
                            echo'</td>';
...
<div class="box footer">   
        <form id="form-simpan">
           <div class="form-row col">
            ...
            $jml = 0;
            $ost = 0;
            $sql = mysqli_query($conn1,"select sum(amount) as nominal, sum(outstanding) as outstanding from sb_bankin_arcollection $where");
...
<script type="text/javascript">
    ...
    location.href = "create-bank-in.php";
};
</script>
