<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">FORM PAYMENT VOUCHER MEMO EXIM</h4>
<div class="box">
...
<!--               <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true">
          ...
          inner join mastersupplier mb on a.id_buyer = mb.id_supplier
                    ...
                    inner join sb_pv_memo_temp mtemp on mtemp.no_memo = a.nm_memo
...
<div class="col-md-4 mb-3">            
            ...
            $nama_supp = isset($_POST['nama_supp']) ? $_POST['nama_supp']: null;
            $sql = mysqli_query($conn2," select GROUP_CONCAT(ket) as sup_doc from (select * from sb_supp_doc_temp where ket != '' ) supp_doc_temp ");
            $row = mysqli_fetch_array($sql);
            $sup_doc = $row['sup_doc'];         
    ...
    <tbody>
        <?php
            ...
            while($row1 = mysqli_fetch_array($querys)){
                ...
                $sql22 = mysqli_query($conn2,"select ket from sb_supp_doc_temp where ket = '$nodoc'");
                $row22 = mysqli_fetch_array($sql22);
                $ket = isset($row22['ket']) ? $row22['ket'] : null;

                $sql23 = mysqli_query($conn2,"select ket from sb_supp_doc_temp where ket != 'Sales Order' and ket != 'Purchase Order' and ket != 'PEB' and ket != 'Invoice'");
...
//   where lj.credit != 0 and mtemp.user = '$user')
    ...
    $sql_jurnal = mysqli_query($conn2,"select no_journal from sb_list_journal a inner join sb_pv_memo_temp b on b.no_memo = a.no_journal where b.user = '$user' limit 1");
    $hasil = mysqli_fetch_array($sql_jurnal);
    $no_journal = isset($hasil['no_journal']) ?  $hasil['no_journal'] : null;

    if ($no_journal != null) {
        $sqlpv = mysqli_query($conn1,"select a.id_h,a.nm_memo,a.tgl_memo,a.jns_trans,IF(a.ditagihkan != 'Y','TIDAK','YA') ditagihkan, '' nm_ctg,'' nm_sub_ctg,'' item_name,lj.no_coa, CONCAT( lj.no_coa, ' ', lj.nama_coa) nama_coa, lj.no_costcenter id_cc,lj.nama_costcenter cc_name,lj.keterangan,lj.credit biaya  from memo_h a
  inner join sb_pv_memo_temp mtemp on mtemp.no_memo = a.nm_memo
    inner join sb_list_journal lj on lj.no_journal = a.nm_memo
  where lj.credit != 0 and mtemp.user = '$user' order by a.nm_memo asc");
    }else{
        $sqlpv = mysqli_query($conn1,"select id_h,nm_memo,tgl_memo,jns_trans, ditagihkan,nm_ctg,nm_sub_ctg,item_name,no_coa, nama_coa, id_cc,cc_name, keterangan,sum(biaya ) biaya from (select a.id_h,a.nm_memo,a.tgl_memo,a.jns_trans,IF(a.ditagihkan != 'Y','TIDAK','YA') ditagihkan,mdet.nm_ctg,mdet.nm_sub_ctg,it.item_name,map.no_coa, CONCAT( map.no_coa, ' ', map.nama_coa) nama_coa, map.id_cc,map.cc_name, UPPER(CONCAT(mdet.nm_sub_ctg,' (',ms.supplier, '), BUYER ',mb.supplier, ', ',a.jns_trans, ', ',inv_vendor)) keterangan,mdet.biaya from memo_h a
           inner join mastersupplier ms on a.id_supplier = ms.id_supplier
           inner join mastersupplier mb on a.id_buyer = mb.id_supplier
           inner join memo_det mdet on mdet.id_h = a.id_h
           left join master_memo_item it on it.id = a.id_item
           ...
           map.jns_trans = a.jns_trans and map.ditagihkan = a.ditagihkan or map.id_item = a.id_item
           where mdet.cancel = 'N' and map.status != 'Y' and mtemp.user = '$user'
           GROUP BY mdet.id order by mdet.id_h) a GROUP BY nm_memo,nm_ctg,nm_sub_ctg");
    }
