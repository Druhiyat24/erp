...
$sqlnkb = mysqli_query($conn2,"select max(no_bankout),curr,reff_doc,create_by,create_date from sb_b_bankout_h where id = '$maxid'");
...
$query = "INSERT INTO sb_b_bankout_adj_det (no_bankout,id_coa,no_cc,reff_doc,reff_date,deskripsi,t_debit,t_credit) 
VALUES 
...
$querys_ppn = "INSERT INTO sb_list_journal (no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date) 
VALUES 
...
$sql1lp = mysqli_query($conn2,"select DISTINCT * from (select no_coa,nama_coa,sum(amount) amount,if(memo = '',null,memo) memo from sb_list_payment where no_payment = '$no_pay' GROUP BY no_coa
union
select no_coa,nama_coa,sum(total) amount,if(keterangan = '',null,keterangan) keterangan from sb_saldo_awal where no_pay = '$no_pay' GROUP BY no_coa) a");
...
if ($pph > 0) {
	...
	$sqlpph = mysqli_query($conn1,"select no_coa,nama_coa from mtax where idtax = (select max(a.idtax) idtax from sb_kontrabon a INNER JOIN sb_list_payment b on b.no_kbon = a.no_kbon where b.no_payment = '$no_pay' group by a.no_kbon limit 1)");
$rowpph = mysqli_fetch_array($sqlpph);
$no_coa_pph = $rowpph['no_coa'];
$nama_coa_pph = $rowpph['nama_coa'];


$querys_pph = "INSERT INTO sb_list_journal (no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date) 
VALUES 
...
if(strpos($no_pay, 'PV/NAG/') !== false) {
	// $sql = "Update sb_pv_h set outstanding = '0' where no_pv = '$no_pay'";
