...
<style>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">UPDATE MEMORIAL JOURNAL</h4>
<div class="box">
...
<form id="form-data" method="post">
        <div class="form-row">
            ...
            $sql = mysqli_query($conn2,"select no_mj from sb_memorial_journal where no_mj = '$no_mj'");
            $row = mysqli_fetch_array($sql);

            echo'<input type="text" readonly style="font-size: 14px;" class="form-control-plaintext" id="no_doc" name="no_doc" value="'.$row['no_mj'].'">'
            ?>
        </div>
            ...
            value="<?php 
            $no_mj = base64_decode($_GET['no_mj']);
            $sql = mysqli_query($conn2,"select mj_date from sb_memorial_journal where no_mj = '$no_mj'");
            $row = mysqli_fetch_array($sql);                         
            if(!empty($no_mj)) {
...
<!--             <input type="hidden" style="font-size: 15px;" name="unik_code" id="unik_code" class="form-control" 
            ...
            <label for="nama_supp"><b>Type</b></label>            
              <select class="form-control select2bs4" name="nama_type" id="nama_type" data-dropup-auto="false" data-live-search="true">
                <?php 
            $no_mj = base64_decode($_GET['no_mj']);
            $sql = mysqli_query($conn2,"select DISTINCT a.id_cmj, b.nama_cmj from sb_memorial_journal a left join master_category_mj b on b.id_cmj = a.id_cmj where a.no_mj = '$no_mj'");
            $row = mysqli_fetch_array($sql);  
            $id_cmj = $row['id_cmj'];
            ...
            if(!empty($no_mj)) {
                ...
                $sql = mysqli_query($conn1,"select id_cmj,CONCAT(id_cmj,'-',nama_cmj) as type,nama_cmj from master_category_mj where id_cmj != '$id_cmj'");
 ...
 <div class="form-row">
    ...
    <div class="box body">
        <div class="row">
            ...
            <div class="col-md-12">
            <div class="table">
            <table id="mytable" class="table table-striped table-bordered text-nowrap" cellspacing="0" width="100%" style="font-size: 11px;text-align:center;table-layout: fixed;">
                    <thead>
        <tr><th class="text-center" style="width: 2%">-</th>
            ...
            <th class="text-center" style="width: 9%">Worksheet</th>
            <th class="text-center" style="width: 7%">Curr</th>
    ...
    <td>
        <select class="form-control selectpicker" name="no_ws" id="no_ws" data-live-search="true"> <option value="-" > - </option><?php $sql3 = mysqli_query($conn1,"select DISTINCT kpno no_ws from act_costing where cost_date >= '2022-01-01'"); foreach ($sql3 as $ws) : ?> <option value="<?= $ws["no_ws"]; ?>"><?= $ws["no_ws"]; ?> </option><?php endforeach; ?>
        </select>
    </td>
    <td>
...
<?php
    $no_mj = base64_decode($_GET['no_mj']);
    $sqlpv = mysqli_query($conn1,"select a.no_coa,concat(c.no_coa,' ', c.nama_coa) as coa ,a.no_costcenter, d.cc_name, a.no_reff, a.reff_date,a.buyer,a.no_ws,a.curr,a.rate,a.debit,a.credit,a.keterangan from sb_memorial_journal a left join master_category_mj b on b.id_cmj = a.id_cmj left join mastercoa_v2 c on c.no_coa = a.no_coa left join b_master_cc d on d.no_cc = a.no_costcenter where a.no_mj = '$no_mj'");
    ...
    echo'<tr style="font-size:10px">
            ...
            <td >
                <select class="form-control selectpicker" name="nomor_coa" id="nomor_coa" data-live-search="true"> <option value="'.$row['no_coa'].'" >'.$row['coa'].'</option><option value="-" > - </option>';  $sql = mysqli_query($conn1,"select no_coa as id_coa,concat(no_coa,' ', nama_coa) as coa from mastercoa_v2 where no_coa != '$coa'"); foreach ($sql as $cc) : echo'<option value="'.$cc["id_coa"].'"> '.$cc["coa"].' </option>'; endforeach; ?>
                <?php
                echo '
                </select>
            </td>
        ...
        <?php
            ...
            <td>
                <input type="text" style="font-size: 13px;" class="form-control" name="keterangan[]" placeholder="" value="'.$row['no_reff'].'" autocomplete="off">
            </td>
            <td>
                <input type="text" style="font-size: 13px;" name="tgl_active" id="tgl_active" value="'.$reffdate.'" class="form-control tanggal" 
    ...
    if ($curr == 'IDR') {
        echo '<td>
        <select class="form-control" name="currenc" id="currenc" onchange="ubahrate(this.value)" data-live-search="true"><option value="'.$row['curr'].'" >'.$row['curr'].'</option><option value="USD">USD</option>
        </select>
    </td>';
    }else{
        echo '<td>
        <select class="form-control" name="currenc" id="currenc" onchange="ubahrate(this.value)" data-live-search="true"><option value="'.$row['curr'].'" >'.$row['curr'].'</option><option value="IDR">IDR</option>
        </select>
    </td>';
    }
            echo'<td hidden>
                <input type="text" style="font-size: 13px;" class="form-control" name="keterangan[]" value="'.$row['rate'].'" placeholder="" autocomplete="off"> 
            </td>';
            if ($debit == '0') {
                echo '<td style="text-align: right;">
            <input style="text-align: right;font-size: 13px;" type="number" min="1"  style="font-size: 12px;" class="form-control" id="txt_amount" name="txt_amount"  oninput="modal_input_amt(value)" autocomplete = "off" readonly>
            </td>';
            }else{
            echo '<td style="text-align: right;">
            <input style="text-align: right;font-size: 13px;" type="number" min="1" value="'.$row['debit'].'" style="font-size: 12px;" class="form-control" id="txt_amount" name="txt_amount"  oninput="modal_input_amt(value)" autocomplete = "off">
            </td>';
        }

        if ($credit == '0') {
                echo '<td>
        <input style="text-align: right;font-size: 13px;" type="number" min="1" style="font-size: 12px;" class="form-control" id="txt_amount" name="txt_amount"  oninput="modal_input_amt2(value)" autocomplete = "off" readonly>
    </td>';
            }else{
            echo '<td>
        <input style="text-align: right;font-size: 13px;" type="number" min="1" value="'.$row['credit'].'" style="font-size: 12px;" class="form-control" id="txt_amount" name="txt_amount"  oninput="modal_input_amt2(value)" autocomplete = "off">
    </td>';
        }
            echo '
            <td><input type="text"  style="font-size: 13px;" value="'.$row['keterangan'].'" class="form-control" name="keterangan[]" placeholder="" autocomplete="off"></td>
