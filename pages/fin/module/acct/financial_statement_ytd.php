...
<!-- MAIN -->
<div class="col p-4">
    <h4 class="text-center">FINANCIAL STATEMENT YTD</h4>
    <div class="box">
        <div class="box header">

            <form id="form" action="financial_statement_ytd.php" method="post">
...
<div id="tril-balance" class="tabcontent">
    ...
    if($id == '56'){
        echo '<a style="padding-left: 10px";><button type="button" class="btn btn-info " name="co_sal" id="co_sal" ><i class="fa fa-clipboard" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 14px;color: #fff;text-shadow: 1px 1px 1px #000"> Copy Saldo</i></button></a>';
    }else{
        echo '';
    }
    ?> 
    ...
    <table id="datatable" class="table table-striped table-bordered" style="font-size: 12px;" role="grid" cellspacing="0" width="100%">
        <thead>
            <tr class="thead-dark">
                ...
                <th style="text-align: center;vertical-align: middle;width: 13%;">Category 6</th>
    ...
    // echo  $tahun_akhir;            
            ...
            if(empty($start_date) and empty($end_date)){
             $sql = mysqli_query($conn2,"    
                ...
                left join
                (select no_coa,nama_coa,'' beg_balance,ind_categori1,ind_categori2,ind_categori3,ind_categori6,ind_categori4,id_ctg5 from mastercoa_sb order by no_coa asc) coa
        ...
        if(!$execute5){ 
            ...
            (select no_coa nocoa,nama_coa namacoa,$kata_filter as saldo from sb_saldo_awal_tb order by no_coa asc) saldo
            left join
            (select no_coa,nama_coa,'' beg_balance,ind_categori1,ind_categori2,ind_categori3,ind_categori6 from mastercoa_sb order by no_coa asc) coa
            on coa.no_coa = saldo.nocoa
            left join
            (select no_coa coa_no, sum(credit) credit,sum(debit) debit,IF(sum(debit) = sum(credit),'B','NB') balance,sum(ROUND(credit * rate,2)) credit_idr,sum(ROUND(debit * rate,2)) debit_idr,IF(sum(ROUND(debit * rate,2)) = sum(ROUND(credit * rate,2)),'B','NB') balance_idr from sb_list_journal where tgl_journal BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = '$bulan_awal' and tahun = '$tahun_awal') and (select tgl_akhir from tbl_tgl_tb where bulan = '$bulan_akhir' and tahun = '$tahun_akhir') group by no_coa) 
...
<div style="border:1px solid black;margin-top: 5px;">        
    <table style="font-size: 16px; margin:auto" border="0" role="grid" cellspacing="0" width="64%">
        ...
        <tr>
            <th colspan="4" style="text-align: center;vertical-align: middle;width: 100%;">Per <?php
            $enddate = date("t F Y");
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $enddate = date("t F Y",strtotime($_POST['end_date'])); 
            } echo $enddate; ?></th>
        </tr>                   
    </table>
...
<div id="arus-kas" class="tabcontent">
  ...
  if($tanggal2 < $tanggal1){
    ...
    echo '<a style="padding-right: 10px;" target="_blank" href="ekspor_arus_kas.php?start_date='.$start_date.' && end_date='.$end_date.' && kata_filter='.$kata_filter.'"><button type="button" class="btn btn-success " ><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 14px;color: #fff;text-shadow: 1px 1px 1px #000"> Excel </i></button></a>
...
<div id="explanation" class="tabcontent">
  ...
  if($tanggal2 < $tanggal1){
    ...
    echo '<a style="padding-right: 10px;" target="_blank" href="ekspor_explanation.php?start_date='.$start_date.' && end_date='.$end_date.' && kata_filter='.$kata_filter.'"><button type="button" class="btn btn-success " ><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 14px;color: #fff;text-shadow: 1px 1px 1px #000"> Excel </i></button></a>
...
<div style="border:1px solid black;margin-top: 5px;">        
    <table style="font-size: 16px; margin:auto" border="0" role="grid" cellspacing="0" width="65%">
        ...
        <tr>
            <th colspan="4" style="text-align: center;vertical-align: middle;width: 100%;">UNTUK TAHUN YANG BERAKHIR <?php
            $enddate = date("F Y");
            $startdate = date("F Y");
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $enddate = date("t F Y",strtotime($_POST['end_date'])); 
