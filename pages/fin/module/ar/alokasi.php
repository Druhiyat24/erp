<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">LIST ALOKASI</h4>
<div class="box">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            <th style="text-align: center;vertical-align: middle;">Alokasi Number</th>
            <th style="text-align: center;vertical-align: middle;">Alokasi Date</th>
            ...
            <th style="text-align: center;vertical-align: middle;">Equivalent IDR</th>
    ...
    if ($customer == "ALL") {
        $str = "WHERE DATE_FORMAT(d.tanggal_input, '%Y-%m-%d')  BETWEEN '$start_date' AND '$end_date' ORDER BY a.id";
    } else if ($customer != "ALL") {
        $str = "WHERE DATE_FORMAT(d.tanggal_input, '%Y-%m-%d')  BETWEEN '$start_date' AND '$end_date' AND a.customer = '$customer' ORDER BY a.id";
    } else {

    }

    $sql = mysqli_query($conn1,"SELECT * FROM (select a.id, a.no_alk,a.tgl_alk,a.customer,b.supplier,a.doc_number,c.nama_bank as bank,a.account,a.curr,if(rate = '0', '-',format(a.rate,0)) as rate,if(a.curr = 'IDR',format(a.amount, 0), format(a.amount, 2)) as amount,format(a.eqp_idr, 0) as eqp_idr,a.status,DATE_FORMAT(d.tanggal_input, '%Y-%m-%d') as tanggal_input,d.nama from sb_alokasi AS a INNER JOIN mastersupplier AS b ON a.customer = b.id_supplier inner join masterbank c on c.id = a.bank left join sb_log d on d.doc_number = a.no_alk $str) a
        UNION
        SELECT * FROM (select a.id, a.no_alk,a.tgl_alk,a.customer,b.supplier,a.doc_number,c.nama_bank as bank,a.account,a.curr,if(rate = '0', '-',format(a.rate,0)) as rate,if(a.curr = 'IDR',format(a.amount, 0), format(a.amount, 2)) as amount,format(a.eqp_idr, 0) as eqp_idr,a.status,DATE_FORMAT(d.tanggal_input, '%Y-%m-%d') as tanggal_input,d.nama from tbl_alokasi AS a INNER JOIN mastersupplier AS b ON a.customer = b.id_supplier inner join masterbank c on c.id = a.bank left join tbl_log d on d.doc_number = a.no_alk $str) a");

    while($row = mysqli_fetch_array($sql)){
                   
        echo '<tr style="font-size:12px;text-align:center;">
            <td style="" value = "'.$row['no_alk'].'">'.$row['no_alk'].'</td>
            <td style="" value = "'.$row['tgl_alk'].'">'.date("d-M-Y",strtotime($row['tgl_alk'])).'</td>
            <td style="" value = "'.$row['supplier'].'">'.$row['supplier'].'</td>
            <td style="" value = "'.$row['doc_number'].'">'.$row['doc_number'].'</td>
            <td style="" value = "'.$row['bank'].'">'.$row['bank'].'</td>
            <td style="" value = "'.$row['account'].'">'.$row['account'].'</td>
            <td style="" value = "'.$row['curr'].'">'.$row['curr'].'</td>
            <td style="text-align : right;" value = "'.$row['rate'].'">'.$row['rate'].'</td>
            <td style="text-align : right;" value = "'.$row['amount'].'">'.$row['amount'].'</td>
            <td style=text-align : right;"" value = "'.$row['eqp_idr'].'">'.$row['eqp_idr'].'</td>
            <td style="" value = "'.$row['tanggal_input'].'">'.date("d-M-Y",strtotime($row['tanggal_input'])).'</td>
            <td style="" value="'.$row['nama'].'">'.$row['nama'].'</td>';
            if ($row['status'] == 'CANCEL') {
                echo '<td style="text-align: center;">-</td>';
            }else{
                echo '<td style="width:100px;text-align : center" >
                <a href="print_alokasi.php?id_alokasi='.$row['id'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>
                </td>';
            }
            echo '<td hidden value = "'.$row['id'].'">'.$row['id'].'</td>
            </tr>';
}?>
</tbody>                    
</table>
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#delete", function(){                 
        ...
        var id_inv = $(this).closest('tr').find('td:eq(11)').attr('value');
