<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">LIST INVOICE</h4>
<div class="box">
    <div class="box header">

        <form id="form-data" action="list-invoice.php" method="post">        
        <div class="form-row">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;">Doc Type</th>
            <th style="text-align: center;vertical-align: middle;">Doc Number</th>
            <th style="text-align: center;vertical-align: middle;">Inv Date</th>
    ...
    if ($customer == "ALL") {
        $str = "WHERE tgl.create_date BETWEEN '$start_date' AND '$end_date' ORDER BY a.id";
    } else if ($customer != "ALL") {
        $str = "WHERE tgl.create_date BETWEEN '$start_date' AND '$end_date' AND a.id_customer = '$customer' ORDER BY a.id";
    } else {

    }

    $sql = mysqli_query($conn1,"select * from (select distinct a.no_invoice AS no_invoice, UPPER(b.supplier) AS customer, a.shipp,                                     a.doc_type, a.doc_number, 
                                   ...
                                   FROM  sb_book_invoice AS a INNER JOIN 
                                          ...
                                          sb_invoice_detail as e on a.id=e.id_book_invoice INNER JOIN
                                          (select doc_number,DATE_FORMAT(tanggal_input, '%Y-%m-%d') create_date from sb_log where activity = 'Create Invoice' GROUP BY doc_number) tgl on tgl.doc_number = a.no_invoice
                                    $str ) a
                                    UNION
                                    select * from (select distinct a.no_invoice AS no_invoice, UPPER(b.supplier) AS customer, a.shipp,                                     a.doc_type, a.doc_number, 
                                          DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS inv_date,DATE_FORMAT(a.tgl_inv, '%Y-%m-%d') AS tgl_inv, c.type,  a.status, a.id, c.id_type, b.Id_Supplier AS id_customer,
                                          FORMAT((d.grand_total), 2) AS amount
                                   FROM  tbl_book_invoice AS a INNER JOIN 
                                          ...
                                          (select doc_number,DATE_FORMAT(tanggal_input, '%Y-%m-%d') create_date from tbl_log where activity = 'Create Invoice' GROUP BY doc_number) tgl on tgl.doc_number = a.no_invoice
                                    $str) b");
    ...
    while($row = mysqli_fetch_array($sql)){
        ...
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="width: 150px;" value = "'.$row['doc_type'].'">'.$row['doc_type'].'</td>
            <td style="width: 150px;" value = "'.$row['doc_number'].'">'.$row['doc_number'].'</td>
            <td style="width: 100px;" value = "'.$row['inv_date'].'">'.date("d-M-Y",strtotime($row['inv_date'])).'</td>
            <td style="width: 100px;" value = "'.$row['tgl_inv'].'">'.date("d-M-Y",strtotime($row['tgl_inv'])).'</td>
            <td style="" value = "'.$row['type'].'">'.$row['type'].'</td>
            <td style="" value = "'.$row['status'].'">'.$row['status'].'</td>
            <td style="width:50px; text-align : center;" value="'.$row['amount'].'">'.$row['amount'].'</td>';
            if ($row['status'] == 'CANCEL') {
                echo '<td style="text-align: center;">-</td>';
            }else{
                echo '<td style="width:100px;text-align : center" >
                <a href="pdf_list_invoice.php?id_inv='.$row['id'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>
                </td>';
            }
...
<script type="text/javascript">
    ...
    location.href = "create-list-invoice.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "list-invoice.php";
};
</script>
