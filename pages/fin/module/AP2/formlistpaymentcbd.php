<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM LIST PAYMENT CBD</h2>
<div class="box">
    <div class="box header">
<form id="form-data" method="post">
        <div class="form-row">
            <div class="col-md-3 mb-3">            
            <label for="nopayment"><b>No List Payment CBD</b></label>
            <?php
            $sql = mysqli_query($conn2,"select max(no_payment) from list_payment_cbd where id = (select MAX(id) from list_payment_cbd)");
...
<!--            <div class="col-md-3 mb-3">            
          ...
          <div class="form-group">
            ...
            <label for="nama_supp"><b>Supplier</b></label>
              <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true">
                ...
                <label><b>Kontrabon CBD Date</b></label>
    ...
    <div class="box body">
        <div class="row">
            ...
            $sql = mysqli_query($conn2,"select kontrabon_cbd.no_kbon as no_kbon, kontrabon_cbd.no_po as no_po, kontrabon_cbd.tgl_po as tgl_po, kontrabon_cbd.pph_value as pph, kontrabon_cbd.tgl_kbon as tgl_kbon, SUM(kontrabon_cbd.total) as total, kontrabon_cbd.nama_supp as supplier, kontrabon_h_cbd.balance as balance, kontrabon_cbd.tgl_tempo as tgl_tempo, kontrabon_cbd.curr as matauang, kontrabon_cbd.status as status, kontrabon_cbd.create_user as create_user, kontrabon_cbd.no_faktur as no_faktur, kontrabon_cbd.supp_inv as supp_inv, kontrabon_cbd.tgl_inv as tgl_inv from kontrabon_cbd inner join kontrabon_h_cbd on kontrabon_h_cbd.no_kbon = kontrabon_cbd.no_kbon where kontrabon_cbd.tgl_kbon between '$start_date' and '$end_date' and kontrabon_cbd.nama_supp = '$nama_supp' and kontrabon_cbd.status = 'Approved' and balance != 0 group by no_kbon");
...
<div class="box footer">   
        <form id="form-simpan">
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='listpaymentcbd.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
            </div>
            </div>                                    
        </form>
        </div>
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        var no_po = $(this).closest('tr').find('td:eq(15)').attr('value');
        ...
        $.ajax({
            type:'POST',
            url:'insertlistpaymentcbd.php',
            data: {'no_payment':no_payment, 'tgl_payment':tgl_payment, 'keterangan':keterangan, 'no_kbon':no_kbon, 'tgl_kbon':tgl_kbon, 'no_po':no_po, 'tgl_po':tgl_po, 'nama_supp':nama_supp, 'curr':curr, 'create_user':create_user, 'total_kbon':total_kbon, 'outstanding':outstanding, 'amount':amount, 'tgl_tempo':tgl_tempo, 'balance':balance},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
            success: function(response){
                ...
                window.location = 'listpaymentcbd.php';
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                alert(xhr);
        ...
        if(document.querySelectorAll("input[name='select[]']:checked").length == 0){
            alert("Please check the Kontrabon CBD number");
        }
    });
</script>
