...
<?php
$sql= "select ftr_cbd.id, kontrabon_cbd.no_kbon, kontrabon_cbd.tgl_kbon, ftr_cbd.no_ftr_cbd, ftr_cbd.tgl_ftr_cbd, ftr_cbd.no_po, ftr_cbd.tgl_po, ftr_cbd.supp, ftr_cbd.subtotal as subtotal, ftr_cbd.tax as tax, kontrabon_cbd.tgl_tempo, kontrabon_cbd.supp_inv, kontrabon_cbd.no_faktur, kontrabon_cbd.create_date, ftr_cbd.curr, ftr_cbd.biaya_tambahan, kontrabon_cbd.pph_value from ftr_cbd inner join kontrabon_cbd on kontrabon_cbd.no_cbd = ftr_cbd.no_ftr_cbd and kontrabon_cbd.no_po = ftr_cbd.no_po where kontrabon_cbd.no_kbon = '$no_kbon'";
...
<tr>
	...
	<td align="right"><h4>
      <?php
      $sql1 = mysqli_query($conn2,"select nama_supp from kontrabon_cbd where no_kbon = '$no_kbon'");
...
<!--       <td style="text-align:center;padding-top: -25px;padding-bottom: -10px;">
      <?php
      $querys = mysqli_query($conn2,"select distinct ftr_cbd.no_ftr_cbd from ftr_cbd inner join kontrabon_cbd on kontrabon_cbd.no_cbd = ftr_cbd.no_ftr_cbd where kontrabon_cbd.no_kbon = '$no_kbon'");
	...
	<td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql1 = mysqli_query($conn2,"select create_date from kontrabon_cbd where no_kbon = '$no_kbon'");
      $rows = mysqli_fetch_array($sql1);
	...
	<td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql2 = mysqli_query($conn2,"select tgl_tempo from kontrabon_cbd where no_kbon = '$no_kbon'");
	...
	<!-- <td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql3 = mysqli_query($conn2,"select supp_inv from kontrabon_cbd where no_kbon = '$no_kbon'");
	...
	<td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql4 = mysqli_query($conn2,"select no_faktur from kontrabon_cbd where no_kbon = '$no_kbon'");
...
<table  border="1" cellspacing="0" style="width:100%;font-size:12px;border-spacing:2px;">
  <tr>
      ...
      <th style="width:25%;border: 1px solid black;text-align:center;">No CBD</th>
      <th style="width:15%;border: 1px solid black;text-align:center;">CBD Date</th>
...
<!--       <th colspan="2" style="width:20%;border: 1px solid black;text-align:center;">Quantity</th>
      ...
      <th style="width:20%;border: 1px solid black;text-align:center;">Subtotal</th>
...
while($data=mysqli_fetch_array($query)){
	$no_po = $data['no_po'];
	$no_cbd = $data['no_ftr_cbd'];
	$tgl_cbd = $data['tgl_ftr_cbd'];
	$tgl_po = $data['tgl_po'];	
	...
	// $sum_qty += $qty;
	$sum_sub += $subtotal;
	$sum_total = $sum_sub + $ppn - $pph + $tambahan;
   echo '<tr>
      <td style="width:20%;text-align:center;">'.$no_po.'</td>
	  <td style="width:auto;text-align:center;">'.date("d M Y",strtotime($tgl_po)).'</td>  
      <td style="width:20%;text-align:center;">'.$no_cbd.'</td>
      <td style="width:auto;text-align:center;">'.date("d M Y",strtotime($tgl_cbd)).'</td>
      <td style="width:12%;text-align:right;">'.$curr.' '.number_format($subtotal, 2).'</td>
    </tr>';	
};	
?>
