...
<form id="form-data" method="post">
        <div class="form-row">
            ...
            <input type="text" style="font-size: 14px;" name="tanggal_inv" id="tanggal_inv" class="form-control tanggal" 
            value="<?php             
            if(!empty($_POST['tanggal_inv'])) {
                echo $_POST['tanggal_inv'];
            }
            else{
                echo date("d-m-Y");
            } ?>">
            ...
            <div class="col-md-2 mb-3">            
            <label for="tanggal_faktur"><b>Tanggal Faktur Pajak</b></label>          
...
<script type="text/javascript">
    ...
    var tgl_inv = document.getElementById('tanggal_inv').value;
        ...
        var create_user = '<?php echo $user; ?>';    
        var unik_code = document.getElementById('unik_code').value;     
        if(no_inv != '' && tgl_inv != '' && no_bpb != ''){
        $.ajax({
            type:'POST',
            url:'insert_bpb_fakturinv.php',
        ...
        if(document.getElementById('no_inv').value == ''){
            alert("Please Input No Invoice");
