<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM MAINTAIN FTR DP</h2>
<div class="box">
...
<form id="form-data" method="post">
    ...
    <div class="box body">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;vertical-align: middle;width: 150px; display: none;">No PO</th>
                            <th style="text-align: center;vertical-align: middle;width: 300px">Supplier</th>            
                            <th style="text-align: center;vertical-align: middle;width: 100px">Total</th>
                            <th style="text-align: center;vertical-align: middle;">DP</th>           
            ...
            $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where tgl_ftr_dp between '$start_date' and '$end_date' and supp = '$nama_supp' and status = 'Approved' and kb_inv is null group by no_ftr_dp");


            while($row = mysqli_fetch_array($sql)){
            $tgl_kbon = $row['tgl_ftr_dp'];
            ...
            // $date_diff = round($diff / 86400);                     
                    echo '<tr>
                            ...
                            <td class="dt_out" style="width:100px;" dataout="'.$row['no_ftr_dp'].'">'.$row['no_ftr_dp'].'</td>
                            <td class="dt_out" style="width:100px;" dataout1= "'.date("d F Y",strtotime($row['tgl_ftr_dp'])).'">'.date("d-M-Y",strtotime($row['tgl_ftr_dp'])).'</td>
                            <td class="dt_out" style="width:100px;display: none;" dataout2="'.$row['no_po'].'">'.$row['no_po'].'</td>
                            <td value = "'.$row['supp'].'">'.$row['supp'].'</td>
                            <td style="text-align:right;" class="dt_out" style="width:100px;" dataout3= "'.$row['total'].'">'.number_format($row['total'],2).'</td>      
                            <td style="text-align:right;" class="dt_out" style="width:100px;" dataout6= "'.$row['dp'].'">'.number_format($row['dp'],2).'</td>
                            <td style="text-align:right;" class="dt_out" style="width:100px;" dataout4= "'.$row['balance'].'">'.number_format($row['balance'],2).'</td>
...
<div class="box footer"> 
        ...
        <form id="form-simpan">
            ...
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">No FTR DP</label>
            <div class="col-md-5 mb-3">                              
            ...
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">FTR DP Date</label>
            <div class="col-md-5 mb-3">                              
            ...
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">Pesan</label>
            <div class="col-md-5 mb-3">                              
        ...
        <!-- <form class="form-row col" method="POST" action="upload.php" enctype="multipart/form-data" style="margin-bottom: -15px;">
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='pengajuan_ftrdp.php'"><span class="fa fa-angle-double-left"></span> Back</button>            
            </div>
            </div>                                    
        </form>
        </div>
...
function formatMoney(amount, decimalCount = 2, decimal = ".", thousands = ",") {
  try {
    ...
    var pdp = parseFloat($(this).closest('tr').find('td:eq(6)').attr('dataout6'),10) || 0;
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        var total = $(this).closest('tr').find('td:eq(5)').attr('dataout3');
        var dp = $(this).closest('tr').find('td:eq(6)').attr('dataout6');
        ...
        $.ajax({
            type:'POST',
            url:'insertpengajuan_ftrdp.php',
            data: {'no_cbd':no_cbd, 'tgl_cbd':tgl_cbd, 'no_po':no_po, 'total':total, 'dp':dp, 'curr':curr, 'nama_supp':nama_supp, 'tgl_pengajuan':tgl_pengajuan, 'nama_pengaju':nama_pengaju, 'pesan':pesan},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
            success: function(response){
                ...
                window.location = 'pengajuan_ftrdp.php';
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                alert(xhr);
...
<script type="text/javascript">     
    ...
    $('#txt_tgl_kbon').html('FTR DP Date : ' + tgl_kbon + '');
