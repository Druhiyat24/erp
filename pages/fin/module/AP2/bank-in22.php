...
<table id="mytable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            $sql = mysql_query("select doc_num, customer, date, ref_data, akun, bank, curr, amount, outstanding, status,deskripsi,approve_date from tbl_bankin_arcollection $where group by doc_num order by id asc",$conn1);
               // echo $start_date;
               //  echo $end_date;                                                       
                while($row = mysql_fetch_array($sql)){
