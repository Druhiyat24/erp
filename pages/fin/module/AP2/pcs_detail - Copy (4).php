...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     ...
     $sql = mysqli_query($conn1,"select * from(
        ...
        union (select a.nama_supp,a.no_kbon,DATE_FORMAT(c.create_date, '%Y-%m-%d') as tgl_kbon,DATEDIFF(c.tgl_tempo,c.tgl_kbon) as top, c.tgl_tempo,a.curr,(a.amount + a.pph_value) as bayar,(a.total_kbon + a.pph_value - b.jml_potong + a.dp_value + b.jml_return) as totalori, (b.jml_potong - c.dp_value) as jml_potong  from list_payment a inner join potongan b on b.no_kbon = a.no_kbon left join kontrabon_h c on c.no_kbon = a.no_kbon where DATE_FORMAT(c.create_date, '%Y-%m-%d') < '$start_date' and DATE_FORMAT(a.create_date, '%Y-%m-%d') between '$start_date' and '$end_date' and a.nama_supp = '$nama_supp')
     ) as b order by b.nama_supp asc");
    }
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $sqllp = mysqli_query($conn1,"select no_kbon,tgl_kbon from list_payment where no_kbon = '$no_kbon' and create_date between '$start_date' and '$end_date' GROUP BY no_kbon");
    $rowlp = mysqli_fetch_array($sqllp);
    $no_lp = isset($rowlp['no_kbon']) ? $rowlp['no_kbon'] : null;


    $sqllp2 = mysqli_query($conn1,"select no_kbon,tgl_kbon from list_payment where no_kbon = '$no_kbon' and create_date < '$start_date' GROUP BY no_kbon");
    $rowlp2 = mysqli_fetch_array($sqllp2);
...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn1,"select * from(
        (select '' as abc,a.nama_supp, a.no_payment, DATE_FORMAT(a.create_date, '%Y-%m-%d') as tgl_payment,a.top,a.tgl_tempo,curr, sum(a.amount + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and create_date between '2022-04-14' and '$start_date' GROUP BY no_payment order by create_date asc) union 
     ...
     $sql = mysqli_query($conn1,"select * from(
        (select '' as abc,a.nama_supp, a.no_payment, DATE_FORMAT(a.create_date, '%Y-%m-%d') as tgl_payment,a.top,a.tgl_tempo,curr, sum(a.amount + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and create_date between '2022-04-14' and '$start_date' and a.nama_supp = '$nama_supp' GROUP BY no_payment order by create_date asc) union 
        (select '' as abc,a.nama_supp, a.no_payment, DATE_FORMAT(a.create_date, '%Y-%m-%d') as tgl_payment,a.top,a.tgl_tempo,curr, sum(a.amount + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and create_date between '$start_date' and '$end_date' and a.nama_supp = '$nama_supp' GROUP BY no_payment order by create_date asc) union 
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $sqllp = mysqli_query($conn1,"select list_payment_id,ttl_bayar from payment_ftr where list_payment_id = '$no_payment' and create_date between '$start_date' and '$end_date' GROUP BY list_payment_id");
    $rowlp = mysqli_fetch_array($sqllp);
    $no_lp = isset($rowlp['list_payment_id']) ? $rowlp['list_payment_id'] : null;


    $sqllp2 = mysqli_query($conn1,"select list_payment_id,ttl_bayar from payment_ftr where list_payment_id = '$no_payment' and create_date < '$start_date' GROUP BY list_payment_id");
    $rowlp2 = mysqli_fetch_array($sqllp2);
