<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">MAINTAIN LIST PAYMENT</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="pengajuanpayment.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
        ...
        <div class="form-row">
            ...
            <div class="input-group-append col" onchange="this.form.submit()">                                   
...
<?php
        $querys = mysqli_query($conn2,"select useraccess.menu as menu,useraccess.username as username, useraccess.fullname as fullname, menurole.id as id from useraccess inner join menurole on menurole.menu = useraccess.menu where username = '$user' and useraccess.menu = 'Create Maintain List Payment'");
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;width: 115px">No List payment</th>
            <th style="text-align: center;vertical-align: middle;width: 130px">List Payment Date</th>
            <th style="text-align: center;vertical-align: middle;display: none;">No Kontrabon</th>
            <th style="text-align: center;vertical-align: middle;width: 150px;">Supplier</th>
            <th style="text-align: center;vertical-align: middle;width: 100px">Total KB</th>
            <th style="text-align: center;vertical-align: middle;">Amount</th>
            <th style="text-align: center;vertical-align: middle;width: 110px">Maintain Date</th>
            <th style="text-align: center;vertical-align: middle;">Name</th>
            <th style="text-align: center;vertical-align: middle;width: 150px;">Message</th>
            <th style="text-align: center;vertical-align: middle;width: 80px">Approve by</th>            
            <th style="text-align: center;vertical-align: middle;width: 80px">Cancel by</th>
...
$nama_supp ='';
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where tgl_pengajuan = '$date_now' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select * from pengajuan_payment group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where nama_supp = '$nama_supp' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where status = '$status' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where status = '$status' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_payment where status = '$status' and nama_supp = '$nama_supp' group by id");
    }
    else{
    $sql = mysqli_query($conn2,"select * from pengajuan_payment where status = '$status' and nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id ");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    ...
    $status = $row['status']; 
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td value = "'.$row['total_kbon'].'">'.number_format($row['total_kbon'],2).'</td>
            <td value = "'.$row['total_amount'].'">'.number_format($row['total_amount'],2).'</td>
            <td value = "'.date("d-M-Y",strtotime($row['tgl_pengajuan'])).'">'.date("d-M-Y",strtotime($row['tgl_pengajuan'])).'</td>
            ...
            if($status == 'Approved' and $group != 'STAFF' and $fin == '1'){
                ...
                echo ' <a id="approve" href=""><button type="button" class="btn-xs btn-outline-success" style="border-radius:6px;"><i class="fa fa-paper-plane" style="padding-right: 10px; padding-left: 5px;"> Approve</i></button></a>
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){
        ...
        $.ajax({
            type:'POST',
            url:'approvepengajuan_payment.php',
            data: {'id':id, 'no_payment':no_payment, 'no_kbon':no_kbon, 'approve_user':approve_user},
            // close: function(e){
            //     e.preventDefault();
            // },
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#delete", function(){ 
        ...
        $.ajax({
            type:'POST',
            url:'cancelpengajuan_payment.php',
            data: {'id':id, 'no_payment':no_payment, 'no_kbon':no_kbon, 'cancel_user':cancel_user},
            // close: function(e){
            //     e.preventDefault();
            // },
            success: function(data){                
...
<script type="text/javascript">     
    ...
    $('#txt_tgl_tempo').html('Applicant Name : ' + tgl_tempo + '');
    $('#txt_curr').html('Maintain Date : ' + curr + '');        
...
<script type="text/javascript">
    ...
    location.href = "formpengajuanpayment.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "pengajuanpayment.php";
};
</script>
