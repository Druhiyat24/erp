<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM MAINTAIN KONTRABON CBD</h2>
<div class="box">
    ...
    <div class="box body">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr>
                            <th style="width:10px; text-align: center;"><input type="checkbox" id="select_all"></th>
            ...
            $sql = mysqli_query($conn2,"select no_kbon, no_cbd, tgl_kbon, nama_supp, SUM(subtotal) , SUM(tax) as tax, SUM(total) , curr, create_user, status, status_int, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_cbd where tgl_kbon between '$start_date' and '$end_date' and nama_supp = '$nama_supp' and status = 'Approved' and lp_inv is null group by no_kbon");
            ...
            $date_diff = round($diff / 86400);                     
                    echo '<tr>
                            ...
                            <td class="dt_out" style="width:100px;" dataout1="'.$row['no_cbd'].'">'.$row['no_cbd'].'</td>
...
<div class="box footer"> 
        ...
        <form id="form-simpan">
            ...
            <div class="col-md-5 mb-3">                              
                <input type="text" class="form-control-plaintext" name="nobpb" id="nobpb" value="" placeholder="-" style="font-size: 14px; text-align: left;" readonly>
            </div>
            </div>
            <div class="form-row col">
        ...
        <!-- <form class="form-row col" method="POST" action="upload.php" enctype="multipart/form-data" style="margin-bottom: -15px;">
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='pengajuankb_cbd.php'"><span class="fa fa-angle-double-left"></span> Back</button>         
            </div>           
            </div>
            </div>                                    
        </form>
...
function formatMoney(amount, decimalCount = 2, decimal = ".", thousands = ",") {
  try {
    ...
    var bpb = $(this).closest('tr').find('td:eq(2)').attr('dataout1') || '';
    var tgl = $(this).closest('tr').find('td:eq(3)').attr('value') || '';
    var ttl = parseFloat($(this).closest('tr').find('td:eq(8)').attr('value'),10) || 0;
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        var no_cbd = $(this).closest('tr').find('td:eq(2)').attr('dataout1');
        ...
        $.ajax({
            type:'POST',
            url:'insertpengajuan_cbd.php',
            data: {'no_kbon':no_kbon,'no_cbd':no_cbd, 'nama_supp':nama_supp, 'tgl_pengajuan':tgl_pengajuan, 'nama_pengaju':nama_pengaju, 'pesan':pesan},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
            success: function(response){
                ...
                window.location = 'pengajuankb_cbd.php';
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                alert(xhr);
...
<script type="text/javascript">     
    ...
    var tgl_tempo = $(this).closest('tr').find('td:eq(9)').text();
