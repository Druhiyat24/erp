<?php include '../header.php' ?>
    ...
    <!-- MAIN -->    
    <div class="col p-4">
        <h2 class="text-center">APPROVE BPB RETURN</h2>
<div class="box">
    <div class="box header">
<form id="form-data" action="formapprovebppb.php" method="post">
        <div class="form-row">
            <div class="col-md-6">
    ...
    <div class="box body">
        <div class="row">
            ...
            $sql = mysqli_query($conn2,"select id, no_bppb, tgl_bppb, no_ro, supplier, no_bpb, curr, confirm1, SUM(qty * price) as total from bppb_new where confirm2 = '' and status != 'Cancel' group by no_ro");                
            }else {
            $sql = mysqli_query($conn2,"select id, no_bppb, tgl_bppb, no_ro, supplier, no_bpb, curr, confirm1, SUM(qty * price) as total from bppb_new where supplier = '$nama_supp' and confirm2 = '' and status != 'Cancel' group by no_ro");
            }
            ...
            while($row = mysqli_fetch_array($sql)){                                          
                    echo'<tr>
                            ...
                            <td style="width:100px;" value="'.$row['no_ro'].'">'.$row['no_ro'].'</td>                                                                                                        
                            <td style="width:50px;" value="'.$row['no_bppb'].'">'.$row['no_bppb'].'</td>
                            <td style="width:100px;" value="'.$row['tgl_bppb'].'">'.date("d-M-Y",strtotime($row['tgl_bppb'])).'</td>
...
<script type="text/javascript">     
    ...
    var tgl_bpb = $(this).closest('tr').find('td:eq(4)').text();
    var no_po = $(this).closest('tr').find('td:eq(6)').attr('value');
    ...
    $('#txt_bpb').html(no_ro);
    $('#txt_tglbpb').html('No BPB : ' + tgl_bpb + '');
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#approve", function(){
        ...
        $.ajax({
            type:'POST',
            url:'approvebppb.php',
            data: {'no_ro':no_ro, 'approve_user':approve_user, 'update_user':update_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
                console.log(response);
                window.location = 'formapprovebppb.php';
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#cancel", function(){
        ...
        $.ajax({
            type:'POST',
            url:'cancelbppb.php',
            data: {'no_ro':no_ro, 'update_user':update_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
