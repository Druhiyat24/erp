...
<?php
$sql= "select * from payment_ftr where payment_ftr_id = '$no_pay' ";
...
<tr>
    ...
    <td align="right"><h4>
      <?php
      $sql1 = mysqli_query($conn2,"select nama_supp from payment_ftr where payment_ftr_id = '$no_pay'");
...
<table style="font-size:12px;">
    ...
    <td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql1 = mysqli_query($conn2,"select tgl_pelunasan from payment_ftr where payment_ftr_id = '$no_pay'");
      $rows = mysqli_fetch_array($sql1);
    ...
    <td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql2 = mysqli_query($conn2,"select cara_bayar from payment_ftr where payment_ftr_id = '$no_pay'");
    ...
    <td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql4 = mysqli_query($conn2,"select bank from payment_ftr where payment_ftr_id = '$no_pay'");
    ...
    <td style="text-align:center;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql3 = mysqli_query($conn2,"select account from payment_ftr where payment_ftr_id = '$no_pay'");
...
<table  border="1" cellspacing="0" style="width:100%;font-size:12px;border-spacing:2px;">
  <tr> 
      <th style="width:28%;border: 1px solid black;text-align:center;">No List payment</th>
      ...
      <th style="width:20%;border: 1px solid black;text-align:center;">List Payment Date</th> 
      <th style="width:20%;border: 1px solid black;text-align:center;">Total List Payment</th>      
      <th style="width:15%;border: 1px solid black;text-align:center;">Rate</th>
      <th style="width:17%;border: 1px solid black;text-align:center;">Nominal Pay</th>
...
<tr>
      <td colspan="2" style="width:48%;text-align:center;"><b>Jumlah</b></td>     
      <td style="width:20%;text-align:right;border-left:none;"><?php echo $curr1.' '.number_format($sumtotal, 2) ?></td>
      <td style="width:15%;text-align:right;border-left:none;"></td>
      <td style="width:17%;text-align:right;border-left:none;"><?php echo $curr2.' '.number_format($sumamount, 2) ?></td>                 
    </tr>
...
<br/>
 ...
 <div style="margin-bottom: 2.54cm; page-break-inside: avoid;">
    <table cellpadding="0" cellspacing="0" border="1" width='600';>
      ...
      <tr>
        <td>Payment Number : <?php echo $no_pay ?></td>
      </tr>
      <tr>
        <td>Total Payment : <?php echo $curr1." ".number_format($sumamount, 2) ?></td>
      </tr>
    </table>
    </div>
</body>
