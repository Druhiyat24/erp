...
$sql = mysqli_query($conn2,"select no_po, tgl_po, no_pi, supp, status, SUM(total) as total_po, SUM(dp) as dp_code, SUM(dp_value) as dp_value, SUM(balance) as balance from ftr_dp where no_ftr_dp = '$no_kbon' and status = 'draft' group by no_po");	
	...
	$table = '<table id="mytdmodal" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
            ...
            $table .= '<tr>                       
                            ...
                            <td style="width:50px;" value="'.$row['dp_code'].'">'.$row['dp_code'].'</td>                            
...
echo '<table width="100%" border="0" style="font-size:13px; font-weight: bold;">
