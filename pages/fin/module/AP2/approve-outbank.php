...
<form id="form-data" action="approve-pv.php" method="post">
            ...
            <label for="nama_supp"><b>Supplier</b></label>            
              <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true" onchange="this.form.submit()">
                ...
                <!-- <div style="padding-left: 10px;">
                    ...
                    <button style="border-radius: 5px" id="btnob" type="button" class="btn-primary btn-xs"><span></span> Outgoing Bank</button>
                </div> -->
    ...
    <div class="box body">
        <div class="row">
            ...
            // }else {
            $sql = mysqli_query($conn2,"select no_bankout,bankout_date,nama_supp,curr, amount, outstanding,IF(reff_doc = 'Payment','List Payment',reff_doc) as reff_doc, akun, bank,status,IF(deskripsi = '','-',deskripsi) as deskripsi from b_bankout_h where status = 'Draft' group by no_bankout order by id asc");
            // }
            ...
            while($row = mysqli_fetch_array($sql)){                                          
                    echo'<tr>
                            ...
                            <td style="width:50px; text-align : center" value="'.$row['nama_supp'].'">'.$row['nama_supp'].'</td> 
    ...
    $("#form-simpan").on("click", "#approve", function(){
        ...
        $.ajax({
            ...
            success: function(response){                
                ...
                window.location = 'approve-outbank.php';
        ...
        if(document.querySelectorAll("input[name='select[]']:checked").length >= 1){
            ...
            alert("Silahkan Ceklist No Bank Out ");
        }        
    });
</script>
