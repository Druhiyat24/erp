...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,c.jml_pterms as top, DATE_ADD(a.bpbdate, INTERVAL c.jml_pterms DAY) as due_date,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier where a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '2022-04-14' and '$start_date' group by a.bpbno_int order by bpbdate asc)
union

(select b.Supplier,a.bpbno_int,bpbdate,c.jml_pterms as top, DATE_ADD(a.bpbdate, INTERVAL c.jml_pterms DAY) as due_date,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier where a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '$start_date' and '$end_date' group by a.bpbno_int order by bpbdate asc)

union(select a.nama_supp, a.no_bpb, a.tgl_bpb,b.top, DATE_ADD(a.tgl_bpb, INTERVAL b.top DAY) as due_date, a.curr, a.total from saldo_bpb_ap a left join bpb_new b on b.no_bpb = a.no_bpb group by b.no_bpb)
...
union (select a.nama_supp, a.no_bpb,a.tgl_bpb,c.top, DATE_ADD(a.tgl_bpb, INTERVAL c.top DAY) as due_date,a.curr,(a.subtotal + a.tax) from kontrabon a inner join list_payment b on b.no_kbon = a.no_kbon left join bpb_new c on c.no_bpb = a.no_bpb inner join kontrabon_h d on d.no_kbon = a.no_kbon where a.tgl_bpb < '$start_date' and d.tgl_kbon2 < '$start_date' and b.tgl_payment between '$start_date' and '$start_date' group by c.no_bpb)) as b order by b.Supplier asc");
    }
    else {
     $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,c.jml_pterms as top, DATE_ADD(a.bpbdate, INTERVAL c.jml_pterms DAY) as due_date,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier where a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '2022-04-14' and '$start_date' and b.Supplier = '$nama_supp' group by a.bpbno_int order by bpbdate asc)
union

(select b.Supplier,a.bpbno_int,bpbdate,c.jml_pterms as top, DATE_ADD(a.bpbdate, INTERVAL c.jml_pterms DAY) as due_date,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier where a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '$start_date' and '$end_date' and b.Supplier = '$nama_supp' group by a.bpbno_int order by bpbdate asc)

union(select a.nama_supp, a.no_bpb, a.tgl_bpb,b.top, DATE_ADD(a.tgl_bpb, INTERVAL b.top DAY) as due_date, a.curr, a.total from saldo_bpb_ap a left join bpb_new b on b.no_bpb = a.no_bpb where a.nama_supp = '$nama_supp' group by b.no_bpb)
...
union (select a.nama_supp, a.no_bpb,a.tgl_bpb,c.top, DATE_ADD(a.tgl_bpb, INTERVAL c.top DAY) as due_date,a.curr,(a.subtotal + a.tax) from kontrabon a inner join list_payment b on b.no_kbon = a.no_kbon left join bpb_new c on c.no_bpb = a.no_bpb inner join kontrabon_h d on d.no_kbon = a.no_kbon where a.tgl_bpb < '$start_date' and d.tgl_kbon2 < '$start_date' and b.tgl_payment between '$start_date' and '$start_date' and a.nama_supp = '$nama_supp' group by c.no_bpb)) as b order by b.Supplier asc");
    }
...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn1,"select * from(
     	(select a.nama_supp, a.no_kbon, a.tgl_kbon2,DATEDIFF(a.tgl_tempo,a.tgl_kbon) as top, a.tgl_tempo,a.curr, if(a.balance != a.total,if(a.curr = 'USD', sum((a.total - a.balance) + a.pph_fgn),sum((a.total - a.balance) + a.pph_idr)),0) as bayar, if(a.curr = 'USD', sum(a.subtotal + a.tax),sum(a.subtotal + a.tax)) as totalori,(b.jml_potong - b.jml_return - a.dp_value) as jml_potong from kontrabon_h a inner join potongan b on b.no_kbon = a.no_kbon where a.status !='CANCEL' and a.tgl_kbon2 between '2022-04-14' and '$start_date' GROUP BY a.no_kbon order by a.tgl_kbon2 asc) 
			
     	union (select a.nama_supp, a.no_kbon, a.tgl_kbon2,DATEDIFF(a.tgl_tempo,a.tgl_kbon) as top, a.tgl_tempo,a.curr, if(a.balance != a.total,if(a.curr = 'USD', sum((a.total - a.balance) + a.pph_fgn),sum((a.total - a.balance) + a.pph_idr)),0) as bayar, if(a.curr = 'USD', sum(a.subtotal + a.tax),sum(a.subtotal + a.tax)) as totalori,(b.jml_potong - b.jml_return - a.dp_value) as jml_potong from kontrabon_h a inner join potongan b on b.no_kbon = a.no_kbon where a.status !='CANCEL' and a.tgl_kbon2 between '$start_date' and '$end_date' GROUP BY a.no_kbon order by a.tgl_kbon2 asc)

     	union (select a.nama_supp, a.no_kbon, b.tgl_kbon2,DATEDIFF(b.tgl_tempo,b.tgl_kbon) as top, b.tgl_tempo, a.curr, a.total as bayar, a.total_bpb as total_ori, a.adjust as jml_potong from saldo_kbon_ap a left join kontrabon_h b on b.no_kbon = a.no_kbon)
			
     	union (select a.nama_supp,a.no_kbon,c.tgl_kbon2,DATEDIFF(c.tgl_tempo,c.tgl_kbon) as top, c.tgl_tempo,a.curr,(a.amount + a.pph_value) as bayar,(c.subtotal + c.tax) as totalori,(b.jml_potong - b.jml_return - c.dp_value) as jml_potong  from list_payment a inner join potongan b on b.no_kbon = a.no_kbon left join kontrabon_h c on c.no_kbon = a.no_kbon where c.tgl_kbon2 < '$start_date' and a.tgl_payment between '$start_date' and '$end_date')
     ) as b order by b.nama_supp asc");
    }

    else {
     $sql = mysqli_query($conn1,"select * from(
        (select a.nama_supp, a.no_kbon, a.tgl_kbon2,DATEDIFF(a.tgl_tempo,a.tgl_kbon) as top, a.tgl_tempo,a.curr, if(a.balance != a.total,if(a.curr = 'USD', sum((a.total - a.balance) + a.pph_fgn),sum((a.total - a.balance) + a.pph_idr)),0) as bayar, f(a.curr = 'USD', sum(a.subtotal + a.tax),sum(a.subtotal + a.tax)) as totalori,(b.jml_potong + b.jml_return + a.dp_value) as jml_potong from kontrabon_h a inner join potongan b on b.no_kbon = a.no_kbon where a.status !='CANCEL' and a.tgl_kbon2 between '2022-04-14' and '$start_date' and a.nama_supp = '$nama_supp' GROUP BY a.no_kbon order by a.tgl_kbon2 asc) 
            
        union (select a.nama_supp, a.no_kbon, a.tgl_kbon2,DATEDIFF(a.tgl_tempo,a.tgl_kbon) as top, a.tgl_tempo,a.curr, if(a.balance != a.total,if(a.curr = 'USD', sum((a.total - a.balance) + a.pph_fgn),sum((a.total - a.balance) + a.pph_idr)),0) as bayar, f(a.curr = 'USD', sum(a.subtotal + a.tax),sum(a.subtotal + a.tax)) as totalori,(b.jml_potong + b.jml_return + a.dp_value) as jml_potong from kontrabon_h a inner join potongan b on b.no_kbon = a.no_kbon where a.status !='CANCEL' and a.tgl_kbon2 between '$start_date' and '$end_date' and a.nama_supp = '$nama_supp' GROUP BY a.no_kbon order by a.tgl_kbon2 asc)

        union (select a.nama_supp, a.no_kbon, b.tgl_kbon2,DATEDIFF(b.tgl_tempo,b.tgl_kbon) as top, b.tgl_tempo, a.curr, a.total as bayar, a.total_bpb as total_ori, a.adjust as jml_potong from saldo_kbon_ap a left join kontrabon_h b on b.no_kbon = a.no_kbon where a.nama_supp = '$nama_supp')
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $sqllp = mysqli_query($conn1,"select a.no_kbon,d.tgl_kbon2 from list_payment a inner join kontrabon_h d on d.no_kbon = a.no_kbon where a.no_kbon = '$no_kbon' and a.tgl_payment between '$start_date' and '$end_date' GROUP BY a.no_kbon");
    $rowlp = mysqli_fetch_array($sqllp);
