...
union (select nama_supp, no_bpb, tgl_bpb, top, duedate, curr, total from tbl_tamb_bpb where nama_supp = '$nama_supp')) as b order by b.Supplier asc");
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $bbayar = $row['total'];
     ...
     $sqldate = mysqli_query($conn1,"select a.bppbno_int,b.jml_pterms as top,DATE_ADD(a.bppbdate, INTERVAL b.jml_pterms DAY) as due_date from bppb a left join bpb d on d.bpbno = a.bpbno_ro left JOIN po_header b on b.pono = d.pono inner join mastersupplier c on c.Id_Supplier = a.id_supplier  where a.bppbno_int = '$no_bpb' group by bppbno_int");
...
<?php
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $pph2 = isset($rowlp['pph']) ? $rowlp['pph'] : 0;
    $pph_h = $pph2 / $creff;
