<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM MAINTAIN LIST PAYMENT</h2>
<div class="box">
    ...
    <div class="box body">
        <div class="row">
            ...
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr >
                            <th style="text-align: center;vertical-align: middle;"><input type="checkbox" id="select_all"></i></th>
                            <th style="text-align: center;vertical-align: middle;width: 200px">No List Payment</th>
                            <th style="text-align: center;vertical-align: middle;width: 150px">List Payment Date</th>
                            <th style="text-align: center;vertical-align: middle;display: none;">No Kontra Bon</th>
                            <th style="text-align: center;vertical-align: middle;width: 350px">Supplier</th>
                            <th style="text-align: center;vertical-align: middle;width: 150px">Total KB</th>
                            <th style="text-align: center;vertical-align: middle;width: 150px">Outstanding</th>
                            <th style="text-align: center;vertical-align: middle;width: 150px">Amount</th>
            ...
            $sql = mysqli_query($conn2,"select id, no_payment, tgl_payment,no_kbon,tgl_kbon, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment where tgl_payment between '$start_date' and '$end_date' and nama_supp = '$nama_supp' and status_int = '4' group by no_payment");
            ...
            $date_diff = round($diff / 86400);                     
                    echo '<tr>
                            ...
                            <td class="dt_out" style="width:100px;" dataout="'.$row['no_payment'].'">'.$row['no_payment'].'</td>
                            <td class="dt_out" style="width:100px;" dataout1= "'.date("d F Y",strtotime($row['tgl_payment'])).'">'.date("d-M-Y",strtotime($row['tgl_payment'])).'</td>
                            <td class="dt_out" style="width:100px;display: none;" dataout2="'.$row['no_kbon'].'">'.$row['no_kbon'].'</td>
                            <td value = "'.$row['nama_supp'].'">'.$row['nama_supp'].'</td>
                            <td style="text-align:right;" class="dt_out" style="width:100px;" dataout3= "'.$row['SUM(total_kbon)'].'">'.number_format($row['SUM(total_kbon)'],2).'</td>
                            <td style="text-align:right;" value = "'.$row['SUM(outstanding)'].'">'.number_format($row['SUM(outstanding)'],2).'</td>       
                            <td style="text-align:right;" class="dt_out" style="width:100px;" dataout4= "'.$row['SUM(amount)'].'">'.number_format($row['SUM(amount)'],2).'</td>
...
<div class="box footer"> 
        ...
        <form id="form-simpan">
            ...
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">No List Payment</label>
            <div class="col-md-5 mb-3">                              
                <input type="text" class="form-control-plaintext" name="nopay" id="nopay" value="" placeholder="-" style="font-size: 14px; text-align: left;" readonly>
            </div>
            </div>
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">List Payment Date</label>
            <div class="col-md-5 mb-3">                              
                <input type="text" class="form-control-plaintext" name="tglpay" id="tglpay" value="" placeholder="-" style="font-size: 14px; text-align: left;" readonly>
            </div>
            </div>
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">Total Kontrabon</label>
            <div class="col-md-5 mb-3">                              
                <input type="text" class="form-control-plaintext" name="total_kbon" id="total_kbon" value="" placeholder="0.00" style="font-size: 14px; text-align: left;" readonly>
            </div>
            </div>
            <div class="form-row col">
                <label for="subtotal" class="col-sm-2 col-form-label" style="width: 100px;">Total Amount</label>
            <div class="col-md-5 mb-3">                              
                <input type="text" class="form-control-plaintext" name="total_amount" id="total_amount" value="" placeholder="0.00" style="font-size: 14px; text-align: left;" readonly>
            </div>
            </div>
            <div class="form-row col">
        ...
        <!-- <form class="form-row col" method="POST" action="upload.php" enctype="multipart/form-data" style="margin-bottom: -15px;">
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='pengajuanpayment.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
            </div>
            </div>                                    
        </form>
        </div>
...
function formatMoney(amount, decimalCount = 2, decimal = ".", thousands = ",") {
  try {
    ...
    var kbon = parseFloat($(this).closest('tr').find('td:eq(5)').attr('dataout3'),10) || 0;
    var am = parseFloat($(this).closest('tr').find('td:eq(7)').attr('dataout4'),10) || 0;
    ...
    $("#nopay").val(n_pay);   
    $("#tglpay").val(t_pay); 
    $("#total_kbon").val(formatMoney(ttl_kbon));  
    $("#total_amount").val(formatMoney(ttl_am));                   
});        
</script>
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        $("input[type=checkbox]:checked").each(function () {
        var no_payment = $(this).closest('tr').find('td:eq(1)').attr('dataout');
        var tgl_payment = $(this).closest('tr').find('td:eq(2)').attr('dataout1');
        var no_kbon = $(this).closest('tr').find('td:eq(3)').attr('dataout2');
        var total_kbon = $(this).closest('tr').find('td:eq(5)').attr('dataout3');
        var amount = $(this).closest('tr').find('td:eq(7)').attr('dataout4');
        ...
        $.ajax({
            type:'POST',
            url:'insertpengajuan_payment.php',
            data: {'no_payment':no_payment, 'tgl_payment':tgl_payment, 'no_kbon':no_kbon, 'total_kbon':total_kbon, 'amount':amount, 'nama_supp':nama_supp, 'tgl_pengajuan':tgl_pengajuan, 'nama_pengaju':nama_pengaju, 'pesan':pesan},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
            success: function(response){
                ...
                window.location = 'pengajuanpayment.php';
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                alert(xhr);
