<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST MAINTAIN FTR DP</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="pengajuan_ftrdp.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle; width: 120px">FTR DP Date</th>
...
$nama_supp ='';
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where tgl_pengajuan = '$date_now' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where nama_supp = '$nama_supp' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where status = '$status' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where status = '$status' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where status = '$status' and nama_supp = '$nama_supp' group by id");
    }
    else{
    $sql = mysqli_query($conn2,"select * from pengajuan_ftrdp where status = '$status' and nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id ");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    ...
    $status = $row['status']; 
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="display:none;" value = "'.$row['total'].'">'.number_format($row['total'],2).'</td>
            <td value = "'.$row['total'].'">'.number_format($row['dp'],2).'</td>
            ...
            if($status == 'Approved' and $group != 'STAFF' and $fin == '1'){
                ...
                echo ' <a id="approve" href=""><button style="border-radius: 6px" type="button" class="btn-xs btn-outline-success"><i class="fa fa-paper-plane" style="padding-right: 10px; padding-left: 5px;"> Approve</i></button></a>
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){
        ...
        $.ajax({
            type:'POST',
            url:'approvepengajuan_ftrdp.php',
...
<script type="text/javascript">
    ...
    location.href = "formpengajuan_ftrdp.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "pengajuan_ftrdp.php";
};
</script>
