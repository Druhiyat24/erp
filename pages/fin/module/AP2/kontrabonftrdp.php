...
<div class="box">
    <div class="box header">
        ...
        <form id="form-data" action="kontrabonftrdp.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;display: none;">Tax (PPh)</th>            
            <th style="text-align: center;vertical-align: middle;display: none;">Total Amount</th>
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where tgl_kbon = '$date_now' group by no_kbon");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp group by no_kbon");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where tgl_kbon between '$start_date' and '$end_date' group by no_kbon");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where nama_supp = '$nama_supp' group by no_kbon");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where nama_supp = '$nama_supp' and tgl_kbon between '$start_date' and '$end_date' group by no_kbon");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where status = '$status' group by no_kbon");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where status = '$status' and tgl_kbon between '$start_date' and '$end_date' group by no_kbon");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp where status = '$status' and nama_supp = '$nama_supp' group by no_kbon");
    }
    else{
    $sql = mysqli_query($conn2,"select no_kbon, no_po, tgl_kbon, tgl_po, nama_supp, SUM(subtotal), SUM(dp_value) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_dp here status = '$status' and nama_supp = '$nama_supp' and tgl_kbon between '$start_date' and '$end_date' group by no_kbon");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status'];         
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="display: none;" value = "'.$row['SUM(total)'].'">'.number_format($row['SUM(total)'],2).'</td>
            ...
            if($status == 'Approved' and $group != 'STAFF' and $fin == '1'){
                ...
                <a href="pdf_kontrabondp.php?nokontrabon='.$row['no_kbon'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
               ...
               <a href="pdf_kontrabondp.php?nokontrabon='.$row['no_kbon'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
                ...
                echo ' <a href="pdf_kontrabondp.php?nokontrabon='.$row['no_kbon'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';                
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){                 
        ...
        $.ajax({
            type:'POST',
            url:'approvekbondp.php',
            data: {'no_kbon':no_kbon, 'approve_user':approve_user, 'tgl_kbon':tgl_kbon, 'supp':supp, 'curr':curr, 'no_po':no_po, 'tgl_po':tgl_po, 'total':total},
            // close: function(e){
            //     e.preventDefault();
            // },
...
<script type="text/javascript">     
    ...
    var no_faktur = $(this).closest('tr').find('td:eq(14)').attr('value');
    var supp_inv = $(this).closest('tr').find('td:eq(15)').attr('value');
    var tgl_inv = $(this).closest('tr').find('td:eq(16)').text();                
...
<script type="text/javascript">
    ...
    location.href = "formkontrabonftrdp.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "kontrabonftrdp.php";
};
</script>
