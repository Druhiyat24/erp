<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST MAINTAIN PAYMENT DP</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="pengajuanpaymentdp.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;width: 100px">Total DP</th>
...
$nama_supp ='';
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where tgl_pengajuan = '$date_now' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where nama_supp = '$nama_supp' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where status = '$status' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where status = '$status' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where status = '$status' and nama_supp = '$nama_supp' group by id");
    }
    else{
    $sql = mysqli_query($conn2,"select * from pengajuan_paymentdp where status = '$status' and nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id ");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    ...
    $status = $row['status']; 
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="display: none;" value = "'.$row['total_kbon'].'">'.number_format($row['total_kbon'],2).'</td>
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){
        ...
        $.ajax({
            type:'POST',
            url:'approvepengajuan_paymentdp.php',
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#delete", function(){ 
        ...
        $.ajax({
            type:'POST',
            url:'cancelpengajuan_paymentdp.php',
...
<script type="text/javascript">
    ...
    location.href = "formpengajuanpaymentdp.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "pengajuanpaymentdp.php";
};
</script>
