...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where tgl_ftr_dp = '$date_now'  group by no_ftr_dp");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp  group by no_ftr_dp");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where tgl_ftr_dp between '$start_date' and '$end_date'  group by no_ftr_dp");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where supp = '$nama_supp'  group by no_ftr_dp");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where supp = '$nama_supp' and tgl_ftr_dp between '$start_date' and '$end_date'  group by no_ftr_dp");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where status = '$status'  group by no_ftr_dp");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where status = '$status' and tgl_ftr_dp between '$start_date' and '$end_date'  group by no_ftr_dp");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where status = '$status' and supp = '$nama_supp' group by no_ftr_dp");
    }
    else{
        $sql = mysqli_query($conn2,"select no_ftr_dp, tgl_ftr_dp, no_po, supp, SUM(total) as total, SUM(dp_value) as dp, SUM(total - dp_value) as balance, curr, create_user, status, keterangan from ftr_dp where status = '$status' and supp = '$nama_supp' and tgl_ftr_dp between '$start_date' and '$end_date' group by no_ftr_dp");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status'];         
        echo '<tr style="font-size: 12px;text-align:center;">
            ...
            <td style="width: 100px;" value = "'.$row['tgl_ftr_dp'].'">'.date("d-M-Y",strtotime($row['tgl_ftr_dp'])).'</td>
            ...
            <td style="text-align:right;" value = "'.$row['dp'].'">'.number_format($row['dp'],2).'</td>            
            <td style="text-align:right;" value = "'.$row['balance'].'">'.number_format($row['balance'],2).'</td>
            ...
            if($status == 'Approved' and $group != 'STAFF' and $pur == '1' ){
                ...
                <a href="pdf_ftrdp.php?noftrdp='.$row['no_ftr_dp'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
               ...
               <a href="pdf_ftrdp.php?noftrdp='.$row['no_ftr_dp'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
                ...
                <a id="delete" href=""><button style="border-radius: 6px" type="button" class="btn-xs btn-danger"><i class="fa fa-trash" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Cancel</i></button></a>
                <a href="pdf_ftrdp.php?noftrdp='.$row['no_ftr_dp'].'" target="_blank"><i class="fa fa-print" style="padding-right: 10px; padding-left: 5px;" hidden></i></a>';                
            }elseif($status == 'draft' and $group == 'STAFF' and $pur == '1') {
                echo ' <a href="pdf_ftrdp.php?noftrdp='.$row['no_ftr_dp'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
...
<script type="text/javascript">
    ...
    location.href = "formftrdp.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "ftrdp.php";
};
</script>
