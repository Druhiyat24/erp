<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST MAINTAIN PAYMENT CBD</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="pengajuanpaymentcbd.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
$nama_supp ='';
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where tgl_pengajuan = '$date_now' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd group by id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where nama_supp = '$nama_supp' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where status = '$status' group by id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where status = '$status' and tgl_pengajuan between '$start_date' and '$end_date' group by id");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where status = '$status' and nama_supp = '$nama_supp' group by id");
    }
    else{
    $sql = mysqli_query($conn2,"select * from pengajuan_paymentcbd where status = '$status' and nama_supp = '$nama_supp' and tgl_pengajuan between '$start_date' and '$end_date' group by id ");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){
        ...
        $.ajax({
            type:'POST',
            url:'approvepengajuan_paymentcbd.php',
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#delete", function(){ 
        ...
        $.ajax({
            type:'POST',
            url:'cancelpengajuan_paymentcbd.php',
...
<script type="text/javascript">
    ...
    location.href = "formpengajuanpaymentcbd.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "pengajuanpaymentcbd.php";
};
</script>
