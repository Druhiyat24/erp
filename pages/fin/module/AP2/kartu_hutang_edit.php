...
<div style="margin-left: 30px">
        ...
        echo '<a target="_blank" href="pcsglobal.php?nama_supp='.$nama_supp.' && start_date='.$start_date.' && end_date='.$end_date.'"><button type="button" class="btn btn-success"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> EXCEL </i></button></a>';
        ?>
    </div> 
    <br/>
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th colspan="4" style="text-align: center;vertical-align: middle;width: 80px">Original Currency</th>
         ...
         <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;width: 80px">Deduction</th>
...
left join 
(select a.nama_supp as Supplier,sum(a.jml_potong) potongori, if(jml_potong > '0',if(b.curr = 'USD',sum(a.jml_potong),0),0) as plususd,if(jml_potong < '0',if(b.curr = 'USD',sum(a.jml_potong),0),0) as minusd, if(jml_potong > '0',if(b.curr = 'IDR',sum(a.jml_potong),sum(a.jml_potong * if((select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and tanggal = a.tgl_kbon) is null, (select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and id IN (SELECT MAX(id) FROM masterrate)),(select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and tanggal = a.tgl_kbon)))),0) as plusidr, if(jml_potong < '0',if(b.curr = 'IDR',sum(a.jml_potong),sum(a.jml_potong * if((select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and tanggal = a.tgl_kbon) is null, (select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and id IN (SELECT MAX(id) FROM masterrate)),(select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' and tanggal = a.tgl_kbon)))),0) as minidr  from potongan a INNER JOIN kontrabon_h b on b.no_kbon = a.no_kbon where jml_potong != '0' and a.nama_supp = '$nama_supp' GROUP BY a.nama_supp
 ) d
on ms.namasupplier = d.Supplier
