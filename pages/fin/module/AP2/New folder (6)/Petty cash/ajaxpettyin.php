...
if ($refdoc == 'Cash Out') {
    $sql = mysqli_query($conn1,"select if(a.no_coa = '-','-',CONCAT(b.no_coa,' ',b.nama_coa)) as coa, IF(a.reff_doc = '', '-', a.reff_doc) as reff_doc,IF(a.reff_date = '1970-01-01', '-', DATE_FORMAT(a.reff_date, '%d-%m-%Y')) as reff_date, if(a.deskripsi = '', '-',a.deskripsi) as deskripsi, a.debit, a.credit from c_petty_cashin_det a left join mastercoa_v2 b on b.no_coa = a.no_coa where no_pci = '$no_ib'"); 
    $sql3 = mysqli_query($conn1,"select amount from c_petty_cashin_h where no_pci = '$no_ib'");
    $row3 = mysqli_fetch_assoc($sql3);
    $amount = $row3['amount'];
}elseif ($refdoc == 'None') {
    $sql = mysqli_query($conn1,"select if(a.no_coa = '-', '-',CONCAT(b.no_coa,' ',b.nama_coa)) as coa,if(a.no_costcenter = '-','-',c.cc_name) as cost_center,if(a.buyer = '','-',a.buyer) as buyer ,if(a.no_ws = '','-',a.no_ws) as no_ws,a.curr,a.debit,a.credit from c_petty_cashin_none a left join mastercoa_v2 b on b.no_coa = a.no_coa left join b_master_cc c on c.no_cc = a.no_costcenter where a.no_pci = '$no_ib'");
    $sql3 = mysqli_query($conn1,"select amount from c_petty_cashin_h where no_pci = '$no_ib'");
    $row3 = mysqli_fetch_assoc($sql3);
    $amount = $row3['amount'];
