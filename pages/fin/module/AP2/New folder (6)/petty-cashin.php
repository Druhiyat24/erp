...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    ...
    if($reference == 'ALL' and empty($doc_num) and empty($start_date) and empty($end_date)){
     ...
     $where = "where a.reff_doc = '$reference' and a.tgl_pci between '$start_date' and '$end_date'";
    }elseif($reference == 'ALL' and !empty($doc_num)){
     $where = "where a.no_pci like '%$doc_num%' and a.tgl_pci between '$start_date' and '$end_date'";
    }else{
     $where = "where a.reff_doc = '$reference' and a.no_pci like '%$doc_num%' and a.tgl_pci between '$start_date' and '$end_date'";  
    }

    $sql = mysql_query("select a.no_pci,a.tgl_pci,a.reff,if(a.reff_doc = '','-',a.reff_doc) as reff_doc,if(a.oth_doc = '','-',a.oth_doc) as oth_doc,b.nama_coa,a.amount,if(a.deskripsi = '','-',a.deskripsi) as deskripsi from c_petty_cashin_h a left join mastercoa_v2 b on b.no_coa = a.coa_akun $where group by a.no_pci",$conn1);
