<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST PAYABLE CARD STATEMENT ON LIST PAYMENT</h2>
<div class="box">
    <div class="box header">

        
        <form id="form-data" action="pcs.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
            <label for="nama_supp"><b>Supplier</b></label>            
             ...
             <button id="btnbpb" type="button" class="btn-secondary btn-xs" style="border-radius: 15%"> BPB</button>
              <button id="btnkb" type="button" class="btn-secondary btn-xs" style="border-radius: 10%"> Kontra Bon</button>
               <button id="btnlp" type="button" class="btn-success btn-xs" style="border-radius: 10%"> List Payment</button>
                </div>
...
$nama_supp ='';
    ...
    // }
    // else{
    $sql = mysqli_query($conn2,"select kontrabon_h.no_kbon as no_kbon, kontrabon_h.tgl_kbon as tgl_kbon, kontrabon_h.nama_supp as nama_supp, kontrabon_h.total as total, kontrabon_h.balance as balance, SUM(list_payment.amount) as amount, kontrabon.curr as curr, kontrabon.create_user as create_user, kontrabon_h.status as status, kontrabon.tgl_tempo as tgl_tempo, kontrabon.no_faktur as no_faktur, kontrabon.supp_inv as supp_inv, kontrabon.tgl_inv as tgl_inv,kontrabon.pph_code as pph_code from kontrabon_h inner join kontrabon on kontrabon_h.no_kbon=kontrabon.no_kbon inner join list_payment on kontrabon_h.no_kbon = list_payment.no_kbon where kontrabon_h.status = 'Approved' and kontrabon_h.total != kontrabon_h.balance group by no_kbon");
    // }
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status']; 
