<?php include '../header.php' ?>
    ...
    <!-- MAIN -->    
    <div class="col p-4">
        <h2 class="text-center">APPROVE PETTY CASH IN</h2>
<div class="box">
    <div class="box header">
<form id="form-data" action="approve-petty-cashin.php" method="post">
        <div class="form-row">
            <!-- <div class="col-md-6">
            <label for="nama_supp"><b>Supplier</b></label>            
              <select class="form-control selectpicker" name="nama_supp" id="nama_supp" data-dropup-auto="false" data-live-search="true" onchange="this.form.submit()">
                ...
                <!-- <div style="padding-left: 10px;">
                    <button style="border-radius: 5px" id="btnpv" type="button" class="btn-primary btn-xs"><span></span> Payment Voucher</button>
                    <button style="border-radius: 5px" id="btnib" type="button" class="btn-secondary btn-xs"><span></span> Incoming Bank</button>
    ...
    <div class="box body">
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search no petty cashin..">
                </div>
            <table id="mytable" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr class="thead-dark">
                            ...
                            <th style="width:100px;">Document Number</th>
                            <th style="width:50px;">Date</th>
                            <th style="width:50px;">Refference</th>
                            <th style="width:90px;">Refference Document</th>                                                                              
                            <th style="width:90px;">Other Document</th>
            ...
            // }else {
            $sql = mysqli_query($conn2,"select a.no_pci,a.tgl_pci,a.reff,if(a.reff_doc = '','-',a.reff_doc) as reff_doc,if(a.oth_doc = '','-',a.oth_doc) as oth_doc,b.nama_coa,a.amount,if(a.deskripsi = '','-',a.deskripsi) as deskripsi from c_petty_cashin_h a left join mastercoa_v2 b on b.no_coa = a.coa_akun where a.status = 'draft' group by a.no_pci");
            // }
...
<script type="text/javascript">     
    ...
    var reff = $(this).closest('tr').find('td:eq(3)').attr('value');
    var reff_doc = $(this).closest('tr').find('td:eq(4)').attr('value');
    var oth_doc = $(this).closest('tr').find('td:eq(5)').attr('value');
    var curr = "IDR";
    var akun = $(this).closest('tr').find('td:eq(6)').attr('value');
    var desk = $(this).closest('tr').find('td:eq(8)').text();
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#approve", function(){
        $("input[type=checkbox]:checked").each(function () {                
        var no_pci = $(this).closest('tr').find('td:eq(1)').attr('value');
        var approve_user = '<?php echo $user ?>';

        $.ajax({
            type:'POST',
            url:'approvepetcashin.php',
            data: {'no_pci':no_pci, 'approve_user':approve_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
                console.log(response);
                window.location = 'approve-petty-cashin.php';
        ...
        if(document.querySelectorAll("input[name='select[]']:checked").length >= 1){
            ...
            alert("Silahkan Ceklist No Petty Cash In ");
        }        
    });
</script>
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#cancel", function(){
        ...
        $.ajax({
            ...
            data: {'no_pci':no_pci, 'cancel_user':cancel_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
