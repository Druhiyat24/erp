<?php include '../header.php' ?>
    ...
    <!-- MAIN -->    
    <div class="col p-4">
        <h2 class="text-center">APPROVE CASH OUT</h2>
<div class="box">
    <div class="box header">
<form id="form-data" action="approve-cashout.php" method="post">
        <div class="form-row">
            <!-- <div class="col-md-6">
    ...
    <div class="box body">
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search no cashout..">
                </div>
            ...
            // }else {
            $sql = mysqli_query($conn2,"select no_co, tgl_co,type_co, IF(dokumen = '','-',dokumen) as dokumen, sum(amount) as amount from c_cash_out where status = 'draft' group by no_co");
            // }
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#approve", function(){
        $("input[type=checkbox]:checked").each(function () {                
        var no_co = $(this).closest('tr').find('td:eq(1)').attr('value');
        var approve_user = '<?php echo $user ?>';

        $.ajax({
            type:'POST',
            url:'approvecashout.php',
            data: {'no_co':no_co, 'approve_user':approve_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
                console.log(response);
                window.location = 'approve-cashout.php';
        ...
        if(document.querySelectorAll("input[name='select[]']:checked").length >= 1){
            ...
            alert("Silahkan Ceklist No Cash Out ");
        }        
    });
</script>
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#cancel", function(){
        ...
        $.ajax({
            type:'POST',
            url:'cancelcashout.php',
            data: {'no_co':no_co, 'cancel_user':cancel_user},
            close: function(e){
                e.preventDefault();
            },
            success: function(response){                
