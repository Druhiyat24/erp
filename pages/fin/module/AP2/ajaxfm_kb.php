...
$sql = mysqli_query($conn2,"select bpb_new.no_bpb, bpb_new.pono, bpb_new.tgl_bpb, SUM(bpb_new.qty * bpb_new.price) as sub, SUM((bpb_new.qty * bpb_new.price) * (bpb_new.tax / 100)) as tax, SUM((bpb_new.qty * bpb_new.price) + ((bpb_new.qty * bpb_new.price) * (bpb_new.tax / 100))) - kontrabon.pph_value as total, kontrabon.pph_value as pph, bpb_new.supplier, potongan.jml_return, potongan.jml_potong from bpb_new inner join kontrabon on kontrabon.no_bpb = bpb_new.no_bpb inner join potongan on potongan.no_kbon = kontrabon.no_kbon where kontrabon.no_kbon = '$no_kbon' group by bpb_new.no_bpb");    
    ...
    $table = '<table id="mytdmodal" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
            ...
            $potong = $jml_return - $jml_potong;
            $total = $sub + $tax - $pph - $potong;
            $table .= '<tr>                       
