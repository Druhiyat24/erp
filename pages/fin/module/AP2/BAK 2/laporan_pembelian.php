<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">PURCHASE REPORT DETAIL</h2>
<div class="box">
    <div class="box header">
        <div style="padding-left: 10px;padding-top: 5px;">
            <button style="-ms-transform: skew(7deg);-webkit-transform: skew(7deg);transform: skew(10deg);" id="rpt_det" type="button" class="btn-primary btn-xs"><span></span> Report Detail</button>
            <button style="-ms-transform: skew(7deg);-webkit-transform: skew(7deg);transform: skew(10deg);" id="rpt_glo" type="button" class="btn-secondary btn-xs"><span></span>Report Global</button>
        </div>
        <form id="form-data" action="laporan_pembelian.php" method="post"> 
        ...
        <div class="form-row">
           <div class="col-md-2">
            ...
            <button type="submit" id="submit" name="submit"  value=" Search " style="margin-top: 25px; margin-bottom: 15px;margin-right: 15px;border: 0;
    line-height: 1;
    padding: -2px 8px;
    font-size: 12px;
    text-align: center;
    ...
    background-color: rgb(46, 139, 87);"><i class="fa fa-search" aria-hidden="true"></i> Search</button>
            <button type="submit" id="ekspor" name="ekspor" value=" ekspor " style="margin-top: 25px; margin-bottom: 15px;margin-right: 15px;border: 0;
    line-height: 1;
    padding: -2px 8px;
    font-size: 12px;
    text-align: center;
...
<!-- <table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%"> -->
    ...
    $end_date ='';
     ...
     echo "<script>
   window.open('ekspor_lap_pembelian.php?status=$nama_tipe&nama_supp=$nama_supp&start_date=$start_date&end_date=$end_date');
    var redirect = function() {
        window.location = 'laporan_pembelian.php';
    };
    ...
    if ($nama_tipe == 'Bahan Baku') {
        if ($nama_supp == 'ALL') {
            $where = "where bpbdate between '$start_date' and '$end_date' and d.tipe_sup = 'S' and left(bpbno,1) in ('A','F','B') and left(bpbno,2)!='FG' and tipe_com != 'FOC' || bpbdate between '$start_date' and '$end_date' and d.tipe_sup = 'S' and left(bpbno,1) in ('A','F','B') and left(bpbno,2)!='FG' and tipe_com is null order by bpbno_int";
        }else{
    ...
    $sql = mysqli_query($conn1,"select a.id, if(bpbno_int!='',bpbno_int,bpbno) bpbno,bpbdate,invno,jenis_dok,right(nomor_aju,6) no_aju,tanggal_aju, lpad(bcno,6,'0') bcno,bcdate,d.supplier,a.pono,z.tipe_com,a.id_item,concat(s.itemdesc,' ',add_info) itemdesc,s.color,s.size, a.qty,(a.qty-coalesce(a.qty_reject,0)) as qty_good,coalesce(a.qty_reject,0) as qty_reject, a.unit,tmpjo.kpno ws,tmpjo.styleno,a.curr,z.tax,a.price, ((a.qty-coalesce(a.qty_reject,0)) * a.price) dpp, (((a.qty-coalesce(a.qty_reject,0)) * a.price) + (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100)) total, (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100) ppn, a.jenis_trans, bn.upt_no_inv,bn.upt_tgl_inv,bn.upt_no_faktur, bn.upt_tgl_faktur from bpb a inner join masteritem s on a.id_item=s.id_item left join (select brh.bpbno as nomorbpb, id_jo, id_item, id_rak_loc, group_concat(distinct kode_rak , ' ', nama_rak) rak from bpb_roll_h brh inner join bpb_roll br on brh.id = br.id_h inner join master_rak mr on br.id_rak_loc = mr.id group by brh.bpbno, id_jo, id_item) lr on a.bpbno = lr.nomorbpb and a.id_item = lr.id_item and a.id_jo = lr.id_jo inner join mastersupplier d on a.id_supplier=d.id_supplier LEFT join (select pono,tipe_com,po_header.tax from po_header_draft inner join po_header on po_header_draft.id = po_header.id_draft where po_header.app = 'A') z on a.pono = z.pono left join (select id_jo,kpno,styleno from act_costing ac inner join so on ac.id=so.id_cost inner join jo_det jod on so.id=jod.id_so group by id_jo) tmpjo on tmpjo.id_jo=a.id_jo left join (select no_bpb,upt_dok_inv,upt_dok_faktur,upt_no_inv,upt_tgl_inv,IF(upt_no_faktur is null OR upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur, IF(upt_no_faktur is null OR upt_no_faktur = '',upt_tgl_faktur2,upt_tgl_faktur) upt_tgl_faktur FROM bpb_new where tgl_bpb between '$start_date' and '$end_date' GROUP BY no_bpb) bn on bn.no_bpb = a.bpbno_int $where");
        ...
        if ($nama_supp == 'ALL') {
            $where = "where bpbdate between '$start_date' and '$end_date' and d.tipe_sup = 'S' and left(bpbno,1) in ('C') and left(bpbno,2)!='FG' and tipe_com != 'FOC' || bpbdate between '$start_date' and '$end_date' and d.tipe_sup = 'S' and left(bpbno,1) in ('C') and left(bpbno,2)!='FG' and tipe_com is null order by bpbno_int";
        }else{
    ...
    $sql = mysqli_query($conn1,"select a.id, if(bpbno_int!='',bpbno_int,bpbno) bpbno,bpbdate,invno,jenis_dok,right(nomor_aju,6) no_aju,tanggal_aju, lpad(bcno,6,'0') bcno,bcdate,d.supplier,a.pono,z.tipe_com,a.id_item,s.itemdesc itemdesc,s.color,s.size, a.qty,(a.qty-coalesce(a.qty_reject,0)) as qty_good,coalesce(a.qty_reject,0) as qty_reject, a.unit,tmpjo.kpno ws,tmpjo.styleno,a.curr,z.tax,a.price, ((a.qty-coalesce(a.qty_reject,0)) * a.price) dpp, (((a.qty-coalesce(a.qty_reject,0)) * a.price) + (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100)) total, (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100) ppn, a.jenis_trans, bn.upt_no_inv,bn.upt_tgl_inv,bn.upt_no_faktur, bn.upt_tgl_faktur from bpb a inner join masteritem s on a.id_item=s.id_item left join (select brh.bpbno as nomorbpb, id_jo, id_item, id_rak_loc, group_concat(distinct kode_rak , ' ', nama_rak) rak from bpb_roll_h brh inner join bpb_roll br on brh.id = br.id_h inner join master_rak mr on br.id_rak_loc = mr.id group by brh.bpbno, id_jo, id_item) lr on a.bpbno = lr.nomorbpb and a.id_item = lr.id_item and a.id_jo = lr.id_jo inner join mastersupplier d on a.id_supplier=d.id_supplier LEFT join (select pono,tipe_com,po_header.tax from po_header_draft inner join po_header on po_header_draft.id = po_header.id_draft where po_header.app = 'A') z on a.pono = z.pono left join (select id_jo,kpno,styleno from act_costing ac inner join so on ac.id=so.id_cost inner join jo_det jod on so.id=jod.id_so group by id_jo) tmpjo on tmpjo.id_jo=a.id_jo left join (select no_bpb,upt_dok_inv,upt_dok_faktur,upt_no_inv,upt_tgl_inv,IF(upt_no_faktur is null OR upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur, IF(upt_no_faktur is null OR upt_no_faktur = '',upt_tgl_faktur2,upt_tgl_faktur) upt_tgl_faktur FROM bpb_new where tgl_bpb between '$start_date' and '$end_date' GROUP BY no_bpb) bn on bn.no_bpb = a.bpbno_int $where");
        ...
        if ($nama_supp == 'ALL') {
            $where = "where bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com != 'FOC' || bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com is null order by bpbno_int";
        }else{
            $where = " where bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com != 'FOC' and supplier = '$nama_supp' || bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com is null and supplier = '$nama_supp' order by bpbno_int";
        }
    ...
    $sql = mysqli_query($conn1,"select a.id, if(a.bpbno_int!='',a.bpbno_int,a.bpbno) bpbno,a.bpbdate,a.invno,a.jenis_dok,right(a.nomor_aju,6) no_aju,a.tanggal_aju, lpad(a.bcno,6,'0') bcno,a.bcdate,d.supplier,a.pono,z.tipe_com,a.id_item, CONCAT(s.itemdesc,' ',s.add_info) itemdesc,s.color,s.size, a.qty,(a.qty-coalesce(a.qty_reject,0)) as qty_good,coalesce(a.qty_reject,0) as qty_reject,a.unit, '' ws, '' styleno,a.curr,z.tax,a.price, ((a.qty-coalesce(a.qty_reject,0)) * a.price) dpp, (((a.qty-coalesce(a.qty_reject,0)) * a.price) + (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100)) total, (((a.qty-coalesce(a.qty_reject,0)) * a.price) * z.tax /100) ppn, a.jenis_trans, bn.upt_no_inv,bn.upt_tgl_inv,bn.upt_no_faktur, bn.upt_tgl_faktur from bpb a inner join masteritem s on a.id_item=s.id_item LEFT join (select pono,tipe_com,po_header.tax from po_header_draft inner join po_header on po_header_draft.id = po_header.id_draft) z on a.pono = z.pono LEFT join mastersupplier d on a.id_supplier=d.id_supplier LEFT OUTER JOIN mapping_category AS m ON s.n_code_category=m.n_id left join (select no_bpb,upt_dok_inv,upt_dok_faktur,upt_no_inv,upt_tgl_inv,IF(upt_no_faktur is null OR upt_no_faktur = '',upt_no_faktur2,upt_no_faktur) upt_no_faktur, IF(upt_no_faktur is null OR upt_no_faktur = '',upt_tgl_faktur2,upt_tgl_faktur) upt_tgl_faktur FROM bpb_new where tgl_bpb between '$start_date' and '$end_date' GROUP BY no_bpb) bn on bn.no_bpb = a.bpbno_int $where");
        ...
        if ($nama_supp == 'ALL') {
            ...
            $where3 = " where bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com != 'FOC' and supplier = '$nama_supp' || bpbno_int LIKE '%GEN%' and d.tipe_sup = 'S' AND a.bpbdate BETWEEN '$start_date' and '$end_date' and tipe_com is null and supplier = '$nama_supp' order by bpbno_int";
...
<!-- <script type="text/javascript">
    document.getElementById('ekspor').onclick = function () {
        ...
        $end_date = $_POST['end_date'];
        ?>
    location.href = "ekspor_lap_pembelian.php?status=<?= $nama_tipe;?>&nama_supp=<?= $nama_supp;?>&start_date=<?= $start_date;?>&end_date=<?= $end_date;?>";
};
</script> -->
