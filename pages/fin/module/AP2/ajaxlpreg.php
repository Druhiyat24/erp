...
$sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar, SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where payment_ftr_id = '$no_kbon' GROUP BY list_payment_id");	
