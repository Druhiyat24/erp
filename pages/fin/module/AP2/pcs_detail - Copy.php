...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier left join bpb_new d on d.no_bpb = a.bpbno_int where d.no_bpb is null and a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate >= '$start_date' group by a.bpbno_int order by bpbdate asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a  where a.is_invoiced = 'Waiting' and a.tgl_bpb >= '$start_date' group by a.no_bpb order by tgl_bpb asc
)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a inner join kontrabon e on e.no_bpb = a.no_bpb where a.is_invoiced = 'invoiced' and a.tgl_bpb >= '$start_date' and e.tgl_kbon between '$start_date' and '$end_date' group by a.no_bpb order by tgl_bpb asc
)

union (select nama_supp, no_bpb,tgl_bpb,curr,(subtotal + tax) from kontrabon where tgl_bpb < '$start_date' and tgl_kbon >= '$start_date')) as b order by b.Supplier asc");
    }
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier left join bpb_new d on d.no_bpb = a.bpbno_int where d.no_bpb is null and a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '$start_date' and '$end_date' group by a.bpbno_int order by bpbdate asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a  where a.is_invoiced = 'Waiting' and a.tgl_bpb between '$start_date' and '$end_date' group by a.no_bpb order by tgl_bpb asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a inner join kontrabon e on e.no_bpb = a.no_bpb where a.is_invoiced = 'invoiced' and a.tgl_bpb between '$start_date' and '$end_date' and e.tgl_kbon between '$start_date' and '$end_date' group by a.no_bpb order by tgl_bpb asc
)

union (select nama_supp, no_bpb,tgl_bpb,curr,(subtotal + tax) from kontrabon where tgl_bpb < '$start_date' and tgl_kbon >= '$start_date')) as b order by b.Supplier asc");
    }
    elseif ($nama_supp != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier left join bpb_new d on d.no_bpb = a.bpbno_int where d.no_bpb is null and a.confirm = 'y' and a.price != '0' and status_retur = 'N' and b.Supplier = '$nama_supp' group by a.bpbno_int order by bpbdate asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a  where a.is_invoiced = 'Waiting' and a.Supplier = '$nama_supp' group by a.no_bpb order by tgl_bpb asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a inner join kontrabon e on e.no_bpb = a.no_bpb where a.is_invoiced = 'invoiced' and a.Supplier = '$nama_supp' and e.tgl_kbon > '2022-between '$start_date' and '$end_date' group by a.no_bpb order by tgl_bpb asc
)

union (select nama_supp, no_bpb,tgl_bpb,curr,(subtotal + tax) from kontrabon where tgl_bpb < '2022-04-14' and tgl_kbon > '2022-04-13' and nama_supp = '$nama_supp')) as b order by b.Supplier asc");
    }
    else{
    $sql = mysqli_query($conn1,"select * from((select b.Supplier,a.bpbno_int,bpbdate,a.curr,round(sum((((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) + (((IF(a.qty_reject IS NULL,(a.qty), (a.qty - a.qty_reject))) * a.price) * (c.tax /100)))),2) as total from bpb a INNER JOIN po_header c on c.pono = a.pono INNER JOIN mastersupplier b on b.Id_Supplier = a.id_supplier left join bpb_new d on d.no_bpb = a.bpbno_int where d.no_bpb is null and a.confirm = 'y' and a.price != '0' and status_retur = 'N' and bpbdate between '$start_date' and '$end_date' and b.Supplier = '$nama_supp' group by a.bpbno_int order by bpbdate asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a  where a.is_invoiced = 'Waiting' and a.tgl_bpb between '$start_date' and '$end_date' and a.Supplier = '$nama_supp' group by a.no_bpb order by tgl_bpb asc)
union

(select a.supplier, a.no_bpb, a.tgl_bpb,a.curr, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total from bpb_new  a inner join kontrabon e on e.no_bpb = a.no_bpb where a.is_invoiced = 'invoiced' and a.tgl_bpb between '$start_date' and '$end_date' and a.Supplier = '$nama_supp' and e.tgl_kbon between '$start_date' and '$end_date' group by a.no_bpb order by tgl_bpb asc
)

union (select nama_supp, no_bpb,tgl_bpb,curr,(subtotal + tax) from kontrabon where tgl_bpb < '$start_date' and tgl_kbon >= '$start_date' and nama_supp = '$nama_supp')) as b order by b.Supplier asc");
}
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $sqlap = mysqli_query($conn1,"select no_bpb, is_invoiced from bpb_new where no_bpb = '$no_bpb' and is_invoiced = 'Invoiced' GROUP BY no_bpb");
    $rowap = mysqli_fetch_array($sqlap);
    $no_bpbap = isset($rowap['no_bpb']) ? $rowap['no_bpb'] : null;

    $sqlap12 = mysqli_query($conn1,"select no_bpb, total from saldo_awal_bpb where no_bpb = '$no_bpb' and tgl_bpb < '$start_date' GROUP BY no_bpb");
    $rowap12 = mysqli_fetch_array($sqlap12);
    $no_bpbap12 = isset($rowap12['no_bpb']) ? $rowap12['no_bpb'] : null;

    if($no_bpbap12 != null){
        $sa_awal = $rowap12['total'];
    }else{
        $sa_awal = 0;
    }
...
<?php
   ...
   if(empty($nama_supp) and empty($start_date) and empty($end_date)){
    ...
    elseif ($nama_supp == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn1,"select * from((select '' as abc,a.nama_supp, a.no_payment, a.tgl_payment,curr, sum(a.total_kbon + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and tgl_payment between '$start_date' and '$end_date' GROUP BY no_payment order by tgl_payment asc) union 
        (select '1' as abc,supplier as nama_supp, no_pay as no_payment, tgl_pay as tgl_payment,valuta as curr, sum(total + pph) as total, if(status_int = '5',sum(total + pph),0) as bayar from saldo_awal where status != 'Cancel' GROUP BY no_pay order by tgl_pay asc)) as b order by b.nama_supp asc");
    }
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn1,"select * from((select '' as abc,a.nama_supp, a.no_payment, a.tgl_payment,curr, sum(a.total_kbon + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and tgl_payment between '$start_date' and '$end_date' GROUP BY no_payment order by tgl_payment asc) union 
        (select '1' as abc,supplier as nama_supp, no_pay as no_payment, tgl_pay as tgl_payment,valuta as curr, sum(total + pph) as total, if(status_int = '5',sum(total + pph),0) as bayar from saldo_awal where status != 'Cancel' GROUP BY no_pay order by tgl_pay asc)) as b order by b.nama_supp asc");
    ...
    elseif ($nama_supp != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn1,"select * from((select '' as abc,a.nama_supp, a.no_payment, a.tgl_payment,curr, sum(a.total_kbon + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and a.nama_supp = '$nama_supp' and tgl_payment >= '$start_date' GROUP BY no_payment order by tgl_payment asc)
        union 
        (select '1' as abc,supplier as nama_supp, no_pay as no_payment, tgl_pay as tgl_payment,valuta as curr, sum(total + pph) as total, if(status_int = '5',sum(total + pph),0) as bayar from saldo_awal where status != 'Cancel' and supplier = '$nama_supp' GROUP BY no_pay order by tgl_pay asc)) as b order by b.nama_supp asc");
    }
    else{
    $sql = mysqli_query($conn1,"select * from((select '' as abc,a.nama_supp, a.no_payment, a.tgl_payment,curr, sum(a.total_kbon + a.pph_value) as total, if(a.status_int = '5',sum(a.total_kbon + a.pph_value),0) as bayar from list_payment a left join saldo_awal b on b.no_pay = a.no_payment where b.no_pay is null and a.status != 'Cancel' and a.nama_supp = '$nama_supp' and tgl_payment between '$start_date' and '$end_date' GROUP BY no_payment order by tgl_payment asc)
        union 
   ...
   while($row = mysqli_fetch_array($sql)){
    ...
    $sqllp = mysqli_query($conn1,"select list_payment_id,ttl_bayar from payment_ftr where list_payment_id = '$no_payment' GROUP BY list_payment_id");
