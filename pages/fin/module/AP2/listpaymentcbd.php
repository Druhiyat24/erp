<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST PAYMENT CBD</h2>
<div class="box">
    <div class="box header">

        <form id="form-data" action="listpaymentcbd.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where tgl_payment = '$date_now'  group by no_payment");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd  group by no_payment");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where tgl_payment between '$start_date' and '$end_date'  group by no_payment");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where nama_supp = '$nama_supp'  group by no_payment");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where nama_supp = '$nama_supp' and tgl_payment between '$start_date' and '$end_date'  group by no_payment");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where status = '$status'  group by no_payment");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where status = '$status' and tgl_payment between '$start_date' and '$end_date'  group by no_payment");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where status = '$status' and nama_supp = '$nama_supp' group by no_payment");
    }
    else{
    $sql = mysqli_query($conn2,"select no_payment, tgl_payment, nama_supp, SUM(total_kbon), SUM(outstanding), SUM(amount), curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where status = '$status' and nama_supp = '$nama_supp' and tgl_payment between '$start_date' and '$end_date' group by no_payment");
    }
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status'];         
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="display: none;" value = "'.$row['tgl_tempo'].'">'.date("d-M-Y",strtotime($row['tgl_tempo'])).'</td>
            ...
            if($status == 'Approved' and $group != 'STAFF' and $fin == '1'){
                ...
                <a href="pdf_list_payment_cbd.php?no_payment='.$row['no_payment'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
                ...
                echo ' <a href="pdf_list_payment_cbd.php?no_payment='.$row['no_payment'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';                
...
<script type="text/javascript">
    $("table tbody tr").on("click", "#approve", function(){                 
        ...
        $.ajax({
            type:'POST',
            url:'approvelistpaymentcbd.php',
            data: {'no_payment':no_payment, 'approve_user':approve_user},
            // close: function(e){
            //     e.preventDefault();
            // },
...
<script type="text/javascript">
    ...
    location.href = "formlistpaymentcbd.php";
};
</script>
<script type="text/javascript">
    document.getElementById('reset').onclick = function () {
    location.href = "listpaymentcbd.php";
};
</script>
