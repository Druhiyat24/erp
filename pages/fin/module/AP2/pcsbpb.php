<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST PAYABLE CARD STATEMENT BPB</h2>
<div class="box">
    <div class="box header">
        ...
        <div class="form-row">
            ...
            <label for="nama_supp"><b>Supplier</b></label>            
             ...
             <button id="btnbpb" type="button" class="btn-success btn-xs" style="border-radius: 15%"> BPB</button>
              <button id="btnkb" type="button" class="btn-secondary btn-xs" style="border-radius: 10%"> Kontra Bon</button>
...
$nama_supp ='';
    ...
    if($nama_supp == 'ALL' and  empty($start_date) and $end_date = $date_now){
     $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, end_date, SUM(qty * price) as sub, SUM((qty * price) * (tax / 100)) as tax, SUM((qty * price) + ((qty * price) * (tax / 100))) as total, top, confirm1, confirm2, supplier, tgl_po,itemdesc from bpb_new where  is_invoiced = 'Waiting' and confirm2 != '' group by no_bpb");
    }
    elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {            
     $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, end_date, SUM(qty * price) as sub, SUM((qty * price) * (tax / 100)) as tax, SUM((qty * price) + ((qty * price) * (tax / 100))) as total, top, confirm1, confirm2, supplier, tgl_po,itemdesc from bpb_new where  is_invoiced = 'Waiting' and confirm2 != '' and tgl_bpb between '$start_date' and '$end_date' group by no_bpb");
    }
    elseif ($nama_supp != 'ALL' and !empty($start_date) and !empty($end_date)) {            
     $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, end_date, SUM(qty * price) as sub, SUM((qty * price) * (tax / 100)) as tax, SUM((qty * price) + ((qty * price) * (tax / 100))) as total, top, confirm1, confirm2, supplier, tgl_po,itemdesc from bpb_new where  is_invoiced = 'Waiting' and confirm2 != '' and supplier = '$nama_supp' and tgl_bpb between '$start_date' and '$end_date' group by no_bpb");
    }
    // elseif ($nama_supp == 'ALL' and !empty($start_date) and !empty($end_date)) {
   ...
   while($row = mysqli_fetch_array($sql)){
            $bpb = $row['no_bpb'];
            $querys = mysqli_query($conn2,"select no_bpb, status from kontrabon where no_bpb = '$bpb' and status = 'GMF-PCH'");
        ...
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td value = "'.$row['pono'].'">'.$row['pono'].'</td>
             ...
             <td value = "'.$row['end_date'].'">'.date("d-M-Y",strtotime($row['end_date'])).'</td>
            ...
            <td style="text-align:right;" value = "'.$row['total'].'">'.number_format($row['total'],2).'</td>';
             echo '<td width="70px;">';
                echo '<a href="pdf_pcsbpb.php?nobpb='.$row['no_bpb'].'" target="_blank"><button type="button" class="btn btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"></i></button></a>';                                      
            echo '</td>
