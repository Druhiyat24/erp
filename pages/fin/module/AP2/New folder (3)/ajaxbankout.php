...
if ($refdoc == 'Payment Voucher') {
    ...
    $sql = mysqli_query($conn1,"select if(a.no_coa = '-', '-',CONCAT(b.no_coa,' ',b.nama_coa)) as coa,if(a.no_costcntr = '-','-',c.cc_name) as cost_center,if(a.buyer = '','-',a.buyer) as buyer ,if(a.no_ws = '','-',a.no_ws) as no_ws,a.curr,a.debit,a.credit from b_bankout_none a left join mastercoa_v2 b on b.no_coa = a.no_coa left join b_master_cc c on c.no_cc = a.no_costcntr where a.no_bankout = '$no_ob'");
    $sql3 = mysqli_query($conn1,"select amount,rate,eqv_idr from b_bankout_h where no_bankout = '$no_ob'");
    $row3 = mysqli_fetch_assoc($sql3);
    $amount = $row3['amount'];
    $rate = $row3['rate'];
