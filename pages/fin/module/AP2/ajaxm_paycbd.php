...
$sql2 = mysqli_query($conn2,"select * from pengajuan_paymentcbd where id = '$id' and no_payment = '$no_kbon' group by id ");   
$rows = mysqli_fetch_assoc($sql2);
$sql = mysqli_query($conn2,"select no_payment, tgl_payment, no_kbon, tgl_kbon, nama_supp, SUM(total_kbon) as total_kbon, SUM(outstanding) as outstanding, SUM(amount) as amount, curr, create_user, status, top, tgl_tempo, memo from list_payment_cbd where no_payment = '$no_kbon' group by no_kbon");
