...
<div class="box">
 ...
 <form id="form-data" action="pelunasanftr.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;width: 100px;display: none;">Action</th>                                
        </tr>
    </thead>
    ...
    if(empty($nama_supp) and empty($status) and empty($start_date) and empty($end_date)){
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar, SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where tgl_pelunasan = '$date_now'  group by payment_ftr_id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {            
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar, SUM(nominal)as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr  group by payment_ftr_id");
    }
    elseif ($nama_supp == 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where tgl_pelunasan between '$start_date' and '$end_date'  group by payment_ftr_id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where nama_supp = '$nama_supp'  group by payment_ftr_id");
    }
    elseif ($nama_supp != 'ALL' and $status == 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where nama_supp = '$nama_supp' and tgl_pelunasan between '$start_date' and '$end_date'  group by payment_ftr_id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where keterangan = '$status' group by payment_ftr_id");
    }
    elseif ($nama_supp == 'ALL' and $status != 'ALL' and !empty($start_date) and !empty($end_date)) {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where keterangan = '$status' and tgl_pelunasan between '$start_date' and '$end_date'  group by payment_ftr_id");
    }
    elseif ($nama_supp != 'ALL' and $status != 'ALL' and $start_date == '1970-01-01' and $end_date == '1970-01-01') {
     $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar, SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where keterangan = '$status' and nama_supp = '$nama_supp' group by payment_ftr_id");
    }
    else{
    $sql = mysqli_query($conn2,"select payment_ftr_id, tgl_pelunasan, nama_supp, list_payment_id, tgl_list_payment, no_kbon, tgl_kbon, valuta_ftr, sum(ttl_bayar) as ttl_bayar, cara_bayar, account, bank, valuta_bayar,  SUM(nominal) as nominal, SUM(nominal_fgn) as nominal_fgn, rate, (ttl_bayar - SUM(nominal)) as sisa,status,keterangan from payment_ftr where keterangan = '$status' and nama_supp = '$nama_supp' and tgl_pelunasan between '$start_date' and '$end_date' group by payment_ftr_id");
}
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status']; 
    ...
    if ($val == 'IDR' && $rat != '0') {
        ...
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            echo '<td width="100px;">';
            echo '<a href="pdf_pelunasanftr.php?nopay='.$row['payment_ftr_id'].'" target="_blank"><button style="border-radius: 6px" type="button" class="btn-xs btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"> Pdf</i></button></a>';
...
<script type="text/javascript">
    ...
    location.href = "pelunasanftr.php";
};
</script>
