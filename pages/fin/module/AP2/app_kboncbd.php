...
<form id="form-data" action="app_kbonreg.php" method="post">
    ...
    <div class="box body">
        <div class="row">
            ...
            $sql = mysqli_query($conn2,"select no_kbon, tgl_kbon, nama_supp, SUM(subtotal), SUM(tax) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_cbd where status = 'draft' group by no_kbon");                
            }else {
            $sql = mysqli_query($conn2,"select no_kbon, tgl_kbon, nama_supp, SUM(subtotal), SUM(tax) as tax, SUM(total), curr, create_user, status, tgl_tempo, no_faktur, supp_inv, tgl_inv, pph_code, SUM(pph_value) as pph_value, pph_code from kontrabon_cbd where status = 'draft' and nama_supp = '$nama_supp' group by no_kbon");
            }
