            ...
            while($row = mysqli_fetch_array($sql)){
                    ...
                    echo '<tr>
                         ...
                         while($row = mysqli_fetch_array($sql)){
                            ...
                            $querysaa = mysqli_query($conn2,"select count(no_kbon) as total_no from list_payment where no_kbon = '$no_kbon'");
                            $rowsaa = mysqli_fetch_array($querysaa);
                            $total_no = isset($rowsaa['total_no']) ? $rowsaa['total_no'] : 0;
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            data: {'no_bankout':no_bankout, 'no_coa':no_coa, 'reff_doc':reff_doc, 'reff_date':reff_date, 'deskripsi':deskripsi, 'debit':debit, 'credit':credit, 'no_pay':no_pay, 'pay_date':pay_date, 'due_date':due_date, 'dpp':dpp, 'ppn':ppn, 'pph':pph, 'total':total, 'total_idr':total_idr, 'curr':curr, 'kodelp':kodelp, 'rates':rates, 'amount_input':amount_input, 'curr_h':curr_h, 'rate_h':rate_h},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
