...
<!--            <div class="col-md-3 mb-3">            
    ...
    <div class="box body">
        <div class="row">
            ...
            <?php
            $no_kontrabon = $_GET['nokontrabon'];
            $query = mysqli_query($conn2,"select nama_supp, start_date, end_date from kontrabon where no_kbon = '$no_kontrabon'");
            $rows = mysqli_fetch_array($query);           
            $supp = $rows['nama_supp'];
            $tgl_awal = $rows['start_date'];
            $tgl_akhir = $rows['end_date'];                                                                      

            $sql = mysqli_query($conn2,"select no_bpb, pono, tgl_bpb, SUM(qty * price) as sub, SUM((qty * price) * (tax / 100)) as tax, SUM((qty * price) + ((qty * price) * (tax / 100))) as total, top, confirm1, confirm2, supplier, tgl_po, is_invoiced from bpb where supplier = '$supp' and tgl_bpb between '$start_date' and '$end_date' and confirm2 != '' group by no_bpb");                                                     
            while($row = mysqli_fetch_array($sql)){
            $bpb = $row['no_bpb'];            
            $invoice = $row['is_invoiced'];
            
            $querys = mysqli_query($conn2,"select distinct no_bpb, no_kbon, pph_code, status, idtax from kontrabon where no_bpb = '$bpb' and status !='Cancel'");
            ...
            $q = mysqli_query($conn2,"select distinct kontra.no_bpb, mtax.idtax, mtax.kriteria, mtax.percentage from mtax left join (select no_kbon, no_bpb, idtax from kontrabon) kontra on kontra.idtax = mtax.idtax where category_tax = 'PPH'");
            ...
            $select .= '<option data-idtax="'.$rs['idtax'].'" value="'.$rs['percentage'].'" "'.$isSelected.'">'.$rs['kriteria'].'</option>';
                    ...
                    if($bpb == $n_bpb and $n_kbon == $no_kontrabon and $status == 'draft'){
                        echo'<tr>
                        <td style="width:10px;"><input type="checkbox" id="select" name="select[]" value="" checked=checked;?></td>
                    ...
                    elseif($invoice == 'Waiting'){
                        echo'<tr>
                        <td style="width:10px;"><input type="checkbox" id="select" name="select[]" value=""?></td>
                        <td style="width:50px;" value="'.$row['no_bpb'].'">'.$row['no_bpb'].'</td>
                            <td style="width:50px;" value="'.$row['pono'].'">'.$row['pono'].'</td>                            
                            <td style="width:100px;" value="'.$row['tgl_bpb'].'">'.date("d-M-Y",strtotime($row['tgl_bpb'])).'</td>                            
                            <td style="text-align: right;" class="dt_price" style="width:100px;" data-link="1" data-subtotal="'.$sub.'">'.number_format($sub,2).'</td>
