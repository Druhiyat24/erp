...
<!--            <div class="col-md-3 mb-3">            
    ...
    <div class="box body">
        <div class="row">
            ...
            if ($ref_num == 'Payment') {
                $sql = mysqli_query($conn2,"select curr,nama_supp,no_payment,tgl_payment,tgl_tempo,curr,sum(total_kbon) as total_kbon,sum(pph_value) as pph_value from list_payment where status_int = '4' and nama_supp = '$nama_supp' and tgl_payment BETWEEN '$start_date' and '$end_date' group by no_payment");
            }elseif ($ref_num == 'Payment Voucher') {
                $sql = mysqli_query($conn2,"select a.nama_supp,a.no_pv,a.pv_date,max(b.due_date) as due_date,a.curr,a.subtotal, a.ppn, a.pph ,a.total,a.status from tbl_pv_h a inner join tbl_pv b on b.no_pv = a.no_pv where a.nama_supp = '$nama_supp' and a.pv_date BETWEEN '$start_date' and '$end_date' and a.status = 'Approved' and a.outstanding != '0' group by a.no_pv");
            }else{
                '';
            }
...
<div class="box footer">   
    ...
    <tbody id="tbody3">
        <tr style="display:none">
            ...
            <td>
                <input type="text" style="font-size: 15px;" name="tgl_active" id="tgl_active" class='form-control tanggal' 
            value='<?= date("d-m-Y"); ?>' autocomplete='off' placeholder="dd-mm-yyyy">
            </td>
            <td>
                <input type="text" class="form-control" name="keterangan[]" placeholder="" autocomplete='off'> 
            </td>
    ...
    <tfoot>
          <tr>
            ...
            <button type="button" class="btn btn-danger" onclick="deleteRow('tbody3')">Delete Row</button>
            </td>
          </tr>
    </tfoot>                   
            </table> 
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            data: {'no_bankout':no_bankout, 'no_coa':no_coa, 'reff_doc':reff_doc, 'reff_date':reff_date, 'deskripsi':deskripsi, 'debit':debit, 'credit':credit, 'no_pay':no_pay, 'pay_date':pay_date, 'due_date':due_date, 'dpp':dpp, 'ppn':ppn, 'pph':pph, 'total':total, 'total_idr':total_idr, 'curr':curr},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
