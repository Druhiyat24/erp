<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM CASH OUT</h2>
<div class="box">
...
<form id="form-data" method="post">
        <div class="form-row">
            ...
            <label for="pajak" class="col-form-label" style="width: 150px;"><b>Document Number</b></label>
                <?php
            $sql = mysqli_query($conn2,"select max(no_co) from c_cash_out");
            $row = mysqli_fetch_array($sql);
            $kodepay = $row['max(no_co)'];
            $urutan = (int) substr($kodepay, 13, 5);
            $urutan++;
            $bln = date("m");
            $thn = date("y");
            $huruf = "RCO/NAG/$bln$thn/";
...
<!--             <input type="hidden" style="font-size: 15px;" name="unik_code" id="unik_code" class="form-control" 
            ...
            <label for="nama_supp"><b>Type</b></label>            
              <select class="form-control selectpicker" name="nama_type" id="nama_type" data-dropup-auto="false" data-live-search="true">
                ...
                $sql = mysqli_query($conn1,"select ref_doc from master_forpay where ket = '3' order by id ASC");
 ...
 <div class="form-row">
    ...
    <div class="box body">
        ...
        <tr><th class="text-center" style="width: 2%">-</th>
            ...
            <th class="text-center" style="width: 10%">Request By</th>
...
<div class="box footer">   
        ...
        <!--     <div class="input-group" >
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='cash-out.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
            </div>
            </div>                                    
        </form>
        </div>
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            type:'POST',
            url:'log_create_cashout.php',
            data: {'doc_num':doc_num, 'doc_date':doc_date, 'from_ip':from_ip, 'create_user':create_user, 'pesan':pesan, 'total':total, 'type_co':type_co},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
       ...
       if($('select[name=nama_type] option').filter(':selected').val() == '' || $('select[name=nama_type] option').filter(':selected').val() == '-'){
        alert("Please Select Type Cash Out");
