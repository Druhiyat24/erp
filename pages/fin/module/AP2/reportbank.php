       ...
       <form id="form-data" action="reportbank.php" method="post">        
        <div class="form-row">
            ...
            <label for="accountid" class="col-form-label" ><b>Account </b></label>  
                ...
                <?php 
                        $sqlacc = mysqli_query($conn1,"select upper(bank_name) as bank,curr,bank_account as account,RIGHT(bank_account,4) as kode from b_masterbank where status = 'Active'");
...
<?php
   ...
   if(empty($accountid) and empty($start_date) and empty($end_date)){
    echo '';
     ...
     $sql = mysqli_query($conn1,"select transaksi_date as date, no_doc as doc_num,deskripsi,debit,credit,curr, balance from b_reportbank where akun = '$accountid' and transaksi_date between '$start_date' and '$end_date' ");
    }
