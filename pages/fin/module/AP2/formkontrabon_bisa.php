    ...
    <div class="box body">
        <div class="row">
            ...
            $pono = $row['pono'];
             ...
             $Aquer = mysqli_query($conn2,"select no_po, sum(amount) as amount from list_payment_dp where no_po = '$pono' and status = 'Approved' and status_int = '5'");
            ...
            $Aquerr = mysqli_query($conn2,"select no_po, sum(amount_update) as amount_update from list_payment_cbd where no_po = '$pono' and status = 'Approved' and status_int = '5'");
...
<div class="col-md-12">   
        ...
        <div class="col-md-12">   
            ...
            $querys = mysqli_query($conn2,"select no_bppb, bppbdate, no_ro, no_bpb, total from ttl_bppb where supp = '$nama_supp' GROUP BY no_ro");
            ...
            while($row1 = mysqli_fetch_array($querys)){
                ...
                $Aquer123 = mysqli_query($conn2,"select DISTINCT no_ro, sum(DISTINCT total_ro) as amount from return_kb where no_ro = '$ro_no' and status != 'Cancel'");
                    ...
                    echo '<tr>
                            ...
                            <td style="width:100px;" valuess="'.$row1['bppbdate'].'">'.date("d-M-Y",strtotime($row1['bppbdate'])).'</td>                            
                            <td style="width:50px;" valuess="'.$row1['no_bpb'].'">'.$row1['no_bpb'].'</td>                            
                            <td style="width:100px;text-align:right;" data-total-ro="'.$sisaro.'">'.number_format($sisaro,2).'</td>
                            <td style="width:100px;">
                            <input style="text-align: right;" type="number" min="0" style="font-size: 12px;" class="form-control" id="txt_amount" name="txt_amount" value="" >
                            </td>                                                                                                 
                        </tr>';
                    }else{
                        echo '';
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            'curr':curr, 'ceklist':ceklist, 'cash':cash, 'create_user':create_user, 'sum_sub':sum_sub, 'sum_tax':sum_tax, 'sum_dp':sum_dp, 'sum_pph':sum_pph, 'sum_total':sum_total, 'start_date':start_date, 'end_date':end_date, 'pph':pph, 'idtax':idtax, 'tgl_po':tgl_po, 'ttl_ro':ttl_ro},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
