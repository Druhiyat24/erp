...
<form id="form-data" method="post">
        <div class="form-row">
            ...
            <input type="text" style="font-size: 14px;" name="tanggal" id="tanggal" class="form-control tanggal" onchange="this.form.submit()" 
            value="<?php             
            $start_date ='';
            $end_date ='';
       ...
       <!--      <input type="text" style="font-size: 14px;" name="tgl_perhitungan" id="tgl_perhitungan" class="form-control"> -->
    ...
    <div class="form-row">
            ...
            <input type="text" style="font-size: 14px;" class="form-control tanggal_fil" name="txt_tglsi" id="txt_tglsi" 
            value="<?php 
            ...
            if(!empty($nama_supp)) {
                echo date("d-m-Y",strtotime($startdate . "+$top days"));
            }
            else{
                echo date("d-m-Y");
            } 
    ...
    <div class="box body">
        <div class="row">
           ...
           <div class="tableFix" style="height: 300px;">
            ...
            $sql = mysqli_query($conn2,"select a.no_bpb,a.curr, a.pono, a.tgl_bpb, a.tgl_po, SUM(a.qty * a.price) as sub, if(a.qty is null,SUM((a.qty * a.price) * (a.tax / 100)) ,SUM(((a.qty) * a.price) * (a.tax / 100))) as tax, if(a.qty is null,SUM((a.qty * a.price) + ((a.qty * a.price) * (a.tax / 100))) ,SUM((a.qty * a.price) + (((a.qty) * a.price) * (a.tax / 100)))) as total, a.top, a.confirm1, a.confirm2, a.supplier, a.tgl_po from bpb_new  a  where a.supplier = '$nama_supp' and a.tgl_bpb between '$start_date' and '$end_date' and a.is_invoiced != 'Invoiced' and a.confirm2 != '' and status != 'Cancel' group by a.no_bpb");
            ...
            $pono = isset($row['pono']) ? $row['pono'] : null;
             ...
             $Aquer = mysqli_query($conn2,"select no_po, sum(amount) as amount from list_payment_dp where no_po = '$pono' and status_int = '5'");
            ...
            $Aquerr = mysqli_query($conn2,"select no_po, sum(amount_update) as amount_update from list_payment_cbd where no_po = '$pono' and status_int = '5'");
            $c = mysqli_fetch_array($Aquerr);
            $po_nno = isset($c['no_po']) ? $c['no_po'] : null ;
            $cbd = isset($c['amount_update']) ? $c['amount_update'] : 0 ;

            $Aquerraa = mysqli_query($conn2,"select DISTINCT list_payment_cbd.no_po as no_po, sum(DISTINCT kontrabon_h.dp_value) as amount from list_payment_cbd inner join kontrabon_h on kontrabon_h.no_po = list_payment_cbd.no_po where kontrabon_h.no_po = '$pono' and list_payment_cbd.status_int = '5' and kontrabon_h.`status` != 'Cancel'");
            ...
            if($bpb == $n_bpb and $stat != 'Cancel'){
                echo '';
                    ...
                    echo '<tr>
                            ...
                            <td style="width:100px;" dates="'.date("d-m-Y",strtotime($row['tgl_bpb'])).'" value="'.$row['tgl_bpb'].'">'.date("d-M-Y",strtotime($row['tgl_bpb'])).'</td>                            
...
<div class="col-md-12">   
        ...
        <div class="col-md-12">   
            ...
            $querys = mysqli_query($conn2,"select no_bppb, tgl_bppb, no_ro, no_bpb, sum(qty * price) as total from bppb_new where supplier = '$nama_supp' GROUP BY no_bppb");
...
<script type="text/javascript">
    $(document).ready(function () {
        var tgl = document.getElementById('tanggal2').value;
    $('.tanggal1').datepicker({
        format: "dd-mm-yyyy",
        autoclose:true,
...
function formatMoney(amount, decimalCount = 2, decimal = ".", thousands = ",") {
  try {
    ...
    var curr = $(this).closest('tr').find('td:eq(14)').attr('value'); 
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            'curr':curr, 'ceklist':ceklist, 'cash':cash, 'create_user':create_user, 'sum_sub':sum_sub, 'sum_tax':sum_tax, 'sum_dp':sum_dp, 'sum_pph':sum_pph, 'sum_total':sum_total, 'start_date':start_date, 'end_date':end_date, 'pph':pph, 'idtax':idtax, 'tgl_po':tgl_po, 'ttl_ro':ttl_ro, 'no_bppb':no_bppb, 'unik_code':unik_code},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
