...
$sql2 = mysqli_query($conn2,"select pesan from pengajuan_ftrdp where id = '$id' and no_cbd = '$no_kbon' group by id ");
$rows = mysqli_fetch_assoc($sql2);
$sql = mysqli_query($conn2,"select no_po, tgl_po, no_pi, supp, SUM(total) as total_po, SUM(dp) as dp_code, SUM(dp_value) as dp_value, SUM(balance) as balance, status from ftr_dp where no_ftr_dp = '$no_kbon' and status = 'Waiting' group by no_po");
