...
<!--            <div class="col-md-3 mb-3">            
    ...
    <div class="box body">
        <div class="row">
            ...
            if ($ref_num == 'Payment') {
                $sql = mysqli_query($conn2,"select no_kbon,curr,nama_supp,no_payment,tgl_payment,tgl_tempo,curr,(sum(amount) - (select COALESCE(sum(for_balance),0) from b_bankout_det where no_reff = no_payment)) as total_kbon,sum(pph_value) as pph_value, 'REG' as kode from list_payment where status_int = '4' and nama_supp = '$nama_supp' and tgl_payment BETWEEN '$start_date' and '$end_date'  group by no_payment
union
select no_kbon,valuta,supplier,no_pay,tgl_pay,due_date,valuta,total,pph,'SAL' as kode from saldo_awal where status_int = '4' and no_pay not like '%LP/NAG%' and supplier = '$nama_supp' and tgl_pay BETWEEN '$start_date' and '$end_date' group by no_pay
union
            ...
            while($row = mysqli_fetch_array($sql)){
                    ...
                    echo '<tr>
                         ...
                         while($row = mysqli_fetch_array($sql)){
                            ...
                            $sqlz = mysqli_query($conn1,"select COALESCE(sum(for_balance),0) as balance from b_bankout_det where no_reff = '$no_pay'");
                            $rowz = mysqli_fetch_array($sqlz);
                            ...
                            if ($kode != 'REG') {
                            $querysaa = mysqli_query($conn2,"select if(count(no_kbon) = 0,1,count(no_kbon)) as total_no from list_payment where no_kbon = '$no_kbon'");
                            ...
                            $querys = mysqli_query($conn2,"select sum(b.amount) as sub,sum(a.tax) as ppn from kontrabon_h a inner join list_payment b on b.no_kbon = a.no_kbon where b.no_payment = '$no_pay' group by b.no_payment");
