<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">FORM REALIZATION</h2>
<div class="box">
    <div class="box header">
<form id="form-data" method="post">
        <div class="form-row">
            <div class="col-md-3 mb-3">            
            <label for="pajak" class="col-form-label" style="width: 150px;font-size: 14px;"><b>Document Number</b></label>
                <?php
            $sql = mysqli_query($conn2,"select max(no_rls) from c_realisasi_h");
            $row = mysqli_fetch_array($sql);
            $kodepay = $row['max(no_rls)'];
            $urutan = (int) substr($kodepay, 13, 5);
            $urutan++;
            $bln = date("m");
            $thn = date("y");
            $huruf = "RLS/NAG/$bln$thn/";
            ...
            <label for="total" class="col-form-label" style="width: 150px;"><b>Date</b></label>
                <input type="text" style="font-size: 14px;" name="tgl_doc" id="tgl_doc" class="form-control tanggal" 
            value="<?php 
...
<!--             <input type="hidden" style="font-size: 15px;" name="unik_code" id="unik_code" class="form-control" 
            ...
            <label for="nama_supp"><b>No Cash Advance</b></label>            
              <input type="text" style="font-size: 14px;" class="form-control" id="no_cashadv" name="no_cashadv" value="<?php             

            $sql = mysqli_query($conn2," select no_ca from c_realisasi_temp ");
            $row = mysqli_fetch_array($sql);
            $no_ca = isset($row['no_ca']) ? $row['no_ca'] : null;         
            ...
            <label for="total" class="col-form-label" style="width: 150px;"><b>Value</b></label>
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="amount_cashadv" name="amount_cashadv" value="<?php             
            $sql = mysqli_query($conn2," select round(total,0) as total from c_realisasi_temp ");
            $row = mysqli_fetch_array($sql);
            $total = isset($row['total']) ? $row['total'] : null;         
                ...
                <input type="hidden" style="font-size: 14px;text-align: right;" class="form-control" id="amount_cashadv_h" name="amount_cashadv_h" value="<?php             
...
<div class="form-row">
 ...
 <div class="form-row">
    <div class="modal fade" id="mymodal2" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
        ...
        <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="fa fa-times"></span></button>
        <h4 class="modal-title" id="Heading">Choose Cash Advance</h4>
        </div>
          <div class="modal-body">
          <div class="form-group">
            <form id="modal-form2" method="post">
                <div class="form-row">
                    <table id="doc_support" class="table-bordered" cellspacing="0" width="100%" style="font-size: 14px;text-align:center;">
                    <thead>
                        <tr>                       
                            <th style="width:5%;">Cek</th>
                            <th style="width:23%;">no Cash Advance</th>
                            <th style="width:15%;">Date</th>
                            <th style="width:35%;">Cost Center</th>                                                                                
    ...
    <tbody>
        <?php
            ...
            $querys = mysqli_query($conn2,"select a.no_ca, a.tgl_ca,b.cc_name,a.amount from c_cash_advances a inner join b_master_cc b on b.no_cc = a.no_costcenter group by a.no_ca ");


            while($row1 = mysqli_fetch_array($querys)){
                $no_ca = $row1['no_ca'];

                 $sql22 = mysqli_query($conn2,"select no_ca from c_realisasi_temp");
                $row22 = mysqli_fetch_array($sql22);
                $ket = isset($row22['no_ca']) ? $row22['no_ca'] : null;
                echo '<tr>'; 
                            if ($ket == $no_ca) {
                         echo'<td style="width:10px;"><input type="radio" id="select" name="select[]" value="" <?php if(in_array("1",$_POST[select])) echo "checked=checked";? checked></td>'; 
                     } else{
                    echo'<td style="width:10px;"><input type="radio" id="select" name="select[]" value="" <?php if(in_array("1",$_POST[select])) echo "checked=checked";?></td>'; 
                    }                        
                            echo '                     
                            <td style="" value="'.$row1['no_ca'].'">'.$row1['no_ca'].'</td>
                            <td style="" value="'.$row1['tgl_ca'].'">'.$row1['tgl_ca'].'</td>
                            <td style="" value="'.$row1['cc_name'].'">'.$row1['cc_name'].'</td>
                            <td value="'.$row1['amount'].'">'.number_format($row1['amount'],2).'</td>
                            <td style="display:none" value="'.$row1['amount'].'">'.$row1['amount'].'</td>
                       </tr>';
                   }
    ...
    <div class="box body">
        ...
        <tr><th class="text-center" style="width: 2%">-</th>
            ...
            <th class="text-center" style="width: 10%">Realization</th>
            <th class="text-center" style="width: 12%">Description</th>
            <th class="text-center" style="width: 10%">BPB</th>
...
<div class="box footer">   
        ...
        <!--     <div class="input-group" >
            ...
            <div class="col-md-4">
                ...
                <div class="input-group" >
                <label for="nama_supp" class="col-form-label" style="width: 180px;"><b>Subtotal</b></label>
            ...
            <div class="input-group" >
                <label for="nama_supp" class="col-form-label" style="width: 180px;"><b>Cash</b></label>
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="cash_value_h" name="cash_value_h">
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="cash_value" name="cash_value" placeholder="0.00" readonly>
            </div>
           ...
           <div class="form-row col">
            ...
            <button type="button" style="border-radius: 6px" class="btn-outline-danger btn-sm" name="batal" id="batal" onclick="location.href='realization.php'"><span class="fa fa-angle-double-left"></span> Back</button>           
            </div>
            </div>                                    
        </form>
        </div>
 ...
 function InsertRow(tableID)
{
 ...
 function hitungRow(){
    ...
    var price = parseFloat(document.getElementById("tbody2").rows[i].cells[3].children[0].value,10) || 0;
...
<script type="text/javascript">
    $("input[name=cash_value_h]").keyup(function(){
    var sum_kb = 0;
    var sum_amount = 0;
    var sum_total = 0;
    var sum_balance = 0;               
    var amount = parseFloat(document.getElementById('cash_value_h').value,10) || 0;

    sum_amount = amount;

    $("#cash_value").val(formatMoney(sum_amount));    
...
<script type="text/javascript">
    $("#modal-form2").on("click", "#send2", function(){
        ...
        var no_ca = $(this).closest('tr').find('td:eq(1)').attr('value');
        var tgl_ca = $(this).closest('tr').find('td:eq(2)').attr('value');
        var cc_name = $(this).closest('tr').find('td:eq(3)').attr('value');
        ...
        $.ajax({
            type:'POST',
            url:'insert_catemp.php',
            data: {'doc_num':doc_num, 'no_ca':no_ca, 'tgl_ca':tgl_ca, 'cc_name':cc_name, 'total':total, 'create_user':create_user},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        var no_ca = document.getElementById('no_cashadv').value;  
        var total_ca = parseFloat(document.getElementById('amount_cashadv_h').value,10) || 0;      
        var from_ip = document.getElementById('ambil_ip').value;
        var subtotal = parseFloat(document.getElementById('total_value_h').value,10) || 0;
        var total_cash = parseFloat(document.getElementById('cash_value_h').value,10) || 0;
        var create_user = '<?php echo $user; ?>';
        var hitung = 0;
        hitung = total_ca - (subtotal + total_cash);


        if (total_ca >= 1 && hitung == 0 && no_ca != '') {
        $.ajax({
            type:'POST',
            url:'insert_realisasi_h.php',
            data: {'doc_num':doc_num, 'doc_date':doc_date, 'no_ca':no_ca, 'total_ca':total_ca, 'from_ip':from_ip, 'subtotal':subtotal, 'total_cash':total_cash, 'create_user':create_user},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
            success: function(response){
                console.log(response);
                //  // alert(response);
                window.location = 'realization.php';
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
                alert(xhr);
        ...
        var no_bpb = $(this).closest('tr').find('td:eq(5) input').val();                               
        var nama_supp = $(this).closest('tr').find('td:eq(6) input').val();
        var buyer = $(this).closest('tr').find('td:eq(7) input').val();
        var no_ws = $(this).closest('tr').find('td:eq(7) input').val();
        var no_ca = document.getElementById('no_cashadv').value; 
        var total_ca = parseFloat(document.getElementById('amount_cashadv_h').value,10) || 0;  
        var subtotal = parseFloat(document.getElementById('total_value_h').value,10) || 0;
        var total_cash = parseFloat(document.getElementById('cash_value_h').value,10) || 0;
        ...
        $.ajax({
            type:'POST',
            url:'insert_realisasi.php',
            data: {'doc_num':doc_num, 'doc_date':doc_date, 'no_coa':no_coa, 'no_costcenter':no_costcenter, 'amount':amount, 'deskripsi':deskripsi, 'no_bpb':no_bpb, 'nama_supp':nama_supp, 'buyer':buyer,'no_ws':no_ws},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
        ...
        if(document.getElementById('no_cashadv').value == ''){
        alert("Please Add No Cash Advance");
        }else if(document.getElementById('amount_cashadv_h').value - (document.getElementById('total_value_h').value + document.getElementById('cash_value_h').value) != 0){
        alert("Calculation doesn't match");
        }else{               
...
<script type="text/javascript">     
    $('modal-form_ca').on('click', '#cari_ca', function(){                
    var no_cc = document.getElementById('no_cc').value; 
    var startdate = document.getElementById('star_tdate').value; 
    var enddate = document.getElementById('end_date').value; 

    $.ajax({
    type : 'post',
    url : 'ajax_carica.php',
    data : {'no_cc': no_cc, 'startdate': startdate, 'enddate': enddate},
    success : function(data){
    $('#details2').html(data); //menampilkan data ke dalam modal
        }
    });                  
});
