...
<?php
        $querys = mysql_query($conn2,"select useraccess.menu as menu,useraccess.username as username, useraccess.fullname as fullname, menurole.id as id from useraccess inner join menurole on menurole.menu = useraccess.menu where username = '$user' and useraccess.menu = 'Create List payment'");
...
(select id,ind_name as idnindir, eng_name as engindir from tbl_master_cashflow) indir on indir.id = coa.id_indirect) $where",$conn1);
    ...
    while($row = mysql_fetch_array($sql)){
        ...
        echo '<tr style="font-size:11px;text-align:center;">
            ...
            $querys = mysql_query($conn1,"select Groupp, finance, ap_apprv_lp from userpassword where username = '$user'");
            $rs = mysql_fetch_array($querys);
            $group = $rs['Groupp'];
            $fin = $rs['finance'];
            $app = $rs['ap_apprv_lp'];
