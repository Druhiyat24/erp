...
<?php
$sql= "select no_payment, no_kbon, tgl_kbon, tgl_payment, nama_supp, total_kbon, outstanding, amount, curr, create_user, status, top, tgl_tempo, memo from list_payment_dp where no_payment = '$no_payment' group by no_kbon";
...
<tr>
	...
	<td align="right"><h4>
      <?php
      $sql1 = mysqli_query($conn2,"select nama_supp from list_payment_dp where no_payment = '$no_payment'");
...
<table style="font-size:12px;">
	...
	<td style="text-align:left;padding-left: 5px;padding-top: -15px;padding-bottom: -10px;">
      <?php
      $sql3 = mysqli_query($conn2,"select tgl_payment from list_payment_dp where no_payment = '$no_payment'");
...
<table  border="1" cellspacing="0" style="width:120%;font-size:21px;border-spacing:2px;">
  <tr>
      <th style="width:35%;border: 1px solid black;text-align:center;">No Kontrabon</th>
      <th style="width:24%;border: 1px solid black;text-align:center;">Kontrabon Date</th>
...
while($data=mysqli_fetch_array($query)){
	...
	$amount = $data['amount'];
	$sisa = $outstanding + $amount;

   echo '<tr>
      <td style="width:30%;text-align:center;">'.$no_kbon.'</td>
      <td style="width:20%;text-align:center;">'.date("d M Y",strtotime($tgl_kbon)).'</td>
...
<?php
$sql1= "select no_payment, SUM(total_kbon) as total_kbon, SUM(outstanding) as outstanding, SUM(amount) as amount from list_payment_dp where no_payment = '$no_payment'";
...
while($datas=mysqli_fetch_array($querys)){
	...
	$sum_sisa = $sum_outstanding + $sum_amount;
}
?>
...
<tr>
      <td colspan="3" style="width:35%;text-align:center;"><b>Jumlah</b></td>
    ...
    <table cellpadding="0" cellspacing="0" border="1" width='600';>
        ...
        <tr>    
            <td style="font-size:12px;text-align:center; width: 150px;text-decoration:underline;">
              <?php
            $sql1 = mysqli_query($conn2,"select create_user from list_payment_dp where no_payment = '$no_payment'");
            $rows = mysqli_fetch_array($sql1);
