<?php include '../header.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h2 class="text-center">LIST PAYABLE CARD STATEMENT ON KONTRABON</h2>
<div class="box">
    <div class="box header">

        
        <form id="form-data" action="pcs2.php" method="post">        
        <div class="form-row">
            <div class="col-md-12">
            <label for="nama_supp"><b>Supplier</b></label>            
             ...
             <button id="btnbpb" type="button" class="btn-secondary btn-xs" style="border-radius: 15%"> BPB</button>
              <button id="btnkb" type="button" class="btn-success btn-xs" style="border-radius: 10%"> Kontra Bon</button>
               <button id="btnlp" type="button" class="btn-secondary btn-xs" style="border-radius: 10%"> List Payment</button>
                </div>
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;display: none;">Amount</th>
            ...
            <th style="text-align: center;vertical-align: middle;">Late Days</th>
...
$nama_supp ='';
    ...
    // }
    // else{
    $sql = mysqli_query($conn2,"select kontrabon_h.no_kbon as no_kbon, kontrabon_h.tgl_kbon as tgl_kbon, kontrabon_h.nama_supp as nama_supp, kontrabon_h.total as total, kontrabon_h.balance as balance, kontrabon.curr as curr, kontrabon.create_user as create_user, kontrabon_h.status as status, kontrabon.tgl_tempo as tgl_tempo, kontrabon.no_faktur as no_faktur, kontrabon.supp_inv as supp_inv, kontrabon.tgl_inv as tgl_inv,kontrabon.pph_code as pph_code from kontrabon_h inner join kontrabon on kontrabon_h.no_kbon=kontrabon.no_kbon where kontrabon_h.status = 'Approved' and kontrabon_h.balance = kontrabon_h.total group by no_kbon");
    // }
    while($row = mysqli_fetch_array($sql)){
    if (!empty($row)) {
    $status = $row['status']; 
    ...
    $date_diff = round($diff / 86400);
        ...
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td style="text-align:right;display:none;" value = "'.$row['total'].'">'.number_format($row['total'],2).'</td>         
           ...
           echo '<td width="100px;">';
            ...
            echo '<td width="70px;">';
            if($date_diff >= 1){
                echo '<a href="pdf_pcs.php?nokontrabon='.$row['no_kbon'].'" target="_blank"><button type="button" class="btn btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;"></i></button></a>';
            }else{
                echo " - ";
            }                                        
            echo '</td>';
