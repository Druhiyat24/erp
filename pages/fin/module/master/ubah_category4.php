...
if ($id != '') {
    $sql = mysqli_query($conn1,"select id_ctg4,Concat(id_ctg4,' - ',ind_name) as name from sb_master_coa_ctg4 where id_ctg3 = '$id'");
}else{

}
  
    echo'<option value="" disabled selected="true">Select Category 4</option>';
    while($r=mysqli_fetch_array($sql)){
        ?>
        <option value="<?php echo $r['id_ctg4'] ?>"><?php echo $r['name'] ?></option>
        <?php        
}
