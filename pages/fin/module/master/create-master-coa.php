...
<div class="box footer">   
        <form id="form-simpan">
            ...
            <div class="input-group">
                ...
                <?php 
                        $sqlacc = mysqli_query($conn1,"select a.id_ctg2,a.ind_name as name2,CONCAT(a.id_ctg2,' - ',a.ind_name) as name,b.id_ctg1,b.ind_name from sb_master_coa_ctg2 a inner join sb_master_coa_ctg1 b on b.id_ctg1 = a.id_ctg1 group by a.id");
            ...
            <div class="input-group" >
                    ...
                    <!-- <?php 
                        $sqlacc = mysqli_query($conn1,"select a.id_ctg5,CONCAT(a.id_ctg5,'.') as no_coa,CONCAT(a.id_ctg5,' - ',a.ind_name) as name,b.id_ctg3,b.ind_name as name3,c.id_ctg4,c.ind_name as name4 from sb_master_coa_ctg5 a inner join sb_master_coa_ctg3 b on b.id_ctg3 = a.id_ctg3 inner join sb_master_coa_ctg4 c on c.id_ctg4 = a.id_ctg4 group by a.id");
                ...
                <div class="input-group">
                <label for="accountid" class="col-form-label" style="width: 150px;" ><b>Category 6</b></label>
                <select class="form-control form-control-sm select2bs4" name="nama_ctg6" id="nama_ctg6" required >
                <option value="" disabled selected="true">Select Category 6</option> 
                <?php 
                        $sql6 = mysqli_query($conn1,"select * from sb_master_coa_ctg6");

                        while ($row = mysqli_fetch_array($sql6)) {
                            $data = $row['ind_name'];
                            $id_ctg2 = $row['id_ctg6'];
                            if($row['id_ctg6'] == $_POST['nama_ctg6']){
                                $isSelected  = ' selected="selected"';
                            }else{
                                $isSelected = '';
                            }
                            echo '<option name="nama_ctg6" value="'.$id_ctg2.'"'.$isSelected.'">'. $data .'</option>';    
                        }
                        ?>
                </select>  
            </div>
            ...
            <div class="input-group">
                <label for="accountid" class="col-form-label" style="width: 150px;" ><b>Category 7</b></label>
                <select class="form-control form-control-sm select2bs4" name="nama_ctg7" id="nama_ctg7"  required >
                <option value="" disabled selected="true">Select Category 7</option> 
                <?php 
                        $sql7 = mysqli_query($conn1,"select * from sb_master_coa_ctg7");

                        while ($row = mysqli_fetch_array($sql7)) {
                            $data = $row['ind_name'];
                            $id_ctg2 = $row['id_ctg7'];
                            if($row['id_ctg7'] == $_POST['nama_ctg7']){
                                $isSelected  = ' selected="selected"';
                            }else{
                                $isSelected = '';
                            }
                            echo '<option name="nama_ctg7" value="'.$id_ctg2.'"'.$isSelected.'">'. $data .'</option>';    
                        }
                        ?>
                </select>  
            </div>
                ...
                $sql = mysqli_query($conn1,"select id,ind_name from sb_master_cashflow where id_group <= 3 and status = 'Active'");
                ...
                $sql = mysqli_query($conn1,"select id,ind_name from sb_master_cashflow where id_group >= 4 and status = 'Active'");
                while ($row = mysqli_fetch_array($sql)) {
                    $data = $row['ind_name'];
                    $id = $row['id'];
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        var nama_ctg6 = $('select[name=nama_ctg6] option').filter(':selected').val();
        var nama_ctg7 = $('select[name=nama_ctg7] option').filter(':selected').val();
        ...
        alert("Indirect Can't be empty");
            ...
            type:'POST',
            url:'insertmastercoa.php',
            data: {'no_coa':no_coa, 'nama_coa':nama_coa, 'nama_ctg1_h':nama_ctg1_h,'nama_ctg2':nama_ctg2, 'nama_ctg3_h':nama_ctg3_h, 'nama_ctg4_h':nama_ctg4_h,'nama_ctg5':nama_ctg5,'nama_ctg6':nama_ctg6,'nama_ctg7':nama_ctg7, 'txt_dirdebit':txt_dirdebit, 'txt_dircredit':txt_dircredit, 'txt_indirect':txt_indirect, 'create_user':create_user},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
