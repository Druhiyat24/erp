...
.modal-dialog {
  ...
  vertical-align: middle;
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">LIST CASH FLOW</h4>
<div class="box">
    <div class="box header">

        <form id="form-data" action="master-cash-flow.php" method="post">        
        <div class="form-row">
           <div class="col-md-3">
            <label for="nama_type"><b>Type</b></label>            
              <select class="form-control select2bs4" name="nama_type" id="nama_type" data-dropup-auto="false" data-live-search="true" >
                ...
                ?>
                >DIRECT</option>
                <option value="INDIRECT" <?php
                $nama_type = '';
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $nama_type = isset($_POST['nama_type']) ? $_POST['nama_type']: null;
                }                 
                    if($nama_type == 'INDIRECT'){
                        $isSelected = ' selected="selected"';
                    }else{
                        $isSelected = '';
                    }
                ...
                <div class="col-md-3">
            <label for="nama_group"><b>Group</b></label>            
              <select style="background-color: gray;" class="form-control select2bs4" name="nama_group" id="nama_group" required>
                <option value="ALL">ALL</option>
                <?php
                $nama_group ='';
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $nama_group = isset($_POST['nama_group']) ? $_POST['nama_group']: null;
                }                 
                $sql = mysql_query("select DISTINCT eng_name from tbl_master_group_cf",$conn1);
                while ($row = mysql_fetch_array($sql)) {
                    $data = $row['eng_name'];
                    if($row['eng_name'] == $_POST['nama_group']){
                        $isSelected = ' selected="selected"';
                    }else{
                        $isSelected = '';
                    }
    ...
    $nama_group = isset($_POST['nama_group']) ? $_POST['nama_group']: null; 
    $Status = isset($_POST['Status']) ? $_POST['Status']: null;
    // $where = "where b.type = '$nama_type'"; 

    }

    if($nama_type == 'ALL' and $nama_group == 'ALL' and $Status == 'ALL'){
     $where = "";
    }elseif($nama_type != 'ALL' and $nama_group == 'ALL' and $Status == 'ALL'){
     $where = "where b.type = '$nama_type'";
    }elseif($nama_type == 'ALL' and $nama_group != 'ALL' and $Status == 'ALL'){
     $where = "where b.eng_name = '$nama_group'";
    }elseif($nama_type == 'ALL' and $nama_group == 'ALL' and $Status != 'ALL'){
     $where = "where a.status = '$Status'";
    }elseif($nama_type != 'ALL' and $nama_group != 'ALL' and $Status == 'ALL'){
     $where = "where b.type = '$nama_type' and b.eng_name = '$nama_group'";
    }elseif($nama_type != 'ALL' and $nama_group == 'ALL' and $Status != 'ALL'){
     $where = "where b.type = '$nama_type' and a.status = '$Status'";
    }elseif($nama_type == 'ALL' and $nama_group != 'ALL' and $Status != 'ALL'){
     $where = "where b.eng_name = '$nama_group' and a.status = '$Status'";
    }else{
     $where = "where b.type = '$nama_type' and b.eng_name = '$nama_group' and a.status = '$Status'";  
    }

        echo '<a target="_blank" href="ekspor_master_cf.php?nama_type='.$nama_type.' && nama_group='.$nama_group.' && Status='.$Status.' && where='.$where.'"><button type="button" class="btn btn-success " style= "margin-top: 30px;"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 1rem;color: #fff;text-shadow: 1px 1px 1px #000"> Excel</i></button></a>';
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    <thead>
        <tr class="thead-dark">
            ...
            <th style="text-align: center;vertical-align: middle;width: 15%;">Type</th>
            <th style="text-align: center;vertical-align: middle;width: 27%;">Group</th>
            <th style="text-align: center;vertical-align: middle;width: 28%;">English Name</th>
            <th style="text-align: center;vertical-align: middle;width: 28%;">Indonesian Name</th>
    ...
    $sql = mysqli_query($conn2,"select a.id,b.type,b.eng_name as grup,a.eng_name as cf_name,a.status,a.ind_name as cf_name2 from sb_master_cashflow a inner join tbl_master_group_cf b on b.id=a.id_group $where");
    ...
    while($row = mysqli_fetch_array($sql)){
        ...
        echo '<tr style="font-size:12px;text-align:center;">
            ...
            <td value = "'.$row['cf_name2'].'">'.$row['cf_name2'].'</td>
            ...
            success: function(response){
                ...
                // $('#modal-form2').modal('hide');
                 ...
                 if (response == '') {
                    ...
                    alert("Cash Flow Already Exist");
                    document.getElementById('txt_indname').focus();
                }
                },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr);
...
<script type="text/javascript">
    ...
    location.href = "master-cash-flow.php";
};
</script>
