...
if ($id != '') {
    $sql = mysqli_query($conn1,"select id_ctg2,Concat(id_ctg2,' - ',ind_name) as name from sb_master_coa_ctg2 where id_ctg1 = '$id'");
}else{
    ...
    while($r=mysqli_fetch_array($sql)){
        ?>
        <option value="<?php echo $r['id_ctg2'] ?>"><?php echo $r['name'] ?></option>
        <?php        
}
