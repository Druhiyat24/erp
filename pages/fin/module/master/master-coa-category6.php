...
.modal-dialog {
  ...
  vertical-align: middle;
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">LIST CATEGORY COA 6</h4>
<div class="box">
    <div class="box header">

        <form id="form-data" action="master-coa-category6.php" method="post">
        <div class="form-row">    
        <div style="padding-left: 10px;" class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
            ...
            <button style="border-radius: 3px" id="btnctg6" type="button" class="btn-primary btn-xs"><span></span> Category 6</button>
      ...
      <a class="dropdown-item" href="#">Category 5</a>
        ...
        <div class="form-row" style="padding-top: 5px;"> 
            ...
            <label for="nama_group"><b>Category Name</b></label>            
              <select style="background-color: gray;" class="form-control select2bs4" name="id_ctg6" id="id_ctg6" required>
                <option value="ALL" selected="true">ALL</option>   
                <?php
                $id_ctg6 ='';
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $id_ctg6 = isset($_POST['id_ctg6']) ? $_POST['id_ctg6']: null;
                }  
                $sql = mysqli_query($conn2,"select id_ctg6,Concat(id_ctg6,' - ',ind_name) as name from sb_master_coa_ctg6");
                while ($row = mysqli_fetch_array($sql)) {
                    $data = $row['id_ctg6'];
                    $data2 = $row['name'];
                    if($row['id_ctg6'] == $_POST['id_ctg6']){
    ...
    if($id_ctg6 == 'ALL'){
     ...
     $where = "where id_ctg6 = '$id_ctg6'";  
    }

        echo '<a target="_blank" href="ekspor_coa_category6.php?where='.$where.'"><button type="button" class="btn btn-success " style= "margin-top: 30px;"><i class="fa fa-file-excel-o" aria-hidden="true" style="padding-right: 10px; padding-left: 5px;font-size: 1rem;color: #fff;text-shadow: 1px 1px 1px #000"> Excel</i></button></a>';
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    ...
    if($id_ctg6 == 'ALL'){
     $sql = mysqli_query($conn2,"select id_ctg6,ind_name,eng_name from sb_master_coa_ctg6");
    }else{
     $sql = mysqli_query($conn2,"select id_ctg6,ind_name,eng_name from sb_master_coa_ctg6 where id_ctg6 = '$id_ctg6'");
    }
    $no=0;
    ...
    while($row = mysqli_fetch_array($sql)){
        ...
        echo '<tr style="font-size:12px;text-align:left;">
            ...
            <td value = "'.$row['id_ctg6'].'">'.$row['id_ctg6'].'</td>
...
<div class="form-row">
    <div class="modal fade" id="mymodal3" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
        ...
        <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="fa fa-times"></span></button>
        <h4 class="modal-title" id="Heading">FORM CREATE CATEGORY COA 6</h4>
        </div>
          <div class="modal-body">
          <div class="form-group">
 ...
 <script type="text/javascript">
    $("#modal-form3").on("click", "#send3", function(){ 
        ...
        $.ajax({
            type:'POST',
            url:'insert_ctg6.php',
            data: {'txt_idctg6':txt_idctg6, 'txt_indname':txt_indname, 'txt_engname':txt_engname, 'create_user':create_user},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
