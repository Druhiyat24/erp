...
$sqlnkb = mysqli_query($conn2,"select max(no_pco) from sb_c_petty_cashout_h where coa_akun = '$akun' and YEAR(tgl_pco) = YEAR('$tgl_pco') AND MONTH(tgl_pco) = MONTH('$tgl_pco')");
 ...
 $sqlx = mysqli_query($conn1,"select if(max(id) is null,'0',max(id)) as id FROM sb_c_report_pettycash where akun = '$akun'");
...
$query = "INSERT INTO sb_c_petty_cashout_h (no_pco,tgl_pco,reff,nama_supp,coa_akun,curr,amount,deskripsi,status, create_by,create_date) 
VALUES 
