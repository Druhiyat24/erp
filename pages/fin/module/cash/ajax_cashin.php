...
$no_ib = isset($_POST['no_ib']) ? $_POST['no_ib']: null;
    ...
    $sql = mysqli_query($conn1,"select if(a.no_coa = '-', '-',CONCAT(b.no_coa,' ',b.nama_coa)) as coa,if(a.no_costcenter = '-','-',c.cc_name) as cost_center,if(a.buyer = '','-',a.buyer) as buyer ,if(a.ws = '','-',a.ws) as no_ws, if(a.submit_by = '','-',a.submit_by) as submit_by,a.curr,a.amount from c_cash_in a left join mastercoa_v2 b on b.no_coa = a.no_coa left join b_master_cc c on c.no_cc = a.no_costcenter where a.no_ci = '$no_ib'");
    $sql3 = mysqli_query($conn1,"select sum(amount) as amount from c_cash_in where no_ci = '$no_ib'");
    ...
    $table = '<table id="mytdmodal" class="table table-striped table-bordered" cellspacing="0" width="100%" style="font-size: 12px;text-align:center;">
                    <thead>
                        <tr>                       
                            ...
                            <th style="width:11%;">Submit By</th> 
            ...
            $table .= '<tr>                       
                            ...
                            <td style="" value="'.$row['no_ws'].'">'.$row['no_ws'].'</td>
                             <td style="" value="'.$row['submit_by'].'">'.$row['submit_by'].'</td>
