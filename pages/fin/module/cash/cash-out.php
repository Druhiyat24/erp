<?php include '../header2.php' ?>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">LIST CASH OUT</h4>
<div class="box">
...
<table id="datatable" class="table table-striped table-bordered" role="grid" cellspacing="0" width="100%">
    ...
    $sql = mysqli_query($conn1,"select * from (select no_co, tgl_co,type_co, IF(dokumen = '','-',dokumen) as dokumen, sum(amount) as amount,status,create_by from sb_c_cash_out $where group by no_co) a 
        UNION
        select * from (select no_co, tgl_co,type_co, IF(dokumen = '','-',dokumen) as dokumen, sum(amount) as amount,status,create_by from c_cash_out $where group by no_co) a");
