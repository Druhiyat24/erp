...
<style>
    ...
    <!-- MAIN -->
    <div class="col p-4">
        <h4 class="text-center">FORM MEMORIAL JOURNAL</h4>
<div class="box">
...
<!--             <input type="hidden" style="font-size: 15px;" name="unik_code" id="unik_code" class="form-control" 
            ...
            <label for="nama_supp" class="col-form-label" style="width: 150px;"><b>Type</b></label>            
              <select class="form-control select2bs4" name="nama_type" id="nama_type" data-dropup-auto="false" data-live-search="true" onchange="this.form.submit()">
            ...
            <label for="filter_cmj" class="col-form-label" style="width: 150px;"><b>Type</b></label>            
              <select class="form-control select2bs4" name="filter_cmj" id="filter_cmj" data-live-search="true" onchange="this.form.submit()">
                    <option value="" disabled selected="true">Select from</option>  
                    <?php
                $filter_cmj ='';
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                $filter_cmj = isset($_POST['filter_cmj']) ? $_POST['filter_cmj']: null;
                }                 
                $sql = mysqli_query($conn1,"select 'None' filter union select 'HRIS' filter");
                while ($row = mysqli_fetch_array($sql)) {
                    $data = $row['filter'];
                    if($row['filter'] == $_POST['filter_cmj']){
                        $isSelected = ' selected="selected"';
                    }else{
                        $isSelected = '';
            ...
            <div class="col-md-2 mb-3">            
            <label for="total" class="col-form-label" style="width: 150px;"><b>Periode Akhir</b></label>
                <input type="text" style="font-size: 15px;" name="tgl_hris" id="tgl_hris" class="form-control tanggal" 
            value="<?php 
            $tgl_hris = isset($_POST['tgl_hris']) ? $_POST['tgl_hris']: null;            
            if(!empty($_POST['tgl_hris'])) {
                echo $_POST['tgl_hris'];
            }
            else{
                echo date("Y-m-d");
            ...
            <div class="col-md-3 mb-3">
            <label for="from_hris" class="col-form-label"><b>Search from HRIS:</b></label>
            <br>            
              <button style="border: 0;line-height: 1;padding: 10px 10px;font-size: 1rem;text-align: center;color: #fff;text-shadow: 1px 1px 1px #000;border-radius: 6px;background-color: rgb(95, 158, 160);" type="submit" id="send" name="send" class="btn btn-primary btn-lg" style="width: 100%;"><span class="fa fa-search"></span> HRIS</button>
 ...
 <div class="form-row">
        ...
        <div class="row">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center" style="width: 8%">Reference Date</th>
                    <th class="text-center" style="width: 8%">Buyer</th>
                    <th class="text-center" style="width: 8%">Worksheet</th>
    ...
    <?php
            ...
            $date_now = date("Y-m-d");           
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $tglhris = isset($_POST['tgl_hris']) ? $_POST['tgl_hris']: null;
            if ($tglhris == null) {
                $hris = date("Y-m-d");
            }else{
                $hris = $_POST['tgl_hris'];
                ...
                while($row = mysqli_fetch_array($sql)){
                    ...
                    if ($debit <= 0 && $credit <= 0) {
                        ...
                        <tr>
                            ...
                            <td style="" value="'.$row['reff_date'].'">'.$reffdate.'</td>
                            <td style="" value="'.$row['buyer'].'">'.$row['buyer'].'</td>
                            <td style="" value="'.$row['ws'].'">'.$row['ws'].'</td>
                            <td style="" value="'.$row['curr'].'">'.$row['curr'].'</td>
                            <td style="" value="'.$row['debit'].'">'.number_format($row['debit'],2).'</td>
                            <td style="" value="'.$row['credit'].'">'.number_format($row['credit'],2).'</td>
...
<div class="box footer">   
        ...
        <!--     <div class="input-group" >
            ...
            <div class="col-md-4">
                ...
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="txt_credit" name="txt_credit" placeholder="0.00" readonly value="<?php 
                    $ref = isset($_POST['filter_cmj']) ? $_POST['filter_cmj']: null;
                    if ($ref == 'HRIS') {
                            echo number_format($ttl_credit,2);
                    }else{ } ?>">
            ...
            <div class="input-group" >
                ...
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="txt_debit" name="txt_debit" placeholder="0.00" readonly value="<?php 
                    $ref = isset($_POST['filter_cmj']) ? $_POST['filter_cmj']: null;
                    if ($ref == 'HRIS') {
                            echo number_format($ttl_debit,2);
                    }else{ } ?>">
            ...
            <div class="input-group" >
                ...
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="txt_credit_idr" name="txt_credit_idr" placeholder="0.00" readonly value="<?php 
                    $ref = isset($_POST['filter_cmj']) ? $_POST['filter_cmj']: null;
                    if ($ref == 'HRIS') {
                            echo number_format($ttl_credit,2);
                    }else{ } ?>">
            ...
            <div class="input-group" >
                ...
                <input type="text" style="font-size: 14px;text-align: right;" class="form-control" id="txt_debit_idr" name="txt_debit_idr" placeholder="0.00" readonly value="<?php 
                    $ref = isset($_POST['filter_cmj']) ? $_POST['filter_cmj']: null;
                    if ($ref == 'HRIS') {
                            echo number_format($ttl_debit,2);
                    }else{ } ?>">
...
<script type="text/javascript">
    $("#form-simpan").on("click", "#simpan", function(){
        ...
        $.ajax({
            ...
            data: {'no_mj':no_mj, 'mj_date':mj_date, 'tgl_hris':tgl_hris, 'id_cmj':id_cmj, 'no_coa':no_coa, 'no_costcenter':no_costcenter, 'no_reff':no_reff,'reff_date':reff_date, 'buyer':buyer, 'no_ws':no_ws, 'curr':curr, 'rate':rate, 'debit':debit, 'credit':credit, 'keterangan':keterangan, 'create_user':create_user},
            cache: 'false',
            close: function(e){
                e.preventDefault();
            },
