...
$_json 						= $Proses->generate_json_with_attribut($mod,$username,$data_datatable,$populasi_kolom,$populasi_attribut); // bermain disini
...
$sql = "select {$kolom_x}  from {$main_query} WHERE 1 ".$searchQuery." order by ".$columnName." DESC limit ".$row.",".$rowperpage;	
	...
	public function generate_attr($mod,$username,$_attr,$id_reff,$__data,$id_so_det){
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			if($akses == "1"){
				$_str .= "<a href='?mod={$mod_form}&id_sew_in={$__data["id_sew_in_header"]}&cr=is_edit' ><i class='fa fa-pencil'> </i></a>";
			}
			...
			if($akses == "1"){
				$_str .= "<a href='?mod={$mod_form}&id_sew_in={$__data["id_sew_in_header"]}&cr=is_view' ><i class='fa fa-eye'> </i></a>";
			}						
		}
		return $_str;
	}			
	...
	public function generate_json_with_attribut($mod,$username,$_data,$_populasi_kolom,$_populasi_attribut){
		$j_data = array();
		$outp = "";/* 
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				...
				$_attr = "";
				$_attr = $this->generate_attr($mod,$username,$_attribut,$id_reff,$_data['data'][$i],$id_so_det);
 					if($k == $trigger){
