	...
	SELECT  
			...
			FROM(
				SELECT 	
					...
					BALANCE.so_no, 	
					/*sewing in*/
					(ifnull(BALANCE.tot_qty_sew_in,0))tot_qty_sew_in,
					(ifnull(BALANCE.balance_qty_sew_in,0))balance_qty_sew_in,
					/*sewing out*/
					(ifnull(BALANCE.tot_qty_sew_out,0))tot_qty_sew_out,
					(ifnull(BALANCE.balance_qty_sew_out,0))balance_qty_sew_out,
					/*sewing qc input*/
					(ifnull(BALANCE.tot_qty_sew_qc_in,0))tot_qty_sew_qc_in,
					(ifnull(BALANCE.balance_qty_sew_qc_in,0))balance_qty_sew_qc_in,
					/*sewing qc output*/
					(ifnull(BALANCE.tot_qty_sew_qc_out,0))tot_qty_sew_qc_out,
					(ifnull(BALANCE.balance_qty_sew_qc_out,0))balance_qty_sew_qc_out,
					/*steam output*/
					(ifnull(BALANCE.tot_qty_steam_out,0))tot_qty_steam_out,
					(ifnull(BALANCE.balance_steam_output,0)) balance_steam_output,	
					/*qc final input*/
					(ifnull(BALANCE.tot_qty_qc_final_in,0))tot_qty_qc_final_in,
					(ifnull(BALANCE.balance_qc_final_input,0))balance_qc_final_input,
					/*qc final output*/
					(ifnull(BALANCE.tot_qty_qc_final_out,0))tot_qty_qc_final_out,
					(ifnull(BALANCE.balance_qc_final_output,0))balance_qc_final_output,
					/*packing*/
					(ifnull(BALANCE.tot_qty_packing,0))tot_qty_packing,
					(ifnull(BALANCE.balance_packing,0))balance_packing,
					/*metal detector*/
					(ifnull(BALANCE.tot_qty_mat_dec,0))tot_qty_mat_dec,
					(ifnull(BALANCE.balance_met_det,0))balance_met_det,
					/*qc audit*/
					(ifnull(BALANCE.tot_qty_qc_audit,0))tot_qty_qc_audit,
					(ifnull(BALANCE.balance_qc_audit,0))balance_qc_audit,
					/*finish good*/
					(ifnull(BALANCE.tot_qty_fg,0))tot_qty_fg,
					(ifnull(BALANCE.balance_fg,0))balance_fg,
					/*shipping*/
					(ifnull(BALANCE.tot_shp,0))tot_shp,
					(ifnull(BALANCE.balance_shp,0))balance_shp
					FROM (
						SELECT
							ACT.kpno,
							''id_dc_out_header,
							...
							SOD.cancel = 'N' 
							GROUP BY
							ACT.id,SOD.id)BALANCE
				INNER JOIN(
					SELECT A.id id_costing,A.kpno,A.status,A.deldate,A.id_buyer
						FROM act_costing A
						LEFT JOIN(SELECT * FROM so WHERE cancel_h !='Y') B ON A.id=B.id_cost
						LEFT JOIN so_det C ON B.id = C.id_so
					
					WHERE A.id_buyer ='{$id_buyer}' AND A.status='{$status}' AND A.deldate>='{$from}' AND A.deldate<='{$to}'
...
function sql_balance_cutting($id_cost){
	...
	$sql = "
		...
		LEFT JOIN (
			...
			GROUP BY d_cod.id_item,d_cod.id_so_det,d_cod.id_panel
		...
		LEFT JOIN (
			...
			GROUP BY d_cnd.id_item,d_cnd.id_so_det,d_codn.id_panel
		...
		LEFT JOIN (
			...
			GROUP BY d_cqd.id_item,d_cqd.id_so_det,d_codnn.id_panel
			ORDER BY d_cqd.id_cut_out_detail ASC
		) AS CQD_D ON CQD_D.id_cut_number_detail = CND_D.id_cut_number_detail
		...
		LEFT JOIN (
			...
			GROUP BY d_djd.id_item,d_djd.id_so_det,d_codnnn.id_panel
			ORDER BY d_djd.id_cut_out_detail ASC
		) AS DJD_D ON DJD_D.id_cut_qc_detail = CQD_D.id_cut_qc_detail
		...
		ORDER BY COD_D.id_item ASC,COD_D.id_panel ASC,SOD.id ASC
	";
	return $sql;
...
SELECT  
	...
	SELECT 	CUTT_OUT.kpno,
			...
			FROM(
					...
					(
						...
						SELECT
							...
							A.size,
							A.lot,
							SUM(IFNULL(A.qty_cut_out, 0)) AS tot_qty_cut_out,
							A.id_grouping,
			...
			GROUP BY SOD.id,SOD.color,COD_D.id_panel,COD_D.id_item)CUTT_OUT
...
LEFT JOIN(
	SELECT id_sec_in_detail
			,id_so_det
			,id_panel
			,id_item
			,id_proses
			,SUM(IFNULL(qty_sec_in, 0)) AS tot_qty_sec_in
...
LEFT JOIN(
	SELECT id_sec_out_detail
			,id_so_det
			,id_panel
			,id_item
			,id_proses
			,SUM(IFNULL(qty_sec_out, 0)) AS tot_qty_sec_out
...
LEFT JOIN(
	SELECT 	 id_sec_qc_detail
			,id_so_det
			,id_panel
			,id_item
			,id_proses
			,SUM(IFNULL(qty, 0)) AS tot_qty_sec_qc
