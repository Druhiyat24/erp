...
if($cut_out == "-1"){
	...
	$table = "(
		...
		SELECT 
			...
			num_old.tot_qty_cut_num AS before_cumulative,
			(num_old.tot_qty_cut_num + num_old.balance_qty_cut_num) AS before_balance
		FROM (
			...
			GROUP BY cod.id_grouping,cod.id_panel,cod.lot,cod.id_so_det

		) AS num_new

		LEFT JOIN (
			{$sql_balance}
		) AS num_old ON num_old.id_so_det = num_new.id_so_det 
		AND num_old.id_panel = num_new.id_panel 
		AND num_old.id_item = num_new.id_item 
		AND num_old.lot = num_new.lot
	...
	$table = "(
		...
		SELECT 
			...
			num_old.tot_qty_cut_num AS cumulative,
			num_old.balance_qty_cut_num AS balance,
			(num_old.tot_qty_cut_num - num_new.qty_cut_number) AS before_cumulative,
			(num_old.tot_qty_cut_num + num_old.balance_qty_cut_num) AS before_balance
		FROM (
			SELECT
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputQtyInNum = '';
	$inputQtyInNum .="<input type='number' id='qtyInNum_".$no."' class='form-control forms' value='$qtyInNum' onchange='handleKeyUp(this)' style='min-width: 80px'>";
