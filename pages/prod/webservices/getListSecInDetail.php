...
//$columnName = $_POST['columns'][$columnIndex]['data']; // Column name$_POST['order'][0]['dir']; // asc or desc
$columnName			= 'X.id_so_det,X.urut'; // Column name
$searchValue		= $_POST['search']['value']; // Search value
$id_cost			= $_GET['id_cost'];
$id_sec_in			= $_GET['id_url'];
...
$main_query 				= $Proses->get_main_query($id_cost,$id_sec_in,$id_proses,$id_panel); // bermain disini
...
class Proses {
	...
	public function populasi_kolom(){
		return array(
			...
			"19"=>"bundle_number",
			"20"=>"sack_number",	
			"21"=>"qty_reject_sec_in",
			"22"=>"id_panel",
			"23"=>"id_item",
			"24"=>"id_proses",
			"25"=>"before_cumulative",
			"26"=>"before_balance"
		);
	}
	...
	public function populasi_attribut(){
		return array(
			"0"=>"is___qty_sec_in",
			"1"=>"is___qty_reject_sec_in",
			"2"=>"is___balance_sec_in",
			"3"=>"is___qty_so",
			"4"=>"is___tot_qty_sec_in",
			"5"=>"is___tot_qty_cut_out"
		);
	}


	public function get_main_query($id_cost,$id_sec_in,$id_proses,$id_panel){
		$sql_balance = sql_balance_secondary_from_cutting($id_cost);
		// echo $sql_balance;die();
		$_sql = "(
				SELECT   		
					...
					TABLE_BALANCE.tot_qty_sec_out next_qty,
					TABLE_BALANCE.balance_sec_out,			
					TABLE_BALANCE.tot_qty_sec_qc,
					TABLE_BALANCE.balance_sec_qc,
					ifnull(SEC_IN.id_sec_in_det,0) id_sec_in_det,
					ifnull(SEC_IN.qty_sec_in,0)qty_sec_in,
					(TABLE_BALANCE.balance_sec_in + ifnull(SEC_IN.qty_sec_in,0))balance_plus_qty_sec_in,
					ifnull(SEC_IN.qty_reject_sec_in,0)qty_reject_sec_in,
					(TABLE_BALANCE.tot_qty_sec_in - (ifnull(SEC_IN.qty_sec_in,0) - ifnull(SEC_IN.qty_reject_sec_in,0))) AS before_cumulative,
					(TABLE_BALANCE.tot_qty_sec_in + TABLE_BALANCE.balance_sec_in) AS before_balance
				FROM (
					{$sql_balance}
				) TABLE_BALANCE
				...
				LEFT JOIN (
					SELECT   
						...
						,A.username
						,A.dateinput
						,B.id_sec_in_detail id_sec_in_det
						,B.id_so_det
						,B.qty_sec_in qty_sec_in
						,B.id_proses
						,B.id_panel
						,B.id_item
						,B.qty_reject_sec_in
					...
					INNER JOIN (
						SELECT id_sec_in_detail,id_sec_in_header,id_so_det,qty_sec_in,id_item,id_panel,id_proses,qty_reject_sec_in FROM prod_sec_in_detail 
					) B ON A.id_sec_in_header = B.id_sec_in_header
					WHERE A.id_sec_in_header = '{$id_sec_in}'	
				)SEC_IN	ON 
				TABLE_BALANCE.id_so_det = SEC_IN.id_so_det
				AND TABLE_BALANCE.id_panel = SEC_IN.id_panel
				AND TABLE_BALANCE.id_item = SEC_IN.id_item
				AND TABLE_BALANCE.id_proses = SEC_IN.id_proses
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut,$_key_det){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_sec_in_det
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_sec_in_det'] == 0 ){
					$id_reff= $i;
				} 
				else{
					$id_reff= $_data['data'][$i]['id_sec_in_det'];
				}
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		/* 	$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			if($_attr == "is___qty_so"){
				$disabled = "disabled";
				$data_value = "0";
			}
			else if($_attr == "is___tot_qty_sec_in"){
				$disabled = "disabled";
				$data_value = $__data['before_cumulative'];
			}
			else if($_attr == "is___balance_sec_in"){
				$disabled = "disabled";
				$data_value = $__data['before_balance'];
			}
			else{
				$disabled = "";
				$data_value = "0";
			}
			$_str = "<input type='number'"
					."class='form-control forms {$_attr}'"
					."id='".$_attr."__".$id_reff."'"
					."name='".$_attr."&".$id_reff."'"
					."value='".$_value."'"
					."data-balance='".$__data["balance_plus_qty_sec_in"]."'" 
					."data-id_det='".$__data["id_sec_in_det"]."'" 
					...
					."onchange='handleKeyUp(this.name)' $disabled "
	...
	public function get_id_jo($id_number){
		$_sql = "SELECT 
					A.id_so,A.id_number,B.id_jo 
				FROM prod_spread_report_number A 
