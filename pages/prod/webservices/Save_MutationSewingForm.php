...
if($code == '1'){
		if($format == '1'){
			$no_mut_sew 			= $Proses->get_no_mut_sew($header->date_output);
			$key 						= $Proses->insert($header,$_SESSION['username'],$no_mut_sew);
			$id_mut_sew_header  	= $Proses->get_id($no_mut_sew);
			$key 						= $Proses->insert_detail($header,$detail,$id_mut_sew_header,$_SESSION['username']);
			$result 					= '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_mut_sew_header" : "'.$id_mut_sew_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_mut_sew_header = $header->id_mut_sew_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_mut_sew($_date_output){
		$category ="MUT/SEW";
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_mut_sew = $category."/".$_month.$seq_years."/".$seq;
			return $_no_mut_sew;
	}	
	public function get_id($_no_mut_sew){
		$sql = "SELECT id_mut_sew_header FROM prod_mut_sew_header WHERE no_mut_sew = '{$_no_mut_sew}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_mut_sew_header'];
	}
	public function insert($data,$username,$no_mut_sew)
	{
		...
		$insert = "INSERT INTO prod_mut_sew_header (notes,id_cost,id_line,username,dateinput,date_output,no_mut_sew,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_mut_sew_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_sew_header = '{$data->id_mut_sew_header}'";
	...
	public function insert_detail($header,$_detail,$id_mut_sew_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_mut_sew_detail(id_so_det,id_mut_sew_header,qty,username,dateinput) VALUES ";
...
/* 		print_r($_detail);
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_mut_sew_detail WHERE id_mut_sew_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
						$update = "UPDATE prod_mut_sew_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_sew_detail = '{$_det->is_id_det}'";
