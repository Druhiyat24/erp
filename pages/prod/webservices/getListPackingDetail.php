...
$id_finishing_packing_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_finishing_packing_header )){
	$id_finishing_packing_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_finishing_packing_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_packing"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."'>";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_finishing_packing_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_packing + ifnull(PACKING.qty_packing,0))balance_plus_qty_packing
			,TABLE_BALANCE.urut
			,ifnull(PACKING.id_det,0) id_det
			,ifnull(PACKING.qty_packing,0)qty_packing	
			,TABLE_BALANCE.tot_qty_mat_dec next_qty
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_finishing_packing_header
							,A.date_output
							,A.no_finishing_packing
							,A.notes
							,A.id_cost
							,A.id_line
							,A.username
							,A.dateinput
							,A.time
							,B.id_finishing_packing_detail id_det
							,B.id_so_det
							,B.qty qty_packing							
					FROM prod_finishing_packing_header A
					INNER JOIN(
						SELECT id_finishing_packing_detail,id_so_det,id_finishing_packing_header,qty FROM prod_finishing_packing_detail 
					)B ON A.id_finishing_packing_header = B.id_finishing_packing_header
					WHERE A.id_finishing_packing_header ='{$id_finishing_packing_header}'
				)PACKING ON TABLE_BALANCE.id_so_det = PACKING.id_so_det
		)X";
/*  echo $_sql;
die();  */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"6"=>"unit",
		"7"=>"balance_packing",
		"8"=>"tot_qty_qc_final_out",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_packing",
		"12"=>"balance_plus_qty_packing",
		"13"=>"next_qty",
		"14"=>"tot_qty_packing"
		);
