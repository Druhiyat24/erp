...
if($code == '1'){
		if($format == '1'){
			$no_sew_steam_out 	= $Proses->get_no($header->date_output);
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_sew_steam_out);
			$id_sew_steam_out_header  = $Proses->get_id($no_sew_steam_out);
			$key 		= $Proses->insert_detail($header,$detail,$id_sew_steam_out_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sew_steam_out_header" : "'.$id_sew_steam_out_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_sew_steam_out_header = $header->id_sew_steam_out_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no($_date_output){
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_sew_steam_out = $category."/".$_month.$seq_years."/".$seq;
			return $_no_sew_steam_out;
	}	
	public function get_id($_no_sew_steam_out){
		$sql = "SELECT id_sew_steam_out_header FROM prod_sew_steam_out_header WHERE no_sew_steam_out = '{$_no_sew_steam_out}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_sew_steam_out_header'];
	}
	public function insert($data,$username,$no_sew_steam_out)
	{
		...
		$insert = "INSERT INTO prod_sew_steam_out_header (notes,id_cost,id_line,username,dateinput,date_output,no_sew_steam_out,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sew_steam_out_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sew_steam_out_header = '{$data->id_sew_steam_out_header}'";
	...
	public function insert_detail($header,$_detail,$id_sew_steam_out_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO steam_out(id_so_det,id_sew_steam_out_header,qty,username,dateinput) VALUES ";
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM steam_out WHERE id = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE steam_out SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id = '{$_det->is_id_det}'";
				...
				if($_det->is_qty > 0){
					...
					$insert .=" 
									'{$_det->is_id_so_det}',
									'{$header->id_sew_steam_out_header}',
									'{$_det->is_qty}',
									'{$username}',
									'{$dt}'
									";		
