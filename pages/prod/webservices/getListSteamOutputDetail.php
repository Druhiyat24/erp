...
$id_sew_steam_out_header     =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_sew_steam_out_header )){
	$id_sew_steam_out_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_steam_out_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-qty_current='".$__data["qty_current"]."' data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_tot_qty_steam_out"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_sew_steam_out_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_steam_output + ifnull(ST_OUT.qty_steam_out,0))balance_plus_tot_qty_steam_out
			,TABLE_BALANCE.urut
			,ifnull(ST_OUT.id_det,0) id_det
			,ifnull(ST_OUT.qty_steam_out,0)qty_steam_out
			,(TABLE_BALANCE.tot_qty_steam_in) next_qty
			,(TABLE_BALANCE.tot_qty_steam_out - ifnull(ST_OUT.qty_steam_out,0) ) qty_current
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_steam_out_header
							,A.date_output
							,A.no_sew_steam_out
							,A.notes
							,A.id_cost
							...
							,B.id_so_det
							,B.qty qty_steam_out
					FROM prod_sew_steam_out_header A
					INNER JOIN(
						SELECT id,id_so_det,id_sew_steam_out_header,qty FROM steam_out 
					)B ON A.id_sew_steam_out_header = B.id_sew_steam_out_header
					WHERE A.id_sew_steam_out_header ='{$id_sew_steam_out_header}'
				)ST_OUT ON TABLE_BALANCE.id_so_det = ST_OUT.id_so_det
		)X";
/*    echo $_sql;
die();    */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"10"=>"id_so_det",
		"11"=>"qty_steam_out",
		"12"=>"balance_plus_tot_qty_steam_out",
		"13"=>"next_qty",
		"14"=>"tot_qty_steam_out",
		"15"=>"qty_current"
		);
	}
}
