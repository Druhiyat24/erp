...
$id_rej_qc_fin_header     	=$_GET['id_url'];
...
if(!ISSET($id_rej_qc_fin_header)){
	$id_rej_qc_fin_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_rej_qc_fin_header,$add_more_item); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		if($_attr == "is___checklist"){
			if($__data["id_rej_qc_fin_header"] == "0" ){
				$checked = "";
			}else{
				$checked = "checked";
			}
			$_str .="<input id='".$_attr."__".$__data["id_rej_qc_fin_detail"]."' type='checkbox' onclick='check_uncheck(this)' {$checked} class='{$_attr}' data-id_header='{$__data['id_rej_qc_fin_header']}' data-id_detail='{$__data['id_rej_qc_fin_detail']}' data-id='".$_attr."__".$__data["id_rej_qc_fin_detail"]."' >";	
		}
		if($pecah_[1] == "select"){
			$_str .="<select onchange='checklist_change(this)' data-id='".$pecah_[2]."__".$__data["id_rej_qc_fin_detail"]."' class='form-control select2 sel_defect' id='".$pecah_[2]."__".$__data["id_rej_qc_fin_detail"]."'>";
			$_str .= $this->get_populasi_defect($__data["id_defect"]);
			$_str .= "</select>";
		}
		else{
/* 			$_str = "<input data-id='".$_attr."__".$__data["id_rej_qc_fin_detail"]."' type='text' data-balance='".$__data["balance_plus_sew_qc_out"]."' data-id_rej_qc_fin_detail='".$__data["id_rej_qc_fin_detail"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$__data["id_rej_qc_fin_detail"]."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >"; */
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_rej_qc_fin_header,$add_more_item){
		$where = "AND A.id_cost = '{$id_cost}' AND ifnull(REJECT.id_rej_qc_fin_header,0) = 0 ";
		if($id_rej_qc_fin_header != "X"){
			if($add_more_item == '1'){
				$where = "AND A.id_cost = '{$id_cost}' AND ((ifnull(REJECT.id_rej_qc_fin_header,0) = '{$id_rej_qc_fin_header}') OR (ifnull(REJECT.id_rej_qc_fin_header,0) = 0) ) ";
			}else{
				$where = "AND A.id_cost = '{$id_cost}' AND ifnull(REJECT.id_rej_qc_fin_header,0) = '{$id_rej_qc_fin_header}' ";
			}
		...
		$_sql ="(
					SELECT
					A.id_cost,
					ifnull(REJECT.id_rej_qc_fin_header, 0)id_rej_qc_fin_header,
					REJECT.id_rej_qc_fin_detail,
					F.urut,
					...
					FROM
					prod_qc_final_out_header A 
					INNER JOIN
						(
							SELECT
								id_so_det,
								id_sew_qc_final_out_header,
								id id_qc_fin_out_detail,
								sum(ifnull(rpr, 0)) qty_reject 
							FROM
								prod_qc_final_out_detail 
							WHERE
								ifnull(rpr, 0) > 0 
								AND IFNULL(id_sew_qc_final_out_header, 0) > 0 
							GROUP BY
								id,
								id_so_det
						)
						B 
						ON A.id_sew_qc_final_out_header = B.id_sew_qc_final_out_header 
					INNER JOIN
						(
							SELECT
								id_rej_qc_fin_detail,
								id_rej_qc_fin_header,
								id_qc_fin_out_detail,
								id_defect 
							FROM
								prod_rej_qc_fin_detail
						)
						REJECT 
						ON B.id_qc_fin_out_detail = REJECT.id_qc_fin_out_detail 
					INNER JOIN
						act_costing C 
						ON A.id_cost = C.id 
					INNER JOIN
					...
					INNER JOIN
						so_det E 
						ON E.id_so = D.id 
						AND B.id_so_det = E.id 
					INNER JOIN
						mastersize F 
						ON E.size = F.size 
					WHERE
					B.qty_reject > 0 {$where} )X";
/*     echo $_sql;
die();  */   
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"no_sew_qc_final_out",
		"8"=>"id_rej_qc_fin_header",
		"9"=>"id_rej_qc_fin_detail",
		"10"=>"id_defect"
		);
	}
}
