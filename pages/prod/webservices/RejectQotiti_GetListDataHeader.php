...
$data = $_POST;
$code = $_POST['code'];
$id_reject_qot_header = $_POST['id'];
 //print_r($id_sew_qc_in );die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_reject_qot_header);
...
class Proses {
	...
	public function get_list_data($id_reject_qot_header){
		$sql = "SELECT id_reject_qot_header,date_output,no_reject_qot,notes,id_cost,id_line,username,dateinput,time 
				FROM prod_reject_qot_header WHERE id_reject_qot_header ='{$id_reject_qot_header}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_reject_qot_header":"'.rawurlencode($row[$i]['id_reject_qot_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_reject_qot":"'. rawurlencode($row[$i]["no_reject_qot"]).'",';	
