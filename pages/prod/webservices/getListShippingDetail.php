...
$id_finishing_shp_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_finishing_shp_header)){
	$id_finishing_shp_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_finishing_shp_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_shp"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_finishing_shp_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.tot_qty_qc_audit tot_qty_audit_buyer
			,TABLE_BALANCE.balance_qc_audit balance_audit_buyer
			
	
			,TABLE_BALANCE.tot_qty_fg
			,TABLE_BALANCE.balance_fg			


			,TABLE_BALANCE.tot_shp
			,TABLE_BALANCE.balance_shp			
	
			,(TABLE_BALANCE.balance_shp + ifnull(SHP.qty_shp,0))balance_plus_qty_shp
			,TABLE_BALANCE.urut
			,ifnull(SHP.id_det,0) id_det
			,ifnull(SHP.qty_shp,0)qty_shp		
			,'0' next_qty
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_finishing_shp_header
							,A.date_output
							,A.no_finishing_shp
							,A.notes
							,A.id_cost
							...
							,A.dateinput
							,A.time
							,B.id_finishing_shp_detail id_det
							,B.id_so_det
							,B.qty qty_shp					
					FROM prod_finishing_shp_header A
					INNER JOIN(
						SELECT id_finishing_shp_detail,id_so_det,id_finishing_shp_header,qty FROM prod_finishing_shp_detail 
					)B ON A.id_finishing_shp_header = B.id_finishing_shp_header
					WHERE A.id_finishing_shp_header ='{$id_finishing_shp_header}'
				)SHP ON TABLE_BALANCE.id_so_det = SHP.id_so_det
		)X";
/*  echo $_sql;
die();  */
		return $_sql;
