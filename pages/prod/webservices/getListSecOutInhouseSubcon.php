...
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_populasi_ws($id_cost);
...
class Proses {
	...
	public function get_populasi_ws($id_cost){
		$sql = "SELECT 
				sec_out.id,
				sec_out.isi 
			FROM (
				SELECT 
					IF(mp.nama_pilihan = 'INHOUSE', 'N', 'Y') AS id,
					mp.nama_pilihan AS isi 
				FROM masterpilihan AS mp
				WHERE mp.kode_pilihan = 'MFG_OUT'
			) AS sec_out
			INNER JOIN prod_sec_in_header AS sih ON sih.inhouse_subkon = sec_out.id WHERE sih.id_cost = '{$id_cost}'
			GROUP BY sec_out.id
		";
