...
$id_finishing_metal_detector_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_finishing_metal_detector_header )){
	$id_finishing_metal_detector_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_finishing_metal_detector_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_met_dec"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."'>";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_finishing_metal_detector_header){
		$sql_balance = sql_balance_from_so($id_cost);
		$_sql ="(
	SELECT  TABLE_BALANCE.id_dc_out_header
			,TABLE_BALANCE.kpno
			,TABLE_BALANCE.so_no
			...
			,TABLE_BALANCE.tot_qty_sew_in
			,TABLE_BALANCE.balance_qty_sew_in
			,TABLE_BALANCE.tot_qty_sew_out
			,TABLE_BALANCE.balance_qty_sew_out	
			,TABLE_BALANCE.tot_qty_sew_qc_in
			,TABLE_BALANCE.balance_qty_sew_qc_in
			,TABLE_BALANCE.tot_qty_sew_qc_out tot_qty_qc_out
			,TABLE_BALANCE.balance_qty_sew_qc_out	
			,TABLE_BALANCE.tot_qty_steam_out
			,TABLE_BALANCE.balance_steam_output balance_steam_out


			,TABLE_BALANCE.tot_qty_qc_final_in
			,TABLE_BALANCE.balance_qc_final_input balance_qc_final_in
			
			
			,TABLE_BALANCE.tot_qty_qc_final_out
			,TABLE_BALANCE.balance_qc_final_output balance_qc_final_out		


			,TABLE_BALANCE.tot_qty_packing
			,TABLE_BALANCE.balance_packing				

			,TABLE_BALANCE.tot_qty_mat_dec
			,TABLE_BALANCE.balance_met_det balance_metal_detector
			
			
			,(TABLE_BALANCE.balance_met_det + ifnull(MET_DEC.qty_metal_detector,0))balance_plus_qty_met_dec
			,TABLE_BALANCE.urut
			,ifnull(MET_DEC.id_det,0) id_det
			,ifnull(MET_DEC.qty_metal_detector,0)qty_metal_detector	
			,TABLE_BALANCE.tot_qty_qc_audit next_qty
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_finishing_metal_detector_header
							,A.date_output
							,A.no_finishing_metal_detector
							,A.notes
							,A.id_cost
							,A.id_line
							,A.username
							,A.dateinput
							,A.time
							,B.id_finishing_metal_detector_detail id_det
							,B.id_so_det
							,B.qty qty_metal_detector							
					FROM prod_finishing_metal_detector_header A
					INNER JOIN(
						SELECT id_finishing_metal_detector_detail,id_so_det,id_finishing_metal_detector_header,qty FROM prod_finishing_metal_detector_detail 
					)B ON A.id_finishing_metal_detector_header = B.id_finishing_metal_detector_header
					WHERE A.id_finishing_metal_detector_header ='{$id_finishing_metal_detector_header}'
				)MET_DEC ON TABLE_BALANCE.id_so_det = MET_DEC.id_so_det
		)X";
/*  echo $_sql;
die();  */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_metal_detector",
		"8"=>"tot_qty_packing",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_metal_detector",
		"12"=>"balance_plus_qty_met_dec",	
		"13"=>"next_qty",
		"14"=>"tot_qty_mat_dec"
		);
