...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_out); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			$_str = "<input type='text'"
						...
						."id='".$_attr."__".$id_reff."'" 
						."value='".$_value."'" 
						."data-balance='".$__data["balance_plus_sew_out"]."'" 
						."data-id_sew_out_det='".$__data["id_sew_out_det"]."'" 
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				...
				if($_data['data'][$i]['id_sew_out_det'] == 0 ){
					$id_reff= $i;
				} else{
	...
	public function get_main_query($id_cost,$id_sew_out){
$sql_balance = sql_balance_from_so($id_cost);
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qty_sew_out + ifnull(SEW_OUT.qty_sew_out,0))balance_plus_sew_out
			,TABLE_BALANCE.urut
			,ifnull(SEW_OUT.id_sew_out_det,0) id_sew_out_det
			,ifnull(SEW_OUT.qty_sew_out,0)qty_sew_out
			,(TABLE_BALANCE.tot_qty_sew_qc_in) next_qty
			,(TABLE_BALANCE.tot_qty_sew_out - ifnull(SEW_OUT.qty_sew_out,0) ) qty_current
			FROM(
				{$sql_balance}
			)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_out_header
							,A.date_output
							,A.no_sew_out
							,A.notes
							,A.id_cost
							...
							,B.id_so_det
							,B.qty qty_sew_out
					FROM prod_sew_out_header A
					INNER JOIN(
						SELECT id,id_so_det,id_sew_out_header,qty FROM sew_out 
					)B ON A.id_sew_out_header = B.id_sew_out_header
					WHERE A.id_sew_out_header ='{$id_sew_out}'
				)SEW_OUT ON TABLE_BALANCE.id_so_det = SEW_OUT.id_so_det
...
die();  */
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qty_sew_out",
		"8"=>"tot_qty_sew_out",
		"9"=>"id_sew_out_det",
		"10"=>"id_so_det",
		"11"=>"qty_sew_out",
		"12"=>"balance_plus_sew_out",
		"13"=>"next_qty",
		"14"=>"tot_qty_sew_in",
		"15"=>"qty_current"
		);
