...
if($code == '1'){
		if($format == '1'){
			$no_mut_dc_set 			= $Proses->get_no_mut_dc_set($header->date_output);
			$key 						= $Proses->insert($header,$_SESSION['username'],$no_mut_dc_set);
			$id_mut_dc_set_header  	= $Proses->get_id($no_mut_dc_set);
			$key 						= $Proses->insert_detail($header,$detail,$id_mut_dc_set_header,$_SESSION['username']);
			$result 					= '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_mut_dc_set_header" : "'.$id_mut_dc_set_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_mut_dc_set_header = $header->id_mut_dc_set_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_mut_dc_set($_date_output){
		$category ="MUT/DCSET";
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_mut_dc_set = $category."/".$_month.$seq_years."/".$seq;
			return $_no_mut_dc_set;
	}	
	public function get_id($_no_mut_dc_set){
		$sql = "SELECT id_mut_dc_set_header FROM prod_mut_dc_set_header WHERE no_mut_dc_set = '{$_no_mut_dc_set}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_mut_dc_set_header'];
	}
	public function insert($data,$username,$no_mut_dc_set)
	{
		...
		$insert = "INSERT INTO prod_mut_dc_set_header (notes,id_cost,username,dateinput,date_output,no_mut_dc_set,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$username}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_mut_dc_set_header SET notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_dc_set_header = '{$data->id_mut_dc_set_header}'";
	...
	public function insert_detail($header,$_detail,$id_mut_dc_set_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_mut_dc_set_detail(id_so_det,id_mut_dc_set_header,qty,username,dateinput) VALUES ";
...
/*  		print_r($_detail);
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_mut_dc_set_detail WHERE id_mut_dc_set_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					

					
						$update = "UPDATE prod_mut_dc_set_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_dc_set_detail = '{$_det->is_id_det}'";
