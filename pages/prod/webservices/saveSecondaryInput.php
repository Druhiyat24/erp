...
if($code == '1'){
		if($format == '1'){
			$no_sec_in 			= $Proses->get_no($header->tanggalinput);		
			$key 				= $Proses->insert($header,$_SESSION['username'],$no_sec_in);
			$id_sec_in_header  	= $Proses->get_id($no_sec_in);
			$key 				= $Proses->insert_detail($header,$detail,$id_sec_in_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sec_in_header" : "'.$id_sec_in_header.'"}';
			// $result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sec_in_header" : "0"}';
			echo $result;
			die();
		}else if($format == '2'){
			$id_sec_in_header = $header->id_sec_in_header;
			$key = $Proses->delete_id_panel($detail,$id_sec_in_header);
			$key = $Proses->delete_id_proses($detail,$id_sec_in_header);
...
class Proses {
	...
	public function get_no($_tanggalinput){
		...
		$_no_sec_in = $category."/".$_month.$seq_years."/".$seq;
		return $_no_sec_in;
	}


	public function insert($data,$username,$no_sec_in){
		// print_r($no_sec_in);die();
		$connect = $this->connect();
		...
		$insert = "INSERT INTO prod_sec_in_header (notes,id_cost,username,dateinput,date_output,no_sec_in,inhouse_subkon,dept_subkon) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$username}',
	...
	public function get_id($_no_sec_in){
		// $_no_sec_ini = $_no_sec_in - 1;
		$sql = "SELECT id_sec_in_header FROM prod_sec_in_header WHERE no_sec_in = '{$_no_sec_in}' LIMIT 1";
		// echo $sql;die();
		$tmp_array = $this->eksekusi_query($sql,"get_id");
		
		return $tmp_array[0]['id_sec_in_header'];
	}


	public function insert_detail($header,$_detail,$id_sec_in_header,$username){
		$connect = $this->connect();
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_sec_in_detail(id_item,id_panel,id_proses,id_so_det,id_sec_in_header,qty_sec_in,username,dateinput,qty_reject_sec_in) VALUES ";
	...
	public function delete_id_panel($detail,$id_sec_in_header){
		$populasi_id_panel = $this->ekstrak_array($detail,'is_id_panel');
		
		$del = "DELETE FROM prod_sec_in_detail WHERE id_sec_in_header = {$id_sec_in_header} AND id_panel NOT IN ({$populasi_id_panel})";
		$this->eksekusi_query_insert_update($del,'delete_id_panel');
		return 1;	
	}

	
	public function delete_id_proses($detail,$id_sec_in_header){	
		$populasi_id_proses = $this->ekstrak_array($detail,'is_id_proses');
		
		$del = "DELETE FROM prod_sec_in_detail WHERE id_sec_in_header = {$id_sec_in_header} AND id_proses NOT IN ({$populasi_id_proses})";
	...
	public function update($data,$username){
		$d_out =  date("Y-m-d", strtotime($data->tanggalinput));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sec_in_header SET dept_subkon='{$data->deptsubcon}',inhouse_subkon='{$data->inhousesubcon}', notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sec_in_header = '{$data->id_sec_in_header}'";
	...
	public function update_detail($header,$_detail,$username){
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_sec_in_detail WHERE id_sec_in_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}
				else{
					$update = "UPDATE prod_sec_in_detail SET  qty_reject_sec_in='{$_det->is_rpr}', qty_sec_in='{$_det->is_qty}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sec_in_detail = '{$_det->is_id_det}'";
