...
$id_sew_qc_final_out_header            		 =$_GET['id_url'];

if(!ISSET($id_cost)){
	$id_cost = 'X';
}

if(!ISSET($id_sew_qc_final_out_header )){
	$id_sew_qc_final_out_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_qc_final_out_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-qty_current='".$__data["qty_current"]."' data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_qcf_out"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."'data-next_qty='".$__data["next_qty"]."'>";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_sew_qc_final_out_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qc_final_output + ifnull(QCF_OUT.qty_qc_final_out,0) + ifnull(QCF_OUT.qty_ds,0))balance_plus_qty_qcf_out
			,TABLE_BALANCE.urut
			,ifnull(QCF_OUT.id_det,0) id_det
			,ifnull(QCF_OUT.qty_qc_final_out,0)qty_qc_final_out
			,ifnull(QCF_OUT.rpr,0) rpr
			,ifnull(QCF_OUT.id_defect,0)id_defect
			,ifnull(QCF_OUT.qty_ds,0)qty_ds
			,(TABLE_BALANCE.tot_qty_packing  ) next_qty
			,(TABLE_BALANCE.tot_qty_qc_final_out - ifnull(QCF_OUT.qty_qc_final_ouT,0) ) qty_current
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_qc_final_out_header
							,A.date_output
							,A.no_sew_qc_final_out
							,A.notes
							,A.id_cost
							,A.id_line
							,A.username
							...
							,B.id id_det
							,B.id_so_det
							,B.qty qty_qc_final_out
							,B.qty_ds
							,B.rpr
							,B.id_defect							
					FROM prod_qc_final_out_header A
					LEFT JOIN(
						SELECT id,rpr,id_defect,id_so_det,id_sew_qc_final_out_header,ifnull(qty,0)qty,ifnull(qty_ds,0)qty_ds FROM prod_qc_final_out_detail 
					)B ON A.id_sew_qc_final_out_header = B.id_sew_qc_final_out_header
					WHERE A.id_sew_qc_final_out_header ='{$id_sew_qc_final_out_header}'
				)QCF_OUT ON TABLE_BALANCE.id_so_det = QCF_OUT.id_so_det
		)X";
/*   echo $_sql;
die();   */
		return $_sql;
	...
	public function populasi_attribut(){
		return array(
		"0"=>"is___qty_qc_final_out",
		"1"=>"is___rpr",
		"2"=>"is___select___defect",
		"3"=>"is___qty_ds"	
		);
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qc_final_out",
		"8"=>"tot_qty_qc_final_in",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_qc_final_out",
		"12"=>"balance_plus_qty_qcf_out",
		"13"=>"rpr",
		"14"=>"id_defect",
		"15"=>"next_qty",
		"16"=>"tot_qty_qc_final_out",
		"17"=>"qty_ds",
		"18"=>"qty_current"
		);
