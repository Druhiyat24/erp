...
$id_mut_qc_fin_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_mut_qc_fin_header )){
	$id_mut_qc_fin_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_mut_qc_fin_header); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		//die();die();
		//$_value 	= ($__data["id_mut_qc_fin_detail"] > 0 ? $__data[$_field] : $__data["balance_plus_qty_mut"] );
		$_value 	= $__data["qty_mut"];
		$checkeds 	= ($__data["id_mut_qc_fin_detail"] > 0 ? "checked" : "" );
		$onclick  	= ($__data["balance_plus_qty_mut"] < 1  ? "onclick='return false'" : "" );
		$_str ="";
		if($_attr == "is___checklist"){
 		$_str = "<input type='checkbox' data-qty='".$_value."'  "
					."id='".$_attr."__".$id_reff."'" 
					."data-balance='".$__data["balance_plus_qty_mut"]."'" 
					."data-id_det='".$__data["id_mut_qc_fin_detail"]."'" 
					."data-id_so_det='".$id_so_det."'"
					."data-qty='".$__data["qty_mut"]."'"
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_mut_qc_fin_detail
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_mut_qc_fin_detail'] == 0 ){
					$id_reff= $i;
				} else{
					$id_reff= $_data['data'][$i]['id_mut_qc_fin_detail'];
				}
	...
	public function get_main_query($id_cost,$id_mut_qc_fin_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.unit
			,TABLE_BALANCE.tot_qty_qotiti
			,TABLE_BALANCE.tot_qty_mut_qc_fin_out
			,TABLE_BALANCE.balance_mut_qotiti
			,TABLE_BALANCE.urut
			,ifnull(MUT_D.id_mut_qc_fin_header,0)id_header
			,if(ifnull(MUT_D.id_mut_qc_fin_detail,0) = '0',TABLE_BALANCE.balance_mut_qotiti,ifnull(MUT_D.qty_mut,0))qty_transfer
			,ifnull(MUT_D.id_mut_qc_fin_detail,0) id_mut_qc_fin_detail
			,ifnull(MUT_D.qty_mut,0)qty_mut
			,(TABLE_BALANCE.balance_mut_qotiti + ifnull(MUT_D.qty_mut,0))balance_plus_qty_mut
			,(TABLE_BALANCE.tot_qty_packing  ) next_qty
			,(TABLE_BALANCE.tot_qty_mut_qc_fin_out - ifnull(MUT_D.qty_mut,0) ) qty_current
			FROM(
			{$sql_balance}
		)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_mut_qc_fin_header
							,A.date_output
							,A.notes
							,A.id_cost
							,A.id_line
							...
							,A.dateinput
							,A.time
							,B.id_mut_qc_fin_detail
							,B.id_so_det
							,B.qty qty_mut
					FROM prod_mut_qc_fin_header A
					INNER JOIN(
						SELECT id_mut_qc_fin_detail,id_so_det,id_mut_qc_fin_header,qty FROM prod_mut_qc_fin_detail 
					)B ON A.id_mut_qc_fin_header = B.id_mut_qc_fin_header
					WHERE A.id_mut_qc_fin_header ='{$id_mut_qc_fin_header}'
				)MUT_D	ON TABLE_BALANCE.id_so_det = MUT_D.id_so_det
		)X";
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_mut_qotiti",
		"8"=>"tot_qty_mut_qc_fin_out",
		"9"=>"id_mut_qc_fin_detail",
		"10"=>"id_so_det",
		"11"=>"qty_mut",
		"12"=>"balance_plus_qty_mut",
		"13"=>"next_qty",
		"14"=>"tot_qty_qotiti",
		"15"=>"qty_transfer",
		"16"=>"id_header",
