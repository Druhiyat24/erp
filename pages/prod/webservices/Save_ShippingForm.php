...
if($code == '1'){
		if($format == '1'){
			$no_finishing_shp 	= $Proses->get_no_finishing_shp($header->date_output);
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_finishing_shp);
			$id_finishing_shp_header  = $Proses->get_id($no_finishing_shp);
			$key 		= $Proses->insert_detail($header,$detail,$id_finishing_shp_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_finishing_shp_header" : "'.$id_finishing_shp_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_finishing_shp_header = $header->id_finishing_shp_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_finishing_shp($_date_output){
		$category ="SHP";
		...
		if((intval($populasi_pool['seq']) > 0)){
			if(intval($populasi_pool['years']) == intval($_years)){
				...
				$seq   =sprintf('%05d', (intval($populasi_pool["seq"])+ 1));
			...
			$_no_finishing_shp = $category."/".$_month.$seq_years."/".$seq;
			return $_no_finishing_shp;
	}	
	public function get_id($_no_finishing_shp){
		$sql = "SELECT id_finishing_shp_header FROM prod_finishing_shp_header WHERE no_finishing_shp = '{$_no_finishing_shp}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_finishing_shp_header'];
	}
	public function insert($data,$username,$no_finishing_shp)
	{
		...
		$insert = "INSERT INTO prod_finishing_shp_header (notes,id_cost,id_line,username,dateinput,date_output,no_finishing_shp,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_finishing_shp_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_finishing_shp_header = '{$data->id_finishing_shp_header}'";
	...
	public function insert_detail($header,$_detail,$id_finishing_shp_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_finishing_shp_detail(id_so_det,id_finishing_shp_header,qty,username,dateinput) VALUES ";
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_finishing_shp_detail WHERE id_finishing_shp_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE prod_finishing_shp_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_finishing_shp_detail = '{$_det->is_id_det}'";
				...
				if($_det->is_qty > 0){
					...
					$insert .=" 
									'{$_det->is_id_so_det}',
									'{$header->id_finishing_shp_header}',
									'{$_det->is_qty}',
									'{$username}',
									'{$dt}'
									";		
