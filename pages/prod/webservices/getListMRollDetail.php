...
// print_r($id_so);die();
$query = "SELECT id,id_cost FROM so WHERE id = '{$id_so}'";
$stmtSQL = mysql_query($query);
$rowSQL = mysql_fetch_array($stmtSQL);
$cost = $rowSQL['id_cost'];
...
$table = "(
		...
		FROM (
			...
			FROM (
				SELECT
					...
					mr.roll_no,
					mr.qty_sticker,
					(mr.qty_sticker - mr.binding - mr.actual_balance) AS fabric_used,
					mr.qty_cutting,
					mr.cons_ws,
					mr.cons_m,
					((mr.qty_sticker - mr.binding - mr.actual_balance) / mr.qty_cutting) AS cons_act,
					(((mr.qty_sticker - mr.binding - mr.actual_balance) / mr.qty_cutting) - mr.cons_ws) AS cons_balance,
					(((((mr.qty_sticker - mr.binding - mr.actual_balance) / mr.qty_cutting) - mr.cons_ws) / mr.cons_ws) * 100) AS percentage,
					mr.binding,
					mr.actual_balance,
					(mr.binding + mr.actual_balance + (mr.ampar * mr.spread_sheet)) AS actual_total,
					((mr.binding + mr.actual_balance + (mr.ampar * mr.spread_sheet)) - mr.qty_sticker) AS short_roll,
					mr.spread_sheet,
					srn.ratio,
					(srn.ratio * mr.spread_sheet) AS qty_pcs,
					srn.p_markers,
					srn.efficiency,
					(((mr.ampar * mr.spread_sheet) * srn.efficiency) / 100) AS fabric_total_act,
					(100 - srn.efficiency) AS majun,
					((mr.ampar * mr.spread_sheet) - (((mr.ampar * mr.spread_sheet) * srn.efficiency) / 100)) AS majun_kg,
					mr.ampar,
					(mr.ampar) AS total,
					(mr.ampar * mr.spread_sheet) AS used_total,
					mr.l_fabric
				...
				INNER JOIN (
					...
					INNER JOIN (
						...
						WHERE med.color = '{$id_color}' AND med.id_item = '{$id_item}' AND med.id_cost = '{$cost}'
				...
				WHERE mr.fabric_color = '{$id_color}' AND mr.id_item = '{$id_item}' AND mr.id_cost = '{$cost}'
				GROUP BY mr.id_m_roll
			) AS mgm
			...
			FROM (
				...
				INNER JOIN (
					...
					INNER JOIN (
						SELECT 
							br.id,
							br.roll_no,
							IF(br.lot_no = '' OR br.lot_no IS NULL, 'N/A', br.lot_no) AS lot_no,
							br.roll_qty
						FROM bpb_roll AS br
				...
				INNER JOIN (
					...
					WHERE vpb.id_item_ms_item = '{$id_item}' AND vpb.color = '{$id_color}' AND vpb.id_cost = '{$cost}'
	...
	SELECT 
		...
		ROLL.id_item,
		ROLL.id_number,
		COUNT(ROLL.id_number) as count_duplicated,
		ROLL.id_panel,
		ROLL.nama_panel,
		ROLL.fabric_code,
		ROLL.fabric_name,
	...
	) AS ROLL GROUP BY ROLL.id_number,ROLL.roll_no HAVING count_duplicated > 0 AND ROLL.id_m_roll = ''
) AS X";
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	if($row['cons_act'] == ''){
		...
		$consAct = number_format($row['cons_act'], 3, ".", ",");
	}
	$inputConsAct = '';
	...
	if($row['cons_balance'] == ''){
		...
		$consBal = number_format($row['cons_balance'], 3, ".", ",");
	}
	$inputConsBal = '';
	...
	if($row['percentage'] == ''){
		...
		$percent = number_format($row['percentage'], 3, ".", ",");
	}
	$inputPercent = '';
