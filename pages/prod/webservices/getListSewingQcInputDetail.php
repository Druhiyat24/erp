...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_qc_input); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			$_str = "<input type='text' data-qty_current='".$__data["qty_current"]."' data-balance='".$__data["balance_plus_sew_qc_in"]."' data-id_sew_qc_in_det='".$__data["id_sew_qc_in_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				...
				if($_data['data'][$i]['id_sew_qc_in_det'] == 0 ){
					$id_reff= $i;
				} else{
	...
	public function get_main_query($id_cost,$id_sew_qc_input){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qty_sew_qc_in + ifnull(SEW_QC_IN.qty_sew_qc_in,0))balance_plus_sew_qc_in
			,TABLE_BALANCE.urut
			,ifnull(SEW_QC_IN.id_sew_qc_input_det,0) id_sew_qc_in_det
			,ifnull(SEW_QC_IN.qty_sew_qc_in,0)qty_sew_qc_in
			,(TABLE_BALANCE.tot_qty_sew_qc_out ) next_qty
			,(TABLE_BALANCE.tot_qty_sew_qc_in - ifnull(SEW_QC_IN.qty_sew_qc_in,0) ) qty_current
			FROM(
				{$sql_balance}
			)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_qc_in_header
							,A.date_output
							,A.no_sew_qc_in
							,A.notes
							,A.id_cost
							...
							,A.dateinput
							,A.time
							,B.id_sew_qc_in_detail id_sew_qc_input_det
							,B.id_so_det
							,B.qty_good qty_sew_qc_in
					FROM prod_qc_in_header A
					INNER JOIN(
						SELECT id_sew_qc_in_detail,id_so_det,id_sew_qc_in_header,qty_good FROM prod_qc_in_detail 
					)B ON A.id_sew_qc_in_header = B.id_sew_qc_in_header
					WHERE A.id_sew_qc_in_header ='{$id_sew_qc_input}'
				)SEW_QC_IN ON TABLE_BALANCE.id_so_det = SEW_QC_IN.id_so_det
...
die();   */
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qty_sew_qc_in",
		"8"=>"tot_qty_sew_out",
		"9"=>"id_sew_qc_in_det",
		"10"=>"id_so_det",
		"11"=>"qty_sew_qc_in",
		"12"=>"balance_plus_sew_qc_in",
		"13"=>"next_qty",
		"14"=>"tot_qty_sew_qc_in",
		"15"=>"qty_current"
		);
