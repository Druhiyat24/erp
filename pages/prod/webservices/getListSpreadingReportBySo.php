...
SELECT A.id_mark_entry
		...
		WHERE A.isDelete = '0' AND A.id_mark_entry='{$id}' GROUP BY so.id
) AS X";
### Search 
$searchQuery = " ";
if($searchValue != ''){
...
//echo $row['n_post'];
	...
	$button .="<a href='../prod/?mod=SpreadingReportBySoColor&id=$row[id_mark_entry]&id_so=$row[id_so]' style='color: #3c8dbc;' title='View' class='btn btn-sm'  ><i class='fa fa-eye'> </i></a>";
