...
$id_qotiti_out_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_qotiti_out_header )){
	$id_qotiti_out_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_qotiti_out_header); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		if(ISSET($__data[$_field])){
			$_value 	= ($__data["id_qotiti_out_detail"] > 0 ? $__data[$_field] : "0" );
		}else{
			$_value 	= ($__data["id_qotiti_out_detail"] > 0 ? "" : "0" );
		}
		//echo "VAL=".$__data[$_field];echo "VAL=".$__data[$_field];
		//die();die();
		$checkeds 	= ($__data["id_qotiti_out_detail"] > 0 ? "checked" : "" );
		$onclick  	= ($__data["balance_plus_qty_qot_out"] < 1  ? "onclick='return false'" : "" );
		$_str ="";
		if($_attr == "is___checklist"){
 		$_str = "<input type='checkbox' data-qty='".$_value."'  "
					."id='".$_attr."__".$id_reff."'" 
					."data-balance='".$__data["balance_plus_qty_qot_out"]."'" 
					."data-id_det='".$__data["id_qotiti_out_detail"]."'" 
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_qotiti_out_detail
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_qotiti_out_detail'] == 0 ){
					$id_reff= $i;
				} else{
					$id_reff= $_data['data'][$i]['id_qotiti_out_detail'];
				}
	...
	public function get_main_query($id_cost,$id_qotiti_out_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.balance_qotiti
			,TABLE_BALANCE.urut
			,ifnull(QOT_OUT.id_qotiti_out_detail,0) id_qotiti_out_detail
			,ifnull(QOT_OUT.qty_qot_out,0)qty_qot_out
			,ifnull(QOT_OUT.rpr,0)rpr
			,(TABLE_BALANCE.balance_qotiti + ifnull(QOT_OUT.qty_qot_out,0))balance_plus_qty_qot_out
			,(TABLE_BALANCE.tot_qty_mut_qc_fin_out) next_qty
			,(TABLE_BALANCE.tot_qty_qotiti - ifnull(QOT_OUT.qty_qot_out,0) ) qty_current
			FROM(
			{$sql_balance}
		)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_qotiti_out_header
							,A.date_output
							,A.notes
							,A.id_cost
							,A.id_line
							...
							,B.qty qty_qot_out
							,B.rpr
					FROM prod_qotiti_out_header A
					LEFT JOIN(
						SELECT id_qotiti_out_detail,id_so_det,id_qotiti_out_header,qty,rpr FROM prod_qotiti_out_detail 
					)B ON A.id_qotiti_out_header = B.id_qotiti_out_header
					WHERE A.id_qotiti_out_header ='{$id_qotiti_out_header}'
				)QOT_OUT	ON TABLE_BALANCE.id_so_det = QOT_OUT.id_so_det
		)X";
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"9"=>"id_qotiti_out_detail",
		"10"=>"id_so_det",
		"11"=>"qty_qot_out",
		"12"=>"balance_plus_qty_qot_out",
		"13"=>"next_qty",
		"14"=>"tot_qty_qotiti",
		"15"=>"rpr",
		"16"=>"qty_current"
