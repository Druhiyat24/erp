...
$data = $_POST;
$code = $_POST['code'];
$id_steam_in_header = $_POST['id'];
 //print_r($id_sew_qc_in );die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_steam_in_header);
...
class Proses {
	...
	public function get_list_data($id_steam_in_header){
		$sql = "SELECT id_steam_in_header,date_output,no_steam_in,notes,id_cost,id_line,username,dateinput,time 
				FROM prod_sew_steam_in_header WHERE id_steam_in_header ='{$id_steam_in_header}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_steam_in_header":"'.rawurlencode($row[$i]['id_steam_in_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_steam_in":"'. rawurlencode($row[$i]["no_steam_in"]).'",';	
