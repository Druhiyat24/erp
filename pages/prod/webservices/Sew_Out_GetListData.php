...
$code = $_POST['code'];
$id_sew_out = $_POST['id'];
 //print_r($id_sew_out);die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_sew_out);
...
class Proses {
	...
	public function get_list_data($id_sew_out){
		$sql = "SELECT id_sew_out_header,date_output,no_sew_out,notes,id_cost,id_line,username,dateinput,time 
					FROM prod_sew_out_header WHERE id_sew_out_header ='{$id_sew_out}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_sew_out_header":"'.rawurlencode($row[$i]['id_sew_out_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_sew_out":"'. rawurlencode($row[$i]["no_sew_out"]).'",';	
