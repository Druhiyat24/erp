...
// $query = "SELECT id,id_cost FROM so WHERE id = '{$so}'";
...
// if($row['color'] == $color && $row['item'] == $item && $row['cost'] == $cost){
// 	// echo "Update";
// 	$format = '2';
// }
// else{
...
if($code == '1'){
	$Proses = new Proses();
		if($format == '1'){
			$key = $Proses->insert($detail,$_SESSION['username']);
...
class Proses {
	...
	public function insert($detail,$username){
		// print_r($detail);die();
		$connect = $this->connect();
		$dt = date("Y-m-d h:i:s");
