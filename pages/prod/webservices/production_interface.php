...
function sql_balance_from_so($id_cost){
	if($id_cost == "REPORT"){
		...
		$tot_qty_dc_set = "tot_qty_dc_sets";
	}else{
		$sql_where = "AND ACT.id = '{$id_cost}'";
		$group_by  ="GROUP BY SOD.id";
		$tot_qty_dc_set = "tot_qty_dc_set";
...
/* 	print_r($status);
	...
	$field_sec			= generate_field(field_balance_sec(),'Y','SEC');
	$field_sec_main		= generate_field(field_balance_sec(),'N','SEC');
	$field_cutting 		= generate_field(field_balance_cutting(),'Y','CUTTING');
	$field_cutting_main = generate_field(field_balance_cutting(),'N','CUTTING');
	$from 				 =  date("Y-m-d", strtotime($from));
	$to 				 =  date("Y-m-d", strtotime($to));
	$sql_balance_cutting = sql_balance_cutting("REPORT");
	$sql_balance_sec 	 = sql_balance_secondary_sec("REPORT");
	...
	SELECT  
			...
			FROM(
				...
				INNER JOIN ({$sql_balance_cutting})BALANCE_CUTTING ON BALANCE_CUTTING.id_cost = BALANCE.id_cost
				LEFT JOIN ({$sql_balance_sec})BALANCE_SEC ON BALANCE_SEC.id_cost = BALANCE.id_cost
				INNER JOIN(
...
function generate_field($pop,$sub_query,$category){
	$trigger = count($pop_field_cutting) - 1;	
	$str = "";
	$ifnull_buka =""; 
	$ifnull_tutup ="";	
	if($sub_query== 'Y'){
		if($category == "CUTTING"){
			$deliver = "BALANCE_CUTTING";
		}else{
			$deliver = "BALANCE_SEC";
			$ifnull_buka = "ifnull(";
			$ifnull_tutup = ",0)";
	...
	for($i=0;$i<count($pop);$i++){
		$str .="\n{$ifnull_buka}{$deliver}.{$pop[$i]}{$ifnull_tutup} {$pop[$i]},";
	}
	return $str;
}

function field_balance_cutting(){
		return array(
			"0"=>"tot_qty_cut_out",
			"1"=>"balance_qty_cut_out",
			"2"=>"tot_qty_cut_num",
			"3"=>"balance_qty_cut_num",
			"4"=>"tot_qty_cut_qc",
			"5"=>"balance_qty_cut_qc",
			"6"=>"tot_qty_dc_join",
			"7"=>"balance_qty_dc_join",
			"8"=>"tot_qty_dc_set",
			"9"=>"balance_qty_dc_set"
		);
}


function field_balance_sec(){
		return array(
			"0"=>"tot_qty_sec_in",
			"1"=>"balance_sec_in",
			"2"=>"tot_qty_sec_out",
...
function populasi_table(){
		return array( //prod_cut_out
			"0"=>"prod_cut_out",
		//	"1"=>"prod_mut_dc_set_header",
		//	"2"=>"prod_sew_in_header",
		//	"3"=>"prod_sew_out_header",
		//	"4"=>"prod_qc_in_header",
		//	"5"=>"prod_qc_out_header",
		//	"6"=>"prod_mut_sew_header",
		//	"7"=>"prod_sew_steam_in_header",
		//	"8"=>"prod_sew_steam_out_header",
		//	"9"=>"prod_qc_final_in_header",
		//	"10"=>"prod_qotiti_out_header",
		//	"11"=>"prod_mut_qc_fin_header",
		//	"12"=>"prod_finishing_packing_header",
		//	"13"=>"prod_finishing_metal_detector_header",
		//	"14"=>"prod_finishing_qc_audit_header",
		//	"15"=>"prod_finishing_fg_header",
		//	"16"=>"prod_finishing_shp_header"
		);	
...
function sql_balance_cutting($id_cost){
	if($id_cost =="REPORT"){
		$where_id_cost = "";
		$group_by ="GROUP BY ACT.id";
		$tot_qty_cut_out = "SUM(IFNULL(COD_D.tot_qty_cut_out, 0))";
		$tot_qty_cut_num = "SUM(IFNULL(CND_D.tot_qty_cut_num, 0))";
		$tot_qty_cut_qc  = "SUM(IFNULL(CQD_D.tot_qty_cut_qc, 0))";
		$tot_qty_dc_join = "SUM(IFNULL(DJD_D.tot_qty_dc_join, 0))";
		$tot_qty_dc_set  = "SUM(IFNULL(DSD_D.tot_qty_dc_set, 0))";
	}else{
		$where_id_cost = "AND ACT.id = '{$id_cost}'";
		$group_by ="GROUP BY SOD.id,SOD.color,COD_D.id_panel,COD_D.id_item,COD_D.lot";
		$tot_qty_cut_out = "(IFNULL(COD_D.tot_qty_cut_out, 0))";
		$tot_qty_cut_num = "(IFNULL(CND_D.tot_qty_cut_num, 0))";
		$tot_qty_cut_qc  = "(IFNULL(CQD_D.tot_qty_cut_qc, 0))";
		$tot_qty_dc_join = "(IFNULL(DJD_D.tot_qty_dc_join, 0))";
		$tot_qty_dc_set  = "(IFNULL(DSD_D.tot_qty_dc_set, 0))";
	}
	...
	$sql = "
		SELECT
			...
			/* cutting out */
			{$tot_qty_cut_out} AS tot_qty_cut_out,
			(
				SUM(IFNULL(SOD.qty, 0)) - {$tot_qty_cut_out}
			) AS balance_qty_cut_out,
				
			/* numbering */
			{$tot_qty_cut_num} AS tot_qty_cut_num,
			(
				SUM(IFNULL(COD_D.tot_qty_cut_out, 0)) - {$tot_qty_cut_num}
			) AS balance_qty_cut_num,
			
			/* qc */
			{$tot_qty_cut_qc} AS tot_qty_cut_qc,
			(
				SUM(IFNULL(CND_D.tot_qty_cut_num, 0)) - {$tot_qty_cut_qc}
			) AS balance_qty_cut_qc,

			/* dc join */
			{$tot_qty_dc_join} AS tot_qty_dc_join,
			(
				#SUM(IFNULL(CQD_D.tot_qty_cut_qc, 0)) - IFNULL(DJD_D.tot_qty_dc_join, 0)
				SUM(IFNULL(SQD_D.tot_qty_sec_qc, CQD_D.tot_qty_cut_qc)) - {$tot_qty_dc_join}
			) AS balance_qty_dc_join,
			
			/* dc set */
			{$tot_qty_dc_set} AS tot_qty_dc_set,
			(
				SUM(IFNULL(DJD_D.tot_qty_dc_join, 0)) - {$tot_qty_dc_set}
...
function sql_balance_secondary_sec($id_cost){
	$sql = "SELECT 
				X.cutting_number,
				X.bundle_number,
				X.sack_number,
				...
				X.nama_item,
				X.nama_panel,
				SUM(X.tot_qty_cut_out)tot_qty_cut_out,
				SUM(X.balance_qty_cut_out)balance_qty_cut_out,
				SUM(X.tot_qty_sec_in)tot_qty_sec_in,
				SUM(X.balance_sec_in)balance_sec_in,
				SUM(X.tot_qty_sec_out)tot_qty_sec_out,
				SUM(X.balance_sec_out)balance_sec_out,			
				SUM(X.tot_qty_sec_qc)tot_qty_sec_qc,
				SUM(X.balance_sec_qc)balance_sec_qc	
	FROM (".sql_balance_secondary_from_cutting($id_cost).")X WHERE X.tot_qty_sec_in > 0 GROUP BY X.id_cost";
	return $sql;
}
...
function sql_balance_secondary_from_cutting($id_cost){
	if($id_cost =="REPORT"){
		$where_id_cost = "";
		$group_by ="GROUP BY ACT.id,SOD.id,SOD.color,COD_D.id_panel,COD_D.id_item";
	}else{
		$where_id_cost = "AND ACT.id = '{$id_cost}'";
		$group_by ="GROUP BY SOD.id,SOD.color,COD_D.id_panel,COD_D.id_item";
	}	
