...
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_populasi_line();
...
class Proses {
	...
	public function get_populasi_line(){
		$sql = "SELECT id id,kpno isi FROM act_costing WHERE id IN(SELECT id_cost FROM prod_sew_out_header GROUP BY id_cost) GROUP BY id";
		$row = $this->eksekusi_query($sql,"get_populasi_line");
		$_pop_size = array();
		$outp = '';
