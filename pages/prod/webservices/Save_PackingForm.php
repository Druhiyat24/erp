...
if($code == '1'){
		if($format == '1'){
			$no_finishing_packing 	= $Proses->get_no($header->date_output);
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_finishing_packing);
			$id_finishing_packing_header  = $Proses->get_id($no_finishing_packing);
			$key 		= $Proses->insert_detail($header,$detail,$id_finishing_packing_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_finishing_packing_header" : "'.$id_finishing_packing_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_finishing_packing_header = $header->id_finishing_packing_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no($_date_output){
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_finishing_packing = $category."/".$_month.$seq_years."/".$seq;
			return $_no_finishing_packing;
	}	
	public function get_id($_no_finishing_packing){
		$sql = "SELECT id_finishing_packing_header FROM prod_finishing_packing_header WHERE no_finishing_packing = '{$_no_finishing_packing}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_finishing_packing_header'];
	}
	public function insert($data,$username,$no_finishing_packing)
	{
		...
		$insert = "INSERT INTO prod_finishing_packing_header (notes,id_cost,id_line,username,dateinput,date_output,no_finishing_packing,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_finishing_packing_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_finishing_packing_header = '{$data->id_finishing_packing_header}'";
	...
	public function insert_detail($header,$_detail,$id_finishing_packing_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_finishing_packing_detail(id_so_det,id_finishing_packing_header,qty,username,dateinput) VALUES ";
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_finishing_packing_detail WHERE id_finishing_packing_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE prod_finishing_packing_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_finishing_packing_detail = '{$_det->is_id_det}'";
				...
				if($_det->is_qty > 0){
					...
					$insert .=" 
									'{$_det->is_id_so_det}',
									'{$header->id_finishing_packing_header}',
									'{$_det->is_qty}',
									'{$username}',
									'{$dt}'
									";		
