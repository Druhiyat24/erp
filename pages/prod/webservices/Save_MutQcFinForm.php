...
if($code == '1'){
		if($format == '1'){
			$no_mut_qc_fin 			= $Proses->get_no_mut_qc_fin($header->date_output);
			$key 						= $Proses->insert($header,$_SESSION['username'],$no_mut_qc_fin);
			$id_mut_qc_fin_header  	= $Proses->get_id($no_mut_qc_fin);
			$key 						= $Proses->insert_detail($header,$detail,$id_mut_qc_fin_header,$_SESSION['username']);
			$result 					= '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_mut_qc_fin_header" : "'.$id_mut_qc_fin_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_mut_qc_fin_header = $header->id_mut_qc_fin_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_mut_qc_fin($_date_output){
		$category ="MUT/QCFIN";
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_mut_qc_fin = $category."/".$_month.$seq_years."/".$seq;
			return $_no_mut_qc_fin;
	}	
	public function get_id($_no_mut_qc_fin){
		$sql = "SELECT id_mut_qc_fin_header FROM prod_mut_qc_fin_header WHERE no_mut_qc_fin = '{$_no_mut_qc_fin}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_mut_qc_fin_header'];
	}
	public function insert($data,$username,$no_mut_qc_fin)
	{
		...
		$insert = "INSERT INTO prod_mut_qc_fin_header (notes,id_cost,id_line,username,dateinput,date_output,no_mut_qc_fin,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_mut_qc_fin_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_qc_fin_header = '{$data->id_mut_qc_fin_header}'";
	...
	public function insert_detail($header,$_detail,$id_mut_qc_fin_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_mut_qc_fin_detail(id_so_det,id_mut_qc_fin_header,qty,username,dateinput) VALUES ";
...
die(); */
	...
	//	die(); 
		...
		for($i=0;$i<count($_detail);$i++){
			$_det = $_detail[$i];
			//$_det->is_id_det = $_det->is_id_so_det;
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_mut_qc_fin_detail WHERE id_mut_qc_fin_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
						$update = "UPDATE prod_mut_qc_fin_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_mut_qc_fin_detail = '{$_det->is_id_det}'";
