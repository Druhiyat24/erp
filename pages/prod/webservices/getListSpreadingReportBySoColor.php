...
SELECT A.id_mark_entry
		...
		FROM prod_mark_entry A
			...
			INNER JOIN act_costing D 
				ON A.id_cost = D.id
			INNER JOIN so ON so.id_cost = D.id	
			INNER JOIN so_det sod ON sod.id_so = so.id
			INNER JOIN mastersupplier E ON D.id_buyer = E.Id_Supplier
		WHERE A.isDelete = '0' AND A.id_mark_entry='{$id}' AND so.id='{$id_so}' GROUP BY sod.color
) AS X";
### Search 
$searchQuery = " ";
if($searchValue != ''){
...
//echo $row['n_post'];
	...
	$button .="<a href='../prod/?mod=SpreadingReport_list&id=$row[id_mark_entry]&id_so=$row[id_so]&color=".rawurldecode($row['color'])."' style='color: #3c8dbc;' title='View' class='btn btn-sm'  ><i class='fa fa-eye'> </i></a>";
	$data[] = array(
		"no"=>$no,
