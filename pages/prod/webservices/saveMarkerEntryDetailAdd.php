...
class Proses {
	...
	public function insert($url,$id_cost,$color,$id_panel,$id_item,$id_jo){
		...
		$group_det = "SELECT 
				MAX(id_group_det) AS id 
			FROM prod_mark_entry_detail 
			WHERE id_cost = '{$id_cost}' AND color = '{$color}'
			AND id_panel = '{$id_panel}' AND id_item = '{$id_item}'
		";
		// echo $group_det;die();
	...
	public function deletes($data){
		...
		$panel = $data->panel;
		$item = $data->item;
		$sqlG="DELETE FROM prod_mark_entry_group WHERE id_group = '{$group}'";
		$resultG = $connect->query($sqlG);

		$sql="DELETE FROM prod_mark_entry_detail WHERE id_group = '{$group}'";
		$result = $connect->query($sql);

		$checkSql = "SELECT 
				med.id_group_det,med.id_group
			FROM prod_mark_entry_detail AS med
			WHERE med.id_cost = '{$cost}' AND med.color = '{$color}' AND med.id_panel = '{$panel}' AND med.id_item = '{$item}'
			GROUP BY med.id_group_det
		";

		$det_group = 0;
		$check = mysqli_query($conn_li,$checkSql);
		$resultCheck = $connect->query($checkSql);
		while($rowC = mysqli_fetch_assoc($resultCheck)){
			// echo var_dump($rowC['id_group']);
			$det_group = $det_group + 1;
			$sqlDet = "UPDATE prod_mark_entry_detail SET
					id_group_det	= '{$det_group}'
				WHERE id_group = '{$rowC['id_group']}'
			";
			$resultP = $connect->query($sqlDet);
		}

		$checkSum = "SELECT 
				MAX(med.id_group) AS id_group
			FROM prod_mark_entry_detail AS med
			WHERE med.id_cost = '{$cost}' AND med.color = '{$color}' AND med.id_panel = '{$panel}' AND med.id_item = '{$item}'
			GROUP BY med.id_group_det
		";
		$resultSum = $connect->query($checkSum);
		$rowSum = mysqli_fetch_array($resultSum);
	
		if($rowSum['id_group'] == ''){
			$sqlSum = "DELETE FROM prod_mark_entry_sum
			WHERE id_mark_entry = '{$url}' AND color = '{$color}' AND id_panel = '{$panel}' AND id_item = '{$item}'";
			$sumResult = $connect->query($sqlSum);
			// echo $sqlSum;die();
		}
		else{
			$sqlSum = "true";
