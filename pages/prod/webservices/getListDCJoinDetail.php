...
if($id_url == '-1' OR $id_url == 'undefined'){
	$table = "(
		...
		SELECT 
			...
			join_old.tot_qty_dc_join AS before_cumulative,
			(join_old.tot_qty_dc_join + join_old.balance_qty_dc_join) AS before_balance
		FROM (
			...
			GROUP BY cod.id_grouping,cod.id_panel,cqd.id_so_det,cod.lot
		
		) AS join_new
		
		LEFT JOIN (
			{$sql_balance}
		) AS join_old ON join_old.id_so_det = join_new.id_so_det 
		AND join_old.id_panel = join_new.id_panel 
		AND join_old.id_item = join_new.id_item
		AND join_old.lot = join_new.lot
	...
	$table = "(
		...
		SELECT 
			...
			join_old.tot_qty_dc_join AS cumulative,
			join_old.balance_qty_dc_join AS balance,
			(join_old.tot_qty_dc_join - join_new.qty_dc_join) AS before_cumulative,
			(join_old.tot_qty_dc_join + join_old.balance_qty_dc_join) AS before_balance
		FROM (
			...
			GROUP BY cod.id_grouping,cod.id_panel,djd.id_so_det,cod.lot
...
$columnName = "X.color ASC,X.id_grouping ASC,X.id_panel ASC,X.lot ASC,X.id_so_det ASC"; // Column name 
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputQtyInJoin = '';
	$inputQtyInJoin .="<input type='number' id='qtyInJoin_".$num."' class='form-control forms' value='$qtyInJoin' onchange='handleKeyUp(this)' style='min-width: 80px'>";
	...
	$inputReject = '';
	$inputReject .="<input type='number' id='reject_".$num."' class='form-control forms' value='$reject' onchange='handleKeyUp(this)' style='min-width: 80px'>";
