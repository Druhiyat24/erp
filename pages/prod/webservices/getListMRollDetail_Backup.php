...
$table = "(
	...
	FROM (
		...
		FROM (
			...
			INNER JOIN prod_spread_report_detail AS srd ON srd.id_number = srn.id_number
	...
	) AS rm GROUP BY rm.id_spread_detail
) AS X";
...
// if($row['color'] == $id_color && $row['item'] == $id_item && $row['cost'] == $cost){
