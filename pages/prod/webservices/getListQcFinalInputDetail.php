...
$id_sew_qc_final_in_header     =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_sew_qc_final_in_header )){
	$id_sew_qc_final_in_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_qc_final_in_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-qty_current='".$__data["qty_current"]."' data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_qcf_in"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_sew_qc_final_in_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qc_final_input + ifnull(QCF_IN.qty_qc_final_in,0))balance_plus_qty_qcf_in
			,TABLE_BALANCE.urut
			,ifnull(QCF_IN.id_det,0) id_det
			,ifnull(QCF_IN.qty_qc_final_in,0)qty_qc_final_in
			,(TABLE_BALANCE.tot_qty_qc_final_out  ) next_qty
			,(TABLE_BALANCE.tot_qty_qc_final_in - ifnull(QCF_IN.qty_qc_final_in,0) ) qty_current
			FROM(
			{$sql_balance }
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_qc_final_in_header
							,A.date_output
							,A.no_sew_final_qc_in
							,A.notes
							,A.id_cost
							,A.id_line
							,A.username
							,A.dateinput
							,A.time
							,B.id_sew_qc_final_in_detail id_det
							,B.id_so_det
							,B.qty qty_qc_final_in
					FROM prod_qc_final_in_header A
					INNER JOIN(
						SELECT id_sew_qc_final_in_detail,id_so_det,id_sew_qc_final_in_header,qty FROM prod_qc_final_in_detail 
					)B ON A.id_sew_qc_final_in_header = B.id_sew_qc_final_in_header
					WHERE A.id_sew_qc_final_in_header ='{$id_sew_qc_final_in_header}'
				)QCF_IN ON TABLE_BALANCE.id_so_det = QCF_IN.id_so_det
		)X";
/*    echo $_sql;
die();    */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qc_final_in",
		"8"=>"tot_qty_steam_out",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_qc_final_in",
		"12"=>"balance_plus_qty_qcf_in",
		"13"=>"next_qty",
		"14"=>"tot_qty_qc_final_in",
		"15"=>"qty_current"
		);
	}
}
