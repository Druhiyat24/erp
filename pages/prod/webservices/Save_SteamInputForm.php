...
if($code == '1'){
		if($format == '1'){
			$no_steam_in 			= $Proses->get_no_steam_in($header->date_output);
			$key 						= $Proses->insert($header,$_SESSION['username'],$no_steam_in);
			$id_steam_in_header  	= $Proses->get_id($no_steam_in);
			$key 						= $Proses->insert_detail($header,$detail,$id_steam_in_header,$_SESSION['username']);
			$result 					= '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_steam_in_header" : "'.$id_steam_in_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_steam_in_header = $header->id_steam_in_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_steam_in($_date_output){
		$category ="SEW/STE/IN";
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_steam_in = $category."/".$_month.$seq_years."/".$seq;
			return $_no_steam_in;
	}	
	public function get_id($_no_steam_in){
		$sql = "SELECT id_steam_in_header FROM prod_sew_steam_in_header WHERE no_steam_in = '{$_no_steam_in}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_steam_in_header'];
	}
	public function insert($data,$username,$no_steam_in)
	{
		...
		$insert = "INSERT INTO prod_sew_steam_in_header (notes,id_cost,id_line,username,dateinput,date_output,no_steam_in,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sew_steam_in_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_steam_in_header = '{$data->id_steam_in_header}'";
	...
	public function insert_detail($header,$_detail,$id_steam_in_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_sew_steam_in_detail(id_so_det,id_steam_in_header,qty,username,dateinput) VALUES ";
...
/* 		print_r($_detail);
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_sew_steam_in_detail WHERE id_steam_in_detail = '{$_det->is_id_det}'";
					...
					if($_det->is_checklist == '0'){
						...
						$update = "UPDATE prod_sew_steam_in_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_steam_in_detail = '{$_det->is_id_det}'";
