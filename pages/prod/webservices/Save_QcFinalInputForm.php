...
if($code == '1'){
		if($format == '1'){
			$no_sew_final_qc_in 	= $Proses->get_no($header->date_output);
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_sew_final_qc_in);
			$id_sew_qc_final_in_header  = $Proses->get_id($no_sew_final_qc_in);
			$key 		= $Proses->insert_detail($header,$detail,$id_sew_qc_final_in_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sew_qc_final_in_header" : "'.$id_sew_qc_final_in_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_sew_qc_final_in_header = $header->id_sew_qc_final_in_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no($_date_output){
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_sew_final_qc_in = $category."/".$_month.$seq_years."/".$seq;
			return $_no_sew_final_qc_in;
	}	
	public function get_id($_no_sew_final_qc_in){
		$sql = "SELECT id_sew_qc_final_in_header FROM prod_qc_final_in_header WHERE no_sew_final_qc_in = '{$_no_sew_final_qc_in}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id");

		return $tmp_array[0]['id_sew_qc_final_in_header'];
	}
	public function insert($data,$username,$no_sew_final_qc_in)
	{
		...
		$insert = "INSERT INTO prod_qc_final_in_header (notes,id_cost,id_line,username,dateinput,date_output,no_sew_final_qc_in,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_qc_final_in_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sew_qc_final_in_header = '{$data->id_sew_qc_final_in_header}'";
	...
	public function insert_detail($header,$_detail,$id_sew_qc_final_in_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_qc_final_in_detail(id_so_det,id_sew_qc_final_in_header,qty,username,dateinput) VALUES ";
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_qc_final_in_detail WHERE id_sew_qc_final_in_detail = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE prod_qc_final_in_detail SET qty={$_det->is_qty},userupdate='{$username}',dateupdate='{$dt}' WHERE id_sew_qc_final_in_detail = '{$_det->is_id_det}'";
				...
				if($_det->is_qty > 0){
					...
					$insert .=" 
									'{$_det->is_id_so_det}',
									'{$header->id_sew_qc_final_in_header}',
									'{$_det->is_qty}',
									'{$username}',
									'{$dt}'
									";		
