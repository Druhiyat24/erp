...
$id_finishing_fg_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_finishing_fg_header )){
	$id_finishing_fg_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_finishing_fg_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_fg"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_finishing_fg_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_fg + ifnull(FG.qty_fg,0))balance_plus_qty_fg
			,TABLE_BALANCE.urut
			,ifnull(FG.id_det,0) id_det
			,ifnull(FG.qty_fg,0)qty_fg		
			,TABLE_BALANCE.tot_shp next_qty
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_finishing_fg_header
							,A.date_output
							,A.no_finishing_fg
							,A.notes
							,A.id_cost
							...
							,A.dateinput
							,A.time
							,B.id_finishing_fg_detail id_det
							,B.id_so_det
							,B.qty qty_fg					
					FROM prod_finishing_fg_header A
					INNER JOIN(
						SELECT id_finishing_fg_detail,id_so_det,id_finishing_fg_header,qty FROM prod_finishing_fg_detail 
					)B ON A.id_finishing_fg_header = B.id_finishing_fg_header
					WHERE A.id_finishing_fg_header ='{$id_finishing_fg_header}'
				)FG ON TABLE_BALANCE.id_so_det = FG.id_so_det
		)X";
/*  echo $_sql;
die();  */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"6"=>"unit",
		"7"=>"balance_fg",
		"8"=>"tot_qty_audit_buyer",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_fg",
		"12"=>"balance_plus_qty_fg",
		"13"=>"next_qty",
		"14"=>"tot_qty_fg"
		);
