...
$id_mut_dc_set_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_mut_dc_set_header )){
	$id_mut_dc_set_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_mut_dc_set_header); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		$_field 	= $pecah_[1];
		//echo "VAL=".$__data[$_field];echo "VAL=".$__data[$_field];
		//die();die();
		//$_value 	= ($__data["id_mut_dc_set_detail"] > 0 ? $__data['qty_dc_set'] : $__data["balance_plus_qty_dc_set"] );
		$_value 	=  $__data['qty_dc_set'] ;
		$checkeds 	= ($__data["id_mut_dc_set_detail"] > 0 ? "checked" : "" );
		$onclick  	= ($__data["balance_plus_qty_dc_set"] < 1  ? "onclick='return false'" : "" );
		$_str ="";
		if($_attr == "is___checklist"){
 		$_str = "<input type='checkbox' data-qty='".$_value."'  "
					."id='".$_attr."__".$id_so_det."'" 
					."data-balance='".$__data["balance_plus_qty_dc_set"]."'" 
					."data-id_det='".$__data["id_mut_dc_set_detail"]."'" 
					."data-id_so_det='".$id_so_det."'"
					."data-next_qty='".$__data["next_qty"]."'"					
					."class='{$_attr}' data-id='".$_attr."__".$id_reff."'  {$checkeds} {$onclick} >"; 
		}else{
		$_str = "<input type='text'  "
					...
					."value='".$_value."'"
					."data-balance='".$__data["balance_plus_qty_dc_set"]."'" 
					."data-id_mut_dc_set_detail='".$__data["id_mut_dc_set_detail"]."'" 
					."data-id_so_det='".$id_so_det."'"
					."data-next_qty='".$__data["next_qty"]."'"
					."data-qty_current='".$__data["qty_current"]."'"
					.">";
		}
		return $_str;
	}
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_mut_dc_set_detail
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_mut_dc_set_detail'] == 0 ){
					$id_reff= $i;
				} else{
					$id_reff= $_data['data'][$i]['id_mut_dc_set_detail'];
				}
	...
	public function get_main_query($id_cost,$id_mut_dc_set_header){
...
/* 		if($key_det =='1'){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.unit
			,TABLE_BALANCE.tot_qty_mut_dc_set
			,TABLE_BALANCE.tot_qty_dc_set
			,TABLE_BALANCE.balance_mut_dc_set
			,TABLE_BALANCE.urut
			,ifnull(DC_SET_MUT.id_mut_dc_set_detail,0) id_mut_dc_set_detail
			,ifnull(DC_SET_MUT.qty_dc_set,0)qty_dc_set
			,ifnull(DC_SET_MUT.id_mut_dc_set_header,0)id_header
			,if(ifnull(DC_SET_MUT.id_mut_dc_set_detail,0) = '0',TABLE_BALANCE.balance_mut_dc_set, ( TABLE_BALANCE.balance_mut_dc_set + ifnull(DC_SET_MUT.qty_dc_set,0) ) )qty_transfer			
			,(TABLE_BALANCE.balance_mut_dc_set + ifnull(DC_SET_MUT.qty_dc_set,0))balance_plus_qty_dc_set
			,(TABLE_BALANCE.tot_qty_sew_in ) next_qty
			,(TABLE_BALANCE.tot_qty_mut_dc_set - ifnull(DC_SET_MUT.qty_dc_set,0)) qty_current
			FROM(
			{$sql_balance}
		)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_mut_dc_set_header
							,A.date_output
							,A.notes
							,A.id_cost
							,A.username
							,A.dateinput
							,A.time
							,B.id_mut_dc_set_detail
							,B.id_so_det
							,B.qty qty_dc_set
					FROM prod_mut_dc_set_header A
					INNER JOIN(
						SELECT id_mut_dc_set_detail,id_so_det,id_mut_dc_set_header,qty FROM prod_mut_dc_set_detail 
					)B ON A.id_mut_dc_set_header = B.id_mut_dc_set_header
					WHERE A.id_mut_dc_set_header ='{$id_mut_dc_set_header}'
				)DC_SET_MUT	ON TABLE_BALANCE.id_so_det = DC_SET_MUT.id_so_det
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_mut_dc_set",
		"8"=>"tot_qty_mut_dc_set",
		"9"=>"id_mut_dc_set_detail",
		"10"=>"id_so_det",
		"11"=>"qty_dc_set",
		"12"=>"balance_plus_qty_dc_set",
		"13"=>"next_qty",
		"14"=>"tot_qty_dc_set",
		"15"=>"id_header",
		"16"=>"qty_transfer",
