...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_in); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		//die();die();
		$_value 	= ($__data["id_sew_in_det"] > 0 ? $__data['qty_sew_in'] : $__data["balance_plus_qty_sew_in"] );
		$checkeds 	= ($__data["id_sew_in_det"] > 0 ? "checked" : "" );
		$onclick  	= ($__data["balance_plus_qty_sew_in"] < 1  ? "onclick='return false'" : "" );
		$_str ="";
		if($_attr == "is___checklist"){
 		$_str = "<input type='checkbox' data-qty='".$_value."'  "
					."id='".$_attr."__".$id_reff."'" 
					."data-balance='".$__data["balance_plus_qty_sew_in"]."'" 
					."data-id_sew_in_det='".$__data["id_sew_in_det"]."'" 
	...
	public function get_main_query($id_cost,$id_sew_in){
...
/* 		if($key_det =='1'){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.unit
			,TABLE_BALANCE.tot_qty_mut_dc_set qty_dc_out
			,TABLE_BALANCE.tot_qty_sew_in
			,TABLE_BALANCE.balance_qty_sew_in
			,TABLE_BALANCE.urut
			,ifnull(SEW_IN.id_sew_in_det,0) id_sew_in_det
			,ifnull(SEW_IN.qty_sew_in,0)qty_sew_in
			,ifnull(SEW_IN.id_sew_in_header,0)id_header
			,(TABLE_BALANCE.balance_qty_sew_in + ifnull(SEW_IN.qty_sew_in,0))balance_plus_qty_sew_in
			,if(ifnull(SEW_IN.id_sew_in_det,0) = '0',TABLE_BALANCE.balance_qty_sew_in,ifnull(SEW_IN.qty_sew_in,0))qty_transfer
			,(TABLE_BALANCE.tot_qty_sew_out ) next_qty
			,(TABLE_BALANCE.tot_qty_sew_in - ifnull(SEW_IN.qty_sew_in,0) ) qty_current
			FROM(
			{$sql_balance}
		)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_in_header
							,A.date_output
							,A.no_sew_in
							,A.notes
							,A.id_cost
							...
							,B.id_so_det
							,B.qty qty_sew_in
					FROM prod_sew_in_header A
					INNER JOIN(
						SELECT id,id_so_det,id_sew_in_header,qty FROM sew_in 
					)B ON A.id_sew_in_header = B.id_sew_in_header
					WHERE A.id_sew_in_header ='{$id_sew_in}'
				)SEW_IN	ON TABLE_BALANCE.id_so_det = SEW_IN.id_so_det
...
die();   */
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qty_sew_in",
		"8"=>"qty_dc_out",
		"9"=>"id_sew_in_det",
		"10"=>"id_so_det",
		"11"=>"qty_sew_in",
		"12"=>"balance_plus_qty_sew_in",
		"13"=>"next_qty",
		"14"=>"tot_qty_sew_in",
		"15"=>"id_header",
		"16"=>"qty_transfer",
