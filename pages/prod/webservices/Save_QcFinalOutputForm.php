...
for($i=0;$i<count($detail);$i++){
	...
	//check Format Inputan qty_ds
	$ii=0;
for($i=0;$i<count($detail);$i++){
	$ii = $i+1;
	$_det = $detail[$i];
    $element = $_det->is_qty_ds;
    if (is_numeric($element)) {
        $x= 'TRUE';
    } else {
		$x = 'FALSE';
		$result = '{ "respon":"'.'210'.'", "message":"'.'OK'.'", "records":"0","baris":"'.$ii.'"}';   
		echo $result;
		die();
    }	
}
...
for($i=0;$i<count($detail);$i++){ 
	...
	$_det = $detail[$i];
	//$qty =$_det->is_qty;
	$qty =$_det->is_qty + $_det->is_rpr + $_det->is_qty_ds;	
...
if($code == '1'){
		...
		if($format == '1'){
			...
			$id_sew_qc_final_out_header  = $Proses->get_id($no_sew_qc_final_out,$dt__);
			$key 		= $Proses->insert_detail($header,$detail,$id_sew_qc_final_out_header,$_SESSION['username'],$dt__);
			$populasi_qty_reject 	= $Proses->populasi_qty_reject($dt__,$id_sew_qc_final_out_header,$_SESSION['username']);
			$key					= $Proses->insert_reject($populasi_qty_reject,$id_sew_qc_final_out_header,$_SESSION['username'],$dt__);			
			...
			$key = $Proses->eksekusi_reject($id_sew_qc_final_out_header,$header,$detail,$_SESSION['username'],$dt__);			
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sew_qc_final_out_header" : "'.$id_sew_qc_final_out_header.'"}';
			echo $result;
			die();
		}else{
...
die();		 */	
	...
	public function insert_detail($header,$_detail,$id_sew_qc_final_out_header,$username,$dt)
	{
		$insert = "INSERT INTO prod_qc_final_out_detail(qty_ds,id_so_det,id_sew_qc_final_out_header,qty,username,dateinput,rpr,id_defect) VALUES ";
	...
	public function update_detail($header,$_detail,$username,$dt)
	{
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					...
					$update = "UPDATE prod_qc_final_out_detail SET qty_ds='{$_det->is_qty_ds}',id_defect='{$_det->is_id_defect}', rpr='{$_det->is_rpr}', qty='{$_det->is_qty}',userupdate='{$username}',dateupdate='{$dt}' WHERE id = '{$_det->is_id_det}'";
		...
		public function populasi_qty_reject($dt,$id_sew_qc_final_out_header,$_username){
			$sql = "SELECT id id_det,id_so_det,rpr FROM prod_qc_final_out_detail WHERE id_sew_qc_final_out_header='{$id_sew_qc_final_out_header}'
						AND  ifnull(rpr,0) > 0
			";
/* 			echo $sql;
			die(); */
	...
	public function insert_reject($populasi_qty_reject,$id_sew_qc_final_out_header,$username,$dt)
	{
		//print_r($populasi_qty_reject);
		//die();
		for($i=0;$i<count($populasi_qty_reject);$i++){
		$insert = "";
		$insert .= "INSERT INTO prod_rej_qc_fin_detail(id_so_det,id_qc_fin_out_detail,qty,username,dateinput) VALUES ";			
	...
	//	die(); 
		...
		public function populasi_qty_qc_out_qty_reject($id_sew_qc_final_out_header,$header){
			$_populasi_reject = array();
			$sql = "SELECT A.id id_det
					,A.rpr qty_rej_qc_out
					,A.id_sew_qc_final_out_header
					,A.id_so_det
					,ifnull(B.qty_rej_det,0)qty_rej_det
					FROM prod_qc_final_out_detail A LEFT JOIN(
					SELECT   id_qc_fin_out_detail
						,id_rej_qc_fin_detail
						,SUM(ifnull(qty,0))qty_rej_det
						FROM prod_rej_qc_fin_detail
		  		GROUP BY id_qc_fin_out_detail
				)B ON A.id = B.id_qc_fin_out_detail
				WHERE A.id_sew_qc_final_out_header = '{$id_sew_qc_final_out_header}'	";
		...
		public function insert_reject_detail($pop_reject,$id_sew_qc_final_out_header,$header,$detail,$username,$dt){
			$_populasi_reject = array();
			$_count = $pop_reject["qty_rej_qc_out"] - $pop_reject["qty_rej_det"];
					...
					array_push($_populasi_reject,$__tmp);	
					$this->insert_reject($_populasi_reject,$id_sew_qc_final_out_header,$username,$dt);
					return 1;
			
		}


		public function populasi_id_delete($id_sew_qc_final_out_header,$jumlah_qty_delete,$category,$id_so_det){
			if($category == "FREE"){
				$sql = "	SELECT A.id_rej_qc_fin_detail
							FROM prod_rej_qc_fin_detail A
							INNER JOIN (SELECT id FROM prod_qc_final_out_detail WHERE id_sew_qc_final_out_header = '{$id_sew_qc_final_out_header}' AND id_so_det ='{$id_so_det}' )B ON A.id_qc_fin_out_detail = B.id
							AND ifnull(id_rej_qc_fin_header,0) = 0
				...
				$sql = "SELECT A.id_rej_qc_fin_detail
							FROM prod_rej_qc_fin_detail A
							INNER JOIN (SELECT id FROM prod_qc_final_out_detail WHERE id_sew_qc_final_out_header = '{$id_sew_qc_final_out_header}' AND id_so_det ='{$id_so_det}')B ON A.id_qc_fin_out_detail = B.id 
							WHERE ifnull(id_rej_qc_fin_header,0) != 0
							LIMIT {$jumlah_qty_delete}";
			}
			//echo $sql;
			...
			if(count($tmp_array) > 0 ){
				...
				for($i=0;$i<count($tmp_array);$i++){
					if($i == $trigger){
						$_str .= "'".$tmp_array[$i]["id_rej_qc_fin_detail"]."'";
					}else{
						$_str .= "'".$tmp_array[$i]["id_rej_qc_fin_detail"]."',";
					}
		...
		public function delete_qty_free($id_sew_qc_final_out_header,$category,$jumlah_qty_delete,$id_so_det){

			$populasi_id_delete = $this->populasi_id_delete($id_sew_qc_final_out_header,$jumlah_qty_delete,$category,$id_so_det);
			$where = "id_rej_qc_fin_detail IN({$populasi_id_delete})";
			$delete = "DELETE FROM prod_rej_qc_fin_detail WHERE 1=1 AND   {$where}";
		//	echo $delete;
			$this->eksekusi_query_insert_update($delete,'delete_qty_free');
			return 1;
		}
		public function delete_qty_combine($id_sew_qc_final_out_header,$category,$jumlah_qty_delete,$_count,$id_so_det){
					
			
			//$this->delete_qty_free($id_sew_qc_final_out_header,"FREE",$_count,$id_so_det);
			$this->delete_qty_free($id_sew_qc_final_out_header,$category,$jumlah_qty_delete,$id_so_det);
			return 1;
		}
		
		public function populasi_qty_free($id_sew_qc_final_out_header,$id_so_det){
			$sql = "SELECT A.id_qc_fin_out_detail
							FROM prod_rej_qc_fin_detail A
							INNER JOIN (SELECT id FROM prod_qc_final_out_detail WHERE id_sew_qc_final_out_header = '{$id_sew_qc_final_out_header}' AND id_so_det ='{$id_so_det}')B ON A.id_qc_fin_out_detail = B.id
							AND ifnull(id_rej_qc_fin_header,0) = 0";
		...
		public function populasi_qty_existing($id_sew_qc_final_out_header,$id_so_det){
			$sql = "SELECT A.id_qc_fin_out_detail
							FROM prod_rej_qc_fin_detail A
							INNER JOIN (SELECT id FROM prod_qc_final_out_detail WHERE id_sew_qc_final_out_header = '{$id_sew_qc_final_out_header}' AND id_so_det ='{$id_so_det}')B ON A.id_qc_fin_out_detail = B.id 
							WHERE ifnull(id_rej_qc_fin_header,0) != 0";
		...
		public function delete_reject_detail($pop_reject,$id_sew_qc_final_out_header,$header,$detail,$username,$dt){
			...
			$populasi_qty_existing 	= $this->populasi_qty_existing($id_sew_qc_final_out_header,$pop_reject['id_so_det']);
			$populasi_qty_free 		= $this->populasi_qty_free($id_sew_qc_final_out_header,$pop_reject['id_so_det']);

			$jumlah_qty_delete = ( ($_count) - (count($populasi_qty_free)) <= 0 ? "0" : ($_count) - (count($populasi_qty_free) ) ) ;

			$this->delete_qty_free($id_sew_qc_final_out_header,"FREE",$_count,$pop_reject['id_so_det']);
			$this->delete_qty_combine($id_sew_qc_final_out_header,"EXIST",$jumlah_qty_delete,$_count,$pop_reject['id_so_det']);
			//print_r(count($jumlah_qty_delete));
/* 			echo count($populasi_qty_free)." >= ".$_count;
			die(); 	 */	
//			if(count($populasi_qty_free) >= $_count ){ //id_so_det
//				$this->delete_qty_free($id_sew_qc_final_out_header,"FREE",count($populasi_qty_free),$pop_reject['id_so_det']);
//			}else{
...
//			}
		...
		public function eksekusi_reject($id_sew_qc_final_out_header,$header,$detail,$username,$dt){
			$populasi_qty_qc_out_qty_reject		= $this->populasi_qty_qc_out_qty_reject($id_sew_qc_final_out_header,$header);
			...
			if(count($populasi_qty_qc_out_qty_reject) == '0'){
				...
				for($i=0;$i<count($populasi_qty_qc_out_qty_reject);$i++){
					...
					if($qty_rej_qc_out == $qty_rej_det ){
						...
						$this->insert_reject_detail($populasi_qty_qc_out_qty_reject[$i],$id_sew_qc_final_out_header,$header,$detail,$username,$dt);
						
						
					} if($qty_rej_qc_out < $qty_rej_det)
						$this->delete_reject_detail($populasi_qty_qc_out_qty_reject[$i],$id_sew_qc_final_out_header,$header,$detail,$username,$dt);
