...
$data = $_POST;
$code = $_POST['code'];
$id_mut_dc_set_header = $_POST['id'];
 //print_r($id_sew_qc_in );die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_mut_dc_set_header);
...
class Proses {
	...
	public function get_list_data($id_mut_dc_set_header){
		$sql = "SELECT id_mut_dc_set_header,date_output,no_mut_dc_set,notes,id_cost,username,dateinput,time 
				FROM prod_mut_dc_set_header WHERE id_mut_dc_set_header ='{$id_mut_dc_set_header}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_mut_dc_set_header":"'.rawurlencode($row[$i]['id_mut_dc_set_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_mut_dc_set":"'. rawurlencode($row[$i]["no_mut_dc_set"]).'",';	
