...
if($code == '1'){
		if($format == '1'){
			$no_finishing_qc_audit 			= $Proses->get_no($header->date_output);		
			$key 							= $Proses->insert($header,$_SESSION['username'],$no_finishing_qc_audit);
			$id_finishing_qc_audit_header  	= $Proses->get_id($no_finishing_qc_audit);
			$key 							= $Proses->insert_detail($header,$detail,$id_finishing_qc_audit_header,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_finishing_qc_audit_header" : "'.$id_finishing_qc_audit_header.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_finishing_qc_audit_header = $header->id_finishing_qc_audit_header;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no($_date_output){
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_finishing_qc_audit = $category."/".$_month.$seq_years."/".$seq;
			return $_no_finishing_qc_audit;
	}	
	public function get_id($_no_finishing_qc_audit){
		$sql = "SELECT id_finishing_qc_audit_header FROM prod_finishing_qc_audit_header WHERE no_finishing_qc_audit = '{$_no_finishing_qc_audit}' LIMIT 1";
		$tmp_array = $this->eksekusi_query($sql,"get_id");
		
		return $tmp_array[0]['id_finishing_qc_audit_header'];
	}
	public function insert($data,$username,$no_finishing_qc_audit)
	{
		...
		$insert = "INSERT INTO prod_finishing_qc_audit_header (notes,id_cost,id_line,username,dateinput,date_output,no_finishing_qc_audit,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_finishing_qc_audit_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_finishing_qc_audit_header = '{$data->id_finishing_qc_audit_header}'";
	...
	public function insert_detail($header,$_detail,$id_finishing_qc_audit_header,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO prod_finishing_qc_audit_detail(id_so_det,id_finishing_qc_audit_header,qty,username,dateinput,rpr,id_defect) VALUES ";
		for($i=0;$i<count($_detail);$i++){
			$_det = $_detail[$i];
			$_det->is_id_defect = 0;
					if($_det->is_qty > 0 || $_det->is_qty > 0 ){
						$insert .= "(";
						$insert .=" 
										'{$_det->is_id_so_det}',
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			...
			if($_det->is_id_det > 0){
				if($_det->is_qty == 0){
					$delete = "DELETE FROM prod_finishing_qc_audit_detail WHERE id = '{$_det->is_id_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE prod_finishing_qc_audit_detail SET id_defect='{$_det->is_id_defect}', rpr='{$_det->is_rpr}', qty='{$_det->is_qty}',userupdate='{$username}',dateupdate='{$dt}' WHERE id = '{$_det->is_id_det}'";
				...
				if($_det->is_qty > 0){
					...
					$insert .=" 
									'{$_det->is_id_so_det}',
									'{$header->id_finishing_qc_audit_header}',
									'{$_det->is_qty}',
									'{$username}',
									'{$dt}',
									'{$_det->is_rpr}',
