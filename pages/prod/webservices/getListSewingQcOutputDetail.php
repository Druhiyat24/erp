...
$main_query 				= $Proses->get_main_query($id_cost,$id_sew_qc_output); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-qty_current='".$__data["qty_current"]."' data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_sew_qc_out"]."' data-id_sew_qc_out_det='".$__data["id_sew_qc_out_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				...
				$__id_det = $_data['data'][$i]['id_sew_qc_out_det'];
				//echo $__id_det."-";
				if($__id_det == "0" ){
				//	echo $__id_det;
					$id_reff= $i;
					...
					$id_reff= $_data['data'][$i]['id_sew_qc_out_det'];
				}
			//	echo $id_reff;
	...
	public function get_main_query($id_cost,$id_sew_qc_output){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qty_sew_qc_out + ifnull(SEW_QC_OUT.qty_sew_qc_out,0))balance_plus_sew_qc_out
			,TABLE_BALANCE.urut
			,ifnull(SEW_QC_OUT.id_sew_qc_output_det,0) id_sew_qc_out_det
			,ifnull(SEW_QC_OUT.qty_sew_qc_out,0)qty_sew_qc_out
			,ifnull(SEW_QC_OUT.rpr,0)rpr
			,SEW_QC_OUT.id_defect
			,TABLE_BALANCE.tot_qty_steam_out next_qty
			,TABLE_BALANCE.tot_qty_sew_qc_out - (ifnull(SEW_QC_OUT.qty_sew_qc_out,0))qty_current
			FROM(
				{$sql_balance}
			)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_sew_qc_out_header
							,A.date_output
							,A.no_sew_qc_out
							,A.notes
							,A.id_cost
							...
							,A.dateinput
							,A.time
							,B.id id_sew_qc_output_det
							,B.id_so_det
							,B.qty qty_sew_qc_out
							,B.rpr
							,B.id_defect
					FROM prod_qc_out_header A
					INNER JOIN(
						SELECT id ,id_so_det,id_sew_qc_out_header,qty,rpr,id_defect FROM qc_out 
					)B ON A.id_sew_qc_out_header = B.id_sew_qc_out_header
					WHERE A.id_sew_qc_out_header ='{$id_sew_qc_output}'
				)SEW_QC_OUT ON TABLE_BALANCE.id_so_det = SEW_QC_OUT.id_so_det
		)X";
/*   echo $_sql;
die();   */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_qty_sew_qc_out",
		"8"=>"tot_qty_sew_qc_in",
		"9"=>"id_sew_qc_out_det",
		"10"=>"id_so_det",
		"11"=>"qty_sew_qc_out",
		"12"=>"balance_plus_sew_qc_out",
		"13"=>"rpr",
		"14"=>"id_defect",
		"15"=>"next_qty",
		"16"=>"tot_qty_sew_qc_out",
		"17"=>"qty_current"
		);
	}
}
