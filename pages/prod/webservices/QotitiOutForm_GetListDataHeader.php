...
$data = $_POST;
$code = $_POST['code'];
$id_qotiti_out_header = $_POST['id'];
 //print_r($id_sew_qc_in );die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_qotiti_out_header);
...
class Proses {
	...
	public function get_list_data($id_qotiti_out_header){
		$sql = "SELECT id_qotiti_out_header,date_output,no_qotiti_out,notes,id_cost,id_line,username,dateinput,time 
				FROM prod_qotiti_out_header WHERE id_qotiti_out_header ='{$id_qotiti_out_header}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_qotiti_out_header":"'.rawurlencode($row[$i]['id_qotiti_out_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_qotiti_out":"'. rawurlencode($row[$i]["no_qotiti_out"]).'",';	
