...
$data = $_POST;
$code = $_POST['code'];
$id_mut_sew_header = $_POST['id'];
 //print_r($id_sew_qc_in );die();
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_list_data($id_mut_sew_header);
...
class Proses {
	...
	public function get_list_data($id_mut_sew_header){
		$sql = "SELECT id_mut_sew_header,date_output,no_mut_sew,notes,id_cost,id_line,username,dateinput,time 
				FROM prod_mut_sew_header WHERE id_mut_sew_header ='{$id_mut_sew_header}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_mut_sew_header":"'.rawurlencode($row[$i]['id_mut_sew_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_mut_sew":"'. rawurlencode($row[$i]["no_mut_sew"]).'",';	
