...
$id_finishing_qc_audit_header     =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_finishing_qc_audit_header )){
	$id_finishing_qc_audit_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_finishing_qc_audit_header); // bermain disini
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_audit"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."' data-next_qty='".$__data["next_qty"]."' >";
		}
		return $_str;
	}
	...
	public function get_main_query($id_cost,$id_finishing_qc_audit_header){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.tot_qty_mat_dec tot_qty_metal_detector
			,TABLE_BALANCE.balance_met_det balance_metal_detector
			
	
			,TABLE_BALANCE.tot_qty_qc_audit
			,TABLE_BALANCE.balance_qc_audit balance_audit_buyer
	
			,(TABLE_BALANCE.balance_qc_audit + ifnull(QCA.qty_audit_buyer,0))balance_plus_qty_audit
			,TABLE_BALANCE.urut
			,ifnull(QCA.id_det,0) id_det
			,ifnull(QCA.qty_audit_buyer,0)qty_audit_buyer	
			,ifnull(QCA.rpr,0)rpr
			,QCA.id_defect
			,TABLE_BALANCE.tot_qty_fg next_qty
			FROM(
			{$sql_balance}
	)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_finishing_qc_audit_header
							,A.date_output
							,A.no_finishing_qc_audit
							,A.notes
							,A.id_cost
							,A.id_line
							,A.username
							...
							,B.id id_det
							,B.id_so_det
							,B.qty qty_audit_buyer
							,ifnull(B.rpr,0)rpr
							,B.id_defect
					FROM prod_finishing_qc_audit_header A
					INNER JOIN(
						SELECT id,id_so_det,rpr,id_defect,id_finishing_qc_audit_header,qty FROM prod_finishing_qc_audit_detail 
					)B ON A.id_finishing_qc_audit_header = B.id_finishing_qc_audit_header
					WHERE A.id_finishing_qc_audit_header ='{$id_finishing_qc_audit_header}'
				)QCA ON TABLE_BALANCE.id_so_det = QCA.id_so_det
		)X";
/*   echo $_sql;
die();   */
		return $_sql;
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_audit_buyer",
		"8"=>"tot_qty_metal_detector",
		"9"=>"id_det",
		"10"=>"id_so_det",
		"11"=>"qty_audit_buyer",
		"12"=>"balance_plus_qty_audit",
		"13"=>"rpr",
		"14"=>"id_defect",
		"15"=>"next_qty",
		"16"=>"tot_qty_qc_audit"
