...
for($i=0;$i<count($detail);$i++){
	...
	$qty =  $qty_current + $_det->is_qty_input;
...
if($code == '1'){
		if($format == '1'){
			$no_sew_in 	= $Proses->get_no_sew_in($header->date_output);
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_sew_in);
			$id_sew_in  = $Proses->get_id_sew_in($no_sew_in);
			$key 		= $Proses->insert_detail($header,$detail,$id_sew_in,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sew_in" : "'.$id_sew_in.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_sew_in = $header->id_si;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_sew_in($_date_output){
		$category ="SEW/IN";
	...
	public function get_id_sew_in($_no_sew_in){
		$sql = "SELECT id_sew_in_header FROM prod_sew_in_header WHERE no_sew_in = '{$_no_sew_in}' LIMIT 1";

		$tmp_array = $this->eksekusi_query($sql,"get_id_sew_in");

		return $tmp_array[0]['id_sew_in_header'];
	}
	public function insert($data,$username,$no_sew_in)
	{ 
		...
		$insert = "INSERT INTO prod_sew_in_header (notes,id_cost,id_line,username,dateinput,date_output,no_sew_in,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sew_in_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sew_in_header = '{$data->id_si}'";
	...
	public function insert_detail($header,$_detail,$id_sew_in,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO sew_in(id_so_det,id_sew_in_header,qty,username,dateinput) VALUES ";
		for($i=0;$i<count($_detail);$i++){
			$_det = $_detail[$i];
					if($_det->is_qty_input > 0){
						if($_det->is_checklist == '1'){
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			$_det = $_detail[$i];
			if($_det->is_id_sew_in_det > 0){
				if($_det->is_qty_input == 0){
					$delete = "DELETE FROM sew_in WHERE id = '{$_det->is_id_sew_in_det}'";
					...
					if($_det->is_checklist == '0'){
						...
						$update = "UPDATE sew_in SET qty={$_det->is_qty_input},userupdate='{$username}',dateupdate='{$dt}' WHERE id = '{$_det->is_id_sew_in_det}'";
