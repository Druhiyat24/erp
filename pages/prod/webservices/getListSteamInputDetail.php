...
$id_steam_in_header            		 =$_GET['id_url'];
/*  print_r($_GET);
die();  */
if(!ISSET($id_cost)){
	$id_cost = 'X';
...
if(!ISSET($id_steam_in_header )){
	$id_steam_in_header  = 'X';
}
...
$main_query 				= $Proses->get_main_query($id_cost,$id_steam_in_header); // bermain disini
...
class Proses {
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){
		...
		//die();die();
		$_value 	= ($__data["id_steam_in_detail"] > 0 ? $__data['qty_steam_input'] : $__data["balance_plus_qty_steam_input"] );
		$checkeds 	= ($__data["id_steam_in_detail"] > 0 ? "checked" : "" );
		$onclick  	= ($__data["balance_plus_qty_steam_input"] < 1  ? "onclick='return false'" : "" );
		$_str ="";
		if($_attr == "is___checklist"){
 		$_str = "<input type='checkbox' data-qty='".$_value."'  "
					."id='".$_attr."__".$id_reff."'" 
					."data-balance='".$__data["balance_plus_qty_steam_input"]."'" 
					."data-id_det='".$__data["id_steam_in_detail"]."'" 
		...
		$_str = "<input type='text'  "
					...
					."value='".$_value."'"
					."data-balance='".$__data["balance_plus_qty_steam_input"]."'" 
					."data-id_steam_in_detail='".$__data["id_steam_in_detail"]."'" 
					."data-id_so_det='".$id_so_det."'"
					."data-next_qty='".$__data["next_qty"]."'"
					."data-qty_current='".$__data["qty_current"]."'"
					.">";
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_steam_in_detail
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_steam_in_detail'] == 0 ){
					$id_reff= $i;
				} else{
					$id_reff= $_data['data'][$i]['id_steam_in_detail'];
				}
	...
	public function get_main_query($id_cost,$id_steam_in_header){
...
/* 		if($key_det =='1'){
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,TABLE_BALANCE.unit
			,TABLE_BALANCE.tot_qty_mut_sew_qc_out
			,TABLE_BALANCE.tot_qty_steam_in
			,TABLE_BALANCE.balance_steam_input
			,TABLE_BALANCE.urut
			,ifnull(STEAM_IN.id_steam_in_detail,0) id_steam_in_detail
			,ifnull(STEAM_IN.qty_steam_input,0)qty_steam_input
			,ifnull(STEAM_IN.id_steam_in_header,0)id_header
			,if(ifnull(STEAM_IN.id_steam_in_detail,0) = '0',TABLE_BALANCE.balance_steam_input,ifnull(STEAM_IN.qty_steam_input,0))qty_transfer			
			,(TABLE_BALANCE.balance_steam_input + ifnull(STEAM_IN.qty_steam_input,0))balance_plus_qty_steam_input
			,(TABLE_BALANCE.tot_qty_steam_out ) next_qty
			,(TABLE_BALANCE.tot_qty_steam_in - ifnull(STEAM_IN.qty_steam_input,0) ) qty_current
			FROM(
			{$sql_balance}
		)TABLE_BALANCE
				LEFT JOIN(
					SELECT   A.id_steam_in_header
							,A.date_output
							,A.notes
							,A.id_cost
							,A.id_line
							...
							,A.dateinput
							,A.time
							,B.id_steam_in_detail
							,B.id_so_det
							,B.qty qty_steam_input
					FROM prod_sew_steam_in_header A
					INNER JOIN(
						SELECT id_steam_in_detail,id_so_det,id_steam_in_header,qty FROM prod_sew_steam_in_detail 
					)B ON A.id_steam_in_header = B.id_steam_in_header
					WHERE A.id_steam_in_header ='{$id_steam_in_header}'
				)STEAM_IN	ON TABLE_BALANCE.id_so_det = STEAM_IN.id_so_det
	...
	public function populasi_kolom(){
		...
		"5"=>"qty_so",
		"6"=>"unit",
		"7"=>"balance_steam_input",
		"8"=>"tot_qty_mut_sew_qc_out",
		"9"=>"id_steam_in_detail",
		"10"=>"id_so_det",
		"11"=>"qty_steam_input",
		"12"=>"balance_plus_qty_steam_input",
		"13"=>"next_qty",
		"14"=>"tot_qty_steam_in",
		"15"=>"id_header",
		"16"=>"qty_transfer",
