...
class Proses {
	...
	public function eksekusi_query_datatable($main_query,$kolom_x,$searchQuery,$row,$rowperpage,$columnName){
		$_populasi_datatable;
		$data_table = array();
	//$sql = "select {$kolom_x}  from {$main_query} WHERE 1 ".$searchQuery." order by ".$columnName." ".
...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			if($akses == "1"){
				$_str .= "<a href='?mod={$mod_form}&id_sew_qc_in={$__data["id_sew_qc_in_header"]}&cr=is_edit' ><i class='fa fa-pencil'> </i></a>";
			}
			...
			if($akses == "1"){
				$_str .= "<a href='?mod={$mod_form}&id_sew_qc_in={$__data["id_sew_qc_in_header"]}&cr=is_view' ><i class='fa fa-eye'> </i></a>";
			}						
		}
		return $_str;
	}		
