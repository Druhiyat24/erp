...
$columnName = "X.id_number DESC"; // Column name
...
$table = "(
	...
	INNER JOIN(SELECT id_number FROM prod_spread_report_detail GROUP BY id_number)SRD ON srn.id_number = SRD.id_number
	GROUP BY so.id
) AS X";
### Search 
$searchQuery = " ";
...
//echo $row['n_post'];
	...
	$button = '';		
	$button .="<a href='../prod/?mod=mRollListWsSoColorPage&id_so=$row[id_so]' style='color: #3c8dbc;' title='Edit' class='btn btn-sm' ><i class='fa fa-eye'> </i></a>";
