...
class Proses {
	...
	public function populasi_attribut(){
		return array(
			...
			"2"=>"is___balance_sec_out",
			"3"=>"is___qty_so",
			"4"=>"is___tot_qty_sec_out",
			"5"=>"is___tot_qty_sec_in"
		);
	}
	...
	public function get_main_query($id_cost,$id_sec_out,$id_proses,$id_panel){	
		...
		$_sql ="(
			SELECT   		
				...
				(TABLE_BALANCE.tot_qty_sec_out - (ifnull(SEC_OUT.qty_sec_out,0) - ifnull(SEC_OUT.qty_reject_sec_out,0))) AS before_cumulative,
				(TABLE_BALANCE.tot_qty_sec_out + TABLE_BALANCE.balance_sec_out) AS before_balance				
			FROM (
				{$sql_balance}
			) TABLE_BALANCE
			LEFT JOIN (
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){	
		...
		/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			else if($_attr == "is___tot_qty_sec_out"){
				$disabled = "disabled";
				$data_value = $__data['before_cumulative'];
			}
			else if($_attr == "is___balance_sec_out"){
				$disabled = "disabled";
