...
class getListData {
	...
	//die();		
		...
		$q = " SELECT A.color,B.id_jo,A.id_so,C.bppbno id,D.bppbno_int nama,A.id_number,C.id_supplier  FROM prod_spread_report_number A
	INNER JOIN jo_det B ON A.id_so = B.id_so
	INNER JOIN bppb_req C ON B.id_jo = C.id_jo
	INNER JOIN bppb D ON D.bppbno_req = C.bppbno
	INNER JOIN masteritem MI ON MI.id_item = D.id_item
	INNER JOIN (SELECT color,id_jo,id_item FROM view_portal_bom)V_BOM 
	ON B.id_jo = V_BOM.id_jo AND V_BOM.id_item = MI.id_gen
	WHERE A.id_number='{$id_number}' AND A.id_so = '{$id}' AND V_BOM.color='{$color}' AND C.id_supplier = '432'
	GROUP BY C.bppbno
"; 
/*  echo $q;
die();    */
