...
$table = "(
	...
	FROM (
		...
		SELECT 
			...
			Q.reject,
			Q.qty_cut_out,
			(R.tot_qty_cut_out - Q.qty_cut_out) AS before_cumulative
		FROM (
			SELECT     
		...
		LEFT JOIN (
			{$sql_balance}
		) AS R ON R.id_so_det = Q.id_so_det AND R.id_panel = Q.id_panel AND R.id_item = Q.id_item AND R.lot = Q.lot
		...
		SELECT 
			...
			P.reject,
			P.qty_cut_out,
			IFNULL(O.tot_qty_cut_out, 0) AS before_cumulative
		FROM (
			SELECT 
				'0' AS id_cod,
				coc.id_cat,
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputCutt = '';
	$inputCutt .="<input type='number' id='cutt_".$no."' class='form-control forms' value='$row[cutting_output]' onchange='handleKeyUp(this);cuttNumber(this)'>";

	$inputReject = '';
	$inputReject .="<input type='number' id='reject_".$no."' class='form-control forms' value='$row[reject]' onchange='handleKeyUp(this);rejectNumber(this)'>";
