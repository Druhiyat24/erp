...
for($i=0;$i<count($detail);$i++){
	...
	$qty =  $qty_current + $_det->is_qty_output;
...
if($code == '1'){
		if($format == '1'){
			$no_sew_out 	= $Proses->get_no_sew_out($header->date_output);		
			$key 		= $Proses->insert($header,$_SESSION['username'],$no_sew_out);
			$id_sew_out  = $Proses->get_id_sew_out($no_sew_out);
			$key 		= $Proses->insert_detail($header,$detail,$id_sew_out,$_SESSION['username']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0","id_sew_out" : "'.$id_sew_out.'"}';
			echo $result;
			die();
		}else if($format == '2'){ 
			$id_sew_out = $header->id_sew_out;
...
/* 		array_push($tmp_array,$_arr);
	...
	public function get_no_sew_out($_date_output){
		$category ="SEW/OUT";
		...
		if((intval($populasi_pool['seq']) > 0)){
			...
			$_no_sew_out = $category."/".$_month.$seq_years."/".$seq;
			return $_no_sew_out;
	}	
	public function get_id_sew_out($_no_sew_out){
		$sql = "SELECT id_sew_out_header FROM prod_sew_out_header WHERE no_sew_out = '{$_no_sew_out}' LIMIT 1";
		$tmp_array = $this->eksekusi_query($sql,"get_id_sew_out");
		
		return $tmp_array[0]['id_sew_out_header'];
	}
	public function insert($data,$username,$no_sew_out)
	{
		...
		$insert = "INSERT INTO prod_sew_out_header (notes,id_cost,id_line,username,dateinput,date_output,no_sew_out,time) 
				VALUES (
					'{$data->notes}',
					'{$data->id_cost}',
					'{$data->id_line}',
...
die();		 */	
	...
	public function update($data,$username)
	{
		$d_out =  date("Y-m-d", strtotime($data->date_output));
		$dt = date("Y-m-d H:i:s");
		$update = "UPDATE prod_sew_out_header SET id_line = '{$data->id_line}',notes='{$data->notes}',userupdate='{$username}',dateupdate='{$dt}' WHERE id_sew_out_header = '{$data->id_si}'";
	...
	public function insert_detail($header,$_detail,$id_sew_out,$username)
	{
		$dt = date("Y-m-d H:i:s");
		$insert = "INSERT INTO sew_out(id_so_det,id_sew_out_header,qty,username,dateinput) VALUES ";
	...
	public function update_detail($header,$_detail,$username)
	{
		...
		for($i=0;$i<count($_detail);$i++){
			$_det = $_detail[$i];
			if($_det->is_id_sew_out_det > 0){
				if($_det->is_qty_output == 0){
					$delete = "DELETE FROM sew_out WHERE id = '{$_det->is_id_sew_out_det}'";
					$this->eksekusi_query_insert_update($delete,'update_detail');
				}else{
					$update = "UPDATE sew_out SET qty={$_det->is_qty_output},userupdate='{$username}',dateupdate='{$dt}' WHERE id = '{$_det->is_id_sew_out_det}'";
