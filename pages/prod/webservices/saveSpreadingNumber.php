...
if($code == '1'){
	$Proses = new Proses();
		if($format == '1'){
			...
			$key = $Proses->delete($_POST['data_id']);
			$result = '{ "respon":"'.'200'.'", "message":"'.'OK'.'", "records":"0"}';
			print_r($result);
		}
		else{
...
die();		 */
	...
	public function delete($num_id){
		$connect = $this->connect();

		$sql = "DELETE FROM prod_spread_report_number WHERE id_number = '$num_id'";
		
		$result = $connect->query($sql);
		$this->eksekusi_query_insert_update($sql,'delete');
