...
$table = "(
	...
	SELECT 
		...
		IFNULL(cut_old.tot_qty_cut_out, 0) AS before_cumulative
	FROM (
		...
		AND coc.is_save = 'N'
		GROUP BY mr.id_panel,mr.lot,mr.size
	...
	GROUP BY cut_new.id_so_det,cut_new.id_panel,cut_new.lot
...
$columnName = "X.id_panel ASC, X.lot ASC, X.id_so_det ASC"; // Column name 
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputCutt = '';
	$inputCutt .="<input type='number' id='cutt_".$no."' class='form-control cutt forms' value='$row[cutting_output]' onchange='handleKeyUp(this);cuttNumber(this)'>";

	$inputReject = '';
	$inputReject .="<input type='number' id='reject_".$no."' class='form-control reject forms' value='$row[reject]' onchange='handleKeyUp(this);rejectNumber(this)'>";
	...
	$data[] = array(
		...
		"cumulative_val"=>(ISSET($row['cumulative']) ? $row['cumulative']  : 0 ),
