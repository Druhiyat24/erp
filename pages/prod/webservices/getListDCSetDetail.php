...
if($id_url == '-1' OR $id_url == 'undefined'){
	...
	$table = "(
		...
		SELECT 
			...
			MIN(set_old.tot_qty_dc_set) AS before_cumulative,
			(MIN(set_old.tot_qty_dc_set) + MIN(set_old.balance_qty_dc_set)) AS before_balance
		FROM ( 
	...
	$table = "(
		...
		SELECT 
			...
			MIN(set_old.tot_qty_dc_set) AS cumulative,
			MIN(set_old.balance_qty_dc_set) AS balance,
			(MIN(set_old.tot_qty_dc_set) - set_new.qty_dc_set) AS before_cumulative,
			(MIN(set_old.tot_qty_dc_set) + MIN(set_old.balance_qty_dc_set)) AS before_balance
		FROM (
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputQtyInSet = '';
	$inputQtyInSet .="<input type='number' id='qtyInSet_".$num."' class='form-control forms' value='$qtyInSet' onchange='handleKeyUp(this)' style='min-width: 80px'>";
	...
	$inputLocation = '';
	$inputLocation .="<input type='text' id='location_".$num."' class='form-control loc' value='$location' autocomplete='on' onchange='handleKeyUp(this)' style='min-width: 80px'>";
