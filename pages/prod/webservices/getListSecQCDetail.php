...
$main_query 				= $Proses->get_main_query($id_cost,$id_sec_qc,$id_proses,$id_panel); // bermain disini
...
class Proses {
	...
	public function populasi_attribut(){
		return array(
			"0"=>"is___qty_sec_qc",
			"1"=>"is___qty_rpr",
			"2"=>"is___balance_sec_qc",
			"3"=>"is___qty_so",
			"4"=>"is___next_qty",
			"5"=>"is___tot_qty_sec_out"
		);
	}


	public function get_main_query($id_cost,$id_sec_qc,$id_proses,$id_panel){	
		...
		$_sql = "(
				SELECT   		
					...
					ifnull(SEC_QC.id_sec_qc_det,0) id_sec_qc_det,
					ifnull(SEC_QC.qty,0)qty_sec_qc,
					(TABLE_BALANCE.balance_sec_qc + ifnull(SEC_QC.qty,0))balance_plus_qty_sec_qc,
					ifnull(SEC_QC.qty_rpr,0)qty_rpr,
					(TABLE_BALANCE.tot_qty_sec_qc - (ifnull(SEC_QC.qty,0) - ifnull(SEC_QC.qty_rpr,0))) AS before_cumulative,
					(TABLE_BALANCE.tot_qty_sec_qc + TABLE_BALANCE.balance_sec_qc) AS before_balance
				FROM (
					{$sql_balance}
				) TABLE_BALANCE
				LEFT JOIN (
					SELECT   
						...
						,A.username
						,A.dateinput
						,B.id_sec_qc_detail id_sec_qc_det
						,B.id_so_det
						,B.qty qty
						,B.id_proses
						,B.id_panel
					...
					INNER JOIN (
						SELECT id_sec_qc_detail,id_sec_qc_header,id_so_det,qty,id_item,id_panel,id_proses,qty_reject AS qty_rpr  FROM prod_sec_qc_detail 
					)B ON A.id_sec_qc_header = B.id_sec_qc_header
					WHERE A.id_sec_qc_header ='{$id_sec_qc}'	
				) SEC_QC ON 
				TABLE_BALANCE.id_so_det = SEC_QC.id_so_det
				AND TABLE_BALANCE.id_panel = SEC_QC.id_panel
				AND TABLE_BALANCE.id_item = SEC_QC.id_item
				AND TABLE_BALANCE.id_proses = SEC_QC.id_proses
	...
	public function generate_attr($_attr,$id_reff,$__data,$id_so_det){	
		...
		/* 	$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
			...
			else if($_attr == "is___next_qty"){
				$disabled = "disabled";
				$data_value = $__data['before_cumulative'];
			}
			else if($_attr == "is___balance_sec_qc"){
				$disabled = "disabled";
			...
			$_str = "<input type='number'"
					...
					."name='".$_attr."&".$id_reff."'"
					."value='".$_value."'"
					."data-balance='".$__data["balance_plus_qty_sec_qc"]."'" 
					."data-id_det='".$__data["id_sec_qc_det"]."'" 
	...
	public function generate_json_with_attribut($_data,$_populasi_kolom,$_populasi_attribut,$_key_det){
		...
		for($i=0;$i<count($_data['data']);$i++){ 	
			...
			for($k=0;$k<count($_populasi_attribut);$k++){
				$trigger = count($_populasi_attribut) - 1; //id_sec_qc_det
				$_attribut = $_populasi_attribut[$k];	
				$id_so_det= $_data['data'][$i]['id_so_det'];
				if($_data['data'][$i]['id_sec_qc_det'] == 0 ){
					$id_reff= $i;
				}
				else{
					$id_reff= $_data['data'][$i]['id_sec_qc_det'];
				}
