...
<script type="text/javascript">
  ...
  { var id_jo = $('#cboJO').val();
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_steam.php?modeajax=view_list_jo',
        data: {id_jo: id_jo},
        async: false
    }).responseText;
    if(html)
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_steam.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>
        ...
        <div class='col-md-3'>
          <div class='form-group'>
            ...
            <select class='form-control select2' multiple="multiple" style='width: 100%;' 
              ...
              <?php 
              $sql = "select a.id isi,concat(a.kpno,'|',a.styleno,'|',s.product_group,'|',s.product_item) tampil from 
                act_costing a inner join masterproduct s on a.id_product=s.id order by a.kpno";
              IsiCombo($sql,'','');
              ?>
            </select>
          </div>
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">List Steam Output</h3>
    <a href='../prod/?mod=11' class='btn btn-primary btn-s'>
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $query = mysql_query("select ac.kpno,co.dateinput,co.dateoutput,co.qty qtyout,sod.id,so.so_no,so.buyerno,sod.dest,sod.color,sod.size,sod.qty,so.unit
          from so inner join so_det sod on so.id=sod.id_so
          inner join steam_out co on sod.id=co.id_so_det
          inner join act_costing ac on so.id_cost=ac.id order by co.dateoutput desc "); 
        $no = 1; 
