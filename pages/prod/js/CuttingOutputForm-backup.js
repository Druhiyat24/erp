...
function generate_option(PropHtmlData, judul) {
	...
	for (i = 0; i < PropHtmlData.records.length; i++) {
		...
		var id_cut_in = decodeURIComponent(PropHtmlData.records[i].id_cut_in);
		option += '<option value="' + id_cost + '_' + id_jo + '_' + id_cut_in + '">' + ws + '</option>';
	}
	return option;
}
...
function OptionWS() {
	return new Promise(function (resolve, reject) {
		$.ajax("webservices/getListCutOutWS.php",
			{
				type: "POST",
				processData: false,
				contentType: false,
...
console.log($("#1numbering_"+i).val());
	for(var i=0;i<$detail.length;i++){
		...
		if(isNaN($t_1numbering)){
			alert("Numbering From baris ke-"+i+" Salah Format");
			$("#1numbering_"+i).val("0.00");
			return false;
		}
		if(isNaN($t_2numbering)){
			alert("Numbering To baris ke-"+i+" Salah Format");
			$("#2numbering_"+i).val("0.00")
			return false;
		}				
		if(isNaN($t_2numbering)){
			alert("Numbering To baris ke-"+i+" Salah Format");
		...
		if(isNaN($t_cutt)){
			alert("Cutting To baris ke-"+i+" Salah Format");
			$("#cutt_"+i).val("0.00")
			return false;
		}	
		if(isNaN($t_reject)){
			alert("Reject baris ke-"+i+" Salah Format");
			$("#reject_"+i).val("0.00")
			return false;
		}			
...
/* return false; */
	...
	$.ajax({
		...
		data: { code: '1', format: format, data: $data, detail: $detail, id_header: $id_cut_out },     // multiple data sent using ajax
		success: function (response) {
			data = response;
