$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Mut_Dc_Set.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_mut_dc_set_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListMutationDcSetDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListMutationDcSetDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},			
				"columns": [
					{
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_dc_set); //is___check_list
						},
						"className": "center"
					},
				],
...
/*		   if($tds.is(':checked')){
		...
		_js = {
			//is_checklist			: $checkeds,
			is_id_so_det			: $tds.data('id_so_det'), 
			is_id_det				: $tds.data('id_mut_dc_set_detail'),
			is_balance				: $tds.data('balance'),
			is_qty					: $tds.val(),
			is_next_qty				: $tds.data('next_qty'),
			is_qty_current			: $tds.data('qty_current')
		}		
		detail.push(_js);
	});
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_MutationDcSetForm.php",
			data: { code: '1', format: format, header: str_header, detail: str_det },     // multiple data sent using ajax
			success: function (response) {
				data = response;
				d = JSON.parse(data);
				if (d.respon == '200') {
					alert('Success Saved'); //8WP
					window.location = "../prod/?mod=MUT_DC_SET_PAGE";
					//window.location = "../prod/?mod=8LA&id_mut_dc_set_header="+d.id_mut_dc_set_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Sewing In Must be Smaller Than Qty Sewing Out!");
					return false;
				}					
			}
		});
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_mut_dc_set_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/MutationDcSet_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_mut_dc_set_header			= decodeURIComponent(json.records[0].id_mut_dc_set_header),	
		...
		Data.no_mut_dc_set          = decodeURIComponent(json.records[0].no_mut_dc_set)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_mut_dc_set").val(json.no_mut_dc_set),
...
var getTable = $('#table_detail tbody tr');
	getTable.each(function () {
		var $tds = $(this).find('input');
		console.log($tds[0].id+ " BALANCE : "+$tds.data('balance'));
		if($tds.data('balance') > 0){
			if ($(that).is(':checked')) {
