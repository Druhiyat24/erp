...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_rej_qc_fin_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListRejectQcFinFormDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListRejectQcFinFormDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_RejectQcFin.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=REJ_QC_FIN_PAGE";
					//window.location = "../prod/?mod=11&id_rej_qc_fin_header="+d.id_rej_qc_fin_header;
				}
				if (d.respon == '201') {
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_rej_qc_fin_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/RejectQcFinal_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_rej_qc_fin_header			= decodeURIComponent(json.records[0].id_rej_qc_fin_header),	
		...
		Data.no_rej_qc_fin          = decodeURIComponent(json.records[0].no_rej_qc_fin),
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_rej_qc_fin").val(json.no_rej_qc_fin),
...
function add_more_item(){
	...
	setTimeout(function(){
		table.ajax.url( "webservices/getListRejectQcFinFormDetail.php?id_cost=" + Data.id_cost + "&id_url=" + $id_url +"&add_more_item="+$add_more_item).load()		
	},2000)	
