$(window).on("load", function () {
	...
	get_url_old(url)//G_kondisi,format diinisialisasi disini $.ajax("webservices/getListSecInInhouseSubcon.php",
		.then(X => GenerateTableDetail("webservices/getListSecOutDetail.php?id_cost=0&id_url=0&id_proses=0&id_panel=0"))
		.then(X => Option("webservices/getListSecOutWS.php"))//OPTION LINE)
...
function getDetail() {
	...
	if ($populasi_id_url.G_kondisi == '1') {
		$id_url = $populasi_id_url.id_sec_out_header;
	}
	...
	// GenerateTableDetail("webservices/getListSecOutDetail.php?id_cost=" + $id_cost + "&id_url=" + $id_url + "&id_proses="+$str_id_proses+"&id_panel="+$str_id_panel)

	setTimeout(function () {
		table.ajax.url("webservices/getListSecOutDetail.php?id_cost=" + $id_cost + "&id_url=" + $id_url + "&id_proses=" + $str_id_proses + "&id_panel=" + $str_id_panel).load()
	}, 300);
}
		...
		"columns": [
			...
			},
			{
				...
				"render": function (data) {
					...
					return decodeURIComponent(data.is___tot_qty_sec_out)
				}
				//"className": "center"
			},
			{
				...
				"render": function (data) {
					...
					return decodeURIComponent(data.is___balance_sec_out)
				}
				//"className": "center"
			},
			{
			...
			},
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.is___qty_sec_out);
				},
			},
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.is___qty_reject_sec_out);
				},
			}
		],
...
function handleKeyUp(that) {
	...
	if (index_name[0] == "is___qty_reject_sec_out") {
		var a = parseInt($("#is___qty_reject_sec_out__" + idx).val() != '' ? $("#is___qty_reject_sec_out__" + idx).val() : 0)
		var b = parseInt($("#is___qty_sec_out__" + idx).val() != '' ? $("#is___qty_sec_out__" + idx).val() : 0)
		var c = parseInt($("#is___balance_sec_out__" + idx).attr("data-value"))
		var d = parseInt($("#is___tot_qty_sec_out__" + idx).attr("data-value"))

		var y = d + (b - a)
		var x = c - y
		$("#is___tot_qty_sec_out__" + idx).val(y)
		$("#is___balance_sec_out__" + idx).val(x)
		// console.log(a)
	}
	else if (index_name[0] == "is___qty_sec_out") {
		var a = parseInt($("#is___qty_reject_sec_out__" + idx).val() != '' ? $("#is___qty_reject_sec_out__" + idx).val() : 0)
		var b = parseInt($("#is___qty_sec_out__" + idx).val() != '' ? $("#is___qty_sec_out__" + idx).val() : 0)
		var c = parseInt($("#is___balance_sec_out__" + idx).attr("data-value"))
		var d = parseInt($("#is___tot_qty_sec_out__" + idx).attr("data-value"))
	...
	getTable.each(function (i) {
		...
		if ($tds.length > 0) {
			...
			*/
			...
			_js.is_qty = $("#is___qty_sec_out__" + $serpihan_id[2]).val();
			_js.is_rpr = $("#is___qty_reject_sec_out__" + $serpihan_id[2]).val();
...
function getListDataHeader($_pop_id_url) {
	if ($_pop_id_url.G_kondisi == '1') {
		my_id = $_pop_id_url.id_sec_out_header
...
function inisialisasi_data_header(json, kondisi) {
	if (kondisi == '1') {
		return [
			Data.id_sec_out_header = decodeURIComponent(json.records[0].id_sec_out_header),
			...
			Data.no_sec_out = decodeURIComponent(json.records[0].no_sec_out),
	...
	if (kondisi == '1') {
		setTimeout(function () {
			return [
				...
				$("#no_sec_out").val(json.no_sec_out),
...
function get_panel_proses(my_value) {
	...
	$("#id_panel").empty();
	setTimeout(function () {
		Option("webservices/getListSecOutProcess.php?id_cost=" + my_value) //getListSecInProcess
		...
		Option("webservices/getListSecOutPanel.php?id_cost=" + my_value) //getListSecInProcess
...
function get_insub(cost) {
	Option("webservices/getListSecOutInhouseSubcon.php?id_cost=" + cost)//OPTION LINE   )
		.then(data => generate_option(data, '--Choose Department--'))//OPTION LINE 
		.then(inj_option => injectOptionToHtml(inj_option, 'inhousesubcon')) //OPTION LINE 
}
