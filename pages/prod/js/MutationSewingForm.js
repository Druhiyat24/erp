 $(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Mut_sewing.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_mut_sew_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListMutationSewingDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListMutationSewingDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},			
				"columns": [
					{
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_mut_sew); //is___check_list
						},
						"className": "center"
					},					
...
/* 		   if($tds.is(':checked')){
		...
		_js = {
			...
			is_id_det				: $tds.data('id_mut_sew_detail'),
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_MutationSewingForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					...
					window.location = "../prod/?mod=MUT_SEW_PAGE";
					//window.location = "../prod/?mod=8LA&id_mut_sew_header="+d.id_mut_sew_header;
				}
				if (d.respon == '201') {
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_mut_sew_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/MutationSewing_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_mut_sew_header			= decodeURIComponent(json.records[0].id_mut_sew_header),	
