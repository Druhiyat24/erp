$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Qotiti_Out.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_qotiti_out_header;
	}
 	setTimeout(function(){
		table.ajax.url( "webservices/getListQotitiOutputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300); 
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListQotitiOutputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},					
				"columns": [
					{
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_qot_out);
						},
					},	
					{
						"data": null,
...
var getTable = $('#table_detail tbody tr');
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_QotitiOutputForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					setTimeout(function(){
						window.location = "../prod/?mod=QOT_OUT_PAGE";
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_qotiti_out_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/QotitiOutForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	if(kondisi =='1'){
	return [
		Data.id_qotiti_out_header		= decodeURIComponent(json.records[0].id_qotiti_out_header),	
		...
		Data.no_qotiti_out       = decodeURIComponent(json.records[0].no_qotiti_out)                     
	];                                           
	}
	else{
		return 1;
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_qotiti_out").val(json.no_qotiti_out),
...
var getTable = $('#table_detail tbody tr');
	getTable.each(function () { 
		...
		//console.log($tds[0].id); = $("#"+$tds[0].id).attr('data-balance');
