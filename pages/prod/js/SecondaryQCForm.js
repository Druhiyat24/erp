		...
		"columns": [
			...
			},
			{
				...
				"render": function (data) {
					...
					return decodeURIComponent(data.is___next_qty)
				}
				//"className": "center"
			},
			{
				...
				"render": function (data) {
					...
					return decodeURIComponent(data.is___balance_sec_qc)
				}
				//"className": "center"
			},
...
function handleKeyUp(that) {
	...
	if (index_name[0] == "is___qty_rpr") {
		var a = parseInt($("#is___qty_rpr__" + idx).val() != '' ? $("#is___qty_rpr__" + idx).val() : 0)
		var b = parseInt($("#is___qty_sec_qc__" + idx).val() != '' ? $("#is___qty_sec_qc__" + idx).val() : 0)
		var c = parseInt($("#is___balance_sec_qc__" + idx).attr("data-value"))
		var d = parseInt($("#is___next_qty__" + idx).attr("data-value"))

		var y = d + (b - a)
		var x = c - y
		$("#is___next_qty__" + idx).val(y)
		$("#is___balance_sec_qc__" + idx).val(x)
		// console.log(a)
	}
	else if (index_name[0] == "is___qty_sec_qc") {
		var a = parseInt($("#is___qty_rpr__" + idx).val() != '' ? $("#is___qty_rpr__" + idx).val() : 0)
		var b = parseInt($("#is___qty_sec_qc__" + idx).val() != '' ? $("#is___qty_sec_qc__" + idx).val() : 0)
		var c = parseInt($("#is___balance_sec_qc__" + idx).attr("data-value"))
		var d = parseInt($("#is___next_qty__" + idx).attr("data-value"))
...
function getListDataHeader($_pop_id_url) {
	if ($_pop_id_url.G_kondisi == '1') {
		...
		return new Promise(function (resolve, reject) {
			$.ajax("webservices/Sec_QC_GetListData.php",
				{
					type: "POST",
