$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Steam_In.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_steam_in_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListSteamInputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListSteamInputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},			
				"columns": [
					{
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_SteamInputForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					...
					window.location = "../prod/?mod=STEAM_IN_PAGE";
					//window.location = "../prod/?mod=8LA&id_steam_in_header="+d.id_steam_in_header;
				}
				if (d.respon == '201') {
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_steam_in_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/SteamInput_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_steam_in_header			= decodeURIComponent(json.records[0].id_steam_in_header),	
		...
		Data.no_steam_in          = decodeURIComponent(json.records[0].no_steam_in)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_steam_in").val(json.no_steam_in),
