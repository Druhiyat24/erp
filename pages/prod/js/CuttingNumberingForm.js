$(window).on("load", function () {
	...
	// split = url.split("=")
	// search_url = split[2].split("&")
	// id_url = search_url[0]
	// ws = split[3].split("&")
	// id_cost = ws[0]
	// color = decodeURI(split[4])
	url = window.location.search
	split = url.split("=")
	// alert(split[2])
...
function getOptionWS(_url) {
	...
	return new Promise(function (resolve, reject) {
		$.ajax("webservices/getListCutNumWS.php?id_url=" + _url, {
			type: "POST",
			processData: false,
			contentType: false,
...
function getDataTableDetail(e) {
	...
	if (x_dataTable == '0') {
		...
		TableDetail = $("#tbl_cut_num").DataTable({
			"processing": true,
			"serverSide": true,
			"bSort": false,
			"destroy": true,
			"searching": false,
			"ajax": "webservices/getListCutNumDetail.php?url=" + url + "&cost=" + cost,
			"columns": [
				{
					"data": "num",
					"className": "center"
				...
				},
				{
					...
					"render": function (data) {
						// console.log(data)
						return decodeURIComponent(data.qty_input_number)
					}
				},
				{
					"data": null,
					...
					"render": function (data) {
						// console.log(data)
						return decodeURIComponent(data.reject_qty)
					}
				},
				{
					"data": null,
					...
					"render": function (data) {
						// console.log(data)
						return decodeURIComponent(data.number_qty)
					}
				},
				{
					"data": null,
...
function handleKeyUp(that) {
	...
	if (index_array[0] == "cut") {
		detail[idx].number_cutting_val = vl
	}
	if (index_array[0] == "bundle") {
		detail[idx].number_bundle_val = vl
	}
	if (index_array[0] == "sack") {
		detail[idx].number_sack_val = vl
	}
	if (index_array[0] == "remark") {
		detail[idx].remarks_val = vl
	}
	if (index_array[0] == "qtyInNum") {
		detail[idx].qty_input_number_val = vl

		// var i = parseFloat($("#bal_" + idx).val())
		var j1 = parseFloat($("#qtyInNum_" + idx).val() != '' ? $("#qtyInNum_" + idx).val() : 0)
		var k1 = parseFloat($("#reject_" + idx).val() != '' ? $("#reject_" + idx).val() : 0)
		var x1 = j1 - k1

		$("#qtyNum_" + idx).val(x1)
		detail[idx].number_qty_val = x1
	...
	if (index_array[0] == "reject") {
		...
		// var i = parseFloat($("#bal_" + idx).val())
		var j2 = parseFloat($("#qtyInNum_" + idx).val() != '' ? $("#qtyInNum_" + idx).val() : 0)
		var k2 = parseFloat($("#reject_" + idx).val() != '' ? $("#reject_" + idx).val() : 0)
		var x2 = j2 - k2

		$("#qtyNum_" + idx).val(x2)
		detail[idx].number_qty_val = x2
...
function Save() {
	...
	if (Data.id == "") {
		...
		for (var i = 0; i < detail.length; i++) {
			if (detail[i].number_qty_val > detail[i].balance_val) {
				alert("Qty Number No More Than Balance Row to-" + detail[i].num)
				return false
			}
			else if (detail[i].number_qty_val < 0) {
				alert("Qty Number Cannot be Minus Row to-" + detail[i].num)
				return false
			}
		}
	}
		...
		for (var i = 0; i < detail.length; i++) {
			...
			if (detail[i].number_qty_val < 0) {
				alert("Qty Number Cannot be Minus Row to-" + detail[i].num)
				return false
			}
			// else if (detail[i].number_qty_val > detail[i].balance_val) {
			// 	alert("Qty Number No More Than Balance Row to-" + detail[i].num)
			// 	return false
			// }
		}
	...
	$.ajax({
		type: "POST",
		cache: false,
		url: "webservices/saveCuttingNumbering.php",
		...
		success: function (response) {
			...
			if (d.responds == '200') {
				...
				window.location.href = "?mod=numCut";
				// location.reload()
			}
			else {
...
function getListData($id) {
	return new Promise(function (resolve, reject) {
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Cut_Number_GetListData.php",
			data: { code: '1', id: $id },
			success: function (response) {
				data = response
				//console.log(data);
				...
				if (d.responds == '200') {
					...
					$("#cutNum").val(decodeURIComponent(d.records[0].no_cut))
					resolve(d)
				}
				else {
