$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Qc_F_In.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_qc_final_in_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListQcFinalInputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListQcFinalInputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
					...
					},					
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.balance_qc_final_in);
						},
					},
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_qc_final_in);
						},
					},	
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_QcFinalInputForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=QC_F_I_Page";
					//window.location = "../prod/?mod=QC_F_I_Form&id_sew_qc_final_in_header="+d.id_sew_qc_final_in_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Sewing  Qc Final In Must be Smaller Than Qty Sewing Qc In!");
					return false;
				}					
			}
		});
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_qc_final_in_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/QcFinalInputForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_sew_qc_final_in_header			= decodeURIComponent(json.records[0].id_sew_qc_final_in_header),	
		...
		Data.no_sew_final_qc_in          = decodeURIComponent(json.records[0].no_sew_final_qc_in)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_sew_final_qc_in").val(json.no_sew_final_qc_in),
