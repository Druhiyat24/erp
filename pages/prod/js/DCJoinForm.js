...
function OptionWS($id_edit) {
	return new Promise(function (resolve, reject) {
		/*  */
		$.ajax("webservices/getListDCJoinWS.php?id_url=" + $id_edit,
			{
				type: "POST",
				processData: false,
				contentType: false,
...
function handleKeyUp(that) {
	...
	if (index_array[0] == 'qtyInJoin') {
		Detail[idx].qty_input_join_val = vl

		var i = parseFloat($("#qtyInJoin_" + idx).val() != '' ? $("#qtyInJoin_" + idx).val() : 0)
		var j = parseFloat($("#reject_" + idx).val() != '' ? $("#reject_" + idx).val() : 0)
		var x = i - j

		$("#qtyJoin_" + idx).val(x)
		Detail[idx].qty_dc_join_val = x
	...
	if (index_array[0] == 'reject') {
		Detail[idx].reject_dc_join_val = vl
	...
	return new Promise(function (resolve, reject) {
		...
		if (x_datatable == '0') {
			...
			TableDetail = $('#detail_item').DataTable({
				...
				"destroy": true,
				"searching": false,
				"ajax": "webservices/getListDCJoinDetail.php?id_ws=" + myValue + "&id_url=" + $id_edit,
				"columns": [
					{
						"data": "no",
						"className": "center"
					...
					{ "data": "number_sack" },
					{
						...
						"render": function (data) {
							// console.log(data)
							return decodeURIComponent(data.qty_input_join)
						}
					},
					{
						"data": null,
						"render": function (data) {
							// console.log(data);
							return decodeURIComponent(data.reject_dc_join)
						}
					},
					{
						"data": null,
						"render": function (data) {
							// console.log(data);
							return decodeURIComponent(data.qty_dc_join)
						}
					}
					// {
					// 	"data": null,
...
function Cancel() {
	// alert('123')
	window.location = "/pages/prod/?mod=6WP"
}
...
function Save() {
	...
	if (Data.id == "") {
		...
		for (var i = 0; i < Detail.length; i++) {
			if (Detail[i].qty_dc_join_val > Detail[i].balance_val) {
				alert("Qty DC Join No More Than Balance Row to-" + Detail[i].no)
				return false
			}
			else if (Detail[i].qty_dc_join_val < 0) {
				alert("Qty DC Join Cannot be Minus Row to-" + Detail[i].no)
				return false
			}
		}
	}
		...
		for (var i = 0; i < Detail.length; i++) {
			if (Detail[i].qty_dc_join_val < 0) {
				alert("Qty DC Join Cannot be Minus Row to-" + Detail[i].no + " And =>" + Detail[i].qty_dc_join_val)
				return false
			}
			// else if (Detail[i].qty_dc_join_val > Detail[i].balance_val) {
			// 	alert("Qty DC Join No More Than Balance Row to-" + Detail[i].no)
			// 	return false
			// }
		}
	...
	$.ajax({
		type: "POST",
		cache: false,
		url: "webservices/saveDCJoin.php",
		...
		success: function (response) {
			...
			if (d.respon == '200') {
				alert('Data Succes on Save')
				window.location.href = "?mod=6WP"
			}
			else {
				alert('Data Failed Entered !!!')
			}
		}
	})
}
	...
	if ($G_kondisi == 0) {
		...
		return new Promise(function (resolve, reject) {
			$.ajax({
				type: "POST",
				cache: false,
				url: "webservices/DC_Join_GetListData.php",
				data: { code: '1', id: $id },
				success: function (response) {
					data = response;
					// console.log(data);
					...
					if (d.respond == '200') {
						...
						$("#djnum").val(decodeURIComponent(d.records[0].num))
					}
					else {
						alert('Erorr!')
					}
