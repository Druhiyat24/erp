$(document).ready(function () {
	url = window.location.search;
	get_url(url)
	.then(X						=> auth($populasi_id_url.mod,"page"))
	.then(akses					=> is_route(akses))
	.then(X 					=> GenerateTable())
	
	
	//GenerateTable();
});



function GenerateTable() {
	table = $("#sewing").DataTable({
		'processing': true,
		'serverSide': true,
		"lengthMenu": [[999999999], ["All"]],
		"ajax": {
			"url": "webservices/getListSewingInput.php?mod="+$populasi_id_url.mod,
			"type": "POST"
		},
		"columns": [
			//description
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.no_sew_in);
				},
				"className": "center"
			},
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.buyer);
				},
				"className": "center"
			},
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.styleno);
				},
				"className": "center"
			},
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.kpno);
				},
				"className": "center"
			},		
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.so_no);
				},
				"className": "center"
			},	
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.buyerno);
				},
				"className": "center"
			},	
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.date_output);
				},
				"className": "center"
			},	
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.time);
				},
				"className": "center"
			},		
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.created_by);
				},
				"className": "center"
			},	
			{
				"data": null,
				"render": function (data) {
					return decodeURIComponent(data.is___edit)+" "+decodeURIComponent(data.is___view);
				},
				"className": "center"
			}	
			
		],
		"autoWidth": true,
		"scrollCollapse": true,
		scrollY: "500px",
		scrollX: true,
		scrollCollapse: true,

		"destroy": true,
		//order: [[ 1, "asc" ]],
		"ordering": true,
		/*         fixedColumns:   {
					leftColumns: 7
				}, */
		dom: 'Bfrtip',

	});
}


// $(".addNew").on("click", function addNew() {
// 	// alert("ok");
// 	window.location = "/prod/?mod=2LA";
// })

function addNew_() {
	// alert("ok");
	window.location = "./?mod=8LA&cr=is_add";
}


function edit(id_sew_in) {
	window.location = "/prod/?mod=8LA&id=" + id_sew_in +"&cr="+is_add;
}