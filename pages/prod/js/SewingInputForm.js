$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Si.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_in;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListSewingInputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListSewingInputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columnDefs": [
						{
				...
				"columns": [
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.qty_dc_out);
						},
					},		
					{
						"data": null,
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_sew_in); 
						},
					},	
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___checklist); //is___check_list
						},
						"className": "center"
					},					
...
function Save() {
	...
	getTable.each(function () {
		...
		_js = {
			...
			is_id_sew_in_det		: $tds.data('id_sew_in_det'),
			is_balance				: $tds.data('balance'),
			is_qty_input			: $tds.data('qty'),
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/saveSewingInput.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					...
					window.location = "../prod/?mod=8WP";
					//window.location = "../prod/?mod=8LA&id_sew_in="+d.id_sew_in;
				}
				if (d.respon == '201') {
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_in
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/Sew_In_GetListData.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_si			= decodeURIComponent(json.records[0].id_sew_in_header),	
		...
		Data.no_sew_in          = decodeURIComponent(json.records[0].no_sew_in)   
