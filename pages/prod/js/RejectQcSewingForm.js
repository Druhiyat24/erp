$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Rej_Qc_sew.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_rej_qc_sew_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListRejectQcSewingFormDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
	setTimeout(function(){
		$(".sel_defect").select2();
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListRejectQcSewingFormDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.no_sew_qc_out);
						},
						//"className": "center"
					},					
					{
...
function Save() {
	...
	getTable.each(function () {
		_js = {};
		   ...
		   var $tdselect = $(this).find('td').find('select');
		if($tds.length > 0){
		  // var $tds = $(this).find('input');
		   if($tds.is(':checked')){
			   $checkeds = '1';
			...
			_js.is_id_defect			= $tdselect[0].value;
			_js.is_checklist			= $checkeds
			_js.is_id_det				= $("#"+$tds[0].id).attr('data-id_detail');
			_js.is_id_header			= $("#"+$tds[0].id).attr('data-id_header');	
		}		   
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_RejectQcSewing.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=REJ_QC_SEW_PAGE";
					//window.location = "../prod/?mod=11&id_rej_qc_sew_header="+d.id_rej_qc_sew_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '277') {
					hide_loading();
					alert("Defect Must Be Fill On Row "+d.baris);
					return false;
				}
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_rej_qc_sew_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/RejectQcSewingForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_rej_qc_sew_header			= decodeURIComponent(json.records[0].id_rej_qc_sew_header),	
		...
		Data.no_rej_qc_sew          = decodeURIComponent(json.records[0].no_rej_qc_sew),
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_rej_qc_sew").val(json.no_rej_qc_sew),
...
function checklist_change(that){
	__id = that.id;
	pecah = __id.split("__");
	id_check = "is___checklist__"+pecah[1];
	if(that.value !=""){
...
function add_more_item(){
	...
	setTimeout(function(){
		table.ajax.url( "webservices/getListRejectQcSewingFormDetail.php?id_cost=" + Data.id_cost + "&id_url=" + $id_url +"&add_more_item="+$add_more_item).load()		
	},2000)	
