$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Sew_Qc_In.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_qc_in;
	}
 	setTimeout(function(){
		table.ajax.url( "webservices/getListSewingQcInputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300); 
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListSewingQcInputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
				...
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_sew_qc_in);
						},
					},	
...
function Save() {
	...
	getTable.each(function (i) {
		...
		_js = {
			...
			is_id_sew_qc_in_det		: $tds.data('id_sew_qc_in_det'),
			is_balance				: $tds.data('balance'),
			is_qty_qc_input			: $tds.val(),
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/saveSewingQcInput.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=10WP";
					//window.location = "../prod/?mod=10LA&id_sew_qc_in="+d.id_sew_qc_in;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Sewing Qc In Must be Smaller Than Qty Sewing Qc Out! " +d.baris);
					return false;
				}
				if (d.respon == '207') {
					hide_loading();
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_qc_in
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/Sew_Qc_In_GetListData.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	if(kondisi =='1'){
	return [
		Data.id_sew_qc_in		= decodeURIComponent(json.records[0].id_sew_qc_in_header),	
		...
		Data.no_sew_qc_in       = decodeURIComponent(json.records[0].no_sew_qc_in)                     
	];                                           
	}
	else{
		return 1;
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_sew_qc_in").val(json.no_sew_qc_in),
