...
SELECT 
				...
				FROM invoice_commercial A 
					...
					WHERE IH.n_typeinvoice = '$type_invoice' AND IH.n_post >= '1'
					GROUP BY A.n_id)X";
...
//echo $row['n_post'];
		...
		if($row['n_post'] != 2){
			...
			$button .=" <a href='#' class='btn btn-primary' onclick='print_new(".'"'. $row['n_id'] .'",'.'"'. $row['n_typeinvoice'] .'"'.")' ><i class='fa fa-file-pdf-o'> </i></a>";					
