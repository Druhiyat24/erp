...
async function getListWS($id_customer){
		check_customer("N",$id_customer,"SHP").then(function($responnya) {
	...
	if(ids != 0){
		...
		CallServiceWs($id_customer).then(function($responnya) {
			...
			//GenerateDetail($my_that,'EXS',ids);
				...
				alert("Tidak Dapat Dibuat PL / Edit PL / Inv Karena Buyer di block");
					$("#head_costumer").val("").trigger("change");
					Mydata.head_costumer = "";
