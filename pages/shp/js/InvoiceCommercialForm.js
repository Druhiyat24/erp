        ...
        success: function (response) {
			...
			data = response;
				...
				if(d.message == '1'){
					...
					datas.to                =decodeURIComponent(d.records[0].id_buyer);
					...
					datas.id_buyer				=decodeURIComponent(d.records[0].id_buyer);
					datas.discount			=decodeURIComponent(d.records[0].discount); 
					datas.fg_discount			=decodeURIComponent(d.records[0].fg_discount); 
					console.log(datas.idcoa);
					datas.save            	="UPDATE"; 					
					getNoBPB(datas.bpb_no )
//$("#head_costumer").val(Mydata.id_buyer).trigger("change")					
					setTimeout(function(){ 				
						default_data();
				}, 4000);	
					console.log(datas);
...
/* 	alert(datas.to); */
	...
	await      $("#to").val(datas.to).trigger("change");
	await $("#po").val(datas.po);
	await $("#amount").val(datas.amount);
	await $("#discount").val(datas.discount);
	document.getElementById("fg_discount").checked = true;
	toggleDiscount();
	console.log(datas.idcoa);
...
function handlekeyup(Item){
	...
	}else if(Item.id == "discount"){
		datas.discount = Item.value;
	}	
}
