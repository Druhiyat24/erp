...
if ($mod == "memo_list") {
  ...
  <div class="box">
    <div class="box-header">
      <a href='../shp/?mod=memo_new&id_h=' class='btn btn-primary btn-s'>
    ...
    <div class="box-body">
      <table id="examplefix" class="display responsive" style="width:100%">
        ...
        <tbody>
          ...
          inner join mastersupplier mb on a.id_buyer = mb.id_supplier where jns_inv = 'INVOICE' and tgl_memo >= '$tglf' and tgl_memo <= '$tglt' order by id_h desc");
          $no = 1;
          while ($data = mysql_fetch_array($query)) {
            ...
            <td>$data[status]</td>
            <td>
            <a href='../shp/?mod=memo_edit&id_h=$data[id_h]'
            data-toggle='tooltip' ><i class='fa fa-pencil'></i>
            </a>
            <a href='cetak_memo.php?id_h=$data[id_h]'
...
function getinvoice()
  ...
  { var id_kat_add = $('#cbokat').val();
    ...
    ({  type: "POST",
        ...
        data: {id_kat_add: id_kat_add,id_sub_kat_add:id_sub_kat_add,biaya_add:biaya_add },
        async: false
    }).responseText;
    if(html)
    {  
  ...
  { var html = $.ajax
    ({  type: "POST",
        ...
        data: {id_book_temp: id_book_temp },
        async: false
    }).responseText;
    if(html)
    {  
  ...
  <div class='box'>
    <div class='box-body'>
      <div class='row'>
        <form method='post' name='form' action='save_memo.php?mod=simpan_header'>
          <div class='col-md-5'>
            ...
            <div class='row'>
              <div class='col-md-6'>
                <div class='form-group'>
                  <label>Tanggal :</label>
                  <input type='text' class='form-control' id='datepicker1' name='txtmemodate' required placeholder='Masukkan Request Date' value='<?php echo $reqdate; ?>'>
                </div>
              </div>
              <div class='col-md-6'>
                <div class='form-group'>
                  ...
                  <select class='form-control select2' style='width: 100%;' name='cbokpd' id='cbokpd' required>
                    <?php
                    $sql = "select department_name isi, department_name tampil from masterdepartment group by department_name";
              ...
              <div class='col-md-6'>
                <div class='form-group'>
                  ...
                  <select class='form-control select2' id='jns_trans' name='jns_trans' value='<?php echo $jns_trans; ?>' required>
                    ...
                    <option value="EXPORT" <?php if ($jns_trans == "EXPORT") {echo "selected";} ?>>EXPORT</option>
                    <option value="IMPORT" <?php if ($jns_trans == "IMPORT") {echo "selected"; } ?>>IMPORT</option>
                    <option value="LOKAL" <?php if ($jns_trans == "LOKAL") { echo "selected";} ?>>LOKAL</option>
                  </select>
                </div>
              </div>
              <div class='col-md-6'>
                <div class='form-group'>
                  ...
                  <select class='form-control select2' id='jns_pengiriman' name='jns_pengiriman' value='<?php echo $jns_pengiriman; ?> ' required>
                    ...
                    <option value="UDARA" <?php if ($jns_pengiriman == "UDARA") {echo "selected";} ?>>UDARA</option>
                    <option value="LAUT" <?php if ($jns_pengiriman == "LAUT") {echo "selected";} ?>>LAUT</option>
                    <option value="DARAT" <?php if ($jns_pengiriman == "DARAT") {echo "selected";} ?>>DARAT</option>
                  </select>
                </div>
              </div>
            </div>
              ...
              <div class='col-md-6'>
                <div class='form-group'>
                  ...
                  <select class='form-control select2' id='ditagihkan' name='ditagihkan' value='<?php echo $ditagihkan; ?> ' required>
                    ...
                    <option value="Y" <?php if ($ditagihkan == "Y") {echo "selected";} ?>>Y</option>
                    <option value="N" <?php if ($ditagihkan == "N") {echo "selected";} ?>>N</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          <div class='col-md-7'>
            <div class='form-group'>
              <label>Penjelasan Biaya Invoice :</label>
              <select class='form-control select2' multiple='multiple' style='width: 100%;' name='cboinv' id='cboinv' onchange='get_list_invoice()' required>
              </select>
            </div>
          </div>
          <div class='col-md-7'>
  ...
  <?php }
?>
<?php if ($mod == "memo_edit") {
$id_h=$_GET['id_h']; 
$username =  $_SESSION['username'];
$sql_clear_tmp  = mysql_query("delete from memo_det_tmp where user = '$user'");
$row_cari = mysql_fetch_array($sql_clear_tmp);

$sql_cek_data  = mysql_query("select * from memo_h where id_h = '$id_h'");
...
<script type='text/javascript'>
  ...
  <div class='box'>
    <div class='box-body'>
      <div class='row'>
        <form method='post' name='form' action='save_memo.php?mod=update_header&id_h=<?php echo $id_h; ?>'>
          <div class='col-md-5'>
          <div class='form-group'>
          ...
          <div class='col-md-7'>
            <div class='form-group'>
              <label>Penjelasan Biaya Invoice :</label>
              <!-- <select class='form-control select2' multiple='multiple' style='width: 100%;' name='cboinv' id='cboinv'>
              </select> -->
            </div>
          </div>          
          <div class='col-md-7'>
        ...
        <tbody>
          <?php
          # QUERY TABLE
          $query = mysql_query("select b.no_invoice , so_number, shipp_number,buyerno, season,reff_no, concat(ac.styleno,styleno_prod) style ,sum(det.qty) qty  from memo_inv a
