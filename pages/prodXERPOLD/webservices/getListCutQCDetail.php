		...
		FROM (
			...
			AND cnd.id_cut_number_detail IN (
				SELECT d_num.id_cut_number_detail FROM (
					...
					GROUP BY d_cod.id_grouping,d_cod.id_panel,d_cnd.id_so_det		
				) AS d_num
			)

			GROUP BY cod.id_grouping,cod.id_panel,cnd.id_so_det
		
		) AS qc_new

		LEFT JOIN (
			{$sql_balance}
		) AS qc_old ON qc_old.id_so_det = qc_new.id_so_det AND qc_old.id_panel = qc_new.id_panel AND qc_old.id_item = qc_new.id_item
...
while ($row = mysqli_fetch_assoc($empRecords)) {
	...
	$inputQtyInQC = '';
	$inputQtyInQC .="<input type='text' id='qtyInQC_".$no."' class='form-control forms' value='$qtyInQC' onchange='handleKeyUp(this)' style='min-width: 80px'>";
