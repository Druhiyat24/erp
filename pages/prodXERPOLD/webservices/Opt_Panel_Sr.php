...
if($code == '1'){
	$Proses = new Proses();
	$_outp = $Proses->get_populasi_panel($_GET['id_item'],$_GET['id_jo'],$_GET['color']);
...
class Proses {
	...
	public function get_populasi_panel($id_item,$id_jo,$color){
		$sql = "SELECT CONCAT(A.nama_panel,' | ',B.itemdesc )isi,A.id_panel id FROM view_populasi_panel_rasio A
					INNER JOIN masteritem B ON A.id_item = B.id_item WHERE A.id_item='{$id_item}' AND A.id_jo='{$id_jo}' AND A.color='{$color}' ";
