...
class Proses {
	...
	public function get_nilai($_populasi_mark,$id_mark_entry,$color,$id_cost,$id_panel,$item){
		// print_r($item);die();
		for($i=0;$i<count($_populasi_mark);$i++){
			$_populasi_mark[$i]->update = $this->is_update($_populasi_mark[$i]->size,$color,$id_mark_entry,$id_cost,$id_panel,$item);
			$_populasi_mark[$i]->detail = $this->detail($_populasi_mark,$_populasi_mark[$i]->size,$color,$id_mark_entry,$id_cost,$id_panel,$item);
	...
	public function is_update($size,$color,$id_mark_entry,$id_cost,$id_panel,$id_item){
		...
		$sql ="SELECT 
			...
			FROM(
				...
				FROM prod_mark_entry_detail A
				LEFT JOIN mastersize B ON A.size = B.size
				WHERE A.id_cost='{$id_cost}' AND A.id_mark='{$id_mark_entry}' 
				AND A.size ='{$size}' AND A.id_panel='{$id_panel}' 
				AND A.id_item='{$id_item}' GROUP BY A.size
			)X ORDER BY X.urut
		";
		// echo $sql;die();
	...
	public function detail($_pop_mark,$size,$color,$id_mark_entry,$id_cost,$id_panel,$id_item){
		...
		$sql = "SELECT    
				...
				FROM(
					...
					INNER JOIN prod_mark_entry_sum PS ON PS.id_mark_entry = A.id_mark AND PS.color = A.color 
					AND PS.id_panel = A.id_panel AND PS.id_item = A.id_item
					WHERE A.id_cost='{$id_cost}' AND A.id_mark='{$id_mark_entry}' AND A.color = '{$color}' AND A.size='{$size}'
					AND A.id_panel='{$id_panel}' AND A.id_item='{$id_item}' 
				)X ORDER BY X.id_group_det
		";
		// echo $sql;die();
