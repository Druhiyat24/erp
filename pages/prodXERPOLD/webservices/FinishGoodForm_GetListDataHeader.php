...
class Proses {
	...
	public function get_list_data($id){
		$sql = "SELECT id_finishing_fg_header,date_output,no_finishing_fg,notes,id_cost,id_line,username,dateinput,time 
					FROM prod_finishing_fg_header WHERE id_finishing_fg_header ='{$id}'";
		$row = $this->eksekusi_query($sql,"get_list_data");
		$_pop_size = array();
		$outp = '';
		for($i=0;$i<count($row);$i++){
			if ($outp != "") {$outp .= ",";}
			$outp .= '{"id_finishing_fg_header":"'.rawurlencode($row[$i]['id_finishing_fg_header']).'",';		
			$outp .= '"date_output":"'. rawurlencode(date('d M Y',strtotime($row[$i]["date_output"]))).'",';	
			$outp .= '"no_finishing_fg":"'. rawurlencode($row[$i]["no_finishing_fg"]).'",';	
