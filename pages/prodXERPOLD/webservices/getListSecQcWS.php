...
class Proses {
	...
	public function get_populasi_ws(){
		$sql = "SELECT id id,kpno isi FROM act_costing WHERE id IN(SELECT id_cost FROM prod_sec_out_header GROUP BY id_cost) GROUP BY id";
		$row = $this->eksekusi_query($sql,"get_populasi_ws");
		$_pop_size = array();
		$outp = '';
