...
/* 		$_str = "<input type='checkbox' onclick='check_uncheck(this)' checked class='{$_attr}[]' data-bppbno='{$bppbno_req}' data-id='".$_attr."__".$id_reff."' >"; */
		...
		if($pecah_[1] == "select"){
			...
			$_str = "<input data-id='".$_attr."__".$id_reff."' type='text' data-balance='".$__data["balance_plus_qty_qcf_out"]."' data-id_det='".$__data["id_det"]."' data-id_so_det='".$id_so_det."' class='form-control {$_attr}' id='".$_attr."__".$id_reff."' value='".$_value."'data-next_qty='".$__data["next_qty"]."'>";
		}
		return $_str;
	}
	...
	SELECT  TABLE_BALANCE.id_dc_out_header
			...
			,(TABLE_BALANCE.balance_qc_final_output + ifnull(QCF_OUT.qty_qc_final_out,0))balance_plus_qty_qcf_out
			,TABLE_BALANCE.urut
			...
			{$sql_balance}
				...
				LEFT JOIN(
					...
					INNER JOIN(
						SELECT id,rpr,id_defect,id_so_det,id_sew_qc_final_out_header,qty FROM prod_qc_final_out_detail 
