...
$akses = flookup("mfg_output","userpassword","username='$user'");
if ($akses=="0") 
...
<script type='text/javascript'>
  ...
  { var dateout = document.form.txtdateout.value;
    ...
    var subkon = document.form.txtSubkon.value;
    var nm_subkon = document.form.txtNmSubkon.value;
    var id_pro = document.form.txtPro.value;
    ...
    else if (subkon == '') { swal({ title: 'Inhouse / Subkon Tidak Boleh Kosong', <?php echo $img_alert; ?> }); valid = false;}
    else if (nm_subkon == '') { swal({ title: 'Dept / Subkon Tidak Boleh Kosong', <?php echo $img_alert; ?> }); valid = false;}
    else if (id_pro == '') { swal({ title: 'Process Tidak Boleh Kosong', <?php echo $img_alert; ?> }); valid = false;}
...
<script type="text/javascript">
  ...
  { var id_jo = document.form.txtJOItem.value;
    var id_mfg = document.form.txtPro.value;
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_mfg.php?modeajax=view_list_jo',
        data: {id_jo: id_jo,id_mfg: id_mfg},
        async: false
    }).responseText;
    if(html)
    {  
  ...
  function getListSub()
  { var jen_sub = document.form.txtSubkon.value;
    var html = $.ajax
    ({  type: "POST",
        url: 'ajax_mfg.php?modeajax=get_list_sub',
        data: {jen_sub: jen_sub},
        async: false
    }).responseText;
    if(html)
    {  
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_mfg.php?mod=<?php echo $mod; ?>' onsubmit='return validasi()'>
        <div class='col-md-3'>
        ...
        <div class='col-md-3'>
          <div class='form-group'>
            ...
            <select class='form-control select2' style='width: 100%;' 
              name='txtSubkon' onchange='getListSub()'>
              <?php 
              $sql = "select if(nama_pilihan='INHOUSE','N','Y') isi,nama_pilihan tampil from 
        ...
        <div class='col-md-3'>
          <div class='form-group'>
            <label>Process *</label>
            <select class='form-control select2' style='width: 100%;' 
              name='txtPro' onchange='getListJO()'>
              <?php 
              $sql = "select id isi,concat(cfcode,'-',cfdesc) tampil from 
                mastercf";
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">List Secondary Process Output</h3>
    <a href='../prod/?mod=3' class='btn btn-primary btn-s'>
  ...
  <div class="box-body">
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        <?php
        # QUERY TABLE
        $query = mysql_query("select ac.styleno,ac.kpno,ms.supplier,co.dateinput,co.dateoutput,co.qty qtyout,sod.id,so.so_no,so.buyerno,sod.dest,sod.color,sod.size,sod.qty,so.unit
          from so inner join so_det sod on so.id=sod.id_so
          inner join mfg_out co on sod.id=co.id_so_det 
