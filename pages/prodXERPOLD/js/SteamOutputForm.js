$(window).on("load", function () {
	...
	get_url(url)//G_kondisi,format diinisialisasi disini
		...
		.then(X 					=> Option("webservices/Opt_Ws_Steam_Out.php"))//OPTION WS   )
...
function getDetail(that){
	if($populasi_id_url.G_kondisi == '1'){
		$id_url = $populasi_id_url.id_sew_steam_out_header;
	}
	setTimeout(function(){
		table.ajax.url( "webservices/getListSteamOutputDetail.php?id_cost=" + that.value + "&id_url=" + $id_url).load()		
	},300);
}
	...
	if ($x_datatable == '0') {
		...
		$.fn.dataTable.ext.errMode = 'none';
			table = $("#table_detail").DataTable({
				...
				"ajax": {
					"url": "webservices/getListSteamOutputDetail.php?id_cost=" + $_id_cost + "&id_url=" + $_id_url,
					"type": "POST"
				},
				"columns": [
					{
					...
					},
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.tot_qty_qc_out);
						},
					},	
					{
						"data": null,
					...
					},					
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.balance_steam_out);
						},
					},
				
					{
						"data": null,
						"render": function (data) {
							return decodeURIComponent(data.is___qty_steam_out);
						},
					},	
...
function Save() {
	...
	setTimeout(function () {
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "webservices/Save_SteamOutputForm.php",
			...
			success: function (response) {
				...
				if (d.respon == '200') {
					alert('Success Saved');
					window.location = "../prod/?mod=11L";
					//window.location = "../prod/?mod=11&id_sew_steam_out_header="+d.id_sew_steam_out_header;
				}
				if (d.respon == '201') {
				...
				if (d.respon == '206') {
					hide_loading();
					alert("Qty Steam be Smaller Than Qty Sewing Qc Final In!");
					return false;
				}
...
function getListDataHeader($_pop_id_url){
	if($_pop_id_url.G_kondisi == '1'){
		my_id = $_pop_id_url.id_sew_steam_out_header
	
	return new Promise(function(resolve, reject) {
	$.ajax("webservices/SteamOutputForm_GetListDataHeader.php",
		{
			type: "POST",
...
function inisialisasi_data_header(json,kondisi){
	...
	return [
		Data.id_sew_steam_out_header			= decodeURIComponent(json.records[0].id_sew_steam_out_header),	
		...
		Data.no_sew_steam_out          = decodeURIComponent(json.records[0].no_sew_steam_out)   
...
function inejectHeaderToHtml(json,kondisi){
	if(kondisi == '1'){
		setTimeout(function(){
			...
			$("#time").val(json.time),
			$("#no_sew_steam_out").val(json.no_sew_steam_out),
