$(window).on("load", function () {
	...
	get_url(url)
		...
		.then(_color_item => getDetail($color, $item))
	format = 1;
...
$x_datatable = 0
function getDetail($_color, $_item) {
	...
	if ($x_datatable == '0') {
		...
		TableDetail = $('#tbl_mroll').DataTable({
			...
			"destroy": true,
			"bAutoWidth": false,
			"ajax": "webservices/getListMRollDetail.php?color=" + $_color + "&item=" + $_item,
			"columns": [
				{
					"data": "no",
					"className": "center"
...
function Save() {
	...
	$.ajax({
		...
		data: { code: '1', detail: detail64, color: $color, item: $item },     // multiple data sent using ajax
