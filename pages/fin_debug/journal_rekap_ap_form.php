...
SELECT JH.id_journal
		...
		LEFT JOIN po_header POH ON POH.id = JD.id_po
		LEFT JOIN mastersupplier MSS ON MSS.Id_Supplier = JD.id_supplier
		LEFT JOIN masterpterms MPT ON MPT.id = POH.id_terms

		WHERE 1  AND JH.id_journal NOT IN (SELECT v_nojournal FROM fin_status_journal_ap WHERE 1=1 AND v_status IN('A','W','S') AND is_deleted = 'N' )  AND JH.type_journal = '14' AND JH.fg_post='2'
...
,o.id_po			
    ...
    $sql = "
		...
		,(PO.pono) po_no
		,(PO.jml_pterms) days_pterms
		,DATE_ADD(JOURNAL_HEADER.date_journal, INTERVAL PO.jml_pterms DAY) as  jatuh_tempo			
		,SUPPLIER.Supplier
		,SUPPLIER.supplier_code
		,JOURNAL_DETAIL.amount nilai_kontrabon
		,(JOURNAL_DETAIL.curr)curr
			FROM fin_status_journal_ap A
				...
				ON A.v_nojournal = JOURNAL_HEADER.id_journal
					...
					LEFT JOIN(
						SELECT po_h.id,po_h.pono,po_h.id_terms,terms.days_pterms,poi.curr,po_h.podate,po_h.jml_pterms FROM po_header po_h LEFT JOIN(
					...
					)PO ON PO.id = JOURNAL_DETAIL.id_po	
		LEFT JOIN(
