...
SELECT   INV.id
		,INV.invno
		#,ifnull(if(INV_COM.bpbno ='','N/A',INV_COM.bpbno)
		#	,INV_COM.bpbno )surat_jalan
		,BKBB.BKBB AS surat_jalan
		,INV.id_buyer
		,INV.id_pterms
		,MP.kode_pterms
		,MR.rate
	...
	SELECT 
		...
		) AS BKBB ON BKBB.id_invoice=INV.id
	#LEFT JOIN(
	...
	ON MR.tanggal = INV.invdate
		WHERE (JH.date_journal >='{$d_from}' AND JH.date_journal <='{$d_to}') AND MS.area='I' AND INV.n_post = '2' AND JD.id_coa IS NOT NULL
)X";
...
$searchQuery = " AND (
	...
	OR X.invdate                    LIKE'%".$searchValue."%'
	OR X.surat_jalan                LIKE'%".$searchValue."%'
	OR X.invno                      LIKE'%".$searchValue."%'
	OR X.tgl_kb                     LIKE'%".$searchValue."%'
	OR X.nomor_kb                   LIKE'%".$searchValue."%'
	OR X.v_fakturpajak			    LIKE'%".$searchValue."%'
	OR X.tgl_fakturpajak		    LIKE'%".$searchValue."%'
	...
	OR X.saldo_awal                 LIKE'%".$searchValue."%'
	OR X.debit_usd                  LIKE'%".$searchValue."%'
	OR X.debit                      LIKE'%".$searchValue."%'    
	OR X.lain_lain_usd              LIKE'%".$searchValue."%'
	OR X.lain_lain                  LIKE'%".$searchValue."%'
	OR X.ref_doc_pengurangan        LIKE'%".$searchValue."%'
	OR X.tgl_pengurangan            LIKE'%".$searchValue."%'
	OR X.keterangan_pengurangan      LIKE'%".$searchValue."%'
	OR X.pengurangan_usd            LIKE'%".$searchValue."%'
	OR X.pengurangan                LIKE'%".$searchValue."%'
	OR X.retur                      LIKE'%".$searchValue."%'
	OR X.retur_usd                  LIKE'%".$searchValue."%'
	OR X.pengurangan_lain_lain      LIKE'%".$searchValue."%'
	OR X.pengurangan_lain_lain_usd  LIKE'%".$searchValue."%'
	OR X.saldo_akhir                LIKE'%".$searchValue."%'
