...
class Model{
   ...
   SELECT FD.debit,FH.* FROM
	...
	if(ISSET($type_nya)){
		if($type_nya_ == 'GEN'){
			$where_poi = "AND if(poh.fg_pkp > 0 OR poh.ppn > 0,(bpb.qty*poi.price)+((poh.ppn/100)*poi.price),bpb.qty*poi.price) > 0";
		}else{
			$type_nya = "";
        ...
        $bpb_detail = $this->query("SELECT X.* FROM (
SELECT 
            bpb.bpbno
			,bpb.id id_bpb
			,bpb.id_po_item 
            ...
            WHERE 
                 trim(bpb.bpbno_int) = '".trim($id)."'  $where_poi )X  GROUP BY X.id_bpb
        ")->result();
...
/* 			if($IsGeneral == '1'){
    ...
    public function check_rekap($id, $checking = TRUE)
    {
		...
		$q = "            	SELECT 
                ...
                WHERE v_listcode = '$id' GROUP BY fjd.reff_doc ";
                    ...
                    <div class='form-group'>
                     ...
                     <div class='form-group'>
                         ...
                         <input type='text'  id='datepicker2' autocomplete="off" class='form-control' name='d_cek_giro'
                                placeholder='Masukkan Tanggal Dokumen' value='<?=isset($row)?date('d M Y', strtotime($row->d_cek_giro)):''?>'>
                     </div>
                     <div class='form-group' >
