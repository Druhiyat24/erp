...
class getListData {
	...
	*/
	...
	protected function getLastDate($module){
		...
		$q = "SELECT v_idgroup,id,curr,(max(tanggal) + INTERVAL 1 DAY )tos,min(tanggal)froms, rate,rate_jual,rate_beli,v_codecurr FROM 	fin_mscurrency  $where";
		$stmt = mysql_query($q);
		$lastDate  = '';
	...
	public function get($data){
		...
		$q = "SELECT v_idgroup
			...
			$q = "SELECT v_idgroup,id,curr,max(tanggal)tos,min(tanggal)froms, rate,rate_jual,rate_beli,v_codecurr FROM 		fin_mscurrency GROUP BY v_idgroup having v_codecurr = '$codeCurr'";		
//echo $q;			
		}
		$stmt = mysql_query($q);		
		$id = array();
