...
class getListData {
	...
	public function get_bpb($id_journal){
		include __DIR__ .'/../../../include/conn.php';
		$q = "SELECT reff_doc,reff_doc2,ifnull(MAX(n_pph),0)n_pph FROM fin_journal_d WHERE id_journal = '$id_journal' AND reff_doc IS NOT NULL AND reff_doc !='' AND credit > 0 AND id_journal NOT IN('15207') GROUP BY reff_doc";
		//echo "$q";
		$stmt = mysql_query($q);
		$outp = '';
		$bpb = array();
