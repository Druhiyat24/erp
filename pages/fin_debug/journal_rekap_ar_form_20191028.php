...
FROM invoice_commercial IAC
    ...
    {
        $sql = "
            SELECT far.*, inv.j_price, inv.jqty, inv.Supplier, inv.id_journal, inv.supplier_code,inv.jatuh_tempo,inv.n_amount
            FROM 
...
//        return $id_journal; //$this->conn->insert_id;
    ...
    public function get_master_supplier() 
    {
        return $this->query("
                   SELECT
		DISTINCT(MSS.Supplier)Supplier
FROM invoice_commercial IAC
			LEFT JOIN(
				SELECT SUM(carton) j_carton,id_inv,max(id_so_det)id_so_det FROM invoice_detail GROUP BY id_inv 
			)ID ON IAC.n_idinvoiceheader = ID.id_inv		
...
<!--                <td>--><?//=$no++?><!--</td>-->
                ...
                <td>
                    ...
                    <a href="#" class="btn btn-danger btn-del">Remove</a>
                </td>
            </tr>
            <?php endforeach;?>
          </tbody>
...
<!--                            <td>--><?php //=$l->reff_doc?><!--</td>-->
                            ...
                            <td>
                                <a class="btn btn-primary btn-s btn-add" href="#"
                                   ...
                                   data-amount="<?=number_format($l->j_price)?>"
