...
$sql="SELECT 
		...
		INNER JOIN (SELECT * FROM fin_journal_d WHERE id_journal LIKE '%-PK-%' AND is_retur_bh ='N' AND debit > 0 AND id_coa NOT IN('15204','15207') AND reff_doc IS NOT NULL) FD ON FD.reff_doc = bpb.bpbno_int
			WHERE 1=1
		...
		group by bpb.id
            ...
            SELECT 
				...
				,(if(SUBSTR(bpb.bpbno_int,1,3)='FG/',(ifnull(poh.ppn,0)/100)*((bpb.qty*poi.price)) + bpb.qty*poi.price ,(ifnull(poh.ppn,0)/100)*((bpb.qty*bpb.price)) + bpb.qty*bpb.price ))nilai
				,'N' is_jasa
				,'N' is_pajak
				,'N' is_utang	
				,'N' is_other
		...
		INNER JOIN (SELECT * FROM fin_journal_d WHERE id_journal LIKE '%-PK-%' AND is_retur_bh ='N' AND credit > 0 AND id_coa NOT IN('15204','15207') AND reff_doc IS NOT NULL  ) FD ON FD.reff_doc = bpb.bpbno_int
			WHERE 1=1
                 AND poi.cancel = 'N' 	AND FD.id_journal = '$id' AND poh.id IS NOT NULL
				 AND (SUBSTR(bpb.bpbno_int,1,3)!='WIP' )
...
/*retur */
SELECT 
                bpb.bppbno_int bpbno
				,bpb.bppbno bppbno_int
				 ,bpb.id id_bpb
				,bpb.bppbdate bpdate
				,poh.pono
				,poh.podate
                ...
                ,poi.id id_po_det
				,poi.id_gen
				,((bpb.qty*poi.price) * (-1))nilai_ori
				,( ( (ifnull(poh.ppn,0)/100)*((bpb.qty*poi.price)) + bpb.qty*poi.price) * (-1))nilai
				,'N' is_jasa
				,'N' is_pajak
				,'N' is_utang
				,'N' is_other
...
if($data['is_pajak'] == 'Y' ){
   ...
   <tr>
      ...
      <td style="width:3%;text-align:left;border-right:none"><?php  echo $data['curr']  ?></td>
	  <td style="width:auto;text-align:right;border-left:none"><?php 	echo number_format((float)$data['nilai'], 2, '.', ',') ?>		</td>
