...
header("Content-Disposition: attachment; filename=NERACA $thisday.xls");
...
FROM
(SELECT n_idcoa,v_namacoa,n_saldo,d_dateupdate,SUBSTRING(n_idcoa,'1','1') segment FROM fin_saldoawal WHERE date(d_dateupdate) = DATE_SUB(DATE_FORMAT(CONCAT('$key','01'), '%Y-%m-%d'), INTERVAL 1 DAY)) SALDO
...
<table border="1" cellpadding="5">
        ...
        <?php foreach($tb as $_id_map => $_map): ?>
            <tr class="">
                <td width='150' style='background-color:  #fffad2; text-align:left; color:Black; font-weight:bold;'><?=$_id_map?></td>
                <td width='500' style='background-color:  #fffad2; text-align:left; color:Black; font-weight:bold;'><?=$_map['h']->nm_map?></td>
                <?php foreach($periods as $key => $period):?>
                    <td width='200' class="text-right" style='background-color:  #fffad2; color:Black; font-weight:bold;  text-align:right;'>
