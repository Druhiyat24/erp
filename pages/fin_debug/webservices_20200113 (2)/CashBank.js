...
if(url[1]){
	...
	var vall = $("#idcoa").val();
	var angka = $("#nilai").val();
	var total_detail_debit = $("#total_detail_debit").text();
	var total_detail_credit = $("#total_detail_credit").text();
	console.log(id);
        ...
        success: function (response) {
			...
			console.log(data);
				...
				if(d.message == '1'){
						...
						var r = decodeURIComponent(d.records[0][0].rate);
						console.log(r);
						$("#rate").val(number_format(r));
				}
				if(d.message == '2'){
					alert("Input Tanggal Salah !")
				}
...
async function getDetailsInc(ids,rows){
	...
	function getIdAkunCashBank(Item){
		...
		function formatRupiahInc(angka,id,conv){
			if(conv){
				angka =angka.replace('.',',');
