...
function generateAccountJournal(){
        ...
        type:"POST",
        cache:false,
        url:"webservices/getMasterAccountJournal.php",
        data : { code : '1'  },     // multiple data sent using ajax
        success: function (response) {
			//console.log(response);
				d = JSON.parse(response);
				console.log(d.records);
				if(d.message == '1'){
					 for(x=0;x<d.records.accountjournal.length;x++){
						 console.log('123');
					$('#account').append('<option value="'+d.records.accountjournal[x]+'" >'+d.records.nama[x]+'</option>');						 
					 }
					//console.log(data);
...
function getChild(X,Z,Y){
	...
	if(Mysplit[1] == 'fa-plus'){
        ...
        type:"POST",
        cache:false,
        url:"webservices/getListSubCoa.php",
...
function defaultshow(){
	...
	$(".account").css('display','none');
		...
		function getAccountPeriode(X,dst){
			...
			if(dst == 'acc'){
				data.accountno = X.value;
			}
		}
	function getListData(){
		data.datefrom = $("#period_from").val();
		data.dateto = $("#period_to").val();
		$("#bodyexamle1").children().remove();
		if(data.pencarian == 'P'){
			if(data.accountno == '' ||  data.accountno == undefined){
        ...
        type:"POST",
        cache:false,
        url:"webservices/getListBukuBesar.php",
        ...
        success: function (response) {
			//console.log(response);
				...
				StoreNamaCoa       = d.records.nm_coa;
				StoreSegment       = d.records.segment;
				StorePostTo        = d.records.post_to;
				...
				if(d.message == '1'){
					...
					$(".list").css('display','block');
					 for(x=0;x<StoreSegment.length;x++){
						td += '<tr id="Group'+decodeURIComponent(StorePostTo[x])+'">';
						td += '<td>';
						td += '<i class="fa fa-plus" style="cursor:pointer" id="'+decodeURIComponent(StorePostTo[x])+'" onclick="getChild('+decodeURIComponent(x)+','+StorePostTo[x]+',this)"> </i>';
						td += '</td>';						
						td += '<td>';
						td += decodeURIComponent(StorePostTo[x]);
						td += '</td>';
						
						td += '<td>';
						td += decodeURIComponent(StoreNamaCoa[x]);
						td += '</td>';	
						...
						td += '<td>';
						td += decodeURIComponent(StoreOpening[x]);
						td += '</td>';	
