...
$akses = flookup("F_L_Laba_Rugi","userpassword","username='$user'");
class Config{
    ...
    public function get_report($period_from, $period_to)
    {
        $sql = "
            ...
            WHERE 1=1
                AND mc.fg_active = '1'
                AND mc.fg_posting = '0'
                AND mc.fg_mapping = '{$this->fg_mapping_pnl}'
            ORDER BY 
                mccat.order
                ,mc.id_coa
                ,mc2.id_coa
        ...
        foreach($periods as $key => $period ){
            ...
            $sql_period_f .=
                ...
                (
                    ...
                    WHERE CONCAT(SUBSTR(fh.period,4,4), SUBSTR(fh.period,1,2)) <= '$key'
                        AND fh.fg_post = '2'
                    GROUP BY 
                        fd.id_coa
                        ,fd.curr
            ...
            foreach($query as $r){
                ...
                if($r->fg_posting == '0'){
                    ...
                    foreach($periods as $key => $period){
                        ...
                        @$total[$r->group_report]['debit_'.$key] += $r->{'debit_'.$key};
                        @$total[$r->group_report]['credit_'.$key] += $r->{'credit_'.$key};
                    }
                }
            }
        }

        return array($tb, $total);
    }
}
...
if($period_from and $period_to) {
    list($tb, $total) = $m->get_report($period_from, $period_to);
        ...
        <form method='get' name='form' enctype='multipart/form-data' action="" onsubmit='return validasi()'>
            <div class="panel panel-default">
                ...
                <div class="panel-body row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <input type="hidden" name="mod" value="reppnl" />
    ...
    <div class='box'>
        <div class='box-body'>
            <table class="table table-condensed table-bordered">
                ...
                <tbody>
                <?php foreach($tb as $_id_map => $_map): $last_coa=$_map['h']->id_coa;?>

                    <?php if($_id_map == '31' and !$total_flag[1]): $total_flag[1] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Total Penjualan dan Pendapatan</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?=number_format($h->count_end_balance($total[1]['debit_'.$key], $total[1]['credit_'.$key], $last_coa))?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                    <?php if($_id_map == '33' and !$total_flag[2]): $total_flag[2] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Total Retur Penjualan dan Pendapatan</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?=number_format($h->count_end_balance($total[2]['debit_'.$key], $total[2]['credit_'.$key], $last_coa))?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                    <?php if($_id_map == '35' and !$total_flag[3]): $total_flag[3] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Total Potongan Penjualan dan Pendapatan</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?=number_format($h->count_end_balance($total[3]['debit_'.$key], $total[3]['credit_'.$key], $last_coa))?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                    <?php if($_id_map == '37' and !$total_flag[4]): $total_flag[4] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Total Harga Pokok Penjualan</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?=number_format($h->count_end_balance($total[4]['debit_'.$key], $total[4]['credit_'.$key], $last_coa))?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                        <tr class=" info">
                            ...
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?php
                                    $laba_kotor_d['debit_'.$key] = $total[1]['debit_'.$key] - $total[2]['debit_'.$key] - $total[3]['debit_'.$key] - $total[4]['debit_'.$key];
                                    $laba_kotor_k['credit_'.$key] = $total[1]['credit_'.$key] - $total[2]['credit_'.$key] - $total[3]['credit_'.$key] - $total[4]['credit_'.$key];
                                    ?>
                                    <?=number_format($laba_kotor_k['credit_'.$key]-$laba_kotor_d['debit_'.$key])?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                    <?php if($_id_map == '41' and !$total_flag[5]): $total_flag[5] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Total Beban Usaha</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?=number_format($total[5]['debit_'.$key] - $total[5]['credit_'.$key])?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                    <?php if($_id_map == '42' and !$total_flag[6]): $total_flag[6] = 1?>
                        <tr class=" info">
                            <th>&nbsp;</th>
                            <th class="text-right">Laba Bersih sebelum Pajak</th>
                            <?php foreach($periods as $key => $period):?>
                                <th class="text-right">
                                    <?php
                                    @$laba_bersih_non_pjk['debit_'.$key] = $laba_kotor_d['debit_'.$key] - $total[5]['debit_'.$key] + $total[6]['debit_'.$key];
                                    @$laba_bersih_non_pjk['credit_'.$key] = $laba_kotor_k['credit_'.$key] - $total[5]['credit_'.$key] + $total[6]['credit_'.$key];
                                    ?>
                                    <?=number_format($laba_bersih_non_pjk['debit_'.$key]+$laba_bersih_non_pjk['credit_'.$key])?>
                                </th>
                            <?php endforeach;?>
                        </tr>
                    <?php endif;?>
                ...
                <?php if(!$total_flag[7]): $total_flag[7] = 1?>
                    <tr class=" info">
                        <th>&nbsp;</th>
                        <th class="text-right">Laba Bersih sesudah Pajak</th>
                        <?php foreach($periods as $key => $period):?>
                            <th class="text-right">
                                <?php
                                @$laba_bersih_pjk['debit_'.$key] = $laba_bersih_non_pjk['debit_'.$key] - $total[7]['debit_'.$key];
                                @$laba_bersih_pjk['credit_'.$key] = $laba_bersih_non_pjk['credit_'.$key] - $total[7]['credit_'.$key];
                                ?>
                                <?=number_format($laba_bersih_pjk['debit_'.$key]+$laba_bersih_pjk['credit_'.$key])?>
                            </th>
                        <?php endforeach;?>
                    </tr>
                <?php endif;?>
