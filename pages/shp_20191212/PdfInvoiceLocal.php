...
SELECT IAC.v_noinvoicecommercial
		...
		FROM invoice_commercial IAC
			...
			LEFT JOIN(
				SELECT invdate,invno,id id_invoice,shipped_by,ship_to,measurement,etd,eta,nw,gw,shipper,n_idcoa,manufacture_address,id_pterms,id_buyer FROM invoice_header
...
ID.id,ID.id_inv,ID.id_so_det,SOD.id_so,ID.lot,ID.carton carton,ID.qty
		...
		,SOD.size,BPB.price,SOD.color,SOD.
...
price unit_price			,if(SOD.size = 'XS', SOD.qty ,0)  XS
	...
	WHERE ID.id_inv = '$id' GROUP BY BPB.id_so_det
...
$after_tax = $value_tax  + $my_tot_price;
	...
	echo "<tr>";
		...
		echo "<td colspan='4'>Total Without Taxes</td>";
...
<div style=" margin-bottom: 2.54cm;">	
	...
	<table cellpadding="0" cellspacing="0" border="1" width="600px";>
		...
		<tr>	
			...
			<td style="font-size:12px;text-align:center">(_________________________) </td>
			<td style="font-size:12px;text-align:center">(_________________________) </td>
