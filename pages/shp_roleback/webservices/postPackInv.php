	...
	if($_GET['part'] == "PL" ){
		if(ISSET($_GET['action'])){
			...
			if($_GET['action'] == "BLOCK"){
				$mod ='ApprovalInvoicePage';
				$List = $Save->BlockPL($_GET['id']); 
...
class Save {
	...
	public function BlockPL($id){
		$q = "UPDATE invoice_header SET n_post = '11' WHERE id = '$id'";	
		$result = $this->CompileQuery($q,'CRUD');
		return $result;
	}	
	public function UnBlockPL($id){
		$q = "UPDATE invoice_header SET n_post = '0' WHERE id = '$id'";	
		$result = $this->CompileQuery($q,'CRUD');
		return $result;
	}	
