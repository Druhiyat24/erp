...
{	$cek=flookup("bcno","detail_bm","jenis_dok='$txtjenis_dok' 
	...
	if($cek=="")
	{	$sql="insert into detail_bm (jenis_dok,bcno,bcdate,bm,ppn,pph) 
			values ('$txtjenis_dok','$txtnomor_daftar','".fd($txttanggal_daftar)."','$txtbm','$txtppn','$txtpph') ";
		insert_log($sql,$user);
	}
	else
	{	$sql="update detail_bm set bm='$txtbm',ppn='$txtppn',pph='$txtpph' where  
