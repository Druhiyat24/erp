...
{ $kode_dept = json_encode($_REQUEST['kode_dept']);
  ...
  $sql = "select a.id isi,reqno tampil from 
    ...
    where s.kode_mkt in ($kode_dept) and ifnull(tpo.qty_po,0)<treq.qty_req and a.app='A' and 
    (
...
{	$id_jo = json_encode($_REQUEST['id_jo']);
  ...
  $jenis_item = $_REQUEST['jenis_item'];
  $id_po_draft = $_REQUEST['id_draft_po'];
  ?>
    ...
    <tbody>
      ...
      { $sql="select poi.cancel cancel_poi,POI.qty qty_poi,POI.price price_poi,POI.id id_poi, ac.id id_cs,k.id_jo,0 idsubgroup,k.id_item,l.color,l.size,
        ...
        left join mastersupplier m on k.id_supplier=m.id_supplier
			INNER join po_item_draft POI on POI.id_gen = k.id_item AND POI.id_jo =  jo.id
			INNER join po_header_draft POH on POH.id = POI.id_po_draft 		
                    where 
			POH.id = '$id_po_draft' AND k.id_jo in ($id_jo) 
      ...
      { if ($jenis_company=="VENDOR LG")
        ...
        { $sql="      select POI.cancel cancel_poi,POI.qty qty_poi,POI.price price_poi,POI.id id_poi,ac.id id_cs,k.id_jo,s.id idsubgroup,k.id_item,l.color,l.size,
            ...
            l.qty qty_gmt,k.cons,round(sum(l.qty*k.cons),2) qty_bom,
            POI.unit,m.supplier,ac.kpno,jo_no  
            from bom_jo_item k inner join so_det l on k.id_so_det=l.id
            inner join so on so.id=l.id_so inner join act_costing ac on ac.id=so.id_cost 
			...
			INNER join po_header_draft POH on POH.id = POI.id_po_draft 
            where 
			POH.id = '$id_po_draft' AND
			k.id_jo in ($id_jo) and 
            (k.id_supplier='$id_supplier' or k.id_supplier2='$id_supplier') 
            and k.cancel='N' group by k.id_jo,k.id_item";
			#echo $sql;
...
/*           $qtyPO=($allowq + $data['qty_bom']);
          ...
          { $id=$data['id_item'].":".$data['id_jo'];
            ...
            echo "<tr>";
              echo "<td><input type ='checkbox' checked name ='itemchk[$id]' disabled 
                class='chkclass'></td>"; 
              if($jenis_company=="VENDOR LG")
              ...
              { //$price_cs = "";
				$price_cs = $data['price_poi'];

		  }
			    $to_tal =$to_tal + ($qtyPO*$price_cs); 
				$to_tal_qty =  $to_tal_qty  + $qtyPO;
              echo "<td>
              ...
              <td>
				...
				<input type ='hidden' name ='id_poi[$id]' value='$data[id_poi]' class='id_poiclass'>
              </td>"; 
              echo "
              <td>
					".number_format($qtyPO*$price_cs)."
              </td>";			  
			  echo "<td>";
			  if($data['cancel_poi'] == 'Y' ){
				  echo "Cancelled";
			  }else{
				  echo "<a href='#' onclick='cancel_poi(".'"'.$data['id_poi'].'"'.")'><i class='fa fa-trash-o'></a>";
			  }
			  echo "</td>";
            echo "</tr>";
...
FROM (
    select POI.cancel cancel_poi
    		,POH.id id_poh
			 ...
			 ,rh.notes
			 ,k.id_reqno
			 ,0 idsubgroup,k.id_item,j.color,j.size,
        j.itemdesc item,0 qty_gmt,0 cons,round(sum(k.qty),2) qty_bom,
        k.unit,'' supplier,'' kpno,k.price 
        from reqnon_header rh 
		inner join reqnon_item k on rh.id=k.id_reqno inner join masteritem j on k.id_item=j.id_item  
			INNER join po_item_draft POI on POI.id_jo = rh.id AND POI.id_gen =k.id_item
			INNER join po_header_draft POH on POH.id = POI.id_po_draft 		
        WHERE POH.id='$id_po_draft' AND k.id_reqno in ($id_req) and k.cancel='N' GROUP BY  k.id_reqno,k.id_item
...
INNER JOIN(
    select POI.cancel cancel_poi
        ...
        from reqnon_header rh 
		inner join reqnon_item k on rh.id=k.id_reqno inner join masteritem j on k.id_item=j.id_item  
			INNER join po_item_draft POI on POI.id_jo = rh.id
			INNER join po_header_draft POH on POH.id = POI.id_po_draft 		
        WHERE POH.id='$id_po_draft' AND k.id_reqno in ($id_req) and k.cancel='N' GROUP BY  k.id_reqno,k.id_item
)X	ON YY.id_item = X.id_item AND YY.id_reqno = X.id_reqno 	  
      ...
      { $query = mysql_query($sql); 
        ...
        { echo "<tr>";
            ...
            $id2=$data['id_item'];
            $idjo2=$data['id_reqno'];
            echo "<td><input type ='checkbox' checked='checked' disabled name ='itemchk[$id]' 
              class='chkclass'></td>"; 
            echo "
            <td>$data[item]</td>
            <td>$data[color]</td>
           ...
           // $qtyPO=($allowq+$data['qty_bom']) - $sisa_stock;
		   	$all_qty = ($allowq+$data['qty_bom']) - $sisa_stock;
		   $qtyPO =$data['qty_poi'];
            //$qtyPO=$qtyPO - $qtySdhPO;			
            $pricereq=$data['price_poi'];
			    $to_tal =$to_tal + ($qtyPO*$pricereq); 
				$to_tal_qty =  $to_tal_qty  + $qtyPO;			
            echo "<td>
              <input type ='text' style='width:70px;' name ='itemqty[$id]' class='qtyclass' value='$qtyPO'>
              <input type ='hidden' style='width:70px;' name ='itemqtybts[$id]' class='qtybtsclass'value='$all_qty'>
              </td>"; 
