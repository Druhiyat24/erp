...
<script type="text/javascript">
  ...
  { var id_jo = $('#cboToJO').val();
    var html = $.ajax
    ({  type: "POST",
        url: '../forms/ajax3.php?modeajax=view_list_stock2',
        data: {id_jo: id_jo},
        async: false
    }).responseText;
    if(html)
...
<div class='box'>
  <div class='box-body'>
    <div class='row'>
      <form method='post' name='form' action='s_trans_post.php?mod=<?php echo $mod; ?>' 
        ...
        <div class='col-md-3'>
          ...
          <!-- <div class='form-group'>
            ...
            $sql="select jo.id isi,concat(jo.jo_no,' | ',ac.styleno,' | ',ac.kpno) tampil 
              from jo inner join jo_det jod on jo.id=jod.id_jo
          ...
          <div class='form-group'>
            ...
            <?php
            if ($id_jo!="") { $fil_jo=" where jo.id='$id_jo'"; } else { $fil_jo=" "; } 
            $sql="select jo.id isi,concat(jo.jo_no,' | ',ac.styleno,' | ',ac.kpno) tampil 
              from jo inner join jo_det jod on jo.id=jod.id_jo
              inner join so on jod.id_so=so.id
              inner join act_costing ac on so.id_cost=ac.id 
              $fil_jo group by jo.id";
            echo $sql;
            IsiCombo($sql,'','Pilih JO');
            ?>
            </select>
            ...
            ?>
              ...
              //   alert('test');
              //   var id_jonya = "159";
              //   var id_jotextnya = "JO/0719/00148";
              //   var $newOption = $("<option selected='selected'></option>").val(id_jonya).text(id_jotextnya)
              //   $("#cboToJO").append($newOption).trigger('change');
              </script>
            <?php } ?>
    ...
    <table id="examplefix" class="display responsive" style="width:100%">
      ...
      <tbody>
        ...
        { echo "<tr>";
            ...
            if($data['cancel']=="N" and $data['status_app']=="N")
            { echo "
