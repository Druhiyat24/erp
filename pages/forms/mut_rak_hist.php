...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">List History Mutasi RAK</h3>
  </div>
    ...
    <table id="examplefix" class="display responsive" style="width:100%;font-size:11px;">
      <thead>
        <tr>
          ...
          <th>Unit</th>
          <th>Rak Temp</th>
          <th>Rak Location Lama</th>
          <th>Rak Location Baru</th>
          <th>User</th>
        </tr>
      </thead>
      <tbody>
        <?php
        $sql="select brd.id,ac.kpno,ac.styleno,brh.bpbno,brh.id_jo,brh.id_item,
          mi.goods_code,mi.color,concat(replace(mi.itemdesc,mi.color,''),' ',mi.add_info) itemdesc,
          brd.roll_no,brd.lot_no,brd.roll_qty,concat(mrt.kode_rak,' ',mrt.nama_rak) rak_temp,
          concat(mr.kode_rak,' ',mr.nama_rak) rak_loc_new,concat(mrs.kode_rak,' ',mrs.nama_rak) rak_loc_old, user, tgl_input
          from bpb_roll_h brh inner join bpb_roll_log brd on brh.id=brd.id_h 
          inner join master_rak mr on brd.id_rak_loc=mr.id  
          inner join master_rak mrt on brd.id_rak=mrt.id 
          inner join master_rak mrs on brd.id_rak_old=mrs.id
        ...
        { 
          ...
          echo"
            ...
            <td>$rsbpb[unit]</td>
            <td>$data[rak_temp]</td>
            <td>$data[rak_loc_old]</td>
            <td>$data[rak_loc_new]</td>
            <td>$data[user]</td>
          </tr>";
          $i++;
        };
