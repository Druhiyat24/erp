...
{	
	$sql="select bppbno isi, concat(bppbno,' | ',ac.kpno,' | ',ac.styleno,' | ',mb.supplier) tampil from (
		...
		(select bppbno, id_item, id_jo, sum(qty) qty_req, bppbdate from bppb_req where bppbdate >= '2025-01-01' and bppbno like 'RQ-$tipe_mat%' group by bppbno, id_item, id_jo) br
		left join (select bppbno,bppbno_req, id_item, id_jo, sum(qty) qty_out from bppb where bppbdate >= '2025-01-01' and bppbno_req like 'RQ-$tipe_mat%' group by bppbno_req, id_item, id_jo) bppb 
