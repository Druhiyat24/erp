...
{	if ($deldulu=="Y") { 
	...
	{	$jenis_dok = $data['jenis_dokumen'];
		...
		$id_item = isset($data['id_item']) ? $data['id_item'] : '';
		...
		mysql_query("insert into upload_standard 
			...
			STATUS_KB_SUPPLIER,username,sesi,nm_file,berat_bersih,podate,id_item)
			...
			trim('$usernya'),'$sesinya','',0,'$tgl_trans','$id_item')");
	}
};
...
{	if ($deldulu=="Y") { 
	...
	{	$jenis_dok = $data['jenis_dokumen'];
		...
		$nilai = $data['nilai_barang'];
		$id_item = isset($data['id_item']) ? $data['id_item'] : '';
		$satuan_bc = isset($data['satuan_bc']) ? $data['satuan_bc'] : '';
		$qty_bc = isset($data['qty_bc']) ? $data['qty_bc'] : 0;
		
		mysql_query("insert into upload_standard 
			(JENIS_DOKUMEN,BCNO,BCDATE,SUPPLIER,KODE_BARANG,ITEMDESC,UNIT,QTY,CURR,PRICE,JENIS_BARANG,AREA_SUPPLIER,
			STATUS_KB_SUPPLIER,username,sesi,nm_file,berat_bersih,podate,id_item,satuan_bc,qty_bc)
			values (trim('$jenis_dok'),trim('$no_dok'),'$tgl_dok',trim('$nm_sup'),trim('$kode_barang'),trim('$nm_barang'),
			trim('$satuan'),'$jml',trim('$curr'),'$nilai','$no_trans','','',
			trim('$usernya'),'$sesinya','',0,'$tgl_trans','$id_item','$satuan_bc','$qty_bc')");
	}
};
