...
{	$crinya = $_REQUEST['cri_item'];
	...
	OR $modenya=="cari_list_item_wip_out" OR $modenya=="cari_list_item_gen_out" 
...
{	$crinya = $_REQUEST['cri_item'];
	...
	{	if ($modenya=="cari_list_item" OR $modenya=="cari_list_item_out" 
		...
		else if ($modenya=="cari_list_item_gen" OR $modenya=="cari_list_item_gen_out")
		{	$matnya = "'N'"; 
			$pronya = "General";
		}
    ...
    { if ($nm_company=="PT. Sandrafine Garment")
  		...
  		{ $kodenya = "concat(id_item,' | ',goods_code,'|',itemdesc)"; }
...
else if ($modenya=="cari_jenis_pemasukan")
{	$crinya = $_REQUEST['cri_item'];
	if ($crinya!="")
	{	$sql = "select nama_trans isi,nama_trans tampil from mastertransaksi where 
                          jenis_trans='IN' and jns_gudang = 'FACC' where nama_trans = '$crinya' order by id";
