...
function get_journal_nya($id){
	...
	$Bm = new Basemodel($con_new);
            ...
            SELECT * FROM
            fin_journal_h jh
            WHERE TRIM(reff_doc) LIKE '%".TRIM($id)."%' AND type_journal IN('2','17','1','19') LIMIT 1;
        ")->row();
...
SELECT X.id_journal
       ...
       ,X.id_coa
       ,X.nm_coa
	   ,( (X.percentage_nya/100)* SUM(X.amount_original) )val_pph
...
SELECT     
			...
			,SUM(MASTER.amnt_only_ori)amnt_only_ori
			,SUM(MASTER.value_pengurangan)value_pengurangan
			FROM ( 
				SELECT
					M.id_journal 
					,M.date_journal
					...
					,(SUM(IFNULL(M.amount_original,0)))amnt_only_ori
					,SUM(M.val_ppn)ppn
					,SUM(M.val_pph)pph
					,SUM(value_pengurangan)value_pengurangan
					FROM( 
							...
							,FH.date_journal
							,if(KB.id_bpb IS NULL  OR KB.id_bpb ='' OR KB.id_bpb =0 ,KB.credit,0 ) value_pengurangan
					...
					SELECT
						...
						,SUM(M.amnt_only_ori)amnt_only_ori
						, ( SUM(M.val_ppn) *-1 )ppn
						, ( SUM(M.val_pph) *-1 )pph
						, ((-1)*SUM(M.value_pengurangan))value_pengurangan
						FROM( 
								SELECT ifnull(KB.amount_original,KB.debit) nilai
								,(ifnull(KB.amount_original,0) * (ifnull(KB.n_ppn,0)/100))val_ppn
								,(ifnull(KB.amount_original,0) * (IFNULL(IFNULL(MT_H.percentage,MT.percentage),0)/100))val_pph
								,FH.date_journal
								,if(KB.id_bppb IS NOT NULL,KB.amount_original,0 )amnt_only_ori
								,if(KB.id_bppb IS NULL  OR KB.id_bppb ='' OR KB.id_bppb =0 ,KB.debit,0 )value_pengurangan
								,KB.* FROM fin_journal_d KB
								LEFT JOIN mtax MT ON MT.idtax = KB.n_pph
								LEFT JOIN(SELECT id_journal,n_pph,date_journal FROM fin_journal_h WHERE type_journal = '14')FH ON KB.id_journal=FH.id_journal
								LEFT JOIN mtax MT_H ON MT_H.idtax = FH.n_pph
				...
				SELECT
					...
					WHERE MASTER.amount IS NOT NULL  GROUP BY MASTER.id_journal
	...
	$id = $res[0]->bppbno;
		...
		$sql="
            ...
            FROM 
				...
				WHERE bpb.bppbno = '$id' AND POH.app  ='A' AND POI.cancel !='C'";
