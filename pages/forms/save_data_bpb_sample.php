...
if (isset($_POST['txtjns'])) { $txtbcno=nb($_POST['txtbcno']); } else { $txtbcno=""; }
...
{	# COPAS SAVE ADD
	...
	{ if ($bpbno=="" and $noid=="") 
		{ $txtbpbno = urutkan($mode,$cbomat); 
			...
			{	
				$date=fd($txtbpbdate);
				$cri2="SMP/IN/".date('my',strtotime($date));
				$txtbpbno2=urutkan_inq($cbomat2."-".date('Y',strtotime($date)),$cri2);
			}
		}
		else
		{
			$cekdata = "select bpbno_int, jenis_trans from bpb where bpbno = '$noid' limit 1";
			$cekdataqry = mysql_query($cekdata);
			while ($cekdatarow = mysql_fetch_array($cekdataqry)) {
			$txtbpbno2= $cekdatarow['bpbno_int'];
			$txttujuan= $cekdatarow['jenis_trans'];
}
		} 
	}
	...
	if ($cek=="0")
	{	$sql = "insert into bpb (id_jo,id_item,id_item_bb,id_item_fg,qty,unit,curr,price,remark,jam_masuk,berat_bersih,berat_kotor,nomor_mobil,pono,id_supplier,
			...
			no_fp,tgl_fp,jenis_trans,subtujuan,total_nilai,id_sec)
			values ('$id_jo','$txtid_item','$txtid_item','$txtid_item_fg','$txtqty','$txtunit','$txtcurr','$txtprice',
