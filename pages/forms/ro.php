...
{ var bppbno = document.form.cbosj.value;
  ...
  {
    echo "var jenisdef = document.form.txtdefect.value;";
  }
  echo "
...
<script type="text/javascript">
  ...
  {   var html = $.ajax
    ({  type: "POST",
      ...
      data: {cri_item: cri_item, mode: '<?=$mode?>'},
      async: false
    }).responseText;
    if(html)
    {  
  ...
  function getListKPNo2(cri_item)
  {   var html = $.ajax
    ({  type: "POST",
      url: 'ajax2.php?modeajax=cari_list_bpb_acc',
      data: {cri_item: cri_item, mode: '<?=$mode?>'},
      async: false
    }).responseText;
    if(html)
  ...
  {   var html = $.ajax
    ...
    function getListData2(cri_item)
    {   
      var html = $.ajax
      ({  type: "POST",
        url: 'ajax2_ro.php?modeajax=view_list_sj_ro2',
        data: "cri_item=" +cri_item,
        async: false
      }).responseText;
      if(html)
      ...
      var html = $.ajax
      ({  type: "POST",
        url: 'ajax2.php?modeajax=cari_defect2',
        data: "cri_item=" +cri_item,
        async: false
      }).responseText;
      if(html)
...
# COPAS ADD
  ...
  echo "</div>";
# END COPAS ADD
}elseif($mod=="Retur_accs") {
  echo "<div class='box'>";
  echo "<div class='box-body'>";
  echo "<div class='row'>";
  echo "<form method='post' name='form' action='save_data_ro_acc.php' >";
  echo "<div class='col-md-3'>";
  echo "<div class='form-group'>";
  echo "<label>Filter Tgl BPB *</label>";
  echo "<input type='text' id='datepicker1' class='form-control dtpspc' autocomplete='off' onkeydown='return false' name='txttglcut'  
  placeholder='Masukkan Filter Tgl Surat Jalan' onchange='getListKPNo2(this.value)'>";
  echo "</div>";
  echo "<div class='form-group'>";
  echo "<label>$caption[2] *</label>";
  echo "<select class='form-control select2' style='width: 100%;' name='txtsjno' id='cbosj' onchange='getListData2(this.value)'>";
  echo "</select>";
  echo "</div>";
  echo "</div>";
  echo "<div class='col-md-3'>";
...
# END COPAS ADD
  ...
  <div class="box">
    ...
    if ($mode=="FG")
      ...
      { $fldnyacri=" mid(bppbno,4,1) in ('N','M') "; $mod2="21e"; }
    else if ($mode=="Scrap")
    ...
    <div class="box-header">
      ...
      <a href='../forms/?mod=Retur_accs&mode=<?php echo $mode; ?>' class='btn btn-primary btn-s'>
        <i class='fa fa-plus'></i> New (Accs)
      </a>
    </div>
      ...
      <table id="examplefix3" class="display responsive" style="width:100%">
        ...
        <tbody>
          ...
          $query = mysql_query("SELECT jo_no,ac.kpno wsno,ac.styleno stylenya,a.*,s.goods_code,$fld_desc itemdesc,supplier 
            ...
            where $fldnyacri and right(bppbno,1)='R' and a.bppbdate >='$tglf' and a.bppbdate <='$tglt' 
          ...
          { 
           ...
           {
            echo "
            <td>Confirmed By ".$data['confirm_by']." (".fd_view_dt($data['confirm_date']).")</td>
            <td></td>";
          }
          else if($data['cancel']=="Y")
          {
            echo "
            <td>Cancelled By ".$data['cancel_by']." (".fd_view_dt($data['cancel_date']).")</td>
            <td></td>";
          }
          else
          {
          ...
          { 
            ...
            if($data['confirm']=="Y"){
             if($data['cancel']=="N"){
              echo "
              <td>
              <a href='cetaksj_ro.php?mode=Out&noid=$data[bppbno]' 
