...
$sql = "SELECT jenis_dokumen, bcno, bcdate, trans_no, trans_date, supplier, kode_brg, itemdesc, unit, qty, nilai_barang, a.curr, price, id_item, matclass, COALESCE(mr.rate,1) rate, (nilai_barang * COALESCE(mr.rate,1)) nilai_barang_idr from (SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate, if(a.bppbno_int!='',a.bppbno_int,a.bppbno) trans_no,a.bppbdate trans_date,d.supplier, if(s.goods_code<>'' AND s.goods_code<>'-' AND s.goods_code<>'0' ,s.goods_code,concat('FG ',s.id_item)) kode_brg,s.itemname itemdesc,a.unit,sum(a.qty) qty, round(sum(a.qty*ifnull(a.price_bc,a.price)),2) nilai_barang,s.id_so_det id_item ,IFNULL(NULLIF(a.curr_bc, ''), a.curr) curr,a.price , 'FG' mattype,'BARANG JADI' matclass from bppb a inner join masterstyle s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bppbdate between '$tglf' AND '$tglt' and a.bppbno like 'SJ-FG%' and jenis_dok='BC 2.7' and tujuan in ('DIKEMBALIKAN','DISUBKONTRAKKAN') group by bcno,bppbno,s.goods_code,s.itemname,price
UNION ALL
SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bppbno_int!='',a.bppbno_int,a.bppbno) trans_no,a.bppbdate trans_date,d.supplier, if(s.goods_code<>'' AND s.goods_code<>'-' AND s.goods_code<>'0' ,s.goods_code,concat('FG ',s.id_item)) kode_brg, s.itemdesc,a.unit,sum(a.qty) qty,round(sum(a.qty*ifnull(a.price_bc,a.price)),2) nilai_barang,a.id_item ,IFNULL(NULLIF(a.curr_bc, ''), a.curr) curr,a.price ,s.mattype,s.matclass from bppb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bppbdate between '$tglf' AND '$tglt' and a.bppbno not like 'SJ-FG%' and jenis_dok='BC 2.7' and tujuan in ('DIKEMBALIKAN','DISUBKONTRAKKAN') group by bcno,bppbno,a.id_item,price order by bcdate,bcno) a LEFT JOIN (SELECT tanggal, curr, rate FROM ap_masterrate where v_codecurr = 'PAJAK' GROUP BY tanggal, curr) mr ON mr.tanggal = a.bcdate AND mr.curr = a.curr";
