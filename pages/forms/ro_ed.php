...
{ $id_item = "";
  ...
  $bppbno = $data['bppbno'];
  if($data['bppbno_int']!="") {$bppbno_int = $data['bppbno_int'];} else {$bppbno_int = $data['bppbno'];}
  ...
  echo "<div class='box-body'>";
    echo "<div class='row'>";
      echo "<form method='post' name='form' action='save_data_bppb_out_jo.php?mod=$mod&mode=$mode&noid=$bppbno&id=$id_line' onsubmit='return validasi()'>";
          ...
          <div class='col-md-6'>
            <div class='form-group'>";
              ...
              echo "<input type='text' class='form-control' name='txtbppbno' readonly 
                placeholder='$cmas $c52' value='$bppbno_int'>
            </div>
          </div>";
          echo "
          <div class='col-md-6'>
        ...
        $pono = flookup("s.pono","bppb a inner join bpb s on a.bpbno_ro=s.bpbno","a.bppbno='$bppbno'");
        echo "
        <div class='form-group'>";
          echo "<label>PO #</label>";
          echo "<input type='text' class='form-control' name='txtpono' readonly value='$pono'>
        </div>";
        echo "</div>";
        ?>
        <div class='box-body'>
          ...
          {	echo "<table id='examplefix2' style='width: 100%;'>";
                  ...
                  // <th>Price</th>";
							...
							<tbody>";
							$sql="select breq.price,breq.curr,breq.nomor_rak,breq.id line_item,breq.id_supplier,breq.qty qtyout,mi.goods_code,mi.itemdesc,tbl_in.id_item,tbl_in.id_jo,tbl_in.qty_in,
  							if(tbl_out.qty_out is null,0,tbl_out.qty_out) qty_out,
  							tbl_in.unit,
                ac.kpno,ac.styleno,mbuyer.supplier buyer
  							from masteritem mi inner join (select id_item,id_jo,sum(qty) qty_in,unit from bpb where id_jo='$id_jo' group by id_item,id_jo) as tbl_in 
  							on mi.id_item=tbl_in.id_item
  							left join (select id_item,id_jo,sum(qty) qty_out from bppb where id_jo='$id_jo' group by id_item,id_jo) as tbl_out
  							on tbl_in.id_item=tbl_out.id_item and tbl_in.id_jo=tbl_out.id_jo
  							inner join bppb breq on mi.id_item=breq.id_item 
                left JOIN (select id_so,id_jo from jo_det group by id_jo)  jod on breq.id_jo=jod.id_jo 
                left join (select so.id,id_cost,min(sod.deldate_det) mindeldate from so inner join so_det sod on so.id=sod.id_so group by so.id) so on jod.id_so=so.id 
                left join act_costing ac on so.id_cost=ac.id
                left join mastersupplier mbuyer on ac.id_buyer=mbuyer.id_supplier
