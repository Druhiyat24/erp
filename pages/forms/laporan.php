...
{ 
  ...
  { $fldnyacri=" bpbno_int like 'SCR%' "; $mod2=52; }
  else if ($tipe=="Sample")
  { $fldnyacri=" bpbno_int like 'SMP%' "; $mod2=52; }
  else if ($tipe=="Barang Dalam Proses")
...
{ 
  ...
  { $fldnyacri=" bppbno_int like 'SCR%' and bppbdate "; $mod2=62; }
else if ($tipe=="Sample")
  { $fldnyacri=" bppbno_int like 'SMP%' bppbdate "; $mod2=62; }
  else if ($tipe=="Barang Dalam Proses")
...
{ $from=date('Y-m-d',strtotime($_POST['txtfrom']));
  ...
  $tipenya=$_POST['txttipe'];
  echo "<script>
    window.location.href='index.php?mod=19&mode=Detail_In&frexc=$from&toexc=$to&tpexc=$tipenya&clexc=&suppexc=&dest=xls';
    </script>";
}
else
{ #$from=date('d M Y');
    ...
    <div class='box-body'>
      <div class='row'>
        ...
        <div class='col-md-3'>
          ...
          <div class='form-group'>
            ...
            <?php 
              ...
              $sql = "SELECT 
                      IF(mattype = 'KM','FABRIC FINISHED', matclass) AS isi,
                      IF(matclass = '-', 'ACCESORIES PACKING DAN SEWING', 
                          IF(mattype = 'KM','FABRIC FINISHED', matclass)
                      ) AS tampil
                      FROM masteritem
                      WHERE mattype IN ('A','F','B','G','KM')
                      GROUP BY IF(mattype = 'KM','FABRIC FINISHED', matclass)
                      UNION ALL
                      SELECT 
                          'BARANG JADI STOK' AS isi,
                          'BARANG JADI STOK' AS tampil
                      ORDER BY isi"; 
        ...
        <?php } else { ?>
          ...
          <button type='submit' name='submit' class='btn btn-primary' value='submit'><?php echo $ctam; ?></button>
<!--           <button type='submitexc' name='submitexc' class='btn btn-primary' value='submitexc' id = 'submitexc'>Submit Excel</button> -->
<!--         <a href='?mod=19&mode=Detail_In&frexc=2022-05-01&toexc=2022-05-31&tpexc=Bahan Baku&clexc=&suppexc=&dest=xls'>Save To Excel</a> -->
