	...
	{			
		echo "<table id='examplefix1' class='display responsive' style='width: 100%;font-size:14px;'>";
			echo "
				...
				<tbody>";
					$sql="select a.id_jo,ac.kpno, mp.product_group, c.id_item, c.goods_code, c.itemdesc,sum(a.qty) qty, a.unit, 
					round(sum(a.qty),2) - round(coalesce(bpb.qty,0),2) balance
			...
			left join (select sum(qty) qty, id_item from bpb where bpbdate >= '2022-01-01' and id_jo = '$crinya' and id_supplier = '$crisupp' group by id_item ) bpb on a.id_item = bpb.id_item
