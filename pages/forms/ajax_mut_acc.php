...
#echo $modenya;
	...
	{ 
		...
		$sql_bpb = "select '' isi, 'Select Item' tampil union select id_item isi, CONCAT(id_item,' - ',itemdesc) tampil from (select id_rak_loc,rak,id_item,itemdesc,sum(qty_sisa) qty,unit from (select a.*,b.*,(COALESCE(a.roll_qty,0) - COALESCE(b.qty_out,0) - COALESCE(a.qty_mutasi,0)) qty_sisa from (select bd.*,
          ...
          styleno
          from bpb_det bd
         left join (select bpbno, pono, id_supplier,bpbno_int, bpbdate,invno from bpb 
         group by bpbno) bpb on bd.bpbno = bpb.bpbno
         inner join masteritem mi on bd.id_item = mi.id_item
         left join (select ac.id_buyer, supplier buyer,ac.styleno,jd.id_jo, ac.kpno from jo_det jd
         ...
         (select id_bpb_det,SUM(roll_qty) qty_out from bppb_det where cancel = 'N' and id_rak_loc = '$id_rak_loc' GROUP BY id_bpb_det) b on b.id_bpb_det = a.id) a GROUP BY id_item ) a where qty > 0  ";
		IsiCombo($sql_bpb,'','');
      ...
      <tbody>
        ...
        //  <td><input type = 'number' style='width:50px;' max = '$data[sisa]' name ='itemqty[$id]' value = '$data[sisa]'></td>
          ...
          <tr>
            ...
            <td>$data[tgl_bpb]</td>
            <td>$data[qty_sisa]</td>
            <td><input  class='form-control' type='text' max = '$data[qty_sisa]' name ='qty_mutasi[$id]' value = '$data[qty_sisa]' required></td>
            <td>$data[unit]</td>
            <td>$data[kode_rak]</td>
