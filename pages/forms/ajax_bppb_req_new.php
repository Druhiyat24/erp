...
{	
	...
	if ($id_jo!="")
	{
		if ($tipews == 'STD' && $tipe_mat == 'F')
		{
			...
			inner join masteritem mi on bppb.id_item = mi.id_item 
			where id_jo = '$id_jo' and mi.mattype = '$tipe_mat' and bppbno like 'SJ-F%'
			...
			// left join (select * from bpb where bpbdate >= '2023-01-01') bpb on mi.id_item = bpb.id_item and k.id_jo = bpb.id_jo	line 155	
		{
			...
			// round(COALESCE ( sum( qty_bppb ), 0 ),2) qty_bppb,
			// round(COALESCE (sum(qty_sa),0) + COALESCE ( sum( qty_bpb ), 0 ) - COALESCE ( sum( qty_bppb ), 0 ),2) sisa_stok,
			// round(COALESCE ( br.qty_br, 0 ),2) qty_br,
			// round(COALESCE ( br.qty_br_out, 0 ),2) qty_br_out,
			...
			$sql = "select * from
(select mi.id_item, ac.kpno,jo.jo_no, mi.goods_code, mi.mattype,mi.matclass,mi.itemdesc,k.id_jo from bom_jo_item k 
inner join masteritem mi on k.id_item = mi.id_gen	
inner join jo_det jd on k.id_jo = jd.id_jo
inner join so on jd.id_so = so.id
inner join act_costing ac on so.id_cost = ac.id
inner join jo on jd.id_jo = jo.id
where mattype = 'A' and k.id_jo = '$id_jo'
group by k.id_item
union
select mi.id_item, ac.kpno,jo.jo_no, mi.goods_code, mi.mattype,mi.matclass,mi.itemdesc,sa.id_jo  
from saldoawal_rak sa 
inner join masteritem mi on sa.id_item = mi.id_item	
inner join jo_det jd on sa.id_jo = jd.id_jo
inner join so on jd.id_so = so.id
inner join act_costing ac on so.id_cost = ac.id
inner join jo on jd.id_jo = jo.id
where mattype = 'A' and sa.id_jo = '$id_jo'
group by sa.id_item
) 
mi	
left join (
	select
	id_item,
	sum(qty_sa) + sum(qty_bpb)qty_bpb,
	sum(qty_bppb) + sum(qty_bppb_non_req) qty_bppb,
	sum(qty_sa) + sum(qty_bpb) - sum(qty_bppb) - sum(qty_bppb_non_req) sisa_stok,
	sum(qty_req)qty_br,
	sum(qty_bppb)qty_br_out,
	sum(qty_req) - sum(qty_bppb) sisa_req,
	unit
	from
	(
	select id_item, id_jo,sum(qty) qty_sa,'0' qty_bpb,'0' qty_req,'0' qty_bppb, '0' qty_bppb_non_req,unit
	from saldoawal_rak where id_jo = '$id_jo' and tipe_mat = 'A'
	and periode = '2023-02-01'
	group by id_item, id_jo, unit
	UNION	
	select id_item, id_jo,'0' qty_sa,sum(qty) qty_bpb,'0' qty_req,'0' qty_bppb, '0' qty_bppb_non_req,unit
	from bpb where id_jo = '$id_jo'
	and bpbno like 'A%' and cancel = 'N' and bpb.bpbdate >= '2023-02-01'
	group by id_item, id_jo, unit
	UNION
	select id_item, id_jo,'0' qty_sa,'0' qty_bpb,sum(qty) qty_req, '0' qty_bppb, '0' qty_bppb_non_req,unit from 
	bppb_req br
	where id_jo = '$id_jo' and br.bppbno like 'RQ-A%' and br.bppbdate >= '2023-02-01'
	group by id_item, id_jo, unit
	UNION
	select id_item, id_jo,'0' qty_sa,'0' qty_bpb, '0' qty_req,sum(qty) qty_bppb, '0'qty_bppb_non_req ,unit 
	from bppb 
	where id_jo = '$id_jo' and bppbno_req != '' and bppbno like 'SJ-A%' and bppb.bppbdate >= '2023-02-01'
	group by id_item, id_jo, unit
	UNION
	select id_item, id_jo,'0' qty_sa,'0' qty_bpb, '0' qty_req,'0' qty_bppb, sum(qty)qty_bppb_non_req, unit 
	from bppb 
	where id_jo = '$id_jo' and bppbno_req = '' and bppbno like 'SJ-A%' and bppb.bppbdate >= '2023-02-01'
	group by id_item, id_jo, unit
	) master_transaksi
...
mi.matclass,
mi.itemdesc,
coalesce(qty_in,0) qty_bpb,
coalesce(qty_out,0) qty_bppb,
coalesce(qty_in,0) - coalesce(qty_out,0) sisa_stok,
coalesce(qty_req,0) qty_br,
coalesce(qty_outstanding_req,0) sisa_req,
coalesce(qty_in,0) - coalesce(qty_out,0) - coalesce(qty_outstanding_req,0) qty_avail_req,
k.unit

from
		(
		select id_jo, k.id_item, unit from bom_jo_global_item k
		inner join masteritem mi on k.id_item = mi.id_item
		where id_jo = '$id_jo' and mi.mattype = '$tipe_mat' and k.cancel = 'N' 
		group by k.id_item, k.unit
		) k
left join 
		(
		select bpb.id_item, id_jo, sum(qty) qty_in, unit from bpb 
		inner join masteritem mi on bpb.id_item = mi.id_item
		where id_jo = '$id_jo' and mi.mattype = '$tipe_mat'
		group by bpb.id_item, id_jo, unit
		) a on k.id_item = a.id_item and k.id_jo = a.id_jo and k.unit = a.unit
left join
(
		select bppb.id_item, id_jo, sum(qty) qty_out, unit from bppb 
		inner join masteritem mi on bppb.id_item = mi.id_item
		where id_jo = '$id_jo' and mi.mattype = '$tipe_mat'
		group by bppb.id_item, id_jo, unit
)	b on k.id_item = b.id_item and k.id_jo = b.id_jo and k.unit = b.unit
left join
(
		select br.id_item, br.id_jo, sum(br.qty) qty_req, sum(bppb.qty) qty_q, sum(br.qty) - sum(if(bppb.qty is null, 0 , bppb.qty)) qty_outstanding_req, br.unit from
		(
		select * from bppb_req where id_jo = '$id_jo'
		group by id_item, id_jo, bppbno, unit
		) br
		left join 
		(
		select bppbno_req,sum(qty) qty, id_item, id_jo, unit  from bppb where id_jo = '$id_jo' group by bppbno_req, id_item, unit
		) bppb on br.bppbno = bppb.bppbno_req and br.id_item = bppb.id_item and br.id_jo = bppb.id_jo and br.unit = bppb.unit
		group by id_item, id_jo, unit
) c on k.id_item = c.id_item and k.id_jo = c.id_jo and k.unit = c.unit
					...
					echo "
						<tr style='width: 100%;'>";
							echo "								  
								  ...
								  <td>$qty_sisa_stok</td>
								  <td>
								  <input type ='number' size = '4' step='0.01'  max = '$qty_sisa_stok' name='qtyreq[$i]' id='qtyreq$i' $status class='form-control qtyreqclass' onFocus='startCalcReq();' onBlur='stopCalcReq();'>
