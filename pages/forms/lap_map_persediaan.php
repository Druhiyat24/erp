...
$mod = $_GET['mod'];
$persediaanfilter = $_GET['persediaanfilter'];
...
$from=date('d M Y',strtotime($_POST['txtfrom']));
  ...
  echo "<script>
    window.location.href='index.php?mod=$mod&from=$from&to=$to&persediaanfilter=$persediaanfilter';
    </script>";
}

if(isset($_POST['submitexc'])) //KLIK SUBMIT
{
$pfexc = $_POST['txtpersediaanfilter'];	
$fromexc=date('d M Y',strtotime($_POST['txtfrom']));
  $toexc=date('d M Y',strtotime($_POST['txtto']));
  echo "<script>
  function exportExcel() {
  window.open ('index.php?mod=7780_exc&from=$fromexc&to=$toexc&pfexc=$pfexc&mode=exc&dest=xls', '_blank');
  }
    </script>";
      ...
      <div class='row'>
        ...
        <label>Mapping Persediaan: </label>
            <select id='persediaanfilter' class='form-control select2' style='width: 100%;' name='txtpersediaanfilter'>
			  ...
			  <option value="5" <?php if ($persediaanfilter == 5) { echo 'selected'; }?> >LAIN LAIN</option>
            </select> 
          </div>
		</div>		  
    </div>
    <button type='submit' name='submit' class='btn btn-primary'>Tampilkan</button>
    <button onclick="exportExcel()" name='submitexc' class='btn btn-success'>Export excel</button>
  </div>
        </form>
</div>
<?php
...
<div class="box">
  ...
  {  
  $queryfilter = "and mc.n_id = '$persediaanfilter'";
}
        if($from == '' && $to == '' && $filter == '')
