...
<div class='row'>
    ...
    <div class="box-header">
      <div class='col-md-2'>                            
        <label>No WS : </label>
        <input type='text' class='form-control' id='nows' name='nows' placeholder='Masukkan No WS' value='<?php echo $kpno;?>' >             
      </div>
      <div class='col-md-3'>
          <div>
          <br>
    ...
    <table id="examplefix" class="display responsive" style="width:100%;font-size:10px;">
      ...
      <tbody>
        ...
        $query = mysql_query("SELECT a.*,s.goods_code kode_lama,s.itemdesc desc_lama 
          FROM masterstyle a left join masteritem_odo s on a.id_item_odo=s.id_item_odo where kpno is null ORDER BY a.id_item DESC");          
      } else
      {
         $query = mysql_query("SELECT a.*,s.goods_code kode_lama,s.itemdesc desc_lama 
          FROM masterstyle a left join masteritem_odo s on a.id_item_odo=s.id_item_odo where kpno like '%$kpno%' ORDER BY a.id_item DESC");        
      }
			  $no = 1; 
          ...
          echo "<td><a $cl_hist href='index.php?mod=14&mode=$mode&id=$data[id_item]' $tt_hist</a></td>";
          /*
