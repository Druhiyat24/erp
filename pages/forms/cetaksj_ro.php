...
*/  
  ...
  <table width="100%" style="border:none; font-size:10pt">
    <tr>
      <td width="15%">Tgl. BPPB</td>
      <td width="35%"> : '.fd_view($rsh['bppbdate']).'</td>
      <td width="15%">SJ # / Inv #</td>
      <td width="35%"> : '.$rsh['invno'].'</td>
    </tr>
    <tr>
      <td>Dok. BC</td>
      <td> : '.$rsh['jenis_dok'].' '.$rsh['bcno'].'</td>
    ...
    if(substr($bppbno,-1,1)=="R")
    {
      ...
      $getbc=mysql_fetch_array(mysql_query("select CONCAT(jenis_dok,' ',bcno) bcno, bcdate from (select jenis_dok, GROUP_CONCAT(no_bc) bcno, GROUP_CONCAT(bcdate) bcdate from (select *, GROUP_CONCAT(bcno) no_bc from (select a.*, concat(bcno, ' ')bcno, concat(DATE_FORMAT(bcdate, '%d %b %Y'),' ') bcdate, jenis_dok, CONCAT(jenis_dok,' ',bcno) pabean from (select bppbno_int, bpbno_ro from bppb where bppbno = '$bppbno') a INNER JOIN bpb b where b.bpbno = a.bpbno_ro GROUP BY jenis_dok, bcno) a GROUP BY jenis_dok, bcdate) a GROUP BY jenis_dok) a"));

    $get_bcno=$getbc['bcno'];
    $get_bcdate=$getbc['bcdate'];
    ...
    // <tr>
    //     <td>BC In</td>
    //     <td> : '.$rsh['dok_bc_in'].' '.$rsh['bcno_in'].'</td>
    //     <td>Tgl. BC In</td>
    //     <td> : '.fd_view($rsh['tgl_bc_in']).'</td>
    //   </tr>';
...
<table class='main' repeat_header="1" border="1" cellspacing="0" width="100%" 
  ...
  <thead>
    ...
    <tr class="head">
      ...
      {
        echo "
        <td align='center' width='10%;'>No Po</td>
        <td align='center'>BC In</td>
        <td align='center'>Tgl. BC In</td>
  ...
  <tbody>
    ...
    <?php
      ...
      { 
        if(substr($bppbno,0,1)=="F")
        {
    tampil_data("select $fld_add_4 $fld_add_fg3 ,$fld_mst $fld_add_fg2 ,a.qty,a.unit,a.remark
    ...
    tampil_data("select $fld_add_4 $fld_add_fg3 ,$fld_mst $fld_add_fg2 ,a.qty,a.unit,(a.qty-a.qty_reject)gqty,a.qty_reject,a.remark
        ...
        <tr>
          ...
          <td align='right'>".round($tqty,3)."</td>

          <td style='background-color: #999'></td>

          <td align='right'>".round($tqtyg,3)."</td>

          <td align='right'>".round($tqtyr,3)."</td>
      ...
      { 
        ...
        if(substr($bppbno,-1,1)=="R"){$keterangan ="'Retur'";} else {$keterangan ="a.remark";}
        tampil_data("select $fldws $fld_add_fg3 ,$fld_mst $fld_add_fg2, pono, b.bcno, b.bcdate,a.qty,a.unit,$keterangan  
          from bppb a left JOIN (select id_item,id_jo,bpbno, pono, concat (jenis_dok, ' ',bcno) bcno, DATE_FORMAT(bcdate, '%d %b %Y') bcdate from bpb) b on b.bpbno = a.bpbno_ro and a.id_item = b.id_item and a.id_jo = b.id_jo inner join $tbl_mst s on a.id_item=s.id_item 
...
try {
    ...
    $mpdf=new mPDF('utf-8', $ukuran_kertas, 9 ,'Arial', 5, 10, $space_head, 5, 5, 1, $orientasi_kertas);
