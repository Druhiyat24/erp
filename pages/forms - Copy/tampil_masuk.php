...
<div class='box'>
	...
	<?php
	  ...
	  #CETAK KITE
		...
		{	if ($rpt=='kite_al')
			...
			$sqlkite = "select a.jenis_dok,$fldno,$no_seri no_seri,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,
				...
				where s.mattype in ('A','F','BB') and bpbno not like 'FG%' and a.tujuan not like '%SUBKON%' 
		...
		{	$sqlk = ""; $sqlk2 = ""; $sqlk3 = "";
			...
			$sql = mysql_query ("select if(a.bpbno_int!='',a.bpbno_int,a.bpbno) bpbno,a.bpbdate,if(s.goods_code='',concat('FG ',' ',a.id_item),
				...
				if(d.status_kb like 'SUBKON%' OR a.jenis_dok='SUBKONTRAK',a.qty,0) Dari_CMT,d.supplier from bpb a inner join 
				masterstyle s on a.id_item=s.id_item  left join mastersupplier d on a.id_supplier=d.id_supplier 
				where left(a.bpbno,2)='FG' and bpbdate between '$tglf' and '$tglt' $cril ");
			$no = 1; #nomor awal
			while($data = mysql_fetch_array($sql)){ #ketika data tabel mahasiswa di array kan maka lakukan perulangan hingga mahasiswa terakhir
			echo "
			<tr>
		...
		{	$kode_brg = "if(s.goods_code<>'' AND s.goods_code<>'-' AND s.goods_code<>'0',s.goods_code,
			...
			$sqlk = "SELECT 'BC 2.3 IMPOR PJT' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				...
				round(sum(a.price*a.qty),2) nilai_barang from bpb a 
				inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and d.area='I' 
			...
			$sqlk = "SELECT 'BC 2.3 IMPOR PJT' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				...
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)='FG'  and d.area='I' and a.invno like '%PJT%' 
		...
		{	if ($nm_company=="PT. Jinwoo Engineering Indonesia") 
			...
			$sqlk = "SELECT 'BC 2.3 IMPOR' jenis_dokumen,lpad(a.bcno,6,'0') bcno,
				...
				a.unit,sum(a.qty) qty,a.curr,round(sum(a.price*a.qty),2) nilai_barang 
				from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and 
		...
		{	if ($nm_company=='PT. Geum Cheon Indo')
			{ 	$sqlk = "SELECT 'BC 2.6.2 MASUK' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
					...
					s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join 
					masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt'   
			...
			{ 	$kode_brg = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) ";
				$sqlk = "SELECT 'BC 2.6.2 MASUK' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
					$kode_brg kode_brg,s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join 
					masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
					where bpbdate between '$tglf' and '$tglt' and bpbno not like 'FG%'  
			...
			$sqlk2 = "SELECT 'BC 2.6.2 MASUK' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,
				...
				round(a.price*a.qty,2) nilai_barang from bpb a inner join masterstyle s on a.id_item=s.id_item 
				inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt' 
		...
		{	if ($nm_company=="PT. Jinwoo Engineering Indonesia") 
			...
			$sqlk = "SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,
				...
				round(sum(a.price*a.qty),2) nilai_barang 
				from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)!='FG' and
			...
			$sqlk3 = "SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				...
				d.supplier,if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat('FG ',s.id_item)) kode_brg,
				itemname itemdesc,a.unit,sum(a.qty) qty,a.curr,round(sum(a.price*a.qty),2) nilai_barang from bpb a inner join masterstyle s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)='FG' 
		...
		{	$kode_brg = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) "; 
			...
			$sqlk = "SELECT 'BC 4.0' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
				$kode_brg kode_brg,s.itemdesc,a.unit,sum(a.qty) qty,a.curr,round(sum(a.price*a.qty),2) nilai_barang 
				from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and a.jenis_dok='BC 4.0' 
			...
			$sqlk2 = "SELECT 'BC 4.0' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
				$kode_brg_fg kode_brg,s.itemname itemdesc,a.unit,sum(a.qty) qty,a.curr,round(sum(a.price*a.qty),2) nilai_barang 
				from bpb a inner join masterstyle s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)='FG' and d.area='L' and a.jenis_dok='BC 4.0' and ucase(invno) not like '%SEWA%' 
				and a.tujuan not like '%SUBKON%' group by bcno,bpbno,a.id_item,price order by bcdate,bcno";
			insert_temp_perdok($sqlk2,$user,$sesi,"N");
		} elseif ($rpt=='bc40sewa')
		{	$sqlk = "SELECT 'BC 4.0 MESIN (SEWA)' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,
				...
				if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) kode_brg,
				s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and d.status_kb='NON KB' and d.area='L' and ucase(invno) like '%SEWA%' order by bcdate,bcno";
			insert_temp_perdok($sqlk,$user,$sesi,"Y");
		...
		{	$kode_brg = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) ";
			$sqlk = "SELECT 'BC 4.0 SUBKON' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
				$kode_brg kode_brg,s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang 
				from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and a.jenis_dok='BC 4.0' 
			...
			$sqlk2 = "SELECT 'BC 4.0 SUBKON' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,a.bpbdate trans_date,d.supplier,
				$kode_brg kode_brg,s.itemname itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang 
				from bpb a inner join masterstyle s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)='FG'  and a.jenis_dok='BC 4.0' 
				and ucase(invno) not like '%SEWA%' and a.tujuan like '%SUBKON%' order by bcdate,bcno";
			insert_temp_perdok($sqlk2,$user,$sesi,"N");
		...
		{	$sqlk = "SELECT 'BC 2.0 IMPOR PIB' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				...
				s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and d.area='I' and a.invno like '%PIB%' and a.invno not like '%PIBK%' order by bcdate,bcno";
			insert_temp_perdok($sqlk,$user,$sesi,"Y");
		} elseif ($rpt=='bc21pibk')
		{	$sqlk = "SELECT 'BC 2.1 IMPOR PIBK' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				a.bpbdate trans_date,d.supplier,if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) kode_brg,
				s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and d.area='I' and a.invno like '%PIBK%' order by bcdate,bcno";
			insert_temp_perdok($sqlk,$user,$sesi,"Y");
		...
		{	$sqlk = "SELECT 'BC 2.4 KITTE' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,if(a.bpbno_int!='',a.bpbno_int,a.bpbno) trans_no,
				a.bpbdate trans_date,d.supplier,if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) kode_brg,
				s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier where bpbdate between '$tglf' and '$tglt' and left(bpbno,2)<>'FG'  and d.status_kb='KITTE' and d.area='L'";
			insert_temp_perdok($sqlk,$user,$sesi,"Y");
		} elseif ($rpt=='bc30')
		{	if ($nm_company=="PT. Bangun Sarana Alloy")
			...
			$sqlk = "SELECT 'BC 3.0' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,
				a.bppbdate trans_date,d.supplier,$vkode kode_brg,s.itemname itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,
				a.price ,'FG' mattype,s.id_item 
			...
			$sqlk2 = "SELECT 'BC 3.0' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,
				d.supplier,if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) kode_brg,s.itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,
		...
		{	$kodenya = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item))";
			$sqlk = "SELECT 'BC 2.6.1 KELUAR' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				$kodenya kode_brg,s.itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,a.price ,s.mattype,
				...
				where bppbdate between '$tglf' and '$tglt'   and jenis_dok='BC 2.6.1' and mid(a.bppbno,4,2) not in ('FG') 
			...
			$sqlk2 = "SELECT 'BC 2.6.1 KELUAR' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				$kodenya kode_brg,s.itemname itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,a.price ,'FG' mattype,
				...
				where bppbdate between '$tglf' and '$tglt'   and jenis_dok='BC 2.6.1' and mid(a.bppbno,4,2) in ('FG') and 
		...
		{	$kdnya = "if(s.goods_code<>'' AND s.goods_code<>'-' AND s.goods_code<>'0'
			...
			$sqlk = "SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,
				...
				round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price ,
			...
			$sqlk2 = "SELECT 'BC 2.7' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				$kdnya kode_brg,
				s.itemdesc,a.unit,sum(a.qty) qty,round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price ,s.mattype,s.id_item from bppb a inner join masteritem s on a.id_item=s.id_item 
		...
		{	$kodenya = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item))";
			$sqlk = "SELECT 'BC 4.1 LOKAL' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				$kodenya kode_brg,s.itemdesc,a.unit,sum(a.qty) qty,
				round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price ,
				s.mattype,s.id_item from bppb a inner join masteritem s on a.id_item=s.id_item inner join 
				mastersupplier d on a.id_supplier=d.id_supplier where 
			...
			$sqlk2 = "SELECT 'BC 4.1 LOKAL' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				...
				sum(a.qty) qty,round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price ,
		...
		{	$sqlk = "SELECT 'BC 4.1 LOKAL MESIN (SEWA)' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,
				...
				round(a.qty*a.price,2) nilai_barang,a.curr,a.price ,s.mattype,s.id_item 
		...
		{	$sqlk = "SELECT 'BC 4.1 SUBKON' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item)) kode_brg,s.itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,a.price ,s.mattype,
				s.id_item from bppb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
			...
			$sqlk2 = "SELECT 'BC 4.1 SUBKON' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat('FG ',s.id_item)) kode_brg,itemname itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,a.price ,'FG' mattype,
				s.id_item from bppb a inner join masterstyle s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
		...
		{	$kodenya = "if(goods_code<>'' AND goods_code<>'-' AND goods_code<>'0',goods_code,concat(s.mattype,s.id_item))";
			$sqlk = "SELECT 'BC 2.5 SCRAP' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				$kodenya kode_brg,s.itemdesc,a.unit,sum(a.qty) qty,round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price ,s.mattype,s.id_item 
				from bppb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
		...
		{	if ($nm_company=="PT. Bangun Sarana Alloy")
			...
			{	$sqlk = "SELECT 'BC 2.5 KELUAR' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
					$vkode kode_brg,s.itemdesc,a.unit,a.qty,round(a.qty*a.price,2) nilai_barang,a.curr,a.price,mattype,a.id_item 
			...
			{	$sqlk = "SELECT 'BC 2.5 JUAL LOKAL' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
					$vkode kode_brg,s.itemname itemdesc,a.unit,sum(a.qty) qty,round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price,'FG' mattype,a.id_item 
					from bppb a inner join masterstyle s on a.id_item=s.id_item LEFT join mastersupplier d on a.id_supplier=d.id_supplier 
					where bppbdate between '$tglf' and '$tglt' and mid(bppbno,4,2)='FG' and 
					jenis_dok='BC 2.5' 
				...
				$sqlk = "SELECT 'BC 2.5 JUAL LOKAL' jenis_dokumen,lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
					...
					a.unit,sum(a.qty) qty,round(sum(a.qty*a.price),2) nilai_barang,a.curr,a.price,s.mattype,a.id_item 
		...
		{	$sqlk = "SELECT 
				...
				if(s.goods_code='' OR s.goods_code='-' OR s.goods_code='0',concat(s.mattype,' ',a.id_item),s.goods_code) kode_brg,
				concat(s.itemdesc,' ',s.color,' ',s.size,' ',s.add_info) itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang,
				berat_bersih,berat_kotor,right(nomor_aju,6) nomor_aju,tujuan 
				from bpb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
				where bpbdate between '$tglf' and '$tglt'  and left(bpbno,2)<>'FG' and jenis_dok!='INHOUSE' order by bcdate,bcno,bpbno ";
			...
			$sqlk = "SELECT 
				...
				s.itemname itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang,
		...
		{	$sqlk = "SELECT 
				...
				if(jenis_dok='BC 2.5' and mid(bppbno,4,1) not in ('S'),'BC 2.5','N/A')))))))) jenis_dokumen,
				lpad(a.bcno,6,'0') bcno,a.bcdate,$vtrans_no trans_no,a.bppbdate trans_date,d.supplier,
				if(s.goods_code='' OR s.goods_code='-' OR s.goods_code='0',concat(s.mattype,' ',a.id_item),s.goods_code) kode_brg,
				s.itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang,a.price,
				berat_bersih,berat_kotor,right(nomor_aju,6) nomor_aju,tujuan
				from bppb a inner join masteritem s on a.id_item=s.id_item inner join mastersupplier d on a.id_supplier=d.id_supplier 
			...
			$sqlk = "SELECT 
				...
				if(s.goods_code='' OR s.goods_code='-' OR s.goods_code='0',concat('FG ',a.id_item),s.goods_code) kode_brg,
				s.itemname itemdesc,a.unit,a.qty,a.curr,round(a.price*a.qty,2) nilai_barang,a.price,
				berat_bersih,berat_kotor,right(nomor_aju,6) nomor_aju,tujuan 
	  ...
	  $no_dok_prev=""; $jenis_dok_prev="";
		$sql = mysql_query("select * from upload_standard where username='$user' and sesi='$sesi' 
			order by jenis_dokumen,bcdate,bcno");
		while($data = mysql_fetch_array($sql))
		{	$jenis_dok = $data['JENIS_DOKUMEN'];
			$no_dok = $data['BCNO'];
			if ($no>1)
			{	$cri_next=$data['JENIS_DOKUMEN'].' '.$data['BCNO']; }
			...
			if ($no==1 OR $cri_next!=$cri_prev)
			{	$no++; }
			$cri_prev=$data['JENIS_DOKUMEN'].' '.$data['BCNO'];
		}
		?>
	</table>
	</div>
