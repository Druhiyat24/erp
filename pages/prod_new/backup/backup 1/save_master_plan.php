...
if ($mod == 'simpan') {
	...
	foreach ($usernameArray as $key_1 => $value_1) {
		...
		$sql = "insert into master_plan (id_plan,id,tgl_plan,sewing_line,id_ws,color,create_by,
				smv,jam_kerja,man_power,plan_target,target_effy,set_target,tgl_input,cancel,gambar)
				values ('$id_plan','','$tanggal','$username','$cbows','$cbocolor',
				'$user','$txtsmv','$txtjamkerja','$txtmpwr','$txt_plan_target','$txt_target_eff','$txt_set_target',
...
if ($mod == 'update') {
	...
	smv = '$txtsmv', man_power = '$txtmpwr', plan_target = '$txt_plan_target', target_effy = '$txt_target_eff' $gambar, set_target = '$txt_set_target'
	where id = '$id'";
	insert_log($sql, $user); {
