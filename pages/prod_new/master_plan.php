    ...
    <div class="box-body">
      <table id="examplefix3" class="display responsive" style="width: 100%;font-size:13px;">
        ...
        <tbody>
          <?php
          # QUERY TABLE
          $query = mysql_query("select a.*, ac.kpno,ac.styleno, supplier buyer,sd.qty_order, if(cd.master_plan_id is null, 'ok', 'no') cek_stat
          from master_plan a
          ...
          left join ( select master_plan_id from output_rfts where created_at >= '$tglf 00:00:00' group by master_plan_id  ) cd on a.id = cd.master_plan_id
          where a.tgl_plan >= '$tglf' and a.tgl_plan <= '$tglt'");

          $no = 1;
          while ($data = mysql_fetch_array($query)) {
            $cek_stat = $data['cek_stat'];
        ...
        <form method='post' name='form' action='save_master_plan.php?mod=simpan' enctype="multipart/form-data">
          ...
          <div class='col-md-4'>
            ...
            <div class='row'>
              <div class='col-md-4'>
                <div class='form-group'>
                  <label>Jam Kerja Awal</label>
                  <input type='time' class='form-control' name='txt_jam_kerja_awal' id='txt_jam_kerja_awal' value = '07:00' required >
                </div>
              </div>
...
<?php if ($mod == "master_plan_edit") {
  ...
  $jam_kerja_awal   = $datamp['jam_kerja_awal'];
?>
          ...
          <div class='col-md-4'>
            ...
            <div class='row'>
              <div class='col-md-4'>
                <div class='form-group'>
                  <label>Jam Kerja Awal</label>
                  <input type='time' class='form-control' name='txt_jam_kerja_awal' id='txt_jam_kerja_awal' value='<?php echo $jam_kerja_awal; ?>' required >
                </div>
              </div>
            <div class='form-group' style='padding: 22px'>
              <button type='submit' name='submit' class='btn btn-primary'>Simpan</button>
