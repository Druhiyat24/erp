...
function update_pono($bpb)
{ //A.bpbno LIKE '%R%' AND (A.id_supplier >= 432 AND A.id_supplier <= 458
...
if (!isset($_POST['item'])) {
	...
	foreach ($QtyRI as $key => $value) {
		if (is_numeric($value)) {
			...
			$sql = "insert into bpb (id_item,id_item_fg,qty,qty_temp,unit,curr,price,remark,jam_masuk,berat_bersih,berat_kotor,nomor_mobil,pono,id_supplier,
				...
				'$txtid_gudang','$txtnomor_rak','$retur','$txtbppbno_ri','$txtbppbno_ri','$id_jo','$txtid_so_det','NAK')";
			insert_log($sql, $user);
	...
	echo "<script>window.location.href='../wh/?mod=retur_material&mode=$mode';</script>";
}
