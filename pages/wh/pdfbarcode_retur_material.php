...
?>
    ...
    $sqlquedet = "select b.* from 
    (select * from retur_material a where no_retur = '$noretur' and kode_barang = '$kdbrg' and nama_barang = '$nmbrg' and job_order = '$jo' and unit = '$unit' ) a
    inner join
     (select a.id_barcode,a.roll_no,a.lot_no,a.kode_rak, a.no_retur,b.no_dok,b.supplier,b.no_po,b.no_sj,b.job_order,b.unit,a.ex_id_barcode,b.nama_barang,kode_barang,roll_qty
     from in_material_det a 
     inner join in_material b on a.id_in_material = b.id
     where a.cancel = 'N' and a.retur = 'Y' and no_retur = '$noretur' ) b on a.id_barcode = b.ex_id_barcode
    ...
    while ($rshder = mysql_fetch_array($qdet)) {
      ...
      $head_data = ' 
            <table width="100%" style="border:none; font: size 5px;font-weight:bold">
              ...
              <tr> <td>No BPB / No Retur / Barcode Lama <td>:<td></tr>
              <td>' . $rshder['no_dok'] . ' / ' . $rshder['no_retur'] . ' / ' . $rshder['ex_id_barcode'] . '</td>              
