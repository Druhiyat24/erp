...
$head_cap = "BUKTI PENGELUARAN BARANG";

$sql = "select *,REPLACE(a.no_req,'/','') simpan from       
(select * from out_material where no_out = '$no_dok' limit 1) a
inner join  (select no_req,supplier,tipe_pengeluaran,tgl_req from req_material group by no_req)  b on a.no_req = b.no_req  ";
...
<th align='center' colspan = "5">test</th>
  ...
  <tbody>
    ...
    tampil_data("select kode_barang,nama_barang,job_order,sum(qty) qty,unit from out_material where no_out = '$no_dok' and cancel = 'N' group by kode_barang, nama_barang, unit, job_order", 5);
    $sqlsum = "select sum(qty) jml_qty from out_material where no_out = '$no_dok' and cancel = 'N'";
          ...
          <table width="100%" style="border:none;">
            ...
            <tr>
              <td align="center" style="border:none;"><h2>' . $head_cap . '</h2></td>
                        ...
                        <tr>
              <td align="center" style="border:none;"><h2>' . $rsh['no_out'] . '</h2></td>
            </tr>
          </table>
