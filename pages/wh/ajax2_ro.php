	...
	{	$bpbno=$crinya;
		$sql="select * from (select * from (select a.*,b.*,(COALESCE(a.roll_qty,0) - COALESCE(b.qty_out,0) - COALESCE(a.qty_mutasi,0)) qty_sisa from (select bd.*,
         ...
         order by bpb.bpbdate asc) a left join 
				 (select id_bpb_det,SUM(roll_qty) qty_out from bppb_det where cancel = 'N' GROUP BY id_bpb_det) b on b.id_bpb_det = a.id) a ) a where qty_sisa > 0"; ?>
				...
				{	
					echo "<tr>";
						echo "
							...
							<td>$data[goods_code]</td>
							<td>$data[itemdesc]</td>
							<td><input  class='form-control' type='number' min = '0' max = '$data[qty_sisa]' id='stockajax$i' name ='itemstock[$id]' value = '$data[qty_sisa]' readonly></td>
							<td>$data[unit]</td>
							<td><input  class='form-control' type='number' min = '0' max = '$data[qty_sisa]' id='itemajax$i' name ='item[$id]' value = '$data[qty_sisa]' >
