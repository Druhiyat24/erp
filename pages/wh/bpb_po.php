...
{ ?>
  ...
  <div class="box">
    ...
    { $fldnyacri=" left(a.bpbno,1) in ('N','M') "; $mod2="edit_penerimaan"; }

    else 

    { $fldnyacri=" left(a.bpbno,1) in ('F') and left(a.bpbno,2)!='FG' "; $mod2="edit_penerimaan"; }
    ...
    <div class="box-header">
      ...
      <a href='../wh/?mod=<?php echo 'in_material_new'; ?>&mode=<?php echo $mode; ?>' class='btn btn-primary btn'>
      ...
      <table id="examplefix3" class="display responsive" style="width:100%;font-size:13px;">
        ...
        <tbody>
          ...
          $query = mysql_query("SELECT a.*,s.goods_code,$fld_desc itemdesc,supplier,a.qty_reject,a.nomor_kk_bc, a.last_date_bpb
            ...
            ,count(*) t_bpb_it,sum(if(isnull(brh.bpbno),0,1)) t_bpb_rl,det.*, round((det.qty_rak/a.qty*100),2) persen
            ...
            left join (select id_h idroll_h, ROUND(sum(roll_qty),2) qty_rak from bpb_roll GROUP BY id_h) det on brh.id = det.idroll_h
          ...
          { 
            if ($data['persen'] == "0") {
              ...
              $width = "25";
              $stat = "hidden";
            } else if ($data['persen'] > "0" && $data['persen'] <= "50") {
              $col = "goldenrod";
              $width = "50";
              $stat = "true";
            } else if ($data['persen'] > "50" && $data['persen'] <= "99.9") {
              $col = "CornflowerBlue'";
              $width = "75";
              $stat = "true";
            } else if ($data['persen'] >= "100") {
              $col = "mediumseagreen'";
              $width = "100";
              $stat = "true";
            }
       ...
       echo "
             ...
             aria-valuemin='0' aria-valuemax='100'>$data[persen]%</div></td>
                <td><a href='pdfBarcode_in_material.php?mode=barcode&id=$data[idroll_h]'
           data-toggle='tooltip' title='Preview' target='_blank'><i class='fa fa-barcode' style='visibility:$stat;'></i></a></td>"; 
              }
              else
              ...
              { echo "
                ...
                <td></td>
                <td style='width: 100px'><div class='progress-bar' role='progressbar'  style='width: $width%;background-color:$col ;color: black;' aria-valuenow='50'
             aria-valuemin='0' aria-valuemax='100'>$data[persen]%</div></td>

                <td>

                  <a href='pdfBarcode_in_material.php?mode=barcode&id=$data[idroll_h]'
